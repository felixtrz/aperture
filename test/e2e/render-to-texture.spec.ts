...
interface RenderToTextureStatus extends ExampleStatusBase {
  ...
  readonly mixedMsaaRenderTargetReuse?: {
    readonly mode?: string;
    readonly source?: string;
    readonly renderTargetKey?: string;
    readonly requestedSampleCount?: number;
    ...
    readonly colorTexturesCreated?: number;
    readonly colorTexturesReused?: number;
    readonly reuse?: RenderToTextureStatus["renderTargetReuseStress"];
    readonly renderTargets?: readonly {
      readonly role?: string;
      readonly target?: string;
      readonly key?: string | null;
  ...
  readonly mixedMsaaReusedTargetCrop?: RenderToTextureStatus["mixedMsaaTargetCrop"] & {
    readonly reuse?: RenderToTextureStatus["renderTargetReuseStress"];
  };
  readonly mixedMsaaTargetCrop?: {
    readonly mode?: string;
    readonly source?: string;
    readonly renderTargetKey?: string;
    readonly requestedSampleCount?: number;
    ...
    readonly colorTexturesCreated?: number;
    readonly colorTexturesReused?: number;
    readonly targetCrop?: RenderToTextureStatus["offscreenTargetCrop"];
    readonly renderTargets?: readonly {
      readonly role?: string;
      readonly target?: string;
      readonly key?: string | null;
  ...
  readonly mixedMsaaResizedReusedTargetCrop?: RenderToTextureStatus["mixedMsaaReusedTargetCrop"] & {
    readonly resize?: RenderToTextureStatus["renderTargetResize"];
  };
  ...
  readonly mixedMsaaResizedDualSizeRenderTargets?: RenderToTextureStatus["mixedMsaaDualSizeRenderTargets"] & {
    readonly resize?: RenderToTextureStatus["renderTargetResize"];
  };
  readonly mixedMsaaReusedDualSizeRenderTargets?: RenderToTextureStatus["mixedMsaaDualSizeRenderTargets"] & {
    readonly reuse?: RenderToTextureStatus["renderTargetReuseStress"];
  };
  readonly mixedMsaaResizedReusedDualSizeRenderTargets?: RenderToTextureStatus["mixedMsaaReusedDualSizeRenderTargets"] & {
    readonly resize?: RenderToTextureStatus["renderTargetResize"];
  };
  ...
  readonly mixedMsaaReusedSameTargetClearLoad?: RenderToTextureStatus["mixedMsaaSameTargetClearLoad"] & {
    readonly reuse?: RenderToTextureStatus["renderTargetReuseStress"];
  };
  readonly mixedMsaaResizedSameTargetClearLoad?: RenderToTextureStatus["mixedMsaaSameTargetClearLoad"] & {
    readonly resize?: RenderToTextureStatus["renderTargetResize"];
  };
  readonly sourceView?: {
    readonly ok?: boolean;
...
test("mixed MSAA resized dual-size route displays current and resized aspect-preserved off-screen targets", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-resized-dual-size.html",
    "mixed-msaa-resized-dual-size-status",
  );
  ...
  const primaryAspect =
    status.mixedMsaaResizedDualSizeRenderTargets?.renderTargets?.find(
      (target) => target.role === "primary",
    )?.aspect ?? null;
  const secondaryAspect =
    status.mixedMsaaResizedDualSizeRenderTargets?.renderTargets?.find(
  ...
  await attachExampleStatus(
    "mixed-msaa-resized-dual-size-rendered-status",
    status,
  );

  if (
    !status.readback?.ok ||
    !status.mixedMsaaResizedDualSizeRenderTargets?.currentTextureReadback?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA resized dual-size pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaResizedDualSizeRenderTargets.currentTextureReadback.samples?.find(
      (entry) => entry.id === "current-texture-direct-left",
    );
  const leftSample = status.readback.samples?.find(
    (entry) => entry.id === "left-target-preview-center",
  ...
  expect(
    leftSample,
    "expected resized primary resolved render-target preview sample",
  ).toBeDefined();
  expect(
    rightSample,
    "expected secondary resolved render-target preview sample",
  ...
  expect(
    ...
    "left preview should sample the resized resolved square MSAA target",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      rightSample.pixel,
  ...
  expect(
    pixelDistance(currentSample.pixel, leftSample.pixel),
    "current-texture sample and resized square resolved preview should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(currentSample.pixel, rightSample.pixel),
    "current-texture sample and wide resolved preview should be visually distinct",
  ...
  expect(
    pixelDistance(leftSample.pixel, rightSample.pixel),
    "resized dual-size resolved off-screen previews should be visually distinct",
  ).toBeGreaterThan(80);
});

test("mixed MSAA reuse dual-size route displays current and second resolved aspect-preserved off-screen targets", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-reuse-dual-size.html",
    "mixed-msaa-reuse-dual-size-status",
  );
  ...
  expect(status, JSON.stringify(status, null, 2)).toMatchObject({
    ...
    renderTargetReuseStress: {
      ...
      frames: [
        {
          frame: 1,
          workerVariant: "current-texture-plus-dual-size-offscreen-targets",
          centerExpectation:
  ...
  const primaryAspect =
    status.mixedMsaaReusedDualSizeRenderTargets?.renderTargets?.find(
      (target) => target.role === "primary",
    )?.aspect ?? null;
  const secondaryAspect =
    status.mixedMsaaReusedDualSizeRenderTargets?.renderTargets?.find(
  ...
  await attachExampleStatus(
    "mixed-msaa-reuse-dual-size-rendered-status",
    status,
  );

  if (
    !status.readback?.ok ||
    !status.mixedMsaaReusedDualSizeRenderTargets?.currentTextureReadback?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA reuse dual-size pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaReusedDualSizeRenderTargets.currentTextureReadback.samples?.find(
      (entry) => entry.id === "current-texture-direct-left",
    );
  const leftSample = status.readback.samples?.find(
    (entry) => entry.id === "left-target-preview-center",
  ...
  expect(
    leftSample,
    "expected second-frame primary resolved render-target preview sample",
  ).toBeDefined();
  expect(
    rightSample,
    "expected secondary resolved render-target preview sample",
  ...
  expect(
    ...
    "left preview should sample the second resolved square MSAA target frame",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      rightSample.pixel,
  ...
  expect(
    pixelDistance(currentSample.pixel, leftSample.pixel),
    "current-texture sample and reused square resolved preview should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(currentSample.pixel, rightSample.pixel),
    "current-texture sample and wide resolved preview should be visually distinct",
  ...
  expect(
    pixelDistance(leftSample.pixel, rightSample.pixel),
    "reused dual-size resolved off-screen previews should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(leftSample.pixel, screenClearSample.pixel),
    "second-frame primary preview should not display first-frame clear pixels",
  ).toBeGreaterThan(40);
});

test("mixed MSAA resized reuse dual-size route displays current and second resized aspect-preserved off-screen targets", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-resized-reuse-dual-size.html",
    "mixed-msaa-resized-reuse-dual-size-status",
  );
  ...
  expectStatusJsonSafeForGpu(status);
  expect(status, JSON.stringify(status, null, 2)).toMatchObject({
    example: "mixed-msaa-resized-reuse-dual-size",
    ok: true,
    phase: "display",
    renderingBackend: "webgpu-explicit",
    renderTarget: {
    ...
    mixedMsaaResizedReusedDualSizeRenderTargets: {
      mode: "current-texture-plus-msaa-resized-reused-dual-size-offscreen-render-targets",
      source: "ViewPacket.renderTarget",
      requestedSampleCount: 8,
      sampleCount: 4,
      enabled: true,
  ...
  const primaryAspect =
    status.mixedMsaaResizedReusedDualSizeRenderTargets?.renderTargets?.find(
      (target) => target.role === "primary",
    )?.aspect ?? null;
  const secondaryAspect =
    status.mixedMsaaResizedReusedDualSizeRenderTargets?.renderTargets?.find(
  ...
  await attachExampleStatus(
    "mixed-msaa-resized-reuse-dual-size-rendered-status",
    status,
  );

  if (
    !status.readback?.ok ||
    !status.mixedMsaaResizedReusedDualSizeRenderTargets?.currentTextureReadback
      ?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA resized reuse dual-size pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaResizedReusedDualSizeRenderTargets.currentTextureReadback.samples?.find(
      (entry) => entry.id === "current-texture-direct-left",
    );
  const leftSample = status.readback.samples?.find(
    (entry) => entry.id === "left-target-preview-center",
  ...
  expect(
    leftSample,
    "expected resized second-frame primary resolved render-target preview sample",
  ).toBeDefined();
  expect(
    rightSample,
    "expected secondary resolved render-target preview sample",
  ...
  expect(
    ...
    "left preview should sample the resized second resolved square MSAA target frame",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      rightSample.pixel,
  ...
  expect(
    ...
    "screen clear sample should stay outside both resized resolved preview quads",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, leftSample.pixel),
    "current-texture sample and resized reused square resolved preview should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(currentSample.pixel, rightSample.pixel),
    "current-texture sample and wide resolved preview should be visually distinct",
  ...
  expect(
    pixelDistance(leftSample.pixel, rightSample.pixel),
    "resized reused dual-size resolved off-screen previews should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(leftSample.pixel, screenClearSample.pixel),
    "resized reused primary preview should not display stale-size or first-frame clear pixels",
  ).toBeGreaterThan(40);
});
...
test("mixed MSAA target crop route displays current and cropped resolved off-screen target", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-target-crop.html",
    "mixed-msaa-target-crop-status",
  );
  ...
  expectNormalizedRect(
    status.mixedMsaaTargetCrop?.targetCrop?.view?.viewport,
    [0.3, 0.25, 0.4, 0.5],
  );
  expectNormalizedRect(
    status.mixedMsaaTargetCrop?.targetCrop?.view?.scissor,
    [0.3, 0.25, 0.4, 0.5],
  );
  guard.expectNoWarnings();

  await attachExampleStatus("mixed-msaa-target-crop-rendered-status", status);

  if (
    !status.readback?.ok ||
    !status.mixedMsaaTargetCrop?.currentTextureReadback?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA target crop pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaTargetCrop.currentTextureReadback.samples?.find(
      (entry) => entry.id === "canvas-direct-left",
    );
  ...
  expect(currentSample, "expected current-texture sample").toBeDefined();
  expect(insideSample, "expected resolved inside-crop sample").toBeDefined();
  expect(outsideSample, "expected resolved outside-crop sample").toBeDefined();
  expect(screenClearSample, "expected screen clear sample").toBeDefined();
  ...
  expect(
    ...
    "inside crop sample should come from the resolved off-screen viewport",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      outsideSample.pixel,
  ...
  expect(
    ...
    "screen clear sample should stay outside the displayed crop preview quad",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, insideSample.pixel),
    "current-texture sample and resolved inside-crop sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(insideSample.pixel, outsideSample.pixel),
    "resolved crop should differ inside and outside the viewport",
  ).toBeGreaterThan(40);
  expect(
...
test("mixed MSAA reuse route displays current and second resolved off-screen target", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-reuse.html",
    "mixed-msaa-reuse-status",
  );
  ...
  expect(status, JSON.stringify(status, null, 2)).toMatchObject({
    ...
    renderTargetReuseStress: {
      ...
      frames: [
        {
          frame: 1,
          workerVariant: "mixed-current-and-offscreen-targets",
          centerExpectation: "canvas-plane-plus-offscreen-preview",
          renderTargetKey,
          width: 256,
          height: 256,
          drawCalls: 1,
  ...
  await attachExampleStatus("mixed-msaa-reuse-rendered-status", status);

  if (
    !status.readback?.ok ||
    !status.mixedMsaaRenderTargetReuse?.currentTextureReadback?.ok
  ) {
    test.skip(true, "Mixed MSAA reuse pixel assertion requires readback.");
    return;
  }

  const currentSample =
    status.mixedMsaaRenderTargetReuse.currentTextureReadback.samples?.find(
      (entry) => entry.id === "canvas-direct-left",
    );
  const previewSample = status.readback.samples?.find(
    (entry) => entry.id === "offscreen-preview-center",
  ...
  expect(
    previewSample,
    "expected second-frame resolved render-target preview sample",
  ).toBeDefined();
  expect(screenClearSample, "expected screen clear sample").toBeDefined();
  ...
  expect(
    ...
    "preview should sample the second resolved MSAA off-screen frame",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      previewSample.pixel,
    ...
    "resolved reused preview should not expose the stale first-frame clear pixel",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(
      screenClearSample.pixel,
    ...
    "screen clear sample should stay outside the reused resolved preview quad",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, previewSample.pixel),
    "current-texture sample and reused resolved preview should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(previewSample.pixel, screenClearSample.pixel),
    "reused resolved preview should differ from the screen clear color",
  ).toBeGreaterThan(40);
});

test("mixed MSAA reuse crop route displays current and second cropped resolved off-screen target", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-reuse-crop.html",
    "mixed-msaa-reuse-crop-status",
  );
  ...
  expectNormalizedRect(status.sourceView?.scissor, [0.3, 0.25, 0.4, 0.5]);
  expectNormalizedRect(
    status.mixedMsaaReusedTargetCrop?.targetCrop?.view?.viewport,
    [0.3, 0.25, 0.4, 0.5],
  );
  expectNormalizedRect(
    status.mixedMsaaReusedTargetCrop?.targetCrop?.view?.scissor,
    [0.3, 0.25, 0.4, 0.5],
  );
  expectNormalizedRect(
    status.mixedMsaaReusedTargetCrop?.views?.find(
      (view) => view.role === "offscreen",
    )?.viewport,
    [0.3, 0.25, 0.4, 0.5],
  );
  ...
  await attachExampleStatus("mixed-msaa-reuse-crop-rendered-status", status);

  if (
    !status.readback?.ok ||
    !status.mixedMsaaReusedTargetCrop?.currentTextureReadback?.ok
  ) {
    test.skip(true, "Mixed MSAA reuse crop pixel assertion requires readback.");
    return;
  }

  const currentSample =
    status.mixedMsaaReusedTargetCrop.currentTextureReadback.samples?.find(
      (entry) => entry.id === "canvas-direct-left",
    );
  const insideSample = status.readback.samples?.find(
    (entry) => entry.id === "offscreen-crop-inside",
  ...
  expect(
    insideSample,
    "expected second-frame resolved inside-crop sample",
  ).toBeDefined();
  expect(
    outsideSample,
    "expected second-frame resolved outside-crop sample",
  ).toBeDefined();
  expect(screenClearSample, "expected screen clear sample").toBeDefined();
  ...
  expect(
    ...
    "inside crop sample should come from the second resolved MSAA off-screen frame",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      outsideSample.pixel,
    ...
    "outside crop sample should remain the reused off-screen target clear color",
  ).toBeLessThan(12);
  expect(
    pixelDistance(
      screenClearSample.pixel,
    ...
    "screen clear sample should stay outside the reused crop preview quad",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, insideSample.pixel),
    "current-texture sample and reused resolved inside-crop sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(insideSample.pixel, outsideSample.pixel),
    "reused resolved crop should differ inside and outside the viewport",
  ).toBeGreaterThan(40);
  expect(
    pixelDistance(currentSample.pixel, outsideSample.pixel),
    "current-texture sample should differ from the reused off-screen target clear color",
  ).toBeGreaterThan(40);
});

test("mixed MSAA resized reuse crop route displays current and second resized cropped resolved off-screen target", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-resized-reuse-crop.html",
    "mixed-msaa-resized-reuse-crop-status",
  );
  ...
  expectNormalizedRect(status.sourceView?.scissor, [0.3, 0.25, 0.4, 0.5]);
  expectNormalizedRect(
    status.mixedMsaaResizedReusedTargetCrop?.targetCrop?.view?.viewport,
    [0.3, 0.25, 0.4, 0.5],
  );
  expectNormalizedRect(
    status.mixedMsaaResizedReusedTargetCrop?.targetCrop?.view?.scissor,
    [0.3, 0.25, 0.4, 0.5],
  );
  expectNormalizedRect(
    status.mixedMsaaResizedReusedTargetCrop?.views?.find(
      (view) => view.role === "offscreen",
    )?.viewport,
    [0.3, 0.25, 0.4, 0.5],
  );
  ...
  await attachExampleStatus(
    "mixed-msaa-resized-reuse-crop-rendered-status",
    status,
  );

  if (
    !status.readback?.ok ||
    !status.mixedMsaaResizedReusedTargetCrop?.currentTextureReadback?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA resized reuse crop pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaResizedReusedTargetCrop.currentTextureReadback.samples?.find(
      (entry) => entry.id === "canvas-direct-left",
    );
  const insideSample = status.readback.samples?.find(
    (entry) => entry.id === "offscreen-crop-inside",
  ...
  expect(
    insideSample,
    "expected resized second-frame resolved inside-crop sample",
  ).toBeDefined();
  expect(
    outsideSample,
    "expected resized second-frame resolved outside-crop sample",
  ).toBeDefined();
  expect(screenClearSample, "expected screen clear sample").toBeDefined();
  ...
  expect(
    ...
    "inside crop sample should come from the resized second resolved MSAA off-screen frame",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      outsideSample.pixel,
    ...
    "outside crop sample should remain the resized reused off-screen target clear color",
  ).toBeLessThan(12);
  expect(
    pixelDistance(
      screenClearSample.pixel,
    ...
    "screen clear sample should stay outside the resized reused crop preview quad",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, insideSample.pixel),
    "current-texture sample and resized reused resolved inside-crop sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(insideSample.pixel, outsideSample.pixel),
    "resized reused resolved crop should differ inside and outside the viewport",
  ).toBeGreaterThan(40);
  expect(
    pixelDistance(currentSample.pixel, outsideSample.pixel),
    "current-texture sample should differ from the resized reused off-screen target clear color",
  ).toBeGreaterThan(40);
});
...
test("mixed MSAA reused same render-target clear/load route preserves current and second resolved regions", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-reuse-clear-load.html",
    "mixed-msaa-reuse-clear-load-status",
  );
  ...
  expectStatusJsonSafeForGpu(status);
  expect(status, JSON.stringify(status, null, 2)).toMatchObject({
    example: "mixed-msaa-reuse-clear-load",
    ok: true,
    phase: "display",
    renderingBackend: "webgpu-explicit",
    renderTarget: {
      ...
      frames: [
        {
          ...
          centerExpectation: "current-texture-plus-msaa-clear-base-overlay",
          renderTargetKey,
          width: 256,
          height: 256,
          drawCalls: 1,
    ...
    mixedMsaaReusedSameTargetClearLoad: {
      mode: "current-texture-plus-msaa-reused-same-offscreen-render-target-clear-load",
      source: "ViewPacket.renderTarget",
      renderTargetKey,
      requestedSampleCount: 8,
      sampleCount: 4,
  ...
  await attachExampleStatus(
    "mixed-msaa-reuse-clear-load-rendered-status",
    status,
  );

  if (
    !status.readback?.ok ||
    !status.mixedMsaaReusedSameTargetClearLoad?.currentTextureReadback?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA reuse clear/load pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaReusedSameTargetClearLoad.currentTextureReadback.samples?.find(
      (entry) => entry.id === "current-texture-direct-left",
    );
  const clearSample = status.readback.samples?.find(
    (entry) => entry.id === "offscreen-clear-only",
  ...
  expect(
    clearSample,
    "expected reused MSAA target clear-only sample",
  ).toBeDefined();
  expect(
    baseSample,
    "expected reused MSAA preserved base sample",
  ).toBeDefined();
  expect(overlaySample, "expected reused MSAA overlay sample").toBeDefined();
  expect(
    screenClearSample,
    "expected reused MSAA screen clear sample",
  ).toBeDefined();
  ...
  expect(
    ...
    "reused current-texture sample should come from the direct MSAA swapchain pass",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      clearSample.pixel,
    ...
    "reused mixed MSAA clear-only sample should remain the off-screen target clear color",
  ).toBeLessThan(12);
  expect(
    pixelDistance(
      baseSample.pixel,
    ...
    "reused mixed MSAA base sample should survive the overlay load pass",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      overlaySample.pixel,
    ...
    "reused mixed MSAA overlay sample should come from the second same-target camera",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      screenClearSample.pixel,
    ...
    "reused mixed MSAA screen clear sample should stay outside the displayed preview quad",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, baseSample.pixel),
    "current-texture sample and reused preserved base sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(currentSample.pixel, overlaySample.pixel),
    "current-texture sample and reused overlay sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(baseSample.pixel, overlaySample.pixel),
    "reused mixed MSAA base and overlay samples should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(baseSample.pixel, clearSample.pixel),
    "reused mixed MSAA base sample should differ from target clear",
  ).toBeGreaterThan(40);
  expect(
    pixelDistance(overlaySample.pixel, clearSample.pixel),
    "reused mixed MSAA overlay sample should differ from target clear",
  ).toBeGreaterThan(40);
});

test("mixed MSAA resized same render-target clear/load route preserves current and resolved regions", async ({
  page,
}) => {
  const guard = attachWebGpuValidationConsoleGuard(page);
  const status = await loadExampleStatus<RenderToTextureStatus>(
    page,
    "/examples/mixed-msaa-resized-clear-load.html",
    "mixed-msaa-resized-clear-load-status",
  );
  ...
  await attachExampleStatus(
    "mixed-msaa-resized-clear-load-rendered-status",
    status,
  );

  if (
    !status.readback?.ok ||
    !status.mixedMsaaResizedSameTargetClearLoad?.currentTextureReadback?.ok
  ) {
    test.skip(
      true,
      "Mixed MSAA resized clear/load pixel assertion requires readback.",
    );
    return;
  }

  const currentSample =
    status.mixedMsaaResizedSameTargetClearLoad.currentTextureReadback.samples?.find(
      (entry) => entry.id === "current-texture-direct-left",
    );
  const clearSample = status.readback.samples?.find(
    (entry) => entry.id === "offscreen-clear-only",
  ...
  expect(
    clearSample,
    "expected resized MSAA target clear-only sample",
  ).toBeDefined();
  expect(
    baseSample,
    "expected resized MSAA preserved base sample",
  ).toBeDefined();
  expect(overlaySample, "expected resized MSAA overlay sample").toBeDefined();
  expect(
    screenClearSample,
    "expected resized MSAA screen clear sample",
  ).toBeDefined();
  ...
  expect(
    ...
    "resized current-texture sample should come from the direct MSAA swapchain pass",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      clearSample.pixel,
    ...
    "resized mixed MSAA clear-only sample should remain the off-screen target clear color",
  ).toBeLessThan(12);
  expect(
    pixelDistance(
      baseSample.pixel,
    ...
    "resized mixed MSAA base sample should survive the overlay load pass",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      overlaySample.pixel,
    ...
    "resized mixed MSAA overlay sample should come from the second same-target camera",
  ).toBeLessThan(80);
  expect(
    pixelDistance(
      screenClearSample.pixel,
    ...
    "resized mixed MSAA screen clear sample should stay outside the displayed preview quad",
  ).toBeLessThan(12);
  expect(
    pixelDistance(currentSample.pixel, baseSample.pixel),
    "resized current-texture sample and preserved base sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(currentSample.pixel, overlaySample.pixel),
    "resized current-texture sample and overlay sample should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(baseSample.pixel, overlaySample.pixel),
    "resized mixed MSAA base and overlay samples should be visually distinct",
  ).toBeGreaterThan(80);
  expect(
    pixelDistance(baseSample.pixel, clearSample.pixel),
    "resized mixed MSAA base sample should differ from target clear",
  ).toBeGreaterThan(40);
  expect(
    pixelDistance(overlaySample.pixel, clearSample.pixel),
    "resized mixed MSAA overlay sample should differ from target clear",
  ).toBeGreaterThan(40);
});
