...
function roundTripTree(value: unknown): unknown {
  return decodeTypedArrayTree(JSON.parse(JSON.stringify(encodeTypedArrayTree(value))));
}

function expectTypedArrayEqual(actual: unknown, expected: ArrayBufferView): void {
...
describe("snapshot JSON codec — generic typed-array tree", () => {
  ...
  it("walks typed arrays nested at any depth and leaves plain handles intact", () => {
    const value = {
      ...
      draws: [{ mesh: { kind: "mesh", id: "box-1" }, material: { kind: "material", id: "m" } }],
    };

    const result = roundTripTree(value) as typeof value;

    expectTypedArrayEqual(result.bounds[0]?.worldAabb, value.bounds[0]!.worldAabb);
  ...
  it("rejects an unknown typed-array tag", () => {
    expect(() =>
      decodeTypedArrayTree({ $typedArray: "BigInt64Array", base64: "AAAA" }),
...
describe("snapshot JSON codec — real extracted snapshot", () => {
  ...
  it("round-trips a real snapshot with the meshDraw and transforms preserved", async () => {
    ...
    const json = JSON.parse(JSON.stringify(renderSnapshotToJsonValue(snapshot)));
