...
describe("aperture headless command — argument handling (P1.4)", () => {
  ...
  it("requires a config path", async () => {
    await expectRejectedCode(["--out", "x.json"], "aperture.headless.missingConfig");
  });

  it("requires --out", async () => {
    await expectRejectedCode([PROCEDURAL_CONFIG], "aperture.headless.missingOutput");
  });
...
describe("aperture headless command — end to end, in-process (PA.3)", () => {
  ...
  it("writes a renderable snapshot bundle from a procedural app", async () => {
    ...
    const bundle = JSON.parse(await readFile(out, "utf8")) as {
      ...
      version: number;
      frame: number;
      snapshot: { meshDraws: unknown[] };
      sourceAssets: { entries: unknown[] };
    };
