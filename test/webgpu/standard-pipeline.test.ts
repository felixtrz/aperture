...
describe("browser standard material pipeline bridge", () => {
  ...
  it("builds a motion-vector MRT standard pipeline descriptor", () => {
    ...
    expect(shader.code).toContain(
      "fn fs_main_color(input: VertexOutput, @builtin(front_facing) frontFacing: bool) -> vec4f",
    );
    expect(shader.code).toContain(
  ...
  it("specializes StandardMaterial pipeline keys for scalar clearcoat", () => {
    const batchKey: BatchCompatibilityKey = {
      ...STANDARD_BATCH_KEY,
      pipelineKey: "standard|clearcoat|opaque|back|less|none",
    };
    const plan = createStandardPipelineDescriptorPlan({
      batchKey,
      colorFormat: "bgra8unorm",
    ...
    expect(plan).toMatchObject({
      ...
      plan: {
        keyInput: {
          shaderVariantKey: "direct-lit-metallic-roughness-clearcoat",
          vertexLayoutKey: "primitive-interleaved",
          bindGroupLayoutKeys: [
            "standard/group-0:view-uniform@0",
            "standard/group-1:world-transforms@0",
  ...
  it("specializes StandardMaterial pipeline keys for scalar iridescence", () => {
    const batchKey: BatchCompatibilityKey = {
      ...STANDARD_BATCH_KEY,
      pipelineKey: "standard|iridescence|opaque|back|less|none",
    };
    const plan = createStandardPipelineDescriptorPlan({
      batchKey,
      colorFormat: "bgra8unorm",
    ...
    expect(plan).toMatchObject({
      ...
      plan: {
        keyInput: {
          shaderVariantKey: "direct-lit-metallic-roughness-iridescence",
          vertexLayoutKey: "primitive-interleaved",
          bindGroupLayoutKeys: [
            "standard/group-0:view-uniform@0",
            "standard/group-1:world-transforms@0",
