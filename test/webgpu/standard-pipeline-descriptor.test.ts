...
describe("standard material pipeline descriptor planning", () => {
  ...
  it("specializes clustered point-cookie pipelines with a cube-texture light layout", () => {
    const batchKey = {
      ...STANDARD_BATCH_KEY,
      pipelineKey:
    ...
    expect(
      ...
      layouts: {
        bindGroups: expect.arrayContaining([
          "lights/group-3:light-floats@0,light-metadata@1,cluster-params@16,cluster-cells@17,cluster-indices@18,cluster-metadata@19,cluster-cookie-cube-texture@20,cluster-cookie-sampler@21,cluster-cookie-matrix@22",
        ]),
      },
    });
  });
  ...
  it("specializes flattened point-array shadows plus clustered cookies in one light layout", () => {
    const batchKey = {
      ...STANDARD_BATCH_KEY,
      pipelineKey:
        "standard|clusteredLocalLights|clusteredLocalLightCookies|clusteredLocalLightShadowCookies|clusteredLocalLightPointArrayShadows|clusteredLocalLightArrayShadows|pointShadowMap|shadowMap|opaque|back|less|none",
    };
    const featurePlan = createStandardPipelineShaderFeaturePlan(batchKey);
    const descriptor = createStandardPipelineDescriptorPlan({
      colorFormat: "bgra8unorm",
  ...
  it("specializes atlas local shadows plus clustered cookies in one light layout", () => {
    const batchKey = {
      ...STANDARD_BATCH_KEY,
      pipelineKey:
        "standard|clusteredLocalLights|clusteredLocalLightCookies|clusteredLocalLightShadowCookies|pointShadowMap|shadowMap|opaque|back|less|none",
    };
    const featurePlan = createStandardPipelineShaderFeaturePlan(batchKey);
    const descriptor = createStandardPipelineDescriptorPlan({
      colorFormat: "bgra8unorm",
    ...
    expect(lightGroupKey).toContain("directional-depth@3");
    expect(lightGroupKey).toContain("point-depth-cube@9");
    expect(lightGroupKey).toContain("cluster-cookie-texture@20");
    expect(lightGroupKey).toContain("cluster-cookie-sampler@21");
    expect(lightGroupKey).toContain("cluster-cookie-shadow-matrix@2");
