...
export default class VehicleSystem extends createSystem({
  ...
  #step(delta: number): void {
    ...
    if (targetSpeed < 0 && this.#linearSpeed > 0.01) {
      ...
      this.#linearSpeed = lerp(this.#linearSpeed, targetSpeed * MAX_SPEED, dt * 1.5);
    }
    ...
    this.#driftIntensity =
      Math.abs(this.#linearSpeed - this.#acceleration) + Math.abs(this.#bodyRoll) * 2;
    ...
    if (speedSignal !== undefined) {
      speedSignal.value = Number((Math.abs(this.#linearSpeed) / MAX_SPEED).toFixed(3));
    }
    // For the main-thread audio driver (src/audio.ts).
    const throttleSignal = this.signals["throttle"];
    if (throttleSignal !== undefined) throttleSignal.value = Number(inputZ.toFixed(3));
  ...
  #resolveNodes(): void {
    for (const entity of this.queries.nodes.entities) {
      if (entity.getValue(AppEntitySource, "assetId") !== PLAYER_ASSET) continue;
      const name = entity.getValue(Name, "value");
      if (name === "body") this.#body = entity;
      else if (typeof name === "string" && name.includes("wheel")) {
        this.#wheels.push(entity);
        if (name.includes("front")) this.#frontWheels.push(entity);
      }
    }
