...
// Scripted phases (by frame): idle baseline -> hover the +Z ring at ~20° ->
// press -> arc CCW through 25°/45°/65° on the ring's first-quadrant diagonal ->
// release -> settle (capture afterDrag). Positions are normalized screen coords
// for the ring points (radius 3) under the camera at [0,0,8], fovY 60,
// aspect 480/360 — a region where only the +Z ring's bounds are under the ray.
...
function frameInput(frame) {
  switch (frame) {
    case 4:
      return {
        phase: "hover",
        pointer: { x: 0.729, y: 0.389, pressed: false },
      };
    case 5:
      return { phase: "press", pointer: { x: 0.729, y: 0.389, pressed: true } };
    case 6:
      return { phase: "drag", pointer: { x: 0.721, y: 0.363, pressed: true } };
    case 7:
      return { phase: "drag", pointer: { x: 0.672, y: 0.27, pressed: true } };
    case 8:
      return { phase: "drag", pointer: { x: 0.603, y: 0.206, pressed: true } };
    case 9:
      return {
        phase: "release",
        pointer: { x: 0.603, y: 0.206, pressed: false },
      };
    default:
...
function startWorkerLoop(aperture, app, sourceAssets) {
  const worker = new Worker("/worker-modules/examples/rotate-gizmo.worker.js", {
    name: "aperture-rotate-gizmo-simulation",
    type: "module",
  });
  const loop = {
    frame: 0,
...
function createStatus(report, message, loop) {
  ...
  return {
    ...
    reason: report.ok ? undefined : "rotate-gizmo-render-failed",
    ...
    drag: {
      baselineRotation: baseline?.rotation ?? null,
      afterDragRotation: afterDrag?.rotation ?? null,
    },
    trace,
  };
}
