import { clearColor, registerSheenScene } from "./sheen-scene.js";
...
self.addEventListener("error", (event) => {
  self.postMessage({
    ...
    message: event.message || "The sheen worker raised an error.",
    location: {
...
async function handleMessage(data) {
  try {
    ...
    if (data?.type === "frame") {
      if (scene === null) {
        throw new Error("Sheen worker scene is not initialized.");
      }
