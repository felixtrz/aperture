...
self.addEventListener("error", (event) => {
  self.postMessage({
    ...
    message: event.message || "The MSAA worker raised an error.",
    location: {
...
async function handleMessage(data) {
  try {
    ...
    if (data?.type === "init") {
      scene = createWorkerScene(aperture, data.canvas ?? msaaCanvasSize);
      self.postMessage({
        type: "ready",
        scene: {
    ...
    if (data?.type === "frame") {
      if (scene === null) {
        throw new Error("MSAA worker scene is not initialized.");
      }
...
function createWorkerScene(aperture, canvasSize) {
  ...
  const registered = registerMsaaScene(aperture, app.assets);
