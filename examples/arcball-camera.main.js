...
// A 3x3 readback grid over the trackball target. The arcball drag orbits the eye
// so the unlit side marker sweeps across these points; zooming in grows the box
// so more grid points land on it.
const samplePoints = [];
...
// Scripted phases (by frame): idle baseline -> trackball drag (accumulated
// rotation) -> settle (capture afterRotate while distance is still the baseline)
// -> wheel zoom-in -> settle (capture afterZoom). Stops after the final settle.
const IDLE_END = 3;
const ROTATE_FIRST = 4;
const ROTATE_LAST = 11;
const ROTATE_SETTLE = 12;
const ZOOM_FIRST = 13;
const ZOOM_LAST = 20;
const ZOOM_SETTLE = 21;
const STOP_FRAME = 22;
const DRAG_STEP = 0.16; // normalized sphere-x advanced per rotate frame (~1.3 rad total)
const ZOOM_STEP = -0.4; // wheel per zoom frame (negative = zoom in)

const captured = { baseline: null, afterRotate: null, afterZoom: null };
const trace = [];
...
function frameInput(frame) {
  if (frame >= ROTATE_FIRST && frame <= ROTATE_LAST) {
    // A fixed center->right drag each frame accumulates a rotation about +Y.
    return {
      phase: "rotate",
      drag: { fromX: 0, fromY: 0, toX: DRAG_STEP, toY: 0 },
      wheel: 0,
    };
  }
...
function startWorkerLoop(aperture, app, sourceAssets) {
  const worker = new Worker(
    "/worker-modules/examples/arcball-camera.worker.js",
    {
      name: "aperture-arcball-camera-simulation",
      type: "module",
    },
  );
  const loop = {
...
async function handleWorkerMessage(
  ...
  const report = await app.renderSnapshot(message.snapshot, {
    ...
    clearColor,
    label: "arcball-camera-app-route",
  ...
  if (message.frame === IDLE_END) {
    captured.baseline = { samples, arcball: message.arcball };
  } else if (message.frame === ROTATE_SETTLE) {
    captured.afterRotate = { samples, arcball: message.arcball };
  } else if (message.frame === ZOOM_SETTLE) {
    captured.afterZoom = { samples, arcball: message.arcball };
  }
...
function eyeDistance(arcball) {
  if (arcball === null || arcball === undefined) {
    return null;
  }
  const eye = arcball.eye;
  return Math.hypot(eye[0], eye[1], eye[2]);
}

function createStatus(report, message, loop) {
  const baseline = captured.baseline;
  const afterRotate = captured.afterRotate;
  const afterZoom = captured.afterZoom;
  const ready = baseline !== null && afterRotate !== null && afterZoom !== null;
  const baseEye = baseline?.arcball.eye ?? null;
  const rotateEye = afterRotate?.arcball.eye ?? null;
  return {
    ...baseStatus,
    ok: report.ok,
    phase: ready ? "ready" : (message.phase ?? "render"),
    reason: report.ok ? undefined : "arcball-camera-render-failed",
    ...
    arcball: message.arcball ?? null,
    transforms: {
      eyeXBaseline: baseEye?.[0] ?? null,
      eyeXAfterRotate: rotateEye?.[0] ?? null,
      eyeZBaseline: baseEye?.[2] ?? null,
      eyeZAfterRotate: rotateEye?.[2] ?? null,
      distanceBaseline: baseline?.arcball.distance ?? null,
      distanceAfterRotate: eyeDistance(afterRotate?.arcball ?? null),
      distanceAfterZoom: afterZoom?.arcball.distance ?? null,
    },
    pixels: {
      rotateGridDelta: gridDelta(
        baseline?.samples ?? null,
        afterRotate?.samples ?? null,
      ),
      ...
      afterRotate: afterRotate?.samples ?? null,
