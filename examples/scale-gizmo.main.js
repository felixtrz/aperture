...
// drag rightward in three moves (growing world X → grow scale) -> release ->
// settle (capture afterDrag). HANDLE_X is the normalized screen x where the +X
// handle (world X = size/2 = 1.5) projects under the camera at [0,0,8], fovY 60,
// aspect 480/360.
const HANDLE_X = 0.622;
...
function frameInput(frame) {
  switch (frame) {
    ...
    case 6:
      return { phase: "drag", pointer: { x: 0.72, y: 0.5, pressed: true } };
    case 7:
      return { phase: "drag", pointer: { x: 0.85, y: 0.5, pressed: true } };
    case 8:
      return { phase: "drag", pointer: { x: 0.92, y: 0.5, pressed: true } };
    case 9:
      return { phase: "release", pointer: { x: 0.92, y: 0.5, pressed: false } };
    default:
...
function startWorkerLoop(aperture, app, sourceAssets) {
  const worker = new Worker("/worker-modules/examples/scale-gizmo.worker.js", {
    name: "aperture-scale-gizmo-simulation",
    type: "module",
  });
  const loop = {
    frame: 0,
...
function createStatus(report, message, loop) {
  ...
  return {
    ...
    reason: report.ok ? undefined : "scale-gizmo-render-failed",
    ...
    drag: {
      baselineScale: baseline?.scale ?? null,
      afterDragScale: afterDrag?.scale ?? null,
    },
    trace,
  };
}
