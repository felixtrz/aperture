...
self.addEventListener("error", (event) => {
  self.postMessage({
    ...
    message: event.message || "The clipping-cutaway worker raised an error.",
    location: {
  ...
  try {
    ...
    if (data?.type === "init") {
      scene = createWorkerScene(
        aperture,
        data.canvas ?? clippingCutawayCanvasSize,
      );
      self.postMessage({
        type: "ready",
    ...
    if (data?.type === "frame") {
      if (scene === null) {
        throw new Error("clipping-cutaway worker scene is not initialized.");
      }
...
function createWorkerScene(aperture, canvasSize) {
  ...
  const registered = registerClippingCutawayScene(aperture, app.assets);
  spawnClippingCutawayEntities(aperture, app, registered, canvasSize);

  return { app, materialKey: registered.boxMaterialKey };
}
