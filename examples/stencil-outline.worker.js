...
self.addEventListener("error", (event) => {
  self.postMessage({
    ...
    message: event.message || "The stencil-outline worker raised an error.",
    location: {
  ...
  try {
    ...
    if (data?.type === "init") {
      scene = createWorkerScene(
        aperture,
        data.canvas ?? stencilOutlineCanvasSize,
      );
      self.postMessage({
        type: "ready",
    ...
    if (data?.type === "frame") {
      if (scene === null) {
        throw new Error("stencil-outline worker scene is not initialized.");
      }
...
function createWorkerScene(aperture, canvasSize) {
  ...
  const registered = registerStencilOutlineScene(aperture, app.assets);
  spawnStencilOutlineEntities(aperture, app, registered, canvasSize);

  return { app, materialKey: registered.outlineMaterialKey };
}
