...
async function createCustomWaterScene(aperture, initialized, canvasSize) {
  const workerSnapshot = await requestCustomWaterSnapshot(aperture, canvasSize);
  ...
  const { assets, mesh, materialHandle } =
    createWaterPresentationAssets(aperture);
...
function requestCustomWaterSnapshot(aperture, canvasSize) {
      ...
      (event) => {
        ...
        if (message?.type === "error") {
          ...
          resolve({
            ...
            message: message.message ?? "The custom material worker failed.",
          });
          return;
        }
    ...
    worker.addEventListener(
      ...
      (event) => {
        ...
        resolve({
          ...
          message:
            event.message ||
            "The custom material simulation worker reported an error.",
        });
      },
      { once: true },
    );
...
function createWaterPresentationAssets(aperture) {
