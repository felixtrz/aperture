  ...
  if (canvas === null) {
    ...
    if (!created.ok) {
      ...
      startGlbViewerMain(
        ...
        created.app,
        sourceAssets,
        aperture.createKtx2TextureCompressionSupportFromFeatures(
          created.app.initialization.adapter.features,
        ),
      );
    }
  }
...
function startGlbViewerMain(aperture, app, sourceAssets, textureCompression) {
  const state = {
    app,
    sourceAssets,
    textureCompression,
    ...
    currentSourceTransfer: null,
    activeRegisteredAssetKeys: [],
    latestStatus: null,
    loading: Promise.resolve(),
    standardMaterialShadowReceiverResources: null,
    pendingControlValues: new Map(),
    pendingSampleAssetId: null,
  };
  ...
  state.worker.addEventListener("message", (event) => {
    void handleGlbWorkerMessage(aperture, state, event.data);
  });
  state.worker.addEventListener("error", (event) => {
    publishStatus(
      failure(
        "glb-viewer-worker-error",
        event.message || "The GLB viewer simulation worker failed.",
      ),
    );
  ...
  const initialAsset =
    initialCustomUrl === null
      ? initialSampleSelection.asset
      : {
          id: "custom-url",
          label: "Custom URL",
          url: initialCustomUrl,
          source: "custom",
          format: initialCustomUrl.pathname.toLowerCase().endsWith(".gltf")
            ? "gltf"
            : "glb",
        };
  const initialSceneIndex = undefined;
  ...
  state.worker.postMessage({
    type: "init",
    canvas: canvasSizeForWorker(),
    textureCompression: state.textureCompression,
    initial: {
      asset: serializeViewerAsset(initialAsset),
      keyPrefix: initialLoad.keyPrefix,
      sceneIndex: initialSceneIndex,
      sampleSelection:
        initialCustomUrl === null
          ? initialSampleSelection.status
          : emptySampleSelectionStatus(),
      importedCamera: readInitialImportedCameraControls(),
    },
  });
}

function populateMainAssetSelect() {
  if (!(assetSelect instanceof HTMLSelectElement)) {
    return;
  }
...
function bindMainControls(aperture, state) {
  if (assetSelect instanceof HTMLSelectElement) {
    assetSelect.addEventListener("change", () => {
      const asset =
        findSampleAssetById(assetSelect.value) ?? getDefaultSampleAsset();

      persistSampleAssetSelection(asset.id);
      queueMainAssetLoad(aperture, state, asset).catch((error) => {
        publishStatus(
          failure(
  ...
  if (sceneSelect instanceof HTMLSelectElement) {
    sceneSelect.addEventListener("change", () => {
      ...
      if (!Number.isInteger(sceneIndex) || state.currentAsset === null) {
        return;
      }

      queueMainAssetLoad(aperture, state, state.currentAsset, {
        sceneIndex,
      }).catch((error) => {
        publishStatus(
          failure(
  ...
  bindMainGalleryButton(aperture, state, textureGalleryPreviousButton, -1);
  bindMainGalleryButton(aperture, state, textureGalleryNextButton, 1);
  ...
  if (customUrlForm !== null) {
    customUrlForm.addEventListener("submit", (event) => {
      event.preventDefault();
      queueMainCustomUrlLoad(aperture, state).catch((error) => {
        publishStatus(
          failure(
            "glb-viewer-load-failed",
  ...
  if (cameraResetButton instanceof HTMLButtonElement) {
    cameraResetButton.addEventListener("click", () => {
      state.worker.postMessage({ type: "command", command: "camera-reset" });
    });
  }

  bindMainInputCommand(importedCameraSelect, "imported-camera-select", (node) =>
    Number(node.value),
  );
  bindMainInputCommand(importedCameraToggle, "imported-camera-toggle", (node) =>
    Boolean(node.checked),
  );
  bindMainInputCommand(shadowReceiverToggle, "shadow-receiver", (node) =>
    Boolean(node.checked),
  );
  bindMainInputCommand(shadowCasterToggle, "shadow-caster", (node) =>
    Boolean(node.checked),
  );
  bindMainInputCommand(iblToggle, "ibl", (node) => Boolean(node.checked));
  bindMainInputCommand(animationClipSelect, "animation-clip", (node) =>
    Number.parseInt(node.value, 10),
  );
  bindMainButtonCommand(animationToggleButton, "animation-toggle");
  bindMainButtonCommand(animationCrossFadeButton, "animation-cross-fade");
  bindMainInputCommand(animationLoopSelect, "animation-loop", (node) =>
    String(node.value),
  );
  bindMainInputCommand(
    animationDirectionSelect,
    "animation-direction",
    (node) => String(node.value),
  );
  bindMainInputCommand(animationScrubInput, "animation-scrub", (node) =>
  ...
  bindMainInputCommand(
    ...
    (node) => numberInputValue(node, lightingControlDefaults.pointIntensity),
  );
  bindMainInputCommand(ambientIntensityInput, "ambient-intensity", (node) =>
    numberInputValue(node, lightingControlDefaults.ambientIntensity),
  );
  bindMainOrbitControls(state);

  function bindMainInputCommand(node, command, value) {
    if (
    ...
    const eventName = node instanceof HTMLInputElement ? "input" : "change";

    node.addEventListener(eventName, () => {
      const nextValue = value(node);

      if (node instanceof HTMLInputElement && node.type === "checkbox") {
        state.pendingControlValues.set(command, nextValue);
      }
  ...
  function bindMainButtonCommand(node, command) {
    if (!(node instanceof HTMLButtonElement)) {
      return;
    }

    node.addEventListener("click", () => {
      state.worker.postMessage({ type: "command", command });
    });
  }
}

function bindMainGalleryButton(aperture, state, button, direction) {
  if (!(button instanceof HTMLButtonElement)) {
    return;
  }

  button.addEventListener("click", () => {
    queueMainGalleryOffset(aperture, state, direction).catch((error) => {
      publishStatus(
        failure(
          "glb-viewer-gallery-load-failed",
          error instanceof Error
...
function bindMainOrbitControls(state) {
  if (canvas === null) {
    return;
  }
  ...
  canvas.addEventListener("pointerdown", (event) => {
    ...
    state.worker.postMessage({ type: "command", command: "orbit-start" });
  });
  ...
  canvas.addEventListener("pointerup", (event) => {
    ...
    state.worker.postMessage({ type: "command", command: "orbit-end" });
  });
...
async function queueMainGalleryOffset(aperture, state, direction) {
  if (state.currentAsset?.source !== "sample") {
    return;
  }
  ...
  await queueMainAssetLoad(aperture, state, asset);
}

async function queueMainCustomUrlLoad(aperture, state) {
  ...
  await queueMainAssetLoad(aperture, state, {
    id: "custom-url",
    label: "Custom URL",
    url,
    source: "custom",
...
async function queueMainAssetLoad(aperture, state, asset, options = {}) {
  const load = beginMainAssetLoad(aperture, state, asset, options);

  state.pendingSampleAssetId = asset.source === "sample" ? asset.id : null;
  state.worker.postMessage({
    type: "load-asset",
    asset: serializeViewerAsset(asset),
    keyPrefix: load.keyPrefix,
...
function beginMainAssetLoad(aperture, state, asset, options = {}) {
  const loadSequence = state.loadSequence + 1;
  const keyPrefix = `viewer-${asset.id}-${loadSequence}`;

  state.loadSequence = loadSequence;
  state.currentKeyPrefix = keyPrefix;
  state.currentAsset = asset;
  state.currentSceneIndex = options.sceneIndex;
  state.currentSourceTransfer = null;
  state.workerReady = false;
  unregisterMainActiveAssets(aperture, state);
  state.standardMaterialShadowReceiverResources = null;

  return { keyPrefix };
}

function registerMainSourceAssetsFromWorker(aperture, state, message) {
  if (message.keyPrefix !== state.currentKeyPrefix) {
    return;
  }

  const assetMapping = message.assetMapping ?? null;
  const meshConstruction = message.meshConstruction ?? null;
  state.currentSourceTransfer = message.transfer ?? null;

  if (
    message.ok !== true ||
    assetMapping === null ||
    meshConstruction === null
  ) {
    publishStatus(
      failure(
        "glb-viewer-load-failed",
        message.diagnostics?.[0]?.message ??
          "glTF asset did not produce renderable source assets.",
      ),
    );
    return;
  }
  ...
  markGlbViewerDecodedTexturesLoading({
    aperture,
    registry: state.sourceAssets,
    assetMapping,
    imageDecode: message.imageDecode ?? emptyImageDecodeStatus(),
  });
  ...
  state.activeRegisteredAssetKeys = collectActiveRegisteredAssetKeys(
    registration,
    null,
    null,
  );
...
function unregisterMainActiveAssets(aperture, state) {
  for (const key of state.activeRegisteredAssetKeys) {
    const handle = assetHandleFromKey(aperture, key);

    if (handle !== null) {
      state.sourceAssets.unregister(handle);
    }
  }
...
async function handleGlbWorkerMessage(aperture, state, message) {
  if (message?.type === "source-assets") {
    registerMainSourceAssetsFromWorker(aperture, state, message);
    return;
  }

  if (message?.type === "ready") {
    state.workerReady = true;
    state.workerScene = message.scene ?? null;
    requestGlbWorkerFrame(state);
    return;
  }
  ...
  if (message?.type === "error") {
    publishStatus(
      failure(
        message.reason ?? "glb-viewer-worker-error",
        message.message ?? "The GLB viewer simulation worker failed.",
      ),
    );
    state.worker.terminate();
    return;
  ...
  state.receivedSnapshots += 1;
  const workerStatus = message.status ?? {};

  if (
    !state.workerReady ||
    !isCurrentWorkerSnapshotStatus(state, workerStatus)
  ) {
    return;
  }

  syncMainAuxiliaryAssets(aperture, state, workerStatus);

  const iblResources = createMainIblResources(aperture, state, workerStatus);
  const useShadowReceiverResources =
    workerStatus.shadow?.controls?.receiverEnabled === true &&
    workerStatus.shadow.controls.casterEnabled === true &&
    state.standardMaterialShadowReceiverResources !== null;
  const report = await state.app.renderSnapshot(message.snapshot, {
    frame: message.frame,
    clearColor,
    label: "glb-viewer-worker-snapshot",
    autoStandardMaterialShadowReceiverResources: false,
    ...(useShadowReceiverResources
      ? {
          standardMaterialShadowReceiverResources:
            state.standardMaterialShadowReceiverResources,
        }
      : {}),
    ...(iblResources === null
      ? {}
      : { standardMaterialIblResources: iblResources }),
  });
  ...
  const shadowFrame = await createViewerShadowFrame({
    ...
    reportJson,
    active:
      workerStatus.shadow?.enabled === true
        ? { shadowScene: { controls: workerStatus.shadow.controls } }
        : null,
  });
  const status = createMainFrameStatus({
    aperture,
    state,
    message,
    workerStatus,
  ...
  if ((state.latestStatus?.frame ?? -1) > (status.frame ?? -1)) {
    return;
  }

  publishStatus(status);
  updateMainControlsFromStatus(state, status);

  state.latestStatus = status;
  state.standardMaterialShadowReceiverResources =
    status.shadow?.controls?.receiverEnabled === true &&
    status.shadow?.controls?.casterEnabled === true &&
    shadowFrame !== null &&
...
function requestGlbWorkerFrame(state) {
...
function isCurrentWorkerSnapshotStatus(state, workerStatus) {
  const selectedAsset = workerStatus.selectedAsset;

  if (!isRecord(selectedAsset) || state.currentAsset === null) {
    return false;
  }

  if (
    selectedAsset.id !== state.currentAsset.id ||
    selectedAsset.source !== state.currentAsset.source
  ) {
    return false;
  }

  return (
    typeof selectedAsset.keyPrefix !== "string" ||
    selectedAsset.keyPrefix === state.currentKeyPrefix
  );
}

function syncMainAuxiliaryAssets(aperture, state, status) {
  const activeKeys = new Set(state.activeRegisteredAssetKeys);
  const fit = status.orbit?.fit ?? null;

  if (status.shadow?.enabled === true && isRecord(fit)) {
    const assets = aperture.createRenderAssetCollections({
      registry: state.sourceAssets,
    });
    const floorMesh = aperture.createMeshHandle("glb-viewer-shadow-floor");
    const floorMaterial = aperture.createMaterialHandle(
      "glb-viewer-shadow-floor-standard",
    );

    if (!state.sourceAssets.has(floorMesh)) {
      assets.meshes.add(
        aperture.createBoxMeshAsset({
          label: "GlbViewerShadowReceiverFloor",
          width: Math.max(2.4, fit.size[0] * 2.6),
          height: 0.12,
          depth: Math.max(1.8, fit.size[2] * 2.4),
        }),
        { id: "glb-viewer-shadow-floor" },
      );
    }

    if (!state.sourceAssets.has(floorMaterial)) {
    ...
    activeKeys.add(aperture.assetHandleKey(floorMesh));
    activeKeys.add(aperture.assetHandleKey(floorMaterial));
  }

  if (status.ibl?.controls?.available === true) {
    const environmentMap =
      aperture.createEnvironmentMapHandle("glb-viewer-studio");

    if (!state.sourceAssets.has(environmentMap)) {
      state.sourceAssets.register(environmentMap, {
        label: "GLB viewer studio IBL",
      });
      state.sourceAssets.markReady(environmentMap, {
        label: "GLB viewer studio IBL",
    ...
    activeKeys.add(aperture.assetHandleKey(environmentMap));
  }

  state.activeRegisteredAssetKeys = [...activeKeys];
}

function createMainIblResources(aperture, state, status) {
  if (
    status.ibl?.controls?.available !== true ||
    status.ibl.controls.enabled !== true
  ) {
    return null;
  }

  return createGlbViewerIblResources(aperture, state.app);
}

function createMainFrameStatus({
  aperture,
  state,
  message,
  workerStatus,
  ...
  return {
    ...workerStatus,
    ok: workerStatus.ok === true && report.ok,
    phase: report.ok ? "render" : "render-failed",
    frame: message.frame,
    renderingBackend: "webgpu-explicit",
    transport: {
      mode: "structured-clone-postMessage",
      jsonRoundTrip: false,
      snapshotsReceived: state.receivedSnapshots,
      typedArraysPreserved: typedSnapshot,
      sourceAssets: state.currentSourceTransfer,
    },
    worker: {
      running: state.workerReady,
      scene: state.workerScene,
    ...
    ibl: createMainIblStatus(workerStatus.ibl, iblResources, reportJson),
    shadow: createMainShadowStatus(workerStatus.shadow, shadowFrame),
    ...
    renderWorld: {
      active: state.app.renderWorld.size,
    },
    report: reportJson,
    canvas: {
...
function createMainIblStatus(workerIbl, resources, reportJson) {
  ...
  const enabled =
    workerIbl?.controls?.available === true &&
    workerIbl.controls.enabled === true;

  return {
    ...(workerIbl ?? {
      enabled: false,
      controls: { enabled: iblControls.enabled, available: false },
      ecs: {
    ...
    enabled,
    specularProof: enabled && specularRoute !== undefined,
    resources: {
    ...
    rendering: {
      supported: enabled && diffuseRoute !== undefined,
...
function createMainShadowStatus(workerShadow, shadowFrame) {
  if (workerShadow?.enabled !== true) {
    return (
      workerShadow ?? {
        enabled: false,
        controls: {
  ...
  return {
    ...
    rendering: {
      supported:
        workerShadow.controls.receiverEnabled &&
        workerShadow.controls.casterEnabled &&
...
function updateMainControlsFromStatus(state, status) {
  updateMainSceneSelectControl(status);

  if (assetSelect instanceof HTMLSelectElement) {
    const assetValue = pendingSampleAssetValue(
      state,
      status.selectedAsset?.source === "sample"
        ? status.selectedAsset.id
        : null,
    );
  ...
  if (cameraResetButton instanceof HTMLButtonElement) {
    cameraResetButton.disabled = status.orbit?.resetAvailable !== true;
  }

  setChecked(
    shadowReceiverToggle,
    pendingControlValue(
      state,
      "shadow-receiver",
      status.shadow?.controls?.receiverEnabled,
    ),
  );
  setChecked(
    shadowCasterToggle,
    pendingControlValue(
      state,
      "shadow-caster",
      status.shadow?.controls?.casterEnabled,
    ),
  );
  setChecked(
    iblToggle,
    pendingControlValue(state, "ibl", status.ibl?.controls?.enabled),
  );
  setChecked(
    importedCameraToggle,
    pendingControlValue(
      state,
      "imported-camera-toggle",
      status.importedCamera?.controls?.enabled,
    ),
  );
  setChecked(
    importedLightToggle,
  ...
  setInputValue(
    ...
    status.lighting?.controls?.ambientIntensity,
  );
  updateMainImportedCameraSelect(status);
  updateMainAnimationClipSelect(status);
}

function pendingControlValue(state, command, statusValue) {
  if (typeof statusValue !== "boolean") {
    state.pendingControlValues.delete(command);
    return statusValue;
  }

  if (!state.pendingControlValues.has(command)) {
    return statusValue;
  }

  const pendingValue = state.pendingControlValues.get(command);

  if (pendingValue === statusValue) {
    state.pendingControlValues.delete(command);
    return statusValue;
  }
...
function pendingSampleAssetValue(state, statusAssetId) {
  if (typeof statusAssetId !== "string") {
    state.pendingSampleAssetId = null;
    return null;
  }

  if (state.pendingSampleAssetId === null) {
    return statusAssetId;
  }

  if (state.pendingSampleAssetId === statusAssetId) {
    state.pendingSampleAssetId = null;
    return statusAssetId;
  }

  return state.pendingSampleAssetId;
}
...
function updateMainSceneSelectControl(status) {
  ...
  const scenes = arrayEntries(status.gltf?.metadata?.scene?.scenes);
  ...
  for (const sceneEntry of scenes) {
    ...
    option.textContent =
      sceneEntry.sceneIndex === status.gltf?.metadata?.scene?.defaultSceneIndex
        ? `${name} (default)`
        : name;
...
function updateMainImportedCameraSelect(status) {
  if (!(importedCameraSelect instanceof HTMLSelectElement)) {
    return;
  }

  const cameras = arrayEntries(status.importedCamera?.cameras).filter(
    (camera) => camera.status === "ready",
  );
  const currentValue = importedCameraSelect.value;
  ...
  importedCameraSelect.disabled = cameras.length < 2;

  for (const camera of cameras) {
    importedCameraSelect.append(
      new Option(
        camera.name ??
          camera.cameraName ??
          camera.nodeName ??
          `camera ${camera.cameraIndex}`,
        String(camera.cameraIndex),
      ),
    );
  }

  importedCameraSelect.value = String(
    status.importedCamera?.controls?.selectedCameraIndex ??
      currentValue ??
...
function updateMainAnimationClipSelect(status) {
  if (!(animationClipSelect instanceof HTMLSelectElement)) {
    return;
  }

  const clips = arrayEntries(status.animation?.clips);
  ...
  animationClipSelect.disabled = clips.length < 2;

  if (status.animation?.activeClipIndex !== undefined) {
    animationClipSelect.value = String(status.animation.activeClipIndex);
  }
}

function setChecked(node, value) {
  if (node instanceof HTMLInputElement && typeof value === "boolean") {
    node.checked = value;
  }
}
...
async function loadAsset(aperture, app, scene, asset, options = {}) {
  ...
  void options;
  throw new Error(
    "The GLB viewer main-thread asset loader is retired; source loading is worker-owned.",
  );
}
...
function bindAnimationControlInputs(aperture, scene) {
  ...
  if (animationCrossFadeButton instanceof HTMLButtonElement) {
    animationCrossFadeButton.addEventListener("click", () => {
