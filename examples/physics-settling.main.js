...
try {
  ...
  publishStatus(
    failure(
      "physics-settling-failed",
      error instanceof Error
        ? error.message
        : "Physics settling example failed.",
    ),
  );
}
...
function startWorkerSnapshotLoop(aperture, app, sourceAssets, scene) {
  const worker = new Worker(
    "/worker-modules/examples/physics-settling.worker.js",
    {
      name: "aperture-physics-settling-simulation",
      type: "module",
    },
  );
  const loop = {
...
async function handleWorkerMessage(
  ...
  const { report, reportJson } = await renderPhysicsSettlingSnapshot(
    aperture,
    app,
    message,
  );
  ...
  publishStatus(createPhysicsSettlingStatus(scene, loop, message.physics));
  worker.terminate();
}

async function renderPhysicsSettlingSnapshot(aperture, app, message) {
  const options = {
    frame: message.frame ?? 1,
    clearColor: physicsSettlingClearColor,
    label: "physics-settling",
...
function createPhysicsSettlingStatus(scene, loop, physics) {
