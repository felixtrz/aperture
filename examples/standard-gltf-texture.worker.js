    ...
    if (data?.type === "init") {
      ...
      self.postMessage({
        ...
        scene: {
          ...
          scalarControlMaterialKey:
            scene.scalarControlMaterial === null
              ? null
              : aperture.assetHandleKey(scene.scalarControlMaterial),
...
function createWorkerScene(aperture, canvasSize, scenario) {
  ...
  const registered = registerStandardGltfTextureScene(
    aperture,
    app.assets,
    scenario,
  );
  ...
  if (registered.expectedNormalScaleComparison !== null) {
    app.spawn(
      aperture.withTransform({ translation: [-0.52, 0, 0] }),
      aperture.withMesh(registered.mesh),
      aperture.withMaterial(registered.scalarControlMaterial),
    ...
    app.spawn(
      aperture.withTransform(
        registered.expectedBackface === null ? {} : { rotation: [0, 1, 0, 0] },
      ),
      aperture.withMesh(registered.mesh),
      aperture.withMaterial(registered.material),
      aperture.withRenderLayer(1),
