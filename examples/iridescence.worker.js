import { clearColor, registerIridescenceScene } from "./iridescence-scene.js";
...
self.addEventListener("error", (event) => {
  self.postMessage({
    ...
    message: event.message || "The iridescence worker raised an error.",
    location: {
...
async function handleMessage(data) {
  try {
    ...
    if (data?.type === "frame") {
      if (scene === null) {
        throw new Error("Iridescence worker scene is not initialized.");
      }
...
function createWorkerScene(aperture, canvasSize) {
  ...
  app.spawn(
    aperture.withTransform({ translation: [0.72, 0.22, 3.25] }),
    aperture.withLight({
