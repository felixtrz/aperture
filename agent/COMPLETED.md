...
- Added `?url=` handling to `examples/glb-viewer.js` so a custom `.glb` URL
  can seed the URL field and load on initial page open.
- Reused the existing custom URL loader, load-sequence guard, and ECS
  replay/unload path.
- Extended GLB viewer Playwright coverage with a query-driven initial custom
  URL load and pixel proof.
...
- `task-2020` should fit the GLB viewer orbit camera from loaded asset bounds
  instead of using a fixed camera distance.
...
- Added public runtime helpers `withShadowCaster(enabled)` and
  `withShadowReceiver(enabled)` over the existing renderer-independent shadow
  authoring components.
- Extended extracted mesh draw packets with JSON-safe `castsShadow` and
  `receivesShadow` flags.
- Updated shadow caster draw-list planning to skip ECS-authored non-casters and
  StandardMaterial shadow pipeline-key routing to skip ECS-authored
  non-receivers.
- Updated `examples/gltf-scene.js` so the receiver and caster toggles mutate
  ECS shadow authoring flags on replayed entities instead of filtering only
  example-local renderer inputs.
- Added runtime, extraction, shadow draw-list, and GLTF scene browser coverage.
...
- `task-2021` should prove IBL and shadow receivers together through the
  browser-safe StandardMaterial route.
...
- Added a custom `.glb` URL form to `examples/glb-viewer.html` with focused
  input/button styling in the shared examples stylesheet.
- Reused the existing `loadSequence` stale-load guard and ECS replay teardown
  path for both sample-selection loads and typed custom URL loads.
- Published the selected asset source and formatted URL in JSON-safe viewer
  status.
- Extended GLB viewer Playwright coverage to load a local sample through the
  custom URL control and prove the rendered pixels change after replay.
...
- In-app browser check at `http://127.0.0.1:4173/examples/glb-viewer.html`
  confirmed the custom URL status reports one mesh draw.

Known follow-up:

- `task-2019` should make the custom URL path deep-linkable from a query
  parameter.
...
- Added a browser-safe multi-shadow StandardMaterial receiver contract that can
  bind directional 2D, spot 2D, and point cube shadow resources in group 3.
- Added a combined shadow-map pipeline/layout route and WGSL variant that
  samples all three shadow families without exceeding Chrome's four bind groups.
- Added `examples/multi-light-shadow.html` / `examples/multi-light-shadow.js`
  with directional, spot, and point shadow casters feeding one receiving wall.
- Added targeted bind-group, shader, pipeline, and Playwright coverage proving
  the combined route and six named receiver samples.
- Updated the public progress trackers for the multi-light shadow milestone.
...
- Future IBL/shadow composition should prove environment lighting and shadow
  receivers together through the same browser-safe bind group boundary.
...
- Updated the public tracker and render pipeline comparison for deterministic
  route summary diagnostic-code sorting coverage.
- Refilled the ready queue with `task-1282` through `task-1286`.

Validation run:

- `pnpm run check:progress`

## task-1280 — Route summary diagnostic-code sorting regression audit
...
- Confirmed the diagnostic-code sorting regression is unit-only
  route-summary determinism coverage.
- Confirmed no app-level non-built-in route behavior, source asset contract,
  ECS component, render snapshot, WebGPU upload, shader, draw submission, or
  public API changed.
...
## task-1279 — Route summary diagnostic-code sorting regression

Completed: 2026-05-18

Summary:

- Added route summary coverage with deliberately unsorted diagnostic codes in
  prepare and frame-resource stages.
- Asserted deterministic stage-level and group-level `byCode` ordering.
- Asserted duplicate diagnostic codes merge across stages without exposing raw
...
## task-1278 — Route summary or glTF fidelity plan audit
...
- Confirmed the selected diagnostic-code sorting regression is concrete,
  bounded, and stays on the JSON-safe route-summary unit surface.
- Recommended implementing `task-1279`.

Validation run:

- Documentation-only audit; covered by final formatting and diff checks.

## task-1277 — Route summary or glTF fidelity plan after mixed aggregation
...
- Compared route summary diagnostic-code sorting, a narrow app-status route
  diagnostic, and StandardMaterial/glTF fidelity diagnostics.
- Selected diagnostic-code sorting as the next narrow follow-up.
...
## task-1276 — Tracker and backlog alignment after mixed-family route regression
...
- Updated the public tracker and render pipeline comparison for mixed-family
  route summary aggregation coverage.
- Refilled the ready queue with `task-1277` through `task-1281`.

Validation run:

- `pnpm run check:progress`

## task-1275 — Mixed-family route summary aggregation regression audit
...
- Confirmed the regression is unit-only route-summary coverage and does not add
  app-level non-built-in material rendering behavior.
- Confirmed no source asset, ECS component, render snapshot, WebGPU upload,
  pipeline, shader, draw submission, or public material-family API changed.

Validation run:

- `pnpm exec vitest run test/webgpu/queued-material-route-summary-group.test.ts`

## task-1274 — Mixed-family route summary aggregation regression

Completed: 2026-05-18

Summary:

- Added a mixed route summary group regression covering a prepared built-in
  `standard` route, a prepared test-only `preview-custom` route, and a failed
  `debug-normal` route.
- Asserted deterministic aggregate totals, valid/invalid counts, status counts,
  diagnostic code counts, and JSON-safe output without raw resource keys.

Validation run:

- `pnpm exec vitest run test/webgpu/queued-material-route-summary-group.test.ts`

## task-1273 — Next material route or glTF fidelity plan audit
...
- Confirmed the selected mixed-family route summary regression was concrete and
  stayed within JSON-safe route-summary unit coverage.
- Recommended implementing `task-1274`.

Validation run:

- Documentation-only audit; covered by final formatting and diff checks.

## task-1272 — Next material route or glTF fidelity plan
...
- Compared a mixed-family route summary aggregation regression, a narrow
  app-status route diagnostic, and a StandardMaterial/glTF fidelity diagnostic.
- Selected the mixed-family route summary aggregation regression as the next
  narrow follow-up.
...
## task-1271 — Tracker and backlog alignment after route fixture
...
- Updated the public tracker and render pipeline comparison to mention the
  test-only non-built-in route summary fixture without claiming app-level
  non-built-in material routing.
- Refilled the ready queue with `task-1272` through `task-1276`.

Validation run:

- `pnpm run check:progress`

## task-1270 — Test-only non-built-in route summary fixture audit
...
- Confirmed the `preview-custom` fixture stays unit-level and route-summary-only.
- Confirmed no source asset, ECS component, render snapshot, WebGPU upload,
  pipeline, shader, or public material-family API changed.

Validation run:

- `pnpm exec vitest run test/webgpu/queued-material-route-summary-group.test.ts`

## task-1269 — Test-only non-built-in route summary fixture

Completed: 2026-05-18

Summary:

- Added a `preview-custom` test-only material-family route summary fixture in
  `test/webgpu/queued-material-route-summary-group.test.ts`.
- Built prepare and frame-resource route shell summaries through the generic
  helpers.
- Asserted grouped JSON reports prepared totals without raw facade/backend
  resource keys or GPU handles.

Validation run:

- `pnpm exec vitest run test/webgpu/queued-material-route-summary-group.test.ts`

## task-1268 — Test-only non-built-in route summary plan audit
...
- Confirmed the selected fixture was concrete enough for one focused run and did
  not require app-level non-built-in rendering.
- Recommended implementing the unit-only route-summary fixture next.

Validation run:

- Documentation-only audit; covered by final formatting and diff checks.

## task-1267 — Test-only non-built-in route summary fixture plan
...
- Compared a unit-only route summary fixture, app-status diagnostic fixture, and
  broad app-level non-built-in route migration.
- Selected the unit-only fixture as the narrow follow-up.
...
## task-1222 — Next StandardMaterial sampler/color-space slice plan
...
- Selected StandardMaterial texture format/color-space readiness diagnostics as
  the next narrow source-side fidelity slice.
- Explicitly deferred WebGPU upload behavior, new app routes, IBL, shadows,
  binary GLB loading, and new material families.
- Updated the backlog to make `task-1225` concrete.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1223 — Prepared-resource lifetime pressure after UV1 audit
...
- Confirmed transformed UV1 did not add new prepared resource types, cache-key
  surfaces, bind group layouts, app routes, or diagnostics fields.
- Confirmed UV set and texture transform values remain source material uniform
  data and invalidate prepared StandardMaterial resources through existing
  source material versioning.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1224 — Lighting-boundary readiness contract plan
...
- Selected an environment-map readiness report as the next lighting-boundary
  contract after direct-light readiness diagnostics.
- Defined `task-1227` for a JSON-safe report derived from environment packets
  and environment resource planning.
- Kept IBL shader sampling, skyboxes, shadow maps, environment texture upload,
...
## task-1225 — StandardMaterial texture format/color-space diagnostics

Completed: 2026-05-18

Summary:

- Extended `createStandardMaterialTextureReadinessReport()` to warn when a
  ready texture slot's declared `TextureAsset.colorSpace` disagrees with
  whether `TextureAsset.format` is sRGB encoded.
- Added `actualFormat` to readiness slots and `actualFormat` plus
  `expectedFormatSrgb` to the new
  `standardMaterialTexture.invalidColorSpaceFormat` diagnostic.
- Added targeted tests for sRGB textures using non-sRGB formats and data
  textures using sRGB formats.
- Preserved existing semantic, color-space, UV, transform, texture dependency,
  and sampler dependency behavior.

Validation:

- `pnpm exec vitest run test/materials/standard-texture-readiness.test.ts`

## task-1226 — StandardMaterial texture format/color-space diagnostics audit
...
- Confirmed the implementation stayed source-side and renderer-independent:
  no WebGPU texture creation, sampler changes, source asset mutation, app route,
  IBL, shadow, or prepared-cache-key change.
- Updated the public tracker and render-pipeline comparison to reflect texture
  format/color-space diagnostics and the new environment-map readiness next
  task.
...
## task-1227 — Environment-map readiness report

Completed: 2026-05-18

Summary:

- Added `createEnvironmentMapReadinessReport()` and JSON helpers in
  `packages/webgpu/src/webgpu/environment-map-readiness.ts`.
- The report derives environment counts, null-handle counts, stable
  environment-map resource requirements, optional renderer-owned resource
  readiness, and missing-resource diagnostics from extracted environment
  packets.
- Exported the helper from `@aperture-engine/webgpu` and documented the
  diagnostics/planning-only boundary in `docs/RENDER_FRAME_READINESS.md`.
- Added focused tests for empty snapshots, null handles, repeated environment
  handles, JSON-safe output, and missing optional renderer resource state.
...
## task-1228 — Environment-map readiness report audit
...
- Confirmed the readiness helper is derived from extracted environment packets
  and optional renderer resource keys, not ECS state or app-owned scene state.
- Confirmed JSON helpers omit raw environment handles, WebGPU objects, backend
  cache maps, source payloads, queues, encoders, and devices.
- Confirmed IBL shader sampling, skyboxes, environment texture upload, shadows,
  binary GLB loading, and app route changes remain deferred.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1229 — Texture fidelity summary coverage for format diagnostics

Completed: 2026-05-18

Summary:

- Updated `createStandardMaterialTextureFidelitySummary()` so
  `standardMaterialTexture.invalidColorSpaceFormat` contributes to
  `colorSpaceIssueCount`.
- Extended the focused summary test to prove the new diagnostic appears in
  `byIssue` and remains grouped without exposing handles or source assets.
...
## task-1230 — Texture fidelity summary format diagnostic audit
...
- Confirmed the summary remains opt-in and compact, emitting only counts by
  field and diagnostic code.
- Confirmed no app-frame default report field, WebGPU upload behavior, app
  route, material family, IBL path, skybox path, or shadow path was added.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1231 — Next material route or PBR fidelity slice plan
...
- Selected a browser/status regression for
  `standardMaterialTexture.invalidColorSpaceFormat` as the next narrow vertical
  slice.
- Defined `task-1232` and follow-up route-cleanup planning tasks while keeping
  WebGPU upload behavior, app route structure changes, IBL, shadows, binary GLB
  loading, and broad material-family rewrites deferred.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1232 — StandardMaterial format/color-space browser diagnostic fixture

Completed: 2026-05-18

Summary:

- Added a glTF-shaped `base-color-format-color-space-mismatch` browser fixture
  that mutates fixture texture metadata to pair `colorSpace: "srgb"` with a
  non-sRGB `rgba8unorm` texture format.
- Verified app status exposes
  `standardMaterialTexture.invalidColorSpaceFormat` with material field,
  texture role, color-space, and format context before submitting a misleading
  successful draw.
- Kept the scenario inside existing source asset metadata and JSON-safe status
  reporting without app route, upload, IBL, shadow, binary GLB, or
...
## task-1233 — StandardMaterial format/color-space browser fixture audit
...
- Confirmed the fixture uses app status and source asset metadata, not
  renderer-owned ECS/game state or hidden scene graph behavior.
- Confirmed WebGPU upload behavior, app route structure, IBL, shadows, binary
  GLB loading, and material-family routing stayed unchanged.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1234 — Next generic material route cleanup plan
...
- Selected a reusable route scratch reset regression for the built-in app
  resource collector.
- Deferred shader feature work, IBL, shadows, binary GLB loading, GLB viewer
  behavior, and broad material-family rewrites.

Validation:

- Formatting/check validation is covered in the current run handoff.

## task-1235 — Reusable route scratch reset regression
...
- Added a focused regression in
  `test/webgpu/queued-built-in-app-resource-set.test.ts` that reuses one
  `QueuedBuiltInAppRouteCollectorScratch` and one `MaterialQueueScratch` across
  unsupported and valid collections.
- Asserted the valid StandardMaterial collection has one routed item, no
  diagnostics, no stale unsupported-family text, no stale route-report
  diagnostic, and no raw GPU-handle serialization.
- Preserved the existing generic route item shape without adding new material
  families or app route behavior.
...
## task-1236 — Generic route cleanup test audit
...
- Confirmed the regression remains generic route hygiene rather than
  StandardMaterial-only product behavior.
- Confirmed source assets remain renderer-independent and WebGPU resources stay
  backend-owned.
...
## task-1237 — Next route or fidelity slice after scratch reset plan
...
- Compared a generic route summary stale-state regression against another
  StandardMaterial/glTF fidelity diagnostic.
- Selected the route summary stale-state regression as the next slice because it
  strengthens generic material route diagnostics before more
  StandardMaterial-specific fixture expansion.

Validation:

- Documentation-only planning slice; covered by touched-file Prettier, progress
  checks, and `git diff --check` in the current run.

## task-1238 — Generic route summary stale-state regression

Completed: 2026-05-18

Summary:

- Added a reusable route report shell regression in
  `test/webgpu/material-queue-route-report.test.ts`.
- Reused one material queue route report shell across a failed route report and
  a clean routed report.
- Asserted the clean JSON report has no stale unsupported-family, skipped,
  blend-preset, diagnostic-code, or route-report state.
...
## task-1239 — Generic route summary stale-state regression audit
...
- Confirmed the route report shell regression is generic diagnostics hygiene,
  not StandardMaterial-only product behavior.
- Confirmed the change is test-only and does not move WebGPU resources into ECS
  or source assets.

Validation:

- Documentation audit backed by the `task-1238` targeted test, TypeScript,
  build, lint, and full test validation.

## task-1240 — Next StandardMaterial glTF fidelity diagnostic plan
...
- Compared unsupported required material-extension browser diagnostics,
  additional sampler/texture diagnostics, and route summary group cleanup.
- Selected unsupported required glTF material-extension browser diagnostics as
  the next fidelity slice.
...
## task-1244 — Selected StandardMaterial glTF fidelity plan audit
...
- Confirmed the unsupported required material-extension browser/status plan
  stays source-side and fixture-side.
- Confirmed the selected slice does not require WebGPU upload behavior, app
  route structure changes, IBL, shadows, binary GLB loading, GLB viewer
  behavior, or material-family routing.
...
## task-1245 — Unsupported required glTF material extension browser diagnostic

Completed: 2026-05-18

Summary:

- Added an `unsupported-required-material-extension` glTF-shaped browser
  scenario using `KHR_materials_clearcoat`.
- Preserved `extensionName` on JSON-safe asset-mapping diagnostics so browser
  status can identify the unsupported required material extension precisely.
- Added Playwright coverage proving invalid material mapping does not register
  or draw a misleading StandardMaterial and surfaces
  `gltfMaterial.unsupportedRequiredExtension` plus
...
## task-1246 — Unsupported required glTF extension browser diagnostic audit
...
- Confirmed the fixture stays inside source glTF material mapping and JSON-safe
  app/example status.
- Confirmed no WebGPU upload behavior, app route structure, IBL, shadows,
  binary GLB loading, GLB viewer behavior, material-family routing, or shader
  behavior changed.

Validation:

- Documentation audit backed by the `task-1245` targeted unit, TypeScript,
  Playwright, build, lint, and full test validation.

## task-1241 — Tracker and backlog alignment after diagnostics route cleanup
...
- Confirmed the public tracker and render-pipeline comparison reflect the
  completed diagnostics, environment readiness, route scratch/report cleanup,
  and unsupported required glTF extension browser coverage.
- Confirmed the ready backlog has five concrete follow-up tasks and no
  immediate task asks for IBL, shadows, binary GLB loading, GLB viewer behavior,
  WebGL fallback, or a mutable scene graph.
...
## task-1242 — Material-family route migration criteria audit
...
- Identified route summary group clean-after-failed coverage as the smallest
  next observable migration criterion before app-level non-built-in material
  adapter routing.
- Confirmed no source asset, ECS component, render snapshot, or WebGPU resource
  ownership boundary needs to change for that criterion.
...
## task-1243 — Route summary group clean-after-failed regression

Completed: 2026-05-18

Summary:

- Added a route summary group regression in
  `test/webgpu/queued-material-route-summary-group.test.ts`.
- Built a failed prepare/frame-resource group followed by a clean StandardMaterial
  group in the same test.
- Asserted the clean JSON group has no failed statuses, no diagnostic-code
  buckets, no stale unsupported-family text, and no raw facade/backend resource
  keys.
...
## task-1247 — Next post-extension fidelity or route slice plan
...
- Compared invalid glTF render-state browser diagnostics, more route-summary
  permutations, and a test-only non-built-in material-family route criterion.
- Selected invalid glTF render-state browser/status diagnostics as the next
  narrow follow-up and added `task-1251`.

Validation:

- `pnpm exec prettier --check docs/research/NEXT_POST_EXTENSION_FIDELITY_OR_ROUTE_SLICE_PLAN_2026_05_18.md agent/BACKLOG.md`

## task-1248 — Next post-extension slice plan audit
...
- Confirmed the selected invalid render-state fixture is concrete enough for
  one focused run.
- Confirmed the selected follow-up preserves ECS authority, render extraction,
  and WebGPU-only backend ownership.

Validation:

- `pnpm exec prettier --check docs/research/NEXT_POST_EXTENSION_SLICE_PLAN_AUDIT_2026_05_18.md`
...
- Confirmed no extra refill was needed after the selected follow-up was added.
- Recommended auditing route-summary hygiene, then returning to the selected
  StandardMaterial/glTF fidelity slice.

Validation:

- `pnpm exec prettier --check docs/research/REFILL_AFTER_MATERIAL_ROUTE_CRITERIA_AUDIT_2026_05_18.md`

## task-1250 — Route summary group clean-after-failed regression audit
...
- Confirmed the regression strengthens generic route-summary hygiene without
  changing runtime behavior.
- Confirmed no source asset, ECS component, render snapshot, app route,
  prepared-resource, WebGPU resource, IBL, shadow, GLB, or shader boundary
  changed.

Validation:

- Documentation audit backed by the `task-1243` targeted test and formatter
  checks.

## task-1251 — Invalid glTF render-state browser diagnostic fixture

Completed: 2026-05-18

Summary:

- Added an `invalid-render-state` glTF-shaped browser scenario with invalid
  `alphaMode`, `alphaCutoff`, and `doubleSided` source values.
- Preserved `value` on JSON-safe material asset-mapping diagnostics.
- Added unit and Playwright coverage proving invalid mapping skips material
  registration, extracts no mesh draw, creates no pipeline keys, submits no
  draw call, and reports `gltfMaterial.invalidField` plus
  `render.missingMaterialHandle`.
...
## task-1252 — Invalid glTF render-state browser diagnostic audit
...
- Confirmed the fixture stays in source mapping and JSON-safe example status.
- Confirmed diagnostic `value` propagation remains scalar/JSON-safe and does
  not expose raw source objects, texture bytes, sampler objects, WebGPU handles,
  or backend resources.

Validation:

- `pnpm exec prettier --check docs/research/INVALID_GLTF_RENDER_STATE_BROWSER_DIAGNOSTIC_AUDIT_2026_05_18.md`

## task-1253 — Next texture-binding or route-migration slice plan
...
- Compared unresolved glTF texture-binding browser diagnostics, a route
  migration candidate, and prepared-resource cache/lifetime work.
- Selected unresolved glTF texture-binding browser diagnostics as the next
  implementation slice.

Validation:

- `pnpm exec prettier --check docs/research/NEXT_TEXTURE_BINDING_OR_ROUTE_MIGRATION_SLICE_PLAN_2026_05_18.md`

## task-1254 — Next texture-binding or route-migration plan audit
...
- Confirmed the unresolved texture-binding fixture is concrete enough for one
  focused run.
- Confirmed the selected follow-up preserves ECS authority, render extraction,
  and WebGPU-only backend ownership.
...
- `pnpm exec prettier --check docs/research/NEXT_TEXTURE_BINDING_OR_ROUTE_MIGRATION_PLAN_AUDIT_2026_05_18.md`

## task-1255 — Unresolved glTF texture-binding browser diagnostic fixture

Completed: 2026-05-18

Summary:

- Added an `unresolved-texture-binding` glTF-shaped browser scenario whose
  base-color texture source image is missing.
- Added Playwright coverage proving invalid texture/material mapping skips
  texture, sampler, and material registration, extracts no mesh draw, creates
  no pipeline keys, and submits no draw call.
- Verified status exposes `gltfTexture.malformedImage`,
  `gltfMaterial.unresolvedTextureBinding`, and `render.missingMaterialHandle`
  without raw texture bytes or backend resources.
...
## task-1256 — Unresolved glTF texture-binding browser diagnostic audit
...
- Confirmed the fixture stays source-mapping/status-side and does not add
  renderer-owned source repair or WebGPU upload behavior.
- Confirmed JSON status remains free of raw texture bytes, sampler objects,
  WebGPU handles, and backend resource handles.

Validation:

- Documentation audit backed by the `task-1255` targeted unit, TypeScript, and
  Playwright checks.

## task-1257 — Next route, prepared-resource, or glTF fidelity slice plan
...
- Compared route migration, prepared-resource/cache reporting, and malformed
  glTF texture-info browser diagnostics.
- Selected invalid glTF texture-info browser diagnostics as the next narrow
  follow-up.

Validation:

- `pnpm exec prettier --check docs/research/NEXT_ROUTE_PREPARED_OR_GLTF_FIDELITY_SLICE_PLAN_2026_05_18.md`

## task-1258 — Next route/prepared/fidelity plan audit
...
- Confirmed malformed glTF texture-info browser diagnostics are concrete enough
  for one focused run.
- Confirmed the selected follow-up preserves ECS authority, render extraction,
  and WebGPU-only backend ownership.
...
- `pnpm exec prettier --check docs/research/NEXT_ROUTE_PREPARED_OR_GLTF_FIDELITY_PLAN_AUDIT_2026_05_18.md`

## task-1259 — Invalid glTF texture-info browser diagnostic fixture

Completed: 2026-05-18

Summary:

- Added an `invalid-texture-info` glTF-shaped browser scenario with malformed
  base-color texture-info `index` and `texCoord` values.
- Added Playwright coverage proving invalid texture-info mapping skips material
  registration, extracts no mesh draw, creates no pipeline keys, and submits no
  draw call.
- Verified status exposes `gltfMaterial.invalidTextureInfo` with JSON-safe
  field, slot, and scalar value context.
...
## task-1260 — Invalid glTF texture-info browser diagnostic audit
...
- Confirmed the fixture stays source-mapping/status-side and does not add
  renderer-owned source repair or WebGPU upload behavior.
- Confirmed JSON status stays free of raw source texture objects, texture bytes,
  sampler objects, WebGPU handles, and backend resource handles.

Validation:

- Documentation audit backed by the `task-1259` targeted unit, TypeScript, and
  Playwright checks.

## task-1261 — Tracker/backlog alignment after glTF diagnostics
...
- Updated the public tracker and render pipeline comparison for invalid
  render-state, unresolved texture-binding, and invalid texture-info browser
  diagnostics.
- Refilled the ready backlog with `task-1262` through `task-1266`.

Validation:

- `pnpm run check:progress`

## task-1262 — Next route/prepared/fidelity after diagnostics plan
...
- Compared route migration, prepared-resource/cache reporting, and remaining
  glTF fidelity work.
- Selected invalid-source no-prepared-resource browser status coverage as the
  next narrow follow-up.

Validation:

- `pnpm exec prettier --check docs/research/NEXT_ROUTE_PREPARED_OR_GLTF_FIDELITY_AFTER_DIAGNOSTICS_PLAN_2026_05_18.md`

## task-1263 — Next route/prepared/fidelity after diagnostics plan audit
...
- Confirmed the no-prepared-resource browser status assertion is concrete and
  boundary-safe.
- Recommended implementing the assertion without changing prepared-resource
  cache behavior.

Validation:

- `pnpm exec prettier --check docs/research/NEXT_ROUTE_PREPARED_OR_GLTF_FIDELITY_AFTER_DIAGNOSTICS_PLAN_AUDIT_2026_05_18.md`

## task-1264 — Invalid-source no-prepared-resource browser summary

Completed: 2026-05-18

Summary:

- Extended the invalid texture-info browser fixture to assert zero texture
  resources, sampler resources, material buffers, bind groups, pipeline keys,
  draw packages, draw commands, and draw calls.
- Made the example status report `bindGroupsCreated: 0` explicitly on no-work
  frames.
...
## task-1265 — Invalid-source no-prepared-resource browser summary audit
...
- Confirmed the status assertion describes derived renderer results without
  moving GPU/resource ownership into ECS or source assets.
- Confirmed JSON status remains free of raw GPU handles, backend resource
  objects, texture bytes, and sampler objects.

Validation:

- Documentation audit backed by the `task-1264` targeted Playwright check.

## task-1266 — Material route migration readiness after diagnostics audit
...
- Re-evaluated route migration readiness after the glTF diagnostics and
  no-prepared-resource coverage.
- Recommended planning a test-only non-built-in material adapter route summary
  fixture before broader app-level route migration.
