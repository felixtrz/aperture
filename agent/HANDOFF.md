...
- Extracted mesh extraction cache state, cache replay, cache template creation,
  and entity cache keys into
  `packages/render/src/rendering/extraction-mesh-cache.ts`.
- Moved shared vec4 packing into
  `packages/render/src/rendering/extraction-packing.ts`.
- `packages/render/src/rendering/extraction-meshes.ts` is now roughly 877
  lines.
...
- `pnpm --filter @aperture-engine/render run typecheck`
- `pnpm run typecheck:test`
- `pnpm exec eslint packages/render/src/rendering/extraction-meshes.ts packages/render/src/rendering/extraction-mesh-cache.ts packages/render/src/rendering/extraction-packing.ts packages/render/src/rendering/extraction.ts`
- `pnpm exec prettier --check packages/render/src/rendering/extraction-meshes.ts packages/render/src/rendering/extraction-mesh-cache.ts packages/render/src/rendering/extraction-packing.ts`
- `pnpm exec vitest run test/rendering/extraction.test.ts test/rendering/line-list-mesh.test.ts test/webgpu/fixtures/ecs-snapshot-render-frame.test.ts`
- `git diff --check`
...
- `packages/render/src/rendering/extraction-meshes.ts` remains the next
  mesh-specific hotspot. Good next splits are skin/morph extraction or
  instance/material-slot helper modules.
- Broad `test/webgpu/webgpu-app.test.ts` still has pre-existing resource-key
  expectation failures unrelated to these extractions; use targeted subsets
  until those expectations are updated.
...
Continue Track 5 by splitting skin/morph extraction helpers out of
`packages/render/src/rendering/extraction-meshes.ts`.
...
- Extracted mesh draw assembly, mesh extraction cache state, mesh layout key
  helpers, skin/morph/instance extraction helpers, and material-slot resolution
  into `packages/render/src/rendering/extraction-meshes.ts`.
- `packages/render/src/rendering/extraction.ts` is now roughly 287 lines and
  serves as the snapshot/view orchestration entry point.
- Preserved the existing public `createRenderExtractionCache` and
  `RenderExtractionCache` exports via re-export from `extraction.ts`.

### Validation

- `pnpm --filter @aperture-engine/render run typecheck`
- `pnpm run typecheck:test`
- `pnpm exec eslint packages/render/src/rendering/extraction.ts packages/render/src/rendering/extraction-meshes.ts packages/render/src/rendering/extraction-sprites.ts packages/render/src/rendering/extraction-skyboxes.ts packages/render/src/rendering/extraction-fogs.ts packages/render/src/rendering/extraction-lights.ts packages/render/src/rendering/extraction-matrices.ts packages/render/src/rendering/extraction-entities.ts packages/render/src/rendering/extraction-inputs.ts packages/render/src/rendering/extraction-asset-validation.ts packages/render/src/rendering/extraction-diagnostics.ts packages/render/src/rendering/extraction-culling.ts`
- `pnpm exec prettier --check packages/render/src/rendering/extraction.ts packages/render/src/rendering/extraction-meshes.ts packages/render/src/rendering/extraction-sprites.ts packages/render/src/rendering/extraction-skyboxes.ts packages/render/src/rendering/extraction-fogs.ts packages/render/src/rendering/extraction-lights.ts packages/render/src/rendering/extraction-matrices.ts packages/render/src/rendering/extraction-entities.ts packages/render/src/rendering/extraction-inputs.ts packages/render/src/rendering/extraction-asset-validation.ts packages/render/src/rendering/extraction-diagnostics.ts packages/render/src/rendering/extraction-culling.ts`
- `pnpm exec vitest run test/rendering/extraction.test.ts test/rendering/line-list-mesh.test.ts test/webgpu/fixtures/ecs-snapshot-render-frame.test.ts`
- `git diff --check`

### Known issues / remaining work

- `packages/render/src/rendering/extraction-meshes.ts` is now the next
  mesh-specific hotspot at roughly 1062 lines. Good next splits are mesh cache,
  mesh layout key helpers, skin/morph extraction, or instance/material-slot
  helper modules.
- Broad `test/webgpu/webgpu-app.test.ts` still has pre-existing resource-key
  expectation failures unrelated to these extractions; use targeted subsets
  until those expectations are updated.
...
Continue Track 5 by splitting mesh cache and mesh layout helpers out of
`packages/render/src/rendering/extraction-meshes.ts`.
...
- Extracted camera/light/sprite/skybox/fog authoring input readers, temporal
  jitter application, render-target/environment-map readers, and asset handle
  parsing into `packages/render/src/rendering/extraction-inputs.ts`.
- `packages/render/src/rendering/extraction.ts` is now roughly 1948 lines.

### Validation

- `pnpm --filter @aperture-engine/render run typecheck`
- `pnpm run typecheck:test`
- `pnpm exec eslint packages/render/src/rendering/extraction.ts packages/render/src/rendering/extraction-inputs.ts packages/render/src/rendering/extraction-asset-validation.ts packages/render/src/rendering/extraction-diagnostics.ts packages/render/src/rendering/extraction-culling.ts`
- `pnpm exec prettier --check packages/render/src/rendering/extraction.ts packages/render/src/rendering/extraction-inputs.ts packages/render/src/rendering/extraction-asset-validation.ts packages/render/src/rendering/extraction-diagnostics.ts packages/render/src/rendering/extraction-culling.ts`
- `pnpm exec vitest run test/rendering/extraction.test.ts test/rendering/line-list-mesh.test.ts test/webgpu/fixtures/ecs-snapshot-render-frame.test.ts`
- `git diff --check`
...
- Extracted StandardMaterial WGSL sampling injectors from
  `packages/webgpu/src/materials/standard/standard-shader.ts` into
  `packages/webgpu/src/materials/standard/standard-shader-sampling.ts`.
- The new module owns clearcoat, sheen, iridescence, transmission, fog,
  clustered-local light, shadow-map, point-shadow, multi-shadow, diffuse IBL,
  and specular IBL proof WGSL patching.
- `standard-shader.ts` now focuses on base WGSL, texture variant assembly,
  metadata validation, declarations, bindings, and labels.

### Validation

- `pnpm --filter @aperture-engine/webgpu run typecheck`
- `pnpm run typecheck:test`
- `pnpm exec eslint packages/webgpu/src/materials/standard/standard-shader.ts packages/webgpu/src/materials/standard/standard-shader-sampling.ts`
- `pnpm exec prettier --check packages/webgpu/src/materials/standard/standard-shader.ts packages/webgpu/src/materials/standard/standard-shader-sampling.ts`
- `pnpm exec vitest run test/webgpu/standard-shader.test.ts test/webgpu/standard-pipeline-descriptor.test.ts`
- `git diff --check`

### Known issues / remaining work

- Track 3 still needs more StandardMaterial shader splitting: base WGSL chunks,
  variant declaration/binding assembly, and shader label helpers are still in
  `standard-shader.ts`.
- `standard-shader.ts` is now roughly 2,550 lines, down from over 5,400 at the
  start of Track 3.
- Broad `test/webgpu/webgpu-app.test.ts` still has pre-existing resource-key
  expectation failures unrelated to this extraction; use targeted subsets until
  those expectations are updated.
...
Continue Track 3 by extracting StandardMaterial shader declaration/binding
assembly or base WGSL chunks from `standard-shader.ts`.
...
- Extracted draw resource set planning from
  `packages/webgpu/src/app/app.ts` into
  `packages/webgpu/src/app/draw-resource-set.ts`.
- `app.ts` now imports the helper internally and re-exports the existing public
  function and types so package-root behavior remains stable.

### Validation

- `pnpm --filter @aperture-engine/webgpu run typecheck`
- `pnpm run typecheck:test`
- `pnpm exec eslint packages/webgpu/src/app/app.ts packages/webgpu/src/app/draw-resource-set.ts`
- `pnpm exec prettier --check packages/webgpu/src/app/app.ts packages/webgpu/src/app/draw-resource-set.ts`
- `pnpm exec vitest run test/webgpu/index.test.ts`
- `pnpm exec vitest run test/webgpu/webgpu-app.test.ts --testNamePattern "reuses prepared scalar unlit mesh buffers|renders multiple unlit app resource sets|renders mixed unlit and standard app resource sets|renders mixed unlit and matcap app resource sets"`
- `git diff --check`
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA reused
  clear/load route, labels it as
  `render-to-texture/mixed-msaa-reused-clear-load-target`, creates an
  MSAA-enabled app, renders two worker snapshots through the same
  renderer-owned off-screen `ViewPacket.renderTarget` handle while each
  snapshot extracts one current-texture ECS camera plus two off-screen ECS
  cameras targeting the same handle, and clears/stores the first off-screen MSAA
  boundary before loading existing color/depth for the second same-target
  boundary.
- Added `mixedMsaaReusedSameTargetClearLoad` status with current/off-screen
...
- Added Playwright coverage proving the current-texture sample, clear-only
  region, base-preserved region, and overlay region are distinct without stale
  first-frame pixels.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3210`,
  `task-3211`, and `task-3212`.
...
- `pnpm run typecheck:test`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA reused same render-target clear/load" --project=chrome-webgpu-headed`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA reused same render-target clear/load|mixed MSAA same render-target clear/load|mixed MSAA resized same render-target clear/load|MSAA same render-target clear/load" --project=chrome-webgpu-headed`
- Browser route check for `examples/mixed-msaa-reuse-clear-load.html` reported
  `ok: true`, three MSAA target submissions, current/off-screen target
  classifications, a stable render-target key across two worker snapshots,
  target-key reuse across the two off-screen passes, pass-order clear/load ops,
  requested 8/effective 4 MSAA, resolve attachments, reuse pressure,
  current-texture readback, and distinct current, clear-only, base-preserved,
  overlay, and screen-clear samples.
- `pnpm run build`
- `pnpm run check:examples`
- `pnpm exec vitest run test/examples/navigation.test.mjs`
- `pnpm run check:progress`
- `git ls-files --modified --others --exclude-standard | rg -v '^(\\.playwright-mcp/|\\.codex/hooks\\.json$|docs/CORE_PACKAGE_REMOVAL_PLAN\\.md$)' | xargs pnpm exec prettier --check`
- `git diff --check -- ':(exclude).codex/hooks.json' ':(exclude).playwright-mcp/**' ':(exclude)docs/CORE_PACKAGE_REMOVAL_PLAN.md'`
...
Continue the visible-feature queue at `task-3210`: add a mixed current-texture
plus MSAA resized reused same-target clear/load off-screen target route.

## Current Run Update — 2026-05-26T15:03:35Z — Mixed MSAA resized reused dual-size render-target route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA resized
  reuse dual-size route, labels it as
  `render-to-texture/mixed-msaa-resized-reused-dual-size-targets`, creates an
  MSAA-enabled app, replaces the primary renderer-owned off-screen
  `ViewPacket.renderTarget` texture under the same ECS handle, renders two
  worker snapshots through the resized primary handle while also extracting a
  current-texture ECS camera and a differently sized secondary off-screen
  target, and resolves the second primary and wide secondary target textures
  into aspect-preserved visible previews.
- Added `mixedMsaaResizedReusedDualSizeRenderTargets` status with
  current/off-screen classifications, before/after dimensions, stable primary
  target key, secondary target dimensions, per-frame dimensions and draw counts,
...
- Added Playwright coverage proving the current-texture sample plus both
  resolved previews are non-clear and visually distinct without stale-size or
  stale first-frame pixels.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3209`,
  `task-3210`, and `task-3211`.
...
- `pnpm exec eslint examples/render-to-texture.main.js test/e2e/render-to-texture.spec.ts`
- `pnpm run typecheck:test`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA resized reuse dual-size" --project=chrome-webgpu-headed`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA resized reuse dual-size|mixed MSAA reuse dual-size|mixed MSAA resized dual-size|mixed MSAA dual-size" --project=chrome-webgpu-headed`
- Browser route check for
  `examples/mixed-msaa-resized-reuse-dual-size.html` reported `ok: true`, three
  MSAA target submissions, current/off-screen target classifications, stable
  128x128 to 384x384 primary render-target key resize, a stable primary
  off-screen render-target key across two worker snapshots, secondary 384x192
  target dimensions, requested 8/effective 4 MSAA, resolve attachments,
  aspect-preserving display quads, resize pressure, reuse pressure,
  current-texture readback, and distinct current, primary-preview,
  secondary-preview, and screen-clear samples.
...
Continue the visible-feature queue at `task-3209`: add a mixed current-texture
plus MSAA reused same-target clear/load off-screen target route.

## Current Run Update — 2026-05-26T14:43:17Z — Mixed MSAA resized reused viewport-cropped render-target route

Completed `task-3207`.

### What changed

- Added `examples/mixed-msaa-resized-reuse-crop.html` to the render-to-texture
  route family and linked it from related example pages.
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA resized
  reuse crop route, labels it as
  `render-to-texture/mixed-msaa-resized-reused-cropped-target`, creates an
  MSAA-enabled app, replaces a renderer-owned off-screen
  `ViewPacket.renderTarget` texture under the same ECS handle, renders two
  worker snapshots through the resized handle while also extracting a
  current-texture ECS camera, applies a non-full viewport/scissor rectangle to
  the off-screen target, and resolves the second cropped off-screen texture into
  the visible preview.
- Added `mixedMsaaResizedReusedTargetCrop` status with current/off-screen
  classifications, before/after dimensions, stable target key, crop rectangle
  and target-space pixels, per-frame dimensions and draw counts,
  requested/resolved sample count, per-pass MSAA sample count, resolve
  attachment behavior, display samples, resize pressure, reuse pressure, and
  current-texture readback.
- Added Playwright coverage proving the current-texture sample, inside-crop
  preview, outside-clear region, and screen-clear region are distinct without
  stale-size or stale first-frame pixels.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3208`,
  `task-3209`, and `task-3210`.
...
- `pnpm exec eslint examples/render-to-texture.main.js test/e2e/render-to-texture.spec.ts`
- `pnpm run typecheck:test`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA resized reuse crop" --project=chrome-webgpu-headed`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA resized reuse crop|mixed MSAA reuse crop|mixed MSAA resized crop|mixed MSAA reuse route" --project=chrome-webgpu-headed`
- Browser route check for `examples/mixed-msaa-resized-reuse-crop.html`
  reported `ok: true`, two MSAA target submissions, current/off-screen target
  classifications, stable 128x128 to 384x384 render-target key resize, a stable
  off-screen target key across two worker snapshots, target-space crop pixels,
  requested 8/effective 4 MSAA, resolve attachments, resize pressure, reuse
  pressure, current-texture readback, and distinct current, inside-crop,
...
Continue the visible-feature queue at `task-3208`: add a mixed current-texture
plus MSAA resized reused dual-size off-screen target route.

## Current Run Update — 2026-05-26T14:21:17Z — Mixed MSAA reused dual-size render-target route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA reuse
  dual-size route, labels it as
  `render-to-texture/mixed-msaa-reused-dual-size-targets`, creates an
  MSAA-enabled app, renders two worker snapshots through the same primary
  renderer-owned off-screen `ViewPacket.renderTarget` handle while also
  extracting a current-texture ECS camera and a differently sized secondary
  off-screen target, and resolves the second primary and wide secondary target
  textures into aspect-preserved visible previews.
- Added `mixedMsaaReusedDualSizeRenderTargets` status with current/off-screen
...
- Added Playwright coverage proving the current-texture sample plus both
  ...
  first-frame pixels.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3207`,
  `task-3208`, and `task-3209`.
...
- Browser route check for `examples/mixed-msaa-reuse-dual-size.html` reported
  `ok: true`, three MSAA target submissions, current/off-screen target
...
Continue the visible-feature queue at `task-3207`: add a mixed current-texture
plus MSAA resized reused viewport-cropped off-screen target route.

## Current Run Update — 2026-05-26T13:36:00Z — Mixed MSAA resized clear/load render-target route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA resized
  clear/load route, labels it as
  `render-to-texture/mixed-msaa-resized-clear-load-target`, creates an
  MSAA-enabled app, replaces a renderer-owned off-screen
  `ViewPacket.renderTarget` texture under the same ECS handle, extracts one
  current-texture ECS camera plus two off-screen ECS cameras targeting the
  resized handle, clears/stores the first off-screen MSAA boundary, loads
  existing color/depth for the second same-target boundary, and resolves the
  resized target into visible clear/base/overlay samples.
- Added `mixedMsaaResizedSameTargetClearLoad` status with current/off-screen
  ...
  current-texture readback.
- Added Playwright coverage proving the current-texture sample, clear-only
  region, base-preserved region, overlay region, and screen-clear region are
  distinct without stale-size sampling.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3206`,
  `task-3207`, and `task-3208`.
...
- `pnpm exec eslint examples/render-to-texture.main.js test/e2e/render-to-texture.spec.ts`
- `pnpm run typecheck:test`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA resized same render-target clear/load" --reporter=list`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed MSAA resized same render-target clear/load|mixed MSAA same render-target clear/load|mixed MSAA resized dual-size|mixed MSAA resized crop" --reporter=list`
- Browser route check for `examples/mixed-msaa-resized-clear-load.html`
  reported `ok: true`, three MSAA target submissions, current/off-screen target
  classifications, stable 128x128 to 384x384 render-target key resize,
  target-key reuse across the two off-screen passes, requested 8/effective 4
  MSAA, clear/load pass-order attachments, current-texture readback, and
  distinct current, clear-only, base-preserved, overlay, and screen-clear
  samples.
...
Continue the visible-feature queue at `task-3206`: add a mixed current-texture
plus MSAA reused dual-size off-screen target route.

## Current Run Update — 2026-05-26T13:16:59Z — Mixed MSAA resized dual-size render-target route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA resized
  dual-size route, labels it as
  `render-to-texture/mixed-msaa-resized-dual-size-targets`, creates an
  MSAA-enabled app, replaces the primary renderer-owned off-screen
  `ViewPacket.renderTarget` texture under the same ECS handle, extracts one
  current-texture camera plus a differently sized secondary off-screen target,
  and resolves the resized primary and wide secondary target textures into
  aspect-preserved visible previews.
- Added `mixedMsaaResizedDualSizeRenderTargets` status with current/off-screen
...
- Added Playwright coverage proving the current-texture sample plus both
  ...
  sampling.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3205`,
  `task-3206`, and `task-3207`.
...
- Browser route check for `examples/mixed-msaa-resized-dual-size.html` reported
  `ok: true`, three MSAA target submissions, current/off-screen target
  ...
  distinct current, primary-preview, secondary-preview, and screen-clear
  samples.
...
Continue the visible-feature queue at `task-3205`: add a mixed current-texture
plus MSAA resized same-target clear/load off-screen target route.
...
- Added `examples/mixed-msaa-resized-crop.html` to the render-to-texture route
  family and linked it from related example pages.
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA resized
  crop route, labels it as
  `render-to-texture/mixed-msaa-resized-cropped-target`, creates an MSAA-enabled
  app, replaces the renderer-owned off-screen `ViewPacket.renderTarget` texture
  under the same ECS handle, extracts one current-texture camera plus one
  viewport-cropped off-screen camera targeting the resized handle, and resolves
  the cropped off-screen target texture into the visible preview.
- Added `mixedMsaaResizedTargetCrop` status with current/off-screen
  classifications, before/after dimensions, stable target key, crop rectangle
...
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3203`,
  `task-3204`, and `task-3205`.
...
- Browser route check for `examples/mixed-msaa-resized-crop.html` reported
  `ok: true`, two MSAA target submissions, current/off-screen target
  classifications, stable 128x128 to 384x384 render-target key resize,
...
Continue the visible-feature queue at `task-3203`: add a mixed current-texture
plus MSAA reused viewport-cropped off-screen target route.

## Current Run Update — 2026-05-26T12:09:30Z — Mixed MSAA dual-size render-target route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA dual-size
  route, labels it as `render-to-texture/mixed-msaa-dual-size-targets`, creates
  an MSAA-enabled app, extracts one current-texture camera plus two off-screen
  cameras targeting distinct renderer-owned `ViewPacket.renderTarget` handles
  with different dimensions, and resolves the square and wide off-screen target
  textures into aspect-preserved visible previews.
- Added `mixedMsaaDualSizeRenderTargets` status with current/off-screen
...
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3202`,
  `task-3203`, and `task-3204`.
...
- Browser route check for `examples/mixed-msaa-dual-size.html` reported
  `ok: true`, three MSAA target submissions, current/off-screen target
...
- The existing standalone
  `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "MSAA render-target reuse route" --reporter=list`
  run hung independently during the previous slice after the focused mixed-route
  checks passed; this task did not touch that standalone route path.
- A broad `pnpm exec vitest run test/webgpu/frame-boundary.test.ts test/webgpu/webgpu-app.test.ts`
  run still fails 11 pre-existing `webgpu-app.test.ts` expectations around
  verbose pipeline descriptor resource keys. This task did not modify that
...
Continue the visible-feature queue at `task-3202`: add a mixed current-texture
plus MSAA resized viewport-cropped off-screen target route.
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA target
  crop route, labels it as `render-to-texture/mixed-msaa-cropped-target`,
  creates an MSAA-enabled app, extracts one current-texture camera plus one
  off-screen camera targeting a renderer-owned `ViewPacket.renderTarget` handle
  with a non-full viewport/scissor rectangle, and resolves the cropped
  off-screen target texture into the visible preview.
- Added `mixedMsaaTargetCrop` status with current/off-screen classifications,
...
- Added Playwright coverage proving the current-texture sample, inside-crop
  preview, outside-clear region, and screen clear region are non-conflicting.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3200`,
  `task-3201`, and `task-3202`.
...
- `pnpm exec vitest run test/examples/navigation.test.mjs`
- `pnpm run check:progress`
- `git ls-files --modified --others --exclude-standard | rg -v '^(\\.playwright-mcp/|\\.codex/hooks\\.json$)' | xargs pnpm exec prettier --check`
- `git diff --check -- ':(exclude).codex/hooks.json' ':(exclude).playwright-mcp/**'`
- Browser route check for `examples/mixed-msaa-target-crop.html` reported
  `ok: true`, two MSAA target submissions, current/off-screen target
...
- A broad `pnpm exec vitest run test/webgpu/frame-boundary.test.ts test/webgpu/webgpu-app.test.ts`
  ...
  area.
- The in-app browser `iab` surface was unavailable in this session, so local
  browser verification used the Playwright e2e route checks instead.
- The pre-existing working-tree deletion of `.codex/hooks.json` and untracked
  `.playwright-mcp/` scratch directory were not made by this run and remain
  untouched.
...
Continue the visible-feature queue at `task-3200`: add a mixed current-texture
plus MSAA reused off-screen target route.

## Current Run Update — 2026-05-26T11:08:30Z — Mixed MSAA same-target clear/load route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA clear/load
  route, labels it as `render-to-texture/mixed-msaa-clear-load-target`, creates
  an MSAA-enabled app, extracts one current-texture camera plus two off-screen
  cameras targeting the same renderer-owned `ViewPacket.renderTarget` handle,
  stores the first off-screen MSAA boundary, and loads existing color/depth for
  the second same-target boundary.
- Added `mixedMsaaSameTargetClearLoad` status with current/off-screen
  classifications, target-key reuse, pass-order load ops, requested/resolved
  sample count, per-pass MSAA sample count, resolve attachment behavior,
  display samples, and current-texture readback.
- Added Playwright coverage proving the current-texture sample, clear-only
  region, base-preserved region, and overlay region are non-conflicting.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3199`,
  `task-3200`, and `task-3201`.
...
- `pnpm exec vitest run test/examples/navigation.test.mjs`
- `pnpm run check:progress`
- Browser route check for `examples/mixed-msaa-clear-load.html` reported
  `ok: true`, three MSAA target submissions, current/off-screen target
...
Continue the visible-feature queue at `task-3199`: add a mixed current-texture
plus MSAA viewport-cropped off-screen target route.

## Current Run Update — 2026-05-26T10:47:58Z — Mixed MSAA resized render-target route
...
- `examples/render-to-texture.main.js` now recognizes the mixed MSAA resize
  route, labels it as `render-to-texture/mixed-msaa-resized-target`, creates an
  MSAA-enabled app, replaces the renderer-owned off-screen
  `ViewPacket.renderTarget` texture under the same ECS handle before rendering,
  and extracts one current-texture camera plus one off-screen camera targeting
  the resized handle.
- Added `mixedMsaaRenderTargetResize` status with current/off-screen
...
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3198`,
  `task-3199`, and `task-3200`.
...
- `pnpm exec vitest run test/examples/navigation.test.mjs`
- `pnpm run check:progress`
- Browser route check for `examples/mixed-msaa-resize.html` reported `ok: true`,
...
Continue the visible-feature queue at `task-3198`: add a mixed current-texture
plus MSAA same-target clear/load route.
...
- `examples/render-to-texture-assets.js` now registers a clear-colored unlit
  material for reuse-stress first frames.
- `examples/render-to-texture.worker.js` now keeps the reuse-stress mesh
  centered and swaps from the clear material on frame 1 to the green material on
  frame 2. This makes stale first-frame pixels visible without depending on a
  transform update to move the sample point.
- `examples/render-to-texture.main.js` now reports stable render-target key,
  ...
  created/reused pressure, per-frame world translation, and resolve attachment
  behavior for reuse-stress frames.
- Added Playwright coverage proving the second resolved preview is non-clear,
  ...
  pixels.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3194`,
  `task-3195`, and `task-3196`.
...
- `pnpm run typecheck:test`
- `pnpm run build`
- `pnpm exec prettier --check examples/render-to-texture-assets.js examples/render-to-texture.main.js examples/render-to-texture.worker.js test/e2e/render-to-texture.spec.ts examples/render-target-msaa-reuse.html examples/index.html examples/render-to-texture.html examples/render-target-resize.html examples/render-target-reuse.html examples/mixed-camera-targets.html examples/multi-render-targets.html examples/mixed-multi-render-targets.html examples/render-target-dual-size.html examples/mixed-dual-size-render-targets.html examples/mixed-secondary-crop-render-targets.html examples/mixed-msaa-two-targets.html examples/mixed-msaa-secondary-crop.html examples/render-target-msaa.html examples/render-target-msaa-two-targets.html examples/render-target-msaa-secondary-crop.html examples/render-target-secondary-crop.html examples/render-target-viewport-crop.html examples/render-target-clear-load.html`
...
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "MSAA render-target route|MSAA two-target|MSAA cropped secondary|mixed MSAA two-target|mixed MSAA cropped secondary" --reporter=list`
  — 5 passed.
- Browser route check for `examples/render-target-msaa-reuse.html` reported
  `ok: true`, a green resolved preview sample, and MSAA reuse pressure of one
  created and one reused color texture.

### Known issues / remaining work

- A single combined 7-test Playwright command covering the reuse pair plus the
  five adjacent MSAA routes stalled after six tests and was interrupted. The
  same coverage was rerun as focused groups above.
- Full-repo `pnpm test` and full-repo `pnpm run lint` were not rerun in this
  slice; earlier handoff notes document unrelated existing failures in both.
- Full-repo `pnpm run format:check` still has pre-existing unrelated formatting
  debt from earlier handoff notes, including untracked `.playwright-mcp/`
...
Continue the visible-feature queue at `task-3194`: add an MSAA render-target
resize preview route.

## Current Run Update — 2026-05-26T07:34:21Z — Mixed current plus MSAA cropped secondary off-screen target
...
- `examples/render-to-texture.worker.js` now supports a mixed-MSAA-cropped
  ...
  handles while only the secondary off-screen target receives a non-full
...
- `examples/render-to-texture.main.js` now creates both renderer-owned targets
  ...
  inside/outside crop samples, and reports
  `mixedMsaaCroppedSecondaryRenderTargets` status with current/off-screen target
  classifications, requested/resolved sample counts, per-target keys,
  dimensions, draw counts, MSAA sample counts, secondary target-space crop
  pixels, per-pass resolve attachment behavior, display samples, and
  current-texture readback.
...
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3193`,
  `task-3194`, and `task-3195`.
...
Continue the visible-feature queue at `task-3193`: add an MSAA render-target
reuse stress preview route.
...
- Added `examples/mixed-secondary-crop-render-targets.html` to the
  render-to-texture route family and linked it from related example pages.
- `examples/render-to-texture.worker.js` now supports a
  mixed-cropped-secondary worker variant that extracts one current-texture
  camera plus two ECS cameras targeting distinct renderer-owned off-screen
  `ViewPacket.renderTarget` handles while only the secondary off-screen target
  receives a non-full viewport/scissor rectangle.
- `examples/render-to-texture.main.js` now creates both renderer-owned targets,
  displays the primary preview plus secondary inside/outside crop samples, and
  reports `mixedCroppedSecondaryRenderTargets` status with target
  classifications, pass order, per-target keys, dimensions, draw counts,
  display samples, current-texture readback, and secondary target-space crop
  pixels.
- Added Playwright coverage proving the current-texture sample, primary
  preview, secondary inside-crop preview, and secondary outside-clear region
  behave distinctly.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3191`,
  `task-3192`, and `task-3193`.
...
- `pnpm run typecheck:test`
- `pnpm run build`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed cropped secondary" --reporter=list`
- `pnpm exec playwright test test/e2e/render-to-texture.spec.ts --grep "mixed cropped secondary|mixed dual-size render-target|mixed multi render-target|cropped secondary render-target|multiple render targets route|mixed camera targets route" --reporter=list`
  — 7 passed.
- `pnpm run check:examples`
- `pnpm exec vitest run test/examples/navigation.test.mjs` — 7 passed.
- `pnpm run check:progress`
...
Continue the visible-feature queue at `task-3191`: add a mixed current-texture
plus MSAA two-target off-screen preview route.
...
- Added `examples/render-target-msaa-two-targets.html` to the render-to-texture
  route family and linked it from related example pages.
- `examples/render-to-texture.worker.js` now reports the combined
  MSAA-plus-two-target worker variant while extracting two ECS cameras targeting
  distinct renderer-owned off-screen `ViewPacket.renderTarget` handles.
- `examples/render-to-texture.main.js` now creates both renderer-owned targets
  in an MSAA-enabled app, displays both resolved off-screen textures side by
  side, and reports `msaaMultiRenderTargets` status with requested/resolved
  sample counts, per-target keys, dimensions, draw counts, MSAA sample counts,
  and per-pass resolve attachment behavior.
- Added Playwright coverage proving both resolved previews are non-clear and
  visually distinct.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3189`,
  `task-3190`, and `task-3191`.
...
- The in-app Browser connector was unavailable in this session (`iab` not
  exposed), so local browser verification used the repo Playwright harness.
- Full-repo `pnpm test` and full-repo `pnpm run lint` were not rerun in this
  slice; earlier handoff notes document unrelated existing failures in both.
- Full-repo `pnpm run format:check` was run and still fails on pre-existing
  unrelated files, including untracked `.playwright-mcp/page-*.yml` scratch
  files and unrelated source/test files. A targeted Prettier check for all
  files touched by this slice passes.
- A broad `pnpm exec vitest run test/webgpu/frame-boundary.test.ts test/webgpu/webgpu-app.test.ts`
  run still fails 11 pre-existing `webgpu-app.test.ts` expectations around
  verbose pipeline descriptor resource keys. This task did not modify that
  area.
...
Continue the visible-feature queue at `task-3189`: add an MSAA cropped
secondary off-screen render-target preview route.

## Current Run Update — 2026-05-26T06:46:17Z — Mixed current plus dual-size off-screen targets

Completed `task-3187` after `task-3186`.
...
- `examples/render-to-texture.worker.js` now supports a mixed-dual-size mode
  ...
  square and wide renderer-owned `ViewPacket.renderTarget` handles from one
  worker-authored ECS snapshot.
- `examples/render-to-texture.main.js` now creates the square and wide
  renderer-owned targets, displays both off-screen textures with
  aspect-preserving preview quads, and reports `mixedDualSizeRenderTargets`
  status with target classifications, keys, dimensions, pass order, draw
  counts, display samples, aspect mapping, and current-texture readback.
- Added Playwright spec coverage for the route. The official Chrome-channel
  Playwright project is currently blocked in this environment because
  `browser.newPage()` hangs even for `about:blank`; a bundled Chromium
  Playwright assertion script verified the route status, aspect mapping,
  current-texture readback, and current/primary/secondary/screen pixel colors.
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3188`,
  `task-3189`, and `task-3190`.
...
- `pnpm run typecheck:test`
- Bundled Chromium Playwright assertion script for
  `/examples/mixed-dual-size-render-targets.html` — passed status, aspect, and
...
- Official Chrome-channel Playwright and render-control remain blocked in this
  environment: minimal `chromium.launch({ channel: "chrome" })` reaches
  `launched` but hangs at `browser.newPage()` for both headed and headless
  modes. Bundled Chromium can create pages and verified the new route.
- Full-repo `pnpm test` and full-repo `pnpm run lint` were not rerun in this
  slice; earlier handoff notes document unrelated existing failures in both.
- A broad `pnpm exec vitest run test/webgpu/frame-boundary.test.ts test/webgpu/webgpu-app.test.ts`
  run still fails 11 pre-existing `webgpu-app.test.ts` expectations around
...
Continue the visible-feature queue at `task-3188`: add an MSAA two-target
off-screen preview route.

## Current Run Update — 2026-05-26T06:30:15Z — Cropped secondary off-screen target

Completed `task-3186` after `task-3185`.

### What changed

- Added `examples/render-target-secondary-crop.html` to the render-to-texture
  route family and linked it from the related example pages.
- `examples/render-to-texture-assets.js` now defines stable readback samples
  for secondary inside-crop and outside-crop target regions.
...
- `examples/render-to-texture.main.js` now creates both renderer-owned targets,
  displays both textures side by side, and reports
  `croppedSecondaryRenderTargets` status with per-target keys, dimensions, draw
  counts, display samples, and resolved secondary target-space crop pixels.
...
- Updated public tracker pages, `agent/BACKLOG.md`, `agent/CURRENT_TASK.md`,
  and `agent/COMPLETED.md`. The ready queue now continues with `task-3187`,
  `task-3188`, and `task-3189`.
...
- `pnpm run render-control:smoke-all` did not complete for this slice. The
  first attempt timed out waiting for readiness on the unrelated
  `/examples/taa.html` route; a second attempt hung and was killed. A direct
  render-control navigate attempt after that also hung and was killed. The
  examples server was stopped afterward and `lsof -ti tcp:4173` returned no
  process. After formatting, a repeat focused Playwright command also hung and
  was killed after the render-control instability; the earlier focused
  Playwright route proof and the nine-route render-to-texture family proof both
  passed before formatting, and post-format syntax, ESLint, and test typecheck
  still passed.
- Full-repo `pnpm test` and full-repo `pnpm run lint` were not rerun in this
  slice; earlier handoff notes document unrelated existing failures in both.
- A broad `pnpm exec vitest run test/webgpu/frame-boundary.test.ts test/webgpu/webgpu-app.test.ts`
...
Continue the visible-feature queue at `task-3187`: add a mixed current-texture
plus dual-size off-screen render-target route.
...
Completed `task-3184` after `task-3183`.

### What changed

- Added `examples/render-target-dual-size.html` to the render-to-texture route
  family.
- `examples/render-to-texture-assets.js` now defines a stable wide secondary
  off-screen target size.
- `examples/render-to-texture.worker.js` now supports a dual-size mode that
  extracts two ECS cameras targeting distinct renderer-owned off-screen
...
- `examples/render-to-texture.main.js` now creates both renderer-owned targets,
  ...
  `dualSizeRenderTargets` status with per-target dimensions, keys, draw counts,
  display samples, and aspect mapping.
...
- `pnpm run check:progress`
- `pnpm run render-control:smoke-all` — after starting
  `pnpm run examples:serve`, the smoke visited 67 routes including
  `/examples/render-target-dual-size.html`, with zero route status failures and
...
Continue the visible-feature queue at `task-3185`: add an MSAA off-screen
render-target preview route.

## Current Run Update — 2026-05-26T05:57:14Z — Mixed current plus two off-screen targets

Completed `task-3183` after `task-3182`.
...
- `examples/render-to-texture-assets.js` now defines a third unlit material and
  stable current-texture readback sample for the three-target route.
- `examples/render-to-texture.worker.js` now supports a mixed multi-target mode
  that extracts three ECS cameras from one worker-owned world: one
  current-texture camera plus two cameras targeting distinct renderer-owned
  off-screen `ViewPacket.renderTarget` handles.
- `examples/render-to-texture.main.js` now registers the secondary target for
  the new mixed route, displays both off-screen target textures side by side,
  and reports `mixedMultiRenderTargets` status with target classifications,
  keys, pass order, draw counts, display samples, and current-texture readback.
- Added Playwright coverage proving the current-texture sample plus both
  off-screen previews are non-clear and visually distinct.
- Updated example navigation, public tracker pages, `agent/BACKLOG.md`,
  `agent/CURRENT_TASK.md`, and `agent/COMPLETED.md`.
...
- `pnpm run check:progress`
- `pnpm run render-control:smoke-all` — after starting
  `pnpm run examples:serve`, the smoke visited 66 routes including
  `/examples/mixed-multi-render-targets.html`, with zero route status failures
...
Continue the visible-feature queue at `task-3184`: add a dual-size off-screen
render-target preview route.
