...
- Material dependency readiness public contracts now live in
  `packages/render/src/materials/dependency-readiness-types.ts`, and
...
- Packed render snapshot high-level packet encoding and decoding now live in
...
- Vite plugin dev-session bridge registration, session-file writing, endpoint
  normalization, and managed-browser bridge descriptors now live in
...
- CLI dev command parsing, help text, managed-session dispatch, status/log
...
- GLB URI image decode cache/transcoder selection, per-image decode task
  handling, image source classification, bufferView/external image byte
