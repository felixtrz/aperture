...
- **Audio subsystem** — ✅ OPTED IN (2026-06-13) and largely implemented as the main-thread `@aperture-engine/audio` package: audio is a derived view of the worker-authoritative ECS (sibling of the particle pipeline). Shipped: `audio-clip` asset + decode-once cache, `AudioEmitter`/`AudioListener` intent-only components + extraction + transferable snapshot, voice reconciliation (playEpoch/stopEpoch counters, loop, mute, seen-sweep), PannerNode spatial voices + listener pose + distance/cone, voice pooling + max-voices virtualization + stealing, bus mixer (master `DynamicsCompressor` limiter + per-bus FFT taps) + scripted ducking + dual-bus music crossfade, auto-Doppler, occlusion lowpass, game-pause vs tab-suspend, suspend/resume reconciliation, captions/mono-downmix, diagnostics + latency compensation; verified by unit tests (FakeAudioBackend) and a headless `OfflineAudioContext` PCM harness (`pnpm run audio:pcm-check`). Design + phased AU-1..AU-19 roadmap: `docs/AUDIO_SUBSYSTEM_PLAN.md`. Remaining: streaming music via MediaElement (AU-10), AudioWorklet brickwall limiter (AU-13 — the master `DynamicsCompressor` already provides the degraded path), and the worker one-shot command-drain (AU-1.5) + settings/persistence (AU-15) that need the app-facade integration layer.
