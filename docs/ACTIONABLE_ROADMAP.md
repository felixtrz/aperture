...
- **Audio subsystem** — ✅ OPTED IN (2026-06-13) and **fully implemented (AU-1..AU-19)** as the main-thread `@aperture-engine/audio` package: audio is a derived view of the worker-authoritative ECS (sibling of the particle pipeline). Shipped: `audio-clip` asset + decode-once cache, `AudioEmitter`/`AudioListener` intent-only components + extraction + transferable snapshot, voice reconciliation (playEpoch/stopEpoch counters, loop, mute, seen-sweep), PannerNode spatial voices + listener pose (camera fallback) + distance/cone, voice pooling + max-voices virtualization + stealing + mid-loop resume, bus mixer (master `DynamicsCompressor` limiter + AudioWorklet brickwall upgrade + per-bus FFT taps) + scripted ducking + dual-bus music crossfade, auto-Doppler, occlusion lowpass, streaming music (MediaElement), bounded worker one-shot queue, game-pause vs tab-suspend, suspend/resume reconciliation, captions/mono-downmix, volume settings + persistence, diagnostics + latency compensation. Verified by ~60 unit tests (FakeAudioBackend) and a headless `OfflineAudioContext` PCM harness (`pnpm run audio:pcm-check`: non-silence, L/R pan, over-unity limiting). Design + phased roadmap: `docs/AUDIO_SUBSYSTEM_PLAN.md`. Follow-up: wire the engine into the live `create-webgpu-app` snapshot consumer (its public `onSnapshot` hook already lets an app drive `engine.applySnapshot`). Deferred per plan §12: reverb-zone IR pipeline, obstruction, multi-listener, mic/MediaStream, audio-effect bank, seek/scrub, beat-synced music.
