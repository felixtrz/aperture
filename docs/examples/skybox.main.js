...
try {
  ...
  if (canvas === null) {
    ...
    if (!created.ok) {
      ...
      const scene = registerSkyboxScene(aperture, sourceAssets);
  ...
  publishStatus(
    failure(
      "skybox-failed",
      error instanceof Error ? error.message : "Skybox example failed.",
    ),
  );
}

function startWorkerSnapshotLoop(aperture, app, scene) {
  const worker = new Worker("/aperture/worker-modules/examples/skybox.worker.js", {
    name: "aperture-skybox-simulation",
    type: "module",
  });
  const loop = {
    receivedSnapshots: 0,
...
async function handleWorkerMessage(
  ...
  publishStatus(createSkyboxStatus(scene, loop, reportJson.diagnostics));
  worker.terminate();
}

function createSkyboxStatus(scene, loop, diagnostics) {
  return {
    ...baseStatus,
    ok:
      loop.frame?.counts?.meshDraws === 1 &&
      loop.frame?.counts?.skyboxes === 1 &&
      loop.frame?.counts?.drawCalls === 2 &&
