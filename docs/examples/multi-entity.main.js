...
function renderDefaultMultiEntityScenario(context) {
  return renderWorkerBackedScenario(context);
}

function renderStatusScene(renderScene) {
  void renderScene;
  return renderWorkerBackedScenario;
}
...
void renderMaterialAssetStatusScene;
void renderTextureDependencyAssetStatusScene;
void renderMeshAssetStatusScene;

async function renderWorkerBackedScenario(context) {
  const workerResult = await requestMultiEntityWorkerScenario(
    context.aperture,
    context.canvasSize,
  );
  ...
  if (!workerResult.ok) {
    return {
      ...failure("worker", workerResult.reason, workerResult.message),
      ...(workerResult.diagnostics === undefined
        ? {}
        : { diagnostics: workerResult.diagnostics }),
    };
  }

  if (workerResult.result.type === "status") {
    return {
      ...workerResult.result.status,
      ...runtimeStatus(context.aperture, context.initialized),
      worker: workerResult.worker,
      transport: workerResult.transport,
    };
  }
  ...
  return renderMultiEntityScene(
    ...
    workerResult.result.scene,
    workerResult.result.snapshot,
    {
      worker: workerResult.worker,
      transport: workerResult.transport,
    },
...
function requestMultiEntityWorkerScenario(aperture, canvasSize) {
  return new Promise((resolve) => {
    const worker = new Worker(
      "/aperture/worker-modules/examples/multi-entity.worker.js",
      {
        name: "aperture-multi-entity-simulation",
        type: "module",
      },
    );
      ...
      (event) => {
        ...
        if (message?.type === "error") {
          ...
          resolve({
            ...
            message: message.message ?? "The multi-entity worker failed.",
          });
          return;
        }
        ...
        resolve({
          ...
          worker: {
            running: false,
            scenario: message.scenario ?? scenario,
            frame:
              message.result?.type === "snapshot"
                ? message.result.snapshot?.frame
                : null,
          },
          transport: {
            mode: "structured-clone-postMessage",
            jsonRoundTrip: false,
            snapshotsReceived: message.result?.type === "snapshot" ? 1 : 0,
            typedArraysPreserved:
              message.result?.type === "snapshot"
                ? inspectStructuredCloneSnapshot(message.result.snapshot)
                : null,
          },
        });
      },
    ...
    worker.addEventListener(
      ...
      (event) => {
        ...
        resolve({
          ...
          message:
            event.message ||
            "The multi-entity simulation worker reported an error.",
        });
      },
      { once: true },
    );
...
async function renderMultiEntityScene(
  ...
  const snapshot =
    inputSnapshot ??
    aperture.extractRenderSnapshot(scene.world, scene.assets, {
      frame: 1,
    });
  ...
  return {
    ...
    ...(workerMetadata?.worker === undefined
      ? {}
      : { worker: workerMetadata.worker }),
    ...(workerMetadata?.transport === undefined
      ? {}
      : { transport: workerMetadata.transport }),
    extraction: snapshotCounts(snapshot),
    resources: {
      materials: frameResources.resources.materials.length,
      textures: textureResources.textures.length,
