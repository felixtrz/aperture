...
// M3-T5: ?graph=1 folds the shadow caster passes into the forward encoder.
...
try {
  ...
  publishStatus(
    failure(
      "point-shadow-failed",
      error instanceof Error ? error.message : "Point shadow example failed.",
    ),
  );
}
...
function startWorkerSnapshotLoop(aperture, app, scene) {
  const worker = new Worker("/aperture/worker-modules/examples/point-shadow.worker.js", {
    name: "aperture-point-shadow-simulation",
    type: "module",
  });
  const loop = {
...
async function publishFrameStatus(
  ...
  const shadowSamplerResourceReport =
    aperture.createShadowSamplerResourceReport({
      device: app.initialization.device,
      resourceKey: "shadow-sampler:point",
  ...
  const shadowViewProjection =
    aperture.pointShadowViewProjectionPlanReportToJsonValue(
  ...
  const shadowMatrixComputation =
    aperture.pointShadowMatrixComputationReportToJsonValue(
  ...
  const shadowMatrixBuffer =
    aperture.shadowMatrixBufferDescriptorReportToJsonValue(
      aperture.createShadowMatrixBufferDescriptorReport({
        ...
        label: "PointShadowMatrices/storage",
      }),
    );
  ...
  publishStatus({
    ...
    shadow: {
      ...
      rendering: {
        ...
        pipelineKey: pointShadowRoute?.pipelineKey ?? null,
      },
    },
    resources: {
...
function resolvePointShadowDepthView(depthTextureResourceReport, attachment) {
...
function findPointShadowRoute(reportJson) {
  const pipelines =
