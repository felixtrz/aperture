...
const selectedMode = normalizeFogMode(
...
try {
  ...
  if (canvas === null) {
    ...
    if (!created.ok) {
      ...
      const scene = registerFogScene(aperture, sourceAssets);
  ...
  publishStatus(
    failure(
      "fog-failed",
      error instanceof Error ? error.message : "Fog example failed.",
    ),
  );
}

function startWorkerSnapshotLoop(aperture, app, scene) {
  const worker = new Worker("/aperture/worker-modules/examples/fog.worker.js", {
    name: "aperture-fog-simulation",
    type: "module",
  });
  const loop = {
    receivedSnapshots: 0,
...
async function handleWorkerMessage(
  ...
  const report = await app.renderSnapshot(message.snapshot, {
    ...
    clearColor,
    label: `fog-${selectedMode}`,
    readbackSamples: fogReadbackSamples,
  });
  ...
  publishStatus(createFogStatus(scene, loop, reportJson.diagnostics));
  worker.terminate();
}

function createFogStatus(scene, loop, diagnostics) {
  return {
    ...baseStatus,
    ok:
      loop.frame?.counts?.meshDraws === 2 &&
      loop.frame?.counts?.fogs === 1 &&
      loop.frame?.counts?.drawCalls === 1 &&
      loop.frame?.counts?.diagnostics === 0,
    phase: "submit",
