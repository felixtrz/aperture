...
OBJ/FBX/USD/STL/…, no exporters), geometry primitive breadth (20 primitives vs
21+, no shape-path/text/edges/wireframe geometry), animation depth (the mixer now does
N-lane weighted blending + additive layers (F1) and IK ships (two-bone + CCD,
F2), but it still lacks lane sync/events and arbitrary-property tracks),
helpers/gizmos breadth, and the
...
| Feature               | three.js                                                                                                                                                                   | Aperture                                                                                                                                                                                                           | Status |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| Primitives            | 21: box, capsule, circle, cone, cylinder, dodeca/icosa/octa/tetrahedron, extrude, lathe, plane, polyhedron, ring, shape, sphere, torus, torus-knot, tube, edges, wireframe | 20: box, plane, sphere, cone, cylinder, capsule, torus, line-list, circle, ring, torus-knot, polyhedron, tetra/octa/icosa/dodecahedron, rounded-box (G1), extrude, lathe, tube (G2); no shape/text/edges/wireframe | 🟡     |
| Text geometry         | `TextGeometry` (+TTF/Font loaders) addon                                                                                                                                   | — (text is MSDF quads, not extruded geometry)                                                                                                                                                                      | ❌     |
| Shape/curve extrusion | `Shape`/`ShapePath`/Earcut + extrude/lathe/tube                                                                                                                            | Curve primitives (line/quadratic/cubic Bézier, Catmull-Rom, arc) + extrude/lathe/tube builders with in-house ear-clipping triangulation and hole support (G2); no bevel, no `Shape`/`ShapePath` curve input        | 🟡     |
...
| Skinning              | `SkinnedMesh`/`Skeleton`/`Bone`, GPU palettes                                                                                                                              | `Skin` component, 2 joint/weight sets, GPU storage-buffer palettes; custom WGSL materials opt in via `skinned: true` + the `apertureSkin(...)` contract header (parity plan F3)                                    | ✅     |
...
| Helpers (13 core + addons)                                       | ✅ immediate-mode debug-draw overlay (parity plan E3, `examples/debug-draw`): `line`/`aabb`/`box`/`sphere`/`axes`/`grid`/`frustum`/`bones`/`light` from systems, rendered through the shared E1 fat-line pipeline, no-op-compiled-out in production; physics debug geometry (wireframes/contacts/AABBs/joints) re-plumbed onto the same overlay route; translate/rotate/scale transform gizmos with snapping (H2). 🟡 `ArrowHelper` via `line` + gizmo aim ray (no dedicated cone arrow); light gizmo is a frame + aim ray, not per-kind | ✅     |
...
| Controls             | 9 addons: Orbit, Map, Trackball, Arcball, Fly, FirstPerson, PointerLock, Drag, Transform | Orbit, fly/first-person, follow, pointer-lock FPS, map/pan, arcball (Shoemake, 3-DOF) controllers + translate/rotate/scale transform gizmos with snapping (H2); ECS-authoritative, headless-safe (H1) | 🟡     |
...
first-person/pointer-lock are now covered (6 controllers). H2 completed the
transform gizmo: translate **plus** rotate (axis rings, swept-angle) and scale
(axis handles + uniform), each with optional snapping and driven through the same
interaction frame — `TransformControls` parity for the gizmo itself. The remaining
breadth gap is real but narrower: no trackball-with-damping or `Drag`-controls
equivalent, none of the controllers ship momentum/damping or collision, and the
transform gizmos are world-space/world-axis only (no screen-space constant-size
handle scaling or a local/world-space toggle yet).
...
| IK                  | `CCDIKSolver` addon                                                                        | Two-bone (law-of-cosines, pole target) + CCD chain solvers as fixed-step ECS systems writing joint local transforms (F2); no rotation/angle limits | ✅     |
...
| ---------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| Visual editor    | Full web editor, PWA                                                                    | None (transform gizmos + agent devtools)                                                                                             | ❌     |
...
3. **Animation depth** — N-clip mixing + additive layers (F1) and two-bone +
   CCD IK (F2) now ship; still no arbitrary property tracks, lane sync/events,
   or retargeting (§10).
4. **Geometry authoring** — 20 primitives incl. circle/ring/torus-knot,
   platonic solids via one polyhedron builder, rounded box (G1), and curve
   primitives + extrude/lathe/tube via in-house ear clipping (G2); still no
   shape-path/text geometry, no bevel, no edges/wireframe derivation (§7).
...
8. **Camera/controls breadth** — pointer-lock/arcball/map controls (H1) and
   translate/rotate/scale transform gizmos with snapping (H2) now ship; remaining
   gaps are trackball-with-damping / `Drag` controls and no CubeCamera/StereoCamera (§9).
