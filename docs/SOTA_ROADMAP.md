...
**Last updated:** `2026-05-30 07:28 PDT — claude/opus`
...
**Active task:** `M4-T5 (in-progress: code + WGSL/pipeline unit proofs done; acne/peter-panning pixel proofs pending a dedicated grazing-plane route)`
...
**Next recommended task:** `M4-T5 (finish) — add a dedicated single flat grazing-plane shadow route (follow single-light-shadow pattern) so acne-variance (bias=0) and peter-panning (huge bias) are isolable pixel proofs; the bias code + WGSL/pipeline units are already done. Then M4-T8 (alpha-test casters, independent) and M4-T6 (cascade blend; T1+T2 done).`
...
**Gate status:** `pnpm run check = pass (384 files / 2173 tests); all 5 shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader Vulkan + xvfb): csm-directional (frustum-fit + authored params + strength=1 full-dark / strength=0 no-shadow proofs), auto-shadow, point, spot, multi-light. M4-T5 bias WGSL/pipeline unit proofs pass; T5 acne/peter-panning PIXEL proof not isolable in csm scene under SwiftShader (needs dedicated grazing-plane route — see T5 notes).`
...
| [M4](#m4)   | 1    | in-progress | 4/9        | frustum-fit / PCSS / alpha-test shadow routes ⬜                                 | M4-T1..T4 done: practical splits + frustum-fit/texel-snapped matrices + authored shadow params plumbed + strength floor removed (full-darkness reachable, per-light authored)                                                                                                                |
...
| 2026-05-30 07:28 PDT             | M4-T5  | 🟡 in-progress | 2e5ab4d  | `pnpm run check` (384 files / 2173 tests pass); `test/webgpu/{standard-shader,shadow-caster-pipeline-descriptor}.test.ts` (WGSL-gen + pipeline-descriptor bias units pass); csm-directional e2e still green                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Code complete + 2/4 done-when proven. (a) Caster pipeline depthStencil gains depthBias(int)+depthBiasSlopeScale(float) threaded from authored depthBias/slopeBias (shadow-caster-pipeline-descriptor.ts + -resource.ts). (b) Cascaded directional receiver bias reads per-light shadowDepthBias(lightIndex) from light-float slot 25 (removed the dead 0.002 const in the cascaded variant). (c) Normal-offset: cascaded sampleDirectionalShadowFactor takes the surface normal and pushes the receiver position along it by shadowNormalBias(lightIndex)\*scale before the shadow-matrix multiply (slot 26). ShadowRequestPacket + packed codec (stride 10->12) + extraction carry depthBias/normalBias; light-packing packs them (stride 25->27). WGSL-gen unit (#2) + pipeline-descriptor unit (#3) PASS. ⚠️ Acne-variance (#1) and peter-panning (#4) PIXEL proofs are NOT isolable in the csm scene under SwiftShader (region variance is dominated by box/shadow-edge geometry; bias effect too subtle) — a dedicated single flat grazing-plane route is the proper vehicle (follow-up). Non-cascaded/multi/point keep their tuned const bias (0.002/0.0001) for now. |
...
`M4 (Production-quality shadows) IN PROGRESS — 4/9 done (T1,T2,T3,T4) + T5 code-complete (pixel proof pending). Gate green: pnpm run check = 384 files / 2173 tests; all 5 shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). DONE: T2 practical cascade splits (absolute cascadeNearDistance/cascadeFarDistance matching light-packing directionalCascadeFarBounds). T1 frustum-fit cascade matrices (tryFrustumFit: camera-frustum unproject -> light-space AABB -> ortho sized to largest world diagonal = position-invariant/no-swim -> texel-snapped center; byte-identical static fallback; wired into csm route). T3 authored shadow knobs (LightShadowSettings + input + validation + extraction + ShadowRequestPacket + packed codec stride 6->12 [T3 added shadowType/strength/filterRadius/slopeBias at 6-9; T5 added depthBias/normalBias at 10-11], ENCODING_VERSION 5->6, +writeFloat32/readFloat32). T4 strength floor removal (per-light strength at light-float slot 24; mix(1-shadowStrength,1,vis) across all 4 shader variants — per-light in cascaded, first-light helpers elsewhere; demo scenes author strength 0.5-0.55; csm ?shadow-strength= proves full-dark vs no-shadow). T5 IN-PROGRESS: bias CODE done + WGSL-gen (#2) and pipeline-descriptor (#3) unit proofs PASS. Caster pipeline depthStencil gains depthBias/depthBiasSlopeScale; cascaded receiver reads per-light shadowDepthBias(slot 25); cascaded normal-offset pushes receiver pos along normal by shadowNormalBias(slot 26)*0.05 before the matrix multiply; light-packing stride 25->27. REMAINING for T5: done-when #1 (acne-variance) and #4 (peter-panning) PIXEL proofs are NOT isolable in the csm scene under SwiftShader (region variance dominated by box/shadow-edge geometry). Build a DEDICATED single flat grazing-plane shadow route (follow examples/single-light-shadow-assets.js pattern) with a ?shadow-depth-bias= param: bias=0/normalBias=0 -> high-variance acne on the flat plane; authored bias -> low variance; huge bias -> contact brightens (peter-panning). The csm worker/main ALREADY support ?shadow-depth-bias= (override threads ECS->packet->codec->light-float->shader, probe-confirmed). ALSO consider wiring per-light depthBias into the non-cascaded/multi/point shader variants (they still use the tuned 0.002/0.0001 consts). THEN remaining M4 tasks (not started): T8 alpha-test shadow casters (independent), T6 cascade blend (T1+T2 done), T7 PCSS (needs T3+T4 done), T9 shadow atlas packer (needs T3). See each task's Done-when for proofs; build pixel proofs on the csm harness or dedicated routes. ENV: WebGPU e2e turnkey via .claude/hooks/session-start.sh (installs Chrome, regenerates git-ignored playwright.local.config.ts, timeout 150s); run scripts/webgpu-e2e.sh <spec>.`
...
#### `M4-T5` · Thread authored depth bias + slope-scaled pipeline bias + normal-offset bias — 🟡 in-progress
...
- [ ] A render-control route authoring a flat receiver under a grazing-angle directional light shows shadow ACNE (self-shadow stripes) with bias=0/slopeBias=0 and clean shadows with authored bias/slopeBias>0, and the spec asserts the acne-region pixel variance drops below a threshold after bias is applied — ⚠️ NOT proven: the csm scene cannot isolate acne-stripe variance from scene geometry (box edges + shadow boundary dominate) under SwiftShader; a dedicated single-flat-grazing-plane route is the proper vehicle (follow-up). Bias data flow IS verified live: `?shadow-depth-bias=` URL param round-trips ECS→packet→codec→light-float→shader (probe-confirmed) and the WGSL/pipeline are unit-proven below.
...
- [ ] Peter-panning check: with very large bias the spec asserts the contact shadow detaches (caster base region brightens) — ⚠️ NOT proven as an isolated pixel signature under SwiftShader (same dedicated-route follow-up); bias liveness is otherwise verified by the WGSL-gen + descriptor units. — proving bias is live, not dead
