...
**Last updated:** `2026-05-30 07:55 PDT — claude/opus`
...
**Next recommended task:** `M4-T8 — alpha-tested shadow casters (UV + baseColor + discard). Fully independent of the CSM-fit work; touches the caster pipeline only. Then M4-T6 (cascade blend; T1+T2 done), M4-T7 (PCSS; T3+T4 done), M4-T9 (atlas; T3 done).`
...
**Gate status:** `pnpm run check = pass; all shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb): csm-directional (frustum-fit + authored params + strength proofs), auto-shadow, point, spot, multi-light, and shadow-bias (M4-T5 acne→clean + brightening). M4-T5 fully proven (WGSL-gen + pipeline-descriptor units + dedicated shadow-bias pixel route).`
...
| [M4](#m4)   | 1    | in-progress | 5/9        | frustum-fit / PCSS / alpha-test shadow routes ⬜                                 | M4-T1..T5 done: practical splits + frustum-fit matrices + authored params + strength floor removed + authored depth/normal/slope bias (dedicated shadow-bias acne route)                                                                                                                     |
...
`M4 (Production-quality shadows) IN PROGRESS — 5/9 done (T1-T5). Gate green; all shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb): csm-directional, auto-shadow, point, spot, multi-light, shadow-bias. DONE: T2 practical cascade splits; T1 frustum-fit cascade matrices (no swim); T3 authored shadow knobs (ShadowRequestPacket + packed codec stride 6->12, ENCODING_VERSION 5->6); T4 strength floor removal (per-light strength slot 24; mix(1-shadowStrength,1,vis) across all 4 variants); T5 authored depth/normal/slope bias (caster pipeline depthBias+depthBiasSlopeScale; cascaded per-light shadowDepthBias slot 25 + normal-offset slot 26; light-packing stride 25->27) — proven by WGSL-gen + pipeline-descriptor units AND a DEDICATED examples/shadow-bias.* grazing-floor route (test/e2e/shadow-bias.spec.ts: bias=0 acne variance>80 -> authored-bias clean <25 + brightening). NOTE: non-cascaded/multi/point shader variants still use the tuned 0.002/0.0001 receiver-bias consts (cascaded directional is the milestone proof route); wiring per-light bias into them is an optional follow-up. REMAINING M4 (not started): T8 alpha-tested shadow casters (independent — caster pipeline gets a TEXCOORD_0 + baseColor + discard variant; proof: perforated shadow on a checkerboard-alpha quad), T6 cascade blend (T1+T2 done — blend factors across cascade far-bound band; proof: continuous luminance across a boundary-spanning receiver), T7 PCSS (T3+T4 done — blocker search + variable-radius PCF behind shadowType=2; proof: contact-hardening edge-width near<far), T9 shadow atlas packer (T3 done — deterministic packer + static/once/interval update modes; vitest packer unit + multi-light atlas route). Build pixel proofs on the csm harness or dedicated routes (the shadow-bias route is a good template; SwiftShader needs robust large-delta/variance assertions, not subtle precision). ENV: WebGPU e2e turnkey via .claude/hooks/session-start.sh (installs Chrome, rebuilds references/, regenerates git-ignored playwright.local.config.ts, timeout 150s); run scripts/webgpu-e2e.sh <spec>.`
