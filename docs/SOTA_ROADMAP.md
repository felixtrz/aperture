...
**Next recommended task:** `M1-T5 — thread Draco/Meshopt/KTX2 decoder factories and GPU texture-compression support through the app glTF loader`
...
**Gate status:** `pnpm run check = pass; focused glTF quantization/root/meshopt Vitest = pass; pnpm exec tsc --noEmit = pass`
...
| ----------- | ---- | ----------- | ---------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [M1](#m1)   | 0    | in-progress | 4/11       | `auto-shadow` route + compressed-GLB load + picking + `npm publish --dry-run` ⬜ | M1-T4 quantized glTF root/accessor decode complete; app decoder factory wiring remains M1-T5 |
