...
**Last updated:** `2026-05-30 03:48 PDT — claude/opus`
...
**Next recommended task:** `M4-T3 — author + extract shadowType/strength/filterRadius/slopeBias on LightShadowSettings; unblocks T4 (strength), T5 (bias), T7 (PCSS), T9 (atlas). T6 (cascade blend) is also now unblocked (T1+T2 done) but T3 is lower task number and unblocks more.`
...
**Gate status:** `pnpm run check = pass (384 files / 2169 tests); test/e2e/csm-directional-shadow.spec.ts = pass (CSM shadow pixel proof via scripts/webgpu-e2e.sh = SwiftShader Vulkan under xvfb, now exercising the frustum-fit path)`
...
| [M4](#m4)   | 1    | in-progress | 2/9        | frustum-fit / PCSS / alpha-test shadow routes ⬜                                 | M4-T1+T2 done: practical cascade splits + per-cascade frustum-fit/texel-snapped matrices (no swim), wired into the csm route                                                                                                                                                                 |
