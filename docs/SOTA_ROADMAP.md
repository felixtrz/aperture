...
**Last updated:** `2026-05-30 04:09 PDT — claude/opus`
...
**Next recommended task:** `M4-T4 — authored shadow strength replaces the MIN_VISIBILITY floor (depends T3, done). Pack strength into the light float buffer and replace the baked 0.45/0.5 mix floors with mix(1-strength,1,visibility). T5/T8 also ready.`
...
**Gate status:** `pnpm run check = pass (384 files / 2172 tests); test/e2e/csm-directional-shadow.spec.ts = pass (CSM shadow pixel proof via scripts/webgpu-e2e.sh = SwiftShader Vulkan under xvfb; frustum-fit path + authored shadowType/strength/filterRadius/slopeBias surfaced in status)`
...
| [M4](#m4)   | 1    | in-progress | 3/9        | frustum-fit / PCSS / alpha-test shadow routes ⬜                                 | M4-T1+T2+T3 done: practical splits + frustum-fit/texel-snapped matrices (no swim) + authored shadowType/strength/filterRadius/slopeBias plumbed ECS→codec→status                                                                                                                             |
...
`M4 (Production-quality shadows) IN PROGRESS — 3/9. M4-T3 (authored shadow knobs) shipped: LightShadowSettings + input + validation + extraction + ShadowRequestPacket + packed codec (stride 6->10, ENCODING_VERSION 5->6, +writeFloat32/readFloat32) now carry shadowType/strength/filterRadius/slopeBias end-to-end (omitted at defaults for minimal packets); no renderer consumer yet (T4/T5/T7). M4-T1 (frustum-fit cascade matrices) shipped: tryFrustumFit in directional-shadow-matrix-computation.ts unprojects the camera frustum, lerps each cascade's [nearDist,farDist] slice (from M4-T2's absolute distances), fits a light-space AABB, sizes the ortho to the slice's largest world diagonal (ceil → position-invariant extent, no swim), and snaps the in-plane center to texel multiples; byte-identical static-center fallback when no camera frustum supplied; wired into examples/csm-directional-shadow.main.js via the primary view's snapshot matrices. M4-T2 (practical cascade splits) shipped: directional-shadow-view-projection-plan.ts cascadeSplit() now emits absolute cascadeNearDistance/cascadeFarDistance using the same linear/log-average formula as light-packing directionalCascadeFarBounds (so selection and matrix-fit agree), keeping the normalized cascadeNear/cascadeFar fractions for back-compat. Gate green (pnpm run check = 384 files / 2166 tests); csm-directional-shadow.spec.ts passes via scripts/webgpu-e2e.sh (SwiftShader Vulkan + xvfb, regenerated playwright.local.config.ts). NEXT: M4-T4 (authored shadow strength replaces the MIN_VISIBILITY floor) — webgpu-render: pack per-light strength into a free light-float slot in light-packing.ts (audit PACKED_LIGHT_FLOAT_STRIDE; do NOT collide with cascade far-bounds offset+6..+9 or matrixBaseIndex offset+10) and replace the 6 baked mix(0.45/0.5,1,visibility) sites in standard-shader-shadow-sampling.ts with mix(1-shadowStrength(i),1,visibility). The csm scene already authors strength=0.9, so its shadow will darken slightly when this lands (csm spec uses maxDelta>10, still green). ENV NOTE: WebGPU e2e is now turnkey — .claude/hooks/session-start.sh installs Chrome + regenerates playwright.local.config.ts; run proofs with scripts/webgpu-e2e.sh <spec>.`
