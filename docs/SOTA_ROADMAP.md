...
| 2026-05-29 00:02 PDT             | M1-T3 | ✅ done  | pending  | `pnpm exec vitest run test/webgpu/shadow-pass-plan.test.ts test/webgpu/shadow-pass-encoder-assembly-report.test.ts test/webgpu/shadow-pass-command-buffer-submission-report.test.ts test/webgpu/shadows/render-shadow-frame.spec.ts test/webgpu/webgpu-app.test.ts --testNamePattern "shadow pass planning\|shadow pass encoder assembly\|ShadowPassCommandBufferSubmissionReport\|render shadow frame\|auto-renders directional shadow resources"`; `CI=true pnpm exec playwright test test/e2e/auto-shadow.spec.ts --reporter=list --timeout=45000`; `CI=true pnpm exec playwright test test/e2e/point-shadow.spec.ts --reporter=list --timeout=90000`; `pnpm run check` | Widened shadow report shaderSampling fields to booleans, reports true on ready/submitted shadow paths, removes submitted/assembled shaderSamplingDeferred diagnostics, and proves auto-shadow JSON reports shaderSampling true. Spot route status probe passed; the full spot Playwright spec hung before result in this environment. |
...
#### `M1-T3` · Enable shaderSampling status and make full shadow darkness reachable end-to-end — ✅ done (2026-05-29 · pending)
