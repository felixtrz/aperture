...
**Last updated:** `2026-05-30 13:02 PDT — claude/opus`
...
**Next recommended task:** `M5-T4 — persistent rgba16float HDR scene buffer + exposure, move tonemap to a final post stage (no deps, no asset blocker). M5-T6 (SSAO→indirect-only) is also ready (no deps). M5 at 3/6 done; T5 depends on T4.`
...
| [M5](#m5)   | 1    | in-progress | 3/6        | DFG / irradiance / transmission correctness routes ⬜                            | T1 ✅ split-sum DFG (ibl-brdf). T2 ✅ cosine irradiance-convolution (ibl-irradiance). T3 ✅ equirect→cube single-asset auto-IBL (ibl-equirect: one .hdr → project → PMREM+convolution → mirror reflection). Next: T4 (HDR buffer+exposure), T6 (SSAO→indirect).                              |
...
`M5-T1 + M5-T2 + M5-T3 ✅ done; M5 in progress (3/6). Gate green (pnpm run check = 390 files / 2200 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect}.spec.ts pass under SwiftShader+xvfb. T1: split-sum environment-BRDF (DFG) — brdf-lut-compute-pipeline.ts + brdf-lut-resource.ts; iblSpecularBrdf shader variant (analytic Karis DFGApprox) supersedes iblSpecularProof, reusing the specular cube (group3 binding7); examples/ibl-brdf. T2: cosine irradiance convolution — irradiance-convolution-compute-pipeline.ts; createDiffuseIblTextureResourceReport convolves by default (32² cube) when compute is available, opt-out convolveIrradiance:false; examples/ibl-irradiance. T3: equirect→cube single-asset auto-IBL — equirect-to-cube-compute-pipeline.ts (+Z-centered UV u=atan2(x,z)/2π+0.5,v=asin(y)/π+0.5) + equirect-to-cube-resource.ts (projects to a cube with COPY_SRC); examples/ibl-equirect synthesizes a 2:1 flat-RGBE .hdr in-JS → loadHdrFromUri → project → readback 6 faces → PMREM + T2 convolution → mirror sphere. DESIGN NOTES: T1 uses the SOTA-sanctioned analytic DFGApprox (GPU LUT built+validated+reported, not sampled at a binding-8 — optional follow-up; DECISIONS §0017). T3 demonstrates the single-asset auto-chain in the example's resource-building; a public equirectSource field on app-environment-resources.ts is an optional equivalent follow-up. NEXT: M5-T4 (persistent rgba16float HDR scene buffer + exposure, tonemap→final post stage; no deps, no asset blocker). T4 plan (roadmap M5-T4 body): allocate the post scene texture rgba16float (post-processing.ts ~72-81), add an exposure scalar (output-stage-tonemap.ts createOutputTonemapWgsl `_ exposure`), createWebGpuTonemapPostEffect (post-pass.ts contract) as the final HDR→swapchain stage, gate applyOutputTonemapToStandardShader off when the HDR path is active (standard-pipeline.ts:113), surface exposure on CreateWebGpuAppOptions (app.ts:457-474); proof examples/hdr-exposure._ exposure sweep. WATCH: lit-pass color target format becomes rgba16float (scope opt-in to limit blast radius); keep the FINAL stage at swapchain format for readback. Then M5-T6 (SSAO→indirect-only; effort S but needs a depth-prepass or an MRT split — see its watch-out) and M5-T5 (refractive transmission; depends T4). ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present.`
