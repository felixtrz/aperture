...
**Last updated:** `2026-05-30 03:32 PDT — claude/opus`
...
**Next recommended task:** `M4-T1 — frustum-fit + texel-stabilized cascade matrices; consumes the absolute cascadeNearDistance/cascadeFarDistance just added by M4-T2, and unblocks M4-T6 (cascade blend).`
...
**Gate status:** `pnpm run check = pass (384 files / 2166 tests); test/e2e/csm-directional-shadow.spec.ts = pass (CSM shadow pixel proof via scripts/webgpu-e2e.sh = SwiftShader Vulkan under xvfb)`
...
| [M4](#m4)   | 1    | in-progress | 1/9        | frustum-fit / PCSS / alpha-test shadow routes ⬜                                 | M4-T2 done: practical (linear/log) cascade splits emit absolute near/far distances agreeing with shader-side selection bounds                                                                                                                                                                |
...
`M4 (Production-quality shadows) IN PROGRESS — 1/9. M4-T2 (practical cascade splits) shipped: directional-shadow-view-projection-plan.ts cascadeSplit() now emits absolute cascadeNearDistance/cascadeFarDistance using the same linear/log-average formula as light-packing directionalCascadeFarBounds (so selection and matrix-fit agree), keeping the normalized cascadeNear/cascadeFar fractions for back-compat. Gate green (pnpm run check = 384 files / 2166 tests); csm-directional-shadow.spec.ts passes via scripts/webgpu-e2e.sh (SwiftShader Vulkan + xvfb, regenerated playwright.local.config.ts). NEXT: M4-T1 (frustum-fit + texel-stabilized cascade matrices) — extend DirectionalShadowMatrixComputationInput with camera view/projection matrices and consume the new plan.cascadeFarDistance/cascadeNearDistance to build per-cascade frustum slices + light-space AABB + texel snapping (bevy cascade.rs calculate_cascade); fall back to DEFAULT_CENTER byte-identically when no camera frustum supplied. ENV NOTE: WebGPU e2e is now turnkey — .claude/hooks/session-start.sh installs Chrome + regenerates playwright.local.config.ts; run proofs with scripts/webgpu-e2e.sh <spec>.`
