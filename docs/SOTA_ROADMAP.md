...
**Last updated:** `2026-05-30 10:47 PDT — claude/opus`
...
**Next recommended task:** `M5-T1 — split-sum environment-BRDF (DFG) LUT compute pass + bind into specular IBL. M4 is complete (9/9); wave 1 still has M5 (PBR/IBL correctness) not-started, so the wave does not advance yet.`
...
**Gate status:** `pnpm run check = pass (385 files / 2176 tests); ALL M4 proofs green via scripts/webgpu-e2e.sh (SwiftShader + xvfb): csm-directional (frustum-fit + authored params + strength + cascade continuity), shadow-bias (acne + cascade blend + PCSS contact-hardening), alpha-shadow (perforated alpha-test caster), atlas-shadow (shared atlas + once-skip), auto-shadow, point, spot, multi-light. M4 COMPLETE — every task ✅ done with passing named proof.`
...
`clean — M4 (Production-quality shadows) is COMPLETE: all 9 tasks ✅ done and gate-green (pnpm run check = 385 files / 2176 tests). T1 frustum-fit/texel-snapped CSM matrices (no swim); T2 practical linear/log cascade splits; T3 authored shadowType/strength/filterRadius/slopeBias/depthBias/normalBias on ShadowRequestPacket + packed codec; T4 strength replaces the MIN_VISIBILITY floor (full darkness); T5 real depth/normal/slope bias (proven on the shadow-bias acne route); T6 cascade blending (no seam); T7 PCSS contact-hardening behind shadowType=2 (blocker search via textureLoad); T8 alpha-tested perforated casters (SHADOW_CASTER_ALPHA_TEST_WGSL + the alpha-shadow perforated-depth route); T9 deterministic shadow atlas packer + per-frame realtime/once/interval update scheduler (the atlas-shadow shared-atlas + static-skip route). Proof routes: csm-directional, shadow-bias, alpha-shadow, atlas-shadow (all via scripts/webgpu-e2e.sh under SwiftShader + xvfb) plus the existing auto/point/spot/multi shadow specs. Per-light light-float layout grew to stride 29 (slots 24-28 = strength/depthBias/normalBias/filterRadius/shadowType). NEXT milestone in wave 1: M5 (PBR/IBL correctness) — not started. Per the run directive only M4 was in scope; no other milestone was started. ENV: WebGPU e2e is turnkey via .claude/hooks/session-start.sh + scripts/webgpu-e2e.sh.`
