...
**Last updated:** `2026-05-30 06:50 PDT — claude/opus`
...
**Next recommended task:** `M4-T5 — thread authored depth bias + slope-scaled pipeline bias + normal-offset bias (depends T3, done). Pack depthBias/normalBias into light floats, add depthBias/depthBiasSlopeScale to the caster pipeline depthStencil, replace the fixed 0.002 receiver bias, and offset the receiver position along its normal. T8 (alpha-test casters) also ready.`
...
**Gate status:** `pnpm run check = pass (384 files / 2172 tests); all 5 shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader Vulkan + xvfb): csm-directional (frustum-fit + authored params + strength=1 full-dark / strength=0 no-shadow proofs), auto-shadow, point, spot, multi-light`
...
`M4 (Production-quality shadows) IN PROGRESS — 4/9. M4-T4 (authored strength replaces the MIN_VISIBILITY floor) shipped: per-light strength packed at light-float slot 24 (PACKED_LIGHT_FLOAT_STRIDE 24->25) via shadowParamsByLight (all shadow-casting kinds); the baked 0.45/0.5 floors are gone from all 4 shader variants in standard-shader-shadow-sampling.ts, replaced by mix(1-shadowStrength,1,vis) — per-light shadowStrength(lightIndex) in the cascaded path, first-light directionalShadowStrengthValue/spotShadowStrengthValue/pointShadowStrengthValue helpers elsewhere. Default strength=1 reaches full darkness, so point/spot/multi/auto demo scenes now author strength 0.5-0.55 to preserve their visible-but-dark look (old floor was equivalent). The csm route drives strength via ?shadow-strength= URL param; spec proves strength=1 full-dark (delta>40) vs strength=0 no-shadow (delta<15). M4-T3 (authored shadow knobs): LightShadowSettings + input + validation + extraction + ShadowRequestPacket + packed codec (stride 6->10, ENCODING_VERSION 5->6, +writeFloat32/readFloat32) carry shadowType/strength/filterRadius/slopeBias end-to-end (omitted at defaults). M4-T1 (frustum-fit cascade matrices): tryFrustumFit unprojects the camera frustum, fits a light-space AABB, sizes the ortho to the slice's largest world diagonal (position-invariant, no swim), texel-snaps the center; byte-identical static fallback; wired into the csm route. M4-T2 (practical splits): cascadeSplit() emits absolute cascadeNearDistance/cascadeFarDistance matching light-packing directionalCascadeFarBounds. Gate green (pnpm run check = 384 files / 2172 tests); all 5 shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). NEXT: M4-T5 (authored depth/normal/slope bias) — webgpu-render: (a) add depthBias(int)+depthBiasSlopeScale(float) to ShadowCasterPipelineDescriptorMetadata.depthStencil + emit in createBrowserShadowCasterPipelineDescriptor; (b) replace fixed STANDARD_SHADOW_DEPTH_BIAS=0.002 with a per-light packed depthBias (pack into a new light-float slot 25/26 alongside strength); (c) offset receiver world pos along its normal by normalBias*texelWorldSize before the shadow matrix multiply. depthBias/slopeBias/normalBias already authored on ShadowRequestPacket (T3). T8 (alpha-test casters) also ready. ENV NOTE: WebGPU e2e is turnkey — .claude/hooks/session-start.sh installs Chrome + regenerates playwright.local.config.ts (timeout 150s); run proofs with scripts/webgpu-e2e.sh <spec>.`
