...
**Last updated:** `2026-05-30 14:05 PDT — claude/opus`
...
**Next recommended task:** `M5-T5 — Refractive transmission (IOR + thickness + Beer-Lambert). dependsOn M5-T4 now ✅, so it is unblocked and is the lowest-numbered remaining M5 task. Touches render types.ts (ior/thickness/attenuationColor/attenuationDistance), gltf-material-extensions.ts (KHR_materials_volume/ior), standard-shader-extension-sampling.ts (rewrite applyStandardTransmissionSampling: refract+thickness+Beer-Lambert over the now-linear-HDR grab buffer), StandardMaterialUniform packing. Then M5-T6 (SSAO→indirect-only, dependsOn none). M5 at 4/6.`
...
**Gate status:** `pnpm run check = pass (391 files / 2205 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect,hdr-exposure,tonemap-showcase}.spec.ts = pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). M5-T1..T4 ✅ done. T1: split-sum DFG metal sphere. T2: cosine irradiance convolution (environment.diffuse{convolved:true,faceSize:32}). T3: one equirect .hdr → loadHdrFromUri → equirect-to-cube projection → PMREM+convolution. T4: rgba16float HDR scene buffer + appended hdr-tonemap post stage applies exposure+operator+sRGB (hdr-exposure sweep 0.25/1/4 brightens monotonically, lit pipeline bakes no tonemap: token); tonemap-showcase regression green. M4 remains COMPLETE (9/9).`
...
| [M5](#m5)   | 1    | in-progress | 4/6        | DFG / irradiance / transmission correctness routes ⬜                            | T1 ✅ split-sum DFG (ibl-brdf). T2 ✅ cosine irradiance-convolution (ibl-irradiance). T3 ✅ equirect→cube single-asset auto-IBL (ibl-equirect). T4 ✅ rgba16float HDR scene buffer + exposure post-tonemap stage (hdr-exposure). T5 (depends T4, now unblocked) / T6 not started.            |
...
`clean — no in-flight work. M5-T1..T4 ✅ done; M5 in progress (4/6); next M5-T5 then M5-T6. Gate green (pnpm run check = 391 files / 2205 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect,hdr-exposure,tonemap-showcase}.spec.ts pass under SwiftShader+xvfb. T1: split-sum environment-BRDF (DFG) — brdf-lut-compute-pipeline.ts + brdf-lut-resource.ts; iblSpecularBrdf variant (analytic Karis DFGApprox) supersedes iblSpecularProof. T2: cosine irradiance convolution — irradiance-convolution-compute-pipeline.ts; createDiffuseIblTextureResourceReport convolves by default (32² cube), opt-out convolveIrradiance:false. T3: equirect→cube single-asset auto-IBL — equirect-to-cube-compute-pipeline.ts + equirect-to-cube-resource.ts; examples/ibl-equirect synthesizes a 2:1 flat-RGBE .hdr in-JS → project → PMREM + T2 convolution → mirror sphere. T4 (f85b7fe): opt-in exposure? → app.sceneRenderFormat=rgba16float; lit pass renders linear HDR (no in-material tonemap: pipeline-resources passes tonemap:none/output-color:linear when HDR); create-webgpu-app appends post/post-tonemap.ts createWebGpuTonemapPostEffect as the LAST stage (HDR buffer → color\*exposure+operator+sRGB → 8-bit swapchain); attachments.ts routes the swapchain MSAA color attachment to sceneRenderFormat. Default (no exposure) byte-identical. Fixed a WGSL param-shadowing compile bug in the exposure tonemap (let color = color*exposure → renamed param to inputColor; e2e GPU readback caught what string-only vitest missed). examples/hdr-exposure + test/e2e/hdr-exposure.spec.ts (sweep 0.25/1/4, monotonic brightening, pixelDistance>40, lit cacheKey has no tonemap: token). NEXT — M5-T5 (Refractive transmission, dependsOn T4 now ✅): add ior/thickness/attenuationColor/attenuationDistance to render types.ts StandardMaterialAsset + factories defaults (ior=1.5); parse KHR_materials_volume/KHR_materials_ior in gltf-material-extensions.ts; pack into StandardMaterialUniform (standard-shader-source.ts struct + standard-material-buffer.ts — keep std140/wgsl+packing in lockstep); rewrite applyStandardTransmissionSampling (standard-shader-extension-sampling.ts ~473-531): refract(-viewDir,normal,1/ior) + thickness-scaled screen offset over the grab buffer (now linear HDR from T4) + Beer-Lambert exp(-coeff*thickness) from attenuationColor/Distance (guard Distance>0). Proof: extend examples/transmission (or a transmission-ior route) ior=1.0/1.5/2.0 + thick/thin attenuation probes + test/e2e/transmission-ior.spec.ts; vitest for the glTF extension parse. Then M5-T6 (SSAO→indirect-only, dependsOn none). ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present. LESSON: post-effect WGSL must be proven by an actual GPU draw+readback, not string assertions — a shader compile error counts a draw but writes nothing.`
