...
**Last updated:** `2026-05-30 11:45 PDT — claude/opus`
...
**Next recommended task:** `M5-T2 — real cosine irradiance-convolution compute pass for diffuse IBL (no deps; unblocks M5-T3). M5-T1 (split-sum DFG) is done; wave 1 still has M5 in progress (1/6).`
...
**Gate status:** `pnpm run check = pass (386 files / 2183 tests); test/e2e/ibl-brdf.spec.ts = pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb) — split-sum DFG metal sphere: iblSpecularBrdf (not proof) in pipeline key, environment.brdfLut{ready,size:256,format:rg16float}, grazing>facing horizon delta, specularDiagnosticCodes:[]. M5-T1 ✅ done. M4 remains COMPLETE (9/9).`
...
| [M5](#m5)   | 1    | in-progress | 1/6        | DFG / irradiance / transmission correctness routes ⬜                            | T1 ✅ split-sum environment-BRDF (DFG): analytic Karis term supersedes iblSpecularProof + validated GPU rg16float DFG-LUT compute pass; proof route examples/ibl-brdf                                                                                                                        |
...
`M5-T1 ✅ done; M5 in progress (1/6). Gate green: pnpm run check = 386 files / 2183 tests; test/e2e/ibl-brdf.spec.ts passes under SwiftShader+xvfb. SHIPPED: split-sum environment-BRDF (DFG). New packages/webgpu/src/lighting/brdf-lut-compute-pipeline.ts (rg16float GGX DFG integration compute shader + integrateEnvironmentBrdf CPU mirror) and brdf-lut-resource.ts (createBrdfIntegrationLutResource: one-time 256² dispatch → 2d view). New iblSpecularBrdf shader variant supersedes iblSpecularProof: applyStandardSpecularIblBrdfSampling evaluates the split-sum (scale,bias) analytically (Karis DFGApprox) and reuses the specular cube at group3 binding7 (no new binding/layout). withStandardIblPipelineKeys gained an includeSpecularBrdf arg; frame-loop passes hasReadyStandardSpecularIblBrdfResources (diffuse+specular+brdfLut all ready). Proof route examples/ibl-brdf.* + test/e2e/ibl-brdf.spec.ts + test/webgpu/brdf-lut-compute-pipeline.test.ts. DESIGN CHOICE (DECISIONS §0017): live term is the SOTA-bar-sanctioned analytic DFGApprox; the GPU LUT is built+validated+reported but NOT yet sampled at a group-3 binding 8 — that (and the standard-material-ibl-bind-group-layout.ts group-4 metadata) is a clean optional follow-up. NEXT: M5-T2 (real cosine irradiance-convolution compute pass for diffuse IBL; no deps; unblocks T3). T2 model: pmrem-compute-pipeline.ts dispatch structure; wire into ibl-texture-resource-diffuse.ts with hasIrradianceConvolutionDeviceSupport feature-detect; flip ibl-preparation-pass-plan.ts submissionDeferred→executed; proof route examples/ibl-irradiance.*. ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present (pnpm setup:references already run).`
...
#### `M5-T1` · Split-sum environment-BRDF (DFG) LUT compute pass + bind it into specular IBL — ✅ done (2026-05-30 · TBDHASH1)
