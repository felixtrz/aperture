...
**Last updated:** `2026-05-29 14:35 PDT — claude/opus`
...
**Next recommended task:** `M2-T4 — glTF animation import (depends M2-T1, M2-T3, both done); parses gltf.animations into engine AnimationClip assets bound to entity-key targets`
...
**Gate status:** `pnpm run check = pass (375 files / 2139 tests); test/render/gltf-skin-import.spec.ts = pass`
...
| [M2](#m2)   | 1    | in-progress | 3/9        | skinned+animated GLB route (M2-T9) ⬜                                            | T1 sampler + T2 mixer + T3 skin import landed                               |
...
`clean — no in-flight work. M2-T1 (d6c26e6) sampler + M2-T2 (5cbb865) mixer + M2-T3 (43f09a1) skin import landed. Full gate green (375 files / 2139 tests). Next: M2-T4 — glTF animation import. Add packages/render/src/assets/gltf-animation-import.ts parsing gltf.animations into AnimationClip data (M2-T1 shape) using the shared decodeGltfFloatAccessor for sampler input/output, supporting LINEAR/STEP/CUBICSPLINE (do NOT reject CUBICSPLINE); resolve target.node to keyPrefix:node:N entity keys; emit a gltfAnimation.* diagnostic report. App-layer wiring onto SystemGltfLoadedScene + AssetRegistry registration is part of M2-T4.`
