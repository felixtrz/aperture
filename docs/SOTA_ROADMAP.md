...
**Last updated:** `2026-05-30 08:02 PDT — claude/opus`
...
**Next recommended task:** `M4-T7 — PCSS contact-hardening soft shadows behind authored shadowType=2 (depends T3+T4, done). Add a blocker-search + variable-radius PCF directional sampler; also fix the cascaded PCF to honor filterRadiusTexels. Watch the comparison-sampler-can't-return-raw-depth constraint. Then T8 (alpha-test casters), T9 (atlas).`
...
**Gate status:** `pnpm run check = pass; shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb): csm-directional, auto-shadow, point, spot, multi-light, shadow-bias (acne + cascade-continuity). M4-T1..T6 fully proven.`
...
| [M4](#m4)   | 1    | in-progress | 6/9        | frustum-fit / PCSS / alpha-test shadow routes ⬜                                 | M4-T1..T6 done: splits + frustum-fit matrices + authored params + strength floor removed + authored bias (acne route) + cascade blend (no seam)                                                                                                                                              |
...
`M4 (Production-quality shadows) IN PROGRESS — 6/9 done (T1-T6). Gate green; shadow e2e specs pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb): csm-directional, auto-shadow, point, spot, multi-light, shadow-bias. DONE: T1 frustum-fit matrices (no swim); T2 practical splits; T3 authored shadow knobs (ShadowRequestPacket+codec stride 6->12); T4 strength floor removed (slot 24; all 4 variants); T5 authored depth/normal/slope bias (pipeline depthBias+slopeScale; cascaded shadowDepthBias slot 25 + normal-offset slot 26; light-packing stride 25->27; proven by units + dedicated examples/shadow-bias.* acne route); T6 cascade blend (extracted sampleDirectionalCascadeVisibility per-cascade helper; mix current+next cascade factors in a 12% band; proven by WGSL unit + shadow-bias floor continuity maxStep<25). REMAINING M4 (not started): T7 PCSS (deps T3+T4 done) — add a blocker-search + variable-radius PCF directional sampler behind shadowType=2 (already authored/packed); ALSO fix the cascaded sampleDirectionalShadowPcf3x3 to honor filterRadiusTexels (it currently ignores it). WATCH OUT: WGSL comparison samplers can't return raw depth for the blocker search — needs a second non-comparison depth-texture view/sampler of the shadow map, or manual multi-tap compares; verify the shadow-depth-texture bind layout. Proof: contact-hardening (near-contact penumbra width < far) vs uniform PCF. T8 alpha-tested shadow casters (independent) — caster pipeline TEXCOORD_0 + baseColor texture + alphaCutoff discard variant + a SECOND caster bind group; select per-draw by material alphaMode=MASK; proof: perforated shadow on an alpha-cutout quad. T9 shadow atlas packer (deps T3) — deterministic shelf/quadtree packer assigning non-overlapping ShadowAtlasRegions + updateMode (realtime/once/interval) scheduler; vitest packer unit + multi-light shared-atlas route. PROOF TIPS: SwiftShader needs robust large-delta/variance assertions (subtle precision fails); the examples/shadow-bias.* grazing-floor + ?shadow-depth-bias= pattern is a good dedicated-route template. ENV: WebGPU e2e turnkey via .claude/hooks/session-start.sh; run scripts/webgpu-e2e.sh <spec> (timeout 150s).`
