...
**Last updated:** `2026-05-30 19:10 PDT — claude/opus`
...
**Next recommended task:** `M5 COMPLETE (6/6). Per the active directive ("Do not start any other milestone"), stop here — do not begin M3. The next milestone in dependsOn/wave order would be M3 (Render Graph, wave 2, M3-T1 first), but it is out of scope for this run. If a new run picks up: M3-T1 is the lowest-numbered todo in wave 2.`
...
**Gate status:** `pnpm run check = pass (392 files / 2211 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect,hdr-exposure,tonemap-showcase,transmission,transmission-ior,ssao,ssao-indirect,post-effects,taa}.spec.ts = pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). M5 COMPLETE (6/6). T1 split-sum DFG; T2 cosine irradiance convolution; T3 equirect→cube single-asset auto-IBL; T4 rgba16float HDR scene buffer + hdr-tonemap post stage (exposure+operator+sRGB); T5 IOR refraction (refract + viewer-facing normal) + thickness + Beer-Lambert; T6 SSAO→indirect-only (lit pass emits an indirect color channel; SSAO removes indirect*(1-ao), preserving direct+emissive). M1/M2/M4 remain COMPLETE. KNOWN ENV: test/e2e/dof.spec.ts times out under SwiftShader on a clean tree (pre-existing, unrelated to M5).`
...
`clean — no in-flight work. M5 COMPLETE (6/6) — all of M5 (Close core PBR/IBL correctness gaps) is done, committed, pushed, and gate-green. Per the active directive, NO other milestone was started. Gate: pnpm run check = pass (392 files / 2211 tests); e2e proofs pass under SwiftShader+xvfb: ibl-brdf (T1), ibl-irradiance (T2), ibl-equirect (T3), hdr-exposure (T4), transmission-ior (T5), ssao-indirect (T6), plus tonemap-showcase/transmission/ssao/post-effects/taa regressions. T1 split-sum DFG (brdf-lut-compute-pipeline + iblSpecularBrdf analytic Karis variant). T2 cosine irradiance convolution (irradiance-convolution-compute-pipeline; default-on, opt-out convolveIrradiance:false). T3 equirect→cube single-asset auto-IBL (equirect-to-cube-{compute-pipeline,resource}). T4 (f85b7fe) opt-in exposure? → rgba16float HDR scene buffer + appended post/post-tonemap.ts last stage; in-material tonemap skipped when HDR. T5 (bfa7e39) ior/thickness/attenuationColor/attenuationDistance + KHR_materials_ior/volume parse + refract(-viewDir,N,1/ior)+thickness+Beer-Lambert; KEY FIX viewer-facing normal before refract. T6 (64eb481) SSAO→indirect-only via approach A: lit pass emits a 2nd color attachment = indirect (ambient+IBL) via standard-indirect-channel-shader.ts; SSAO removes indirect*(1-ao); indirectColorFormat threaded like motionVectorColorFormat (mutually exclusive, both @location(1); enabled only when an effect requiresIndirectColor + MSAA off + no motion vectors); post-ssao falls back to whole-image multiply otherwise. KEY FIX textureSampleLevel (not textureSample) in the SSAO fragment's non-uniform flow. KNOWN ENV: test/e2e/dof.spec.ts times out under SwiftShader on a clean tree (pre-existing, unrelated to M5). NEXT (out of scope this run): M3 (Render Graph, wave 2) — M3-T1 is the lowest todo. ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present. LESSONS: (1) post-effect/shader WGSL must be proven by an actual GPU draw+readback — a shader compile error still counts a draw but writes nothing (caught the T4 param-shadow + T6 textureSample bugs); (2) for view-dependent shading on transmissive/double-sided surfaces, force the normal to face the viewer before refract/reflect, and read GPU inputs back as color when a term looks input-insensitive; (3) Infinity is not JSON-safe — use a finite sentinel; (4) some e2e specs hardcode env-dependent values (swapchain format, scene draw counts) — assert shapes/relations, not absolutes.`
