...
**Last updated:** `2026-05-30 00:40 PDT — claude/opus`
...
**Next recommended task:** `M2-T9 — end-to-end skinned+animated GLB route (depends M2-T6/T7/T8); engine-status proof is headless-verifiable, pixel/render-control proof is WebGPU-blocked here`
...
**Gate status:** `pnpm run check = pass (383 files / 2163 tests); test/e2e/morph-target-render.spec.ts = pass (3-target morph pixel-diff via playwright.local.config.ts under xvfb + SwiftShader)`
...
| [M2](#m2)   | 1    | in-progress | 8/9        | skinned+animated GLB route (M2-T9) ⬜                                            | T1-T8 done; T7 N-target morph render shipped (storage buffer, pixel-proven via examples/morph-targets); only T9 route/pixel remains |
...
`M2-T1..T8 done and gate-green (383 files / 2163 tests). M2-T7 fully landed (921736e): the N-target morph render via a group(1) storage buffer (deltas @binding 4 + per-instance descriptor @binding 5) with the standard morph shader looping over targetCount — vertex-attribute morph removed; all four done-when boxes pass, including the live pixel proof (test/e2e/morph-target-render.spec.ts, 3-target box, center readback differs between target-2 weight 0 and 1) run via playwright.local.config.ts under xvfb + SwiftShader. ONLY M2-T9 remains for the milestone. M2-T9 (end-to-end skinned + animated GLB route through createWebGpuApp/spawn) needs: examples/animation-skinning.{worker,main}.js + .html + a skinned + ≥3-target-morph + keyframed (LINEAR + a CUBICSPLINE variant) fixture, and test/e2e/animation-skinning.spec.ts asserting (a) pixels differ between two animation times, (b) a CUBICSPLINE clip plays, (c) a >2-target morph contributes, (d) engine-owned status (clip name/time/joint values/morph weights/skin jointCount) — the headless half is already proven by test/app/{skinned-animated-route,gltf-animation-playback,gltf-animation-clips}.test.ts. The morph-targets route (worker+main+scene) and clearcoat-style createWebGpuApp readback harness are the model. WebGPU runs here (xvfb + SwiftShader over localhost).`
