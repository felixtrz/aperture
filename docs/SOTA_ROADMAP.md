...
**Last updated:** `2026-05-30 13:24 PDT — claude/opus`
...
**Active task:** `M5-T4 (in-progress) — exposure WGSL (Done-when #4) landed (ccaffcc); rgba16float scene-render-format threading + createWebGpuTonemapPostEffect final stage + hdr-exposure example/spec remain (plan in HANDOFF.md).`
...
**Next recommended task:** `Continue M5-T4 — exposure WGSL (Done-when #4) done (ccaffcc); next: opt-in exposure option → rgba16float sceneRenderFormat threaded through pipeline-resources.ts colorFormat + cache key + tonemap-skip, post-processing.ts scene texture + a new createWebGpuTonemapPostEffect final stage, and examples/hdr-exposure.* + spec (Done-when #1/#2/#3). Plan in HANDOFF.md. Then M5-T6 (SSAO→indirect, ready), M5-T5 (depends T4). M5 at 3/6 done + T4 in-progress.`
...
**Gate status:** `pnpm run check = pass (390 files / 2201 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect}.spec.ts = pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). M5-T4 in progress (exposure WGSL #4 done, ccaffcc). T1: split-sum DFG metal sphere. T2: cosine irradiance convolution (environment.diffuse{convolved:true,faceSize:32}). T3: one equirect .hdr → loadHdrFromUri → equirect-to-cube projection → PMREM+convolution (source{loader:'loadHdrFromUri',projection:'equirect-to-cube',faceCount:6}, specularPrefiltering:true, mirror reflects the bright band). M5-T1, M5-T2, M5-T3 ✅ done. M4 remains COMPLETE (9/9).`
...
| [M5](#m5)   | 1    | in-progress | 3/6        | DFG / irradiance / transmission correctness routes ⬜                            | T1 ✅ split-sum DFG (ibl-brdf). T2 ✅ cosine irradiance-convolution (ibl-irradiance). T3 ✅ equirect→cube single-asset auto-IBL (ibl-equirect). T4 🟡 in-progress: exposure WGSL (Done-when #4) done; rgba16float buffer + tonemap post stage + example remain. T5/T6 not started.           |
...
`M5-T1 + M5-T2 + M5-T3 ✅ done; M5-T4 in progress; M5 in progress (3/6). Gate green (pnpm run check = 390 files / 2201 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect}.spec.ts pass under SwiftShader+xvfb. T1: split-sum environment-BRDF (DFG) — brdf-lut-compute-pipeline.ts + brdf-lut-resource.ts; iblSpecularBrdf shader variant (analytic Karis DFGApprox) supersedes iblSpecularProof, reusing the specular cube (group3 binding7); examples/ibl-brdf. T2: cosine irradiance convolution — irradiance-convolution-compute-pipeline.ts; createDiffuseIblTextureResourceReport convolves by default (32² cube) when compute is available, opt-out convolveIrradiance:false; examples/ibl-irradiance. T3: equirect→cube single-asset auto-IBL — equirect-to-cube-compute-pipeline.ts (+Z-centered UV u=atan2(x,z)/2π+0.5,v=asin(y)/π+0.5) + equirect-to-cube-resource.ts (projects to a cube with COPY_SRC); examples/ibl-equirect synthesizes a 2:1 flat-RGBE .hdr in-JS → loadHdrFromUri → project → readback 6 faces → PMREM + T2 convolution → mirror sphere. DESIGN NOTES: T1 uses the SOTA-sanctioned analytic DFGApprox (GPU LUT built+validated+reported, not sampled at a binding-8 — optional follow-up; DECISIONS §0017). T3 demonstrates the single-asset auto-chain in the example's resource-building; a public equirectSource field on app-environment-resources.ts is an optional equivalent follow-up. M5-T4 IN PROGRESS (ccaffcc): the exposure math (Done-when #4) is done — createOutputTonemapWgsl(operator, {exposure:true}) emits the exposure-aware tonemap (color\*exposure before the operator), backward-compatible + vitest. RESUME T4 with the remaining steps (FULL step-by-step plan in agent/HANDOFF.md "M5-T4 refined integration plan"): opt-in exposure? on CreateWebGpuAppOptions → app sceneRenderFormat=rgba16float threaded into pipeline-resources.ts colorFormat (73/88/101/113) + pipeline cache key + pass tonemap:undefined for the standard pipeline when HDR (skip applyOutputTonemapToStandardShader, material returns linear HDR); post-processing.ts scene texture (79) uses sceneRenderFormat + APPEND a new post-tonemap.ts createWebGpuTonemapPostEffect (model on post-pass.ts + post-fxaa.ts) as the last stage (rgba16float scene → exposure+tonemap+sRGB → 8-bit swapchain); examples/hdr-exposure.* exposure sweep + test/e2e/hdr-exposure.spec.ts (Done-when #1/#2/#3). The hdr-exposure example uses NO other post effects so the tonemap stage is the only/last effect → no intermediate-format threading. WATCH: lit-pass color attachment format must equal the scene-texture format (opt-in limits blast radius); keep the FINAL stage at swapchain format for readback. Then M5-T6 (SSAO→indirect-only; effort S but needs a depth-prepass or an MRT split — see its watch-out) and M5-T5 (refractive transmission; depends T4). ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present.`
...
#### `M5-T4` · Persistent rgba16float HDR scene buffer; move tonemap+exposure+encode to a final post stage — 🟡 in-progress (2026-05-30 · ccaffcc)
...
**In-progress note (2026-05-30 · ccaffcc):** DONE — the exposure math (Done-when #4): `createOutputTonemapWgsl(operator, { exposure: true })` emits an exposure-aware `apertureOutputTonemap(color, exposure)` (`color * exposure` before the operator), backward-compatible (default form byte-identical, so the in-material path + existing tonemap/standard-shader tests are unchanged) + vitest. REMAINING for the next agent (full plan in agent/HANDOFF.md "M5-T4 refined integration plan"): (a) opt-in `exposure?` on CreateWebGpuAppOptions → app `sceneRenderFormat`=rgba16float; (b) thread `sceneRenderFormat` into pipeline-resources.ts colorFormat (73/88/101/113) + the pipeline cache key + pass `tonemap:undefined` for the standard pipeline when HDR (so applyOutputTonemapToStandardShader is skipped → material returns linear HDR); (c) post-processing.ts scene texture (79) uses sceneRenderFormat + APPEND a new `createWebGpuTonemapPostEffect` (post-tonemap.ts, model on post-pass.ts contract + post-fxaa.ts) as the last stage (samples the rgba16float scene, applies exposure+tonemap+sRGB → 8-bit swapchain); (d) examples/hdr-exposure.\* exposure sweep + test/e2e/hdr-exposure.spec.ts (Done-when #1/#2/#3). The hdr-exposure example uses NO other post effects so the tonemap stage is the only/last effect → no intermediate-format threading needed.
