...
**Last updated:** `2026-05-30 19:52 PDT — claude/opus`
...
**Next recommended task:** `M3-T2 — Single-encoder graph executor (executeFrameGraph): walk compiled.orderedNodes into ONE GPUCommandEncoder + submit once; refactor frame-boundary.ts to extract encodeFrameBoundaryInto while assembleFrameBoundary stays a legacy-preserving wrapper. Depends on M3-T1 (done, 107c61d). It is the non-negotiable foundation the post/forward/shadow ports (T3–T5) all execute through.`
...
**Gate status:** `pnpm run check = pass (393 files / 2217 tests) @ 107c61d. M3 IN PROGRESS (1/7): M3-T1 done — pure, headless FrameGraph data model (render/graph/frame-graph.ts) + compileFrameGraph (render/graph/frame-graph-compile.ts: topo order + store-on-no-clear load/store inference + descriptor-keyed transient aliasing + JSON-safe report; cycle ⇒ ok:false diagnostic, no throw) + ComputePassCommand union (render/passes/compute-pass-commands.ts). Proof: test/webgpu/frame-graph-compile.test.ts (6 cases) green. The graph layer is GPU-free/synchronous (headless/worker-safe invariant upheld); no executor/route wiring yet (that is T2+). M1/M2/M4/M5 remain COMPLETE. KNOWN ENV: test/e2e/dof.spec.ts times out under SwiftShader on a clean tree (pre-existing, unrelated).`
...
| [M3](#m3)   | 2    | in-progress | 1/7        | post+forward+shadow ported under graph + custom-pass example (M3-T7) ⬜          | M3-T1 ✅ (107c61d): pure FrameGraph data model + compileFrameGraph (topo order, store-on-no-clear load/store inference, transient aliasing, JSON-safe report) + ComputePassCommand union. Next: M3-T2 single-encoder executor.                                                                                                  |
...
`M3 IN PROGRESS (1/7) — M3-T1 done (107c61d), gate-green, committed. NEXT: M3-T2 (single-encoder graph executor). Tree is clean after the T1 code + docs commits.

M3-T1 shipped the pure, headless graph model (no GPU): render/graph/frame-graph.ts (createFrameGraph builder, ResourceHandle/PassNode types, declareTransient/declareHistory/importSwapchain/importDepth), render/graph/frame-graph-compile.ts (compileFrameGraph: topo-sort + store-on-no-clear load/store inference + transient aliasing + JSON-safe report; cycle⇒ok:false, no throw), render/passes/compute-pass-commands.ts (ComputePassCommand union). Proof: test/webgpu/frame-graph-compile.test.ts (6 pass). Exported from index.ts + test-support.ts.

M3-T2 concrete next step (depends T1 only): build render/graph/frame-graph-execute.ts = executeFrameGraph(device,queue,compiled,resources) that creates exactly ONE encoder (gpu/command-encoder.ts createCommandEncoderResource), walks compiled.orderedNodes (render node ⇒ begin pass w/ compiled load/store + resolve write handles to views + replay RenderPassCommand[] through the EXISTING render/passes/render-pass-command-executor.ts so bundle-caching/elision is preserved + end; compute node ⇒ begin compute pass + run ComputePassCommand[] + end), then finish ONCE + submitCommandBuffers([one]). KEYSTONE REFACTOR: split render/frame/frame-boundary.ts assembleFrameBoundary (line ~287) into (a) encodeFrameBoundaryInto(encoder, options) doing begin→execute→end on a CALLER-PROVIDED encoder (no create/finish/submit) and (b) keep assembleFrameBoundary as a thin wrapper that creates+finishes+submits around it — so test/webgpu/frame-boundary.test.ts STILL emits ['begin','draw','end','finish','submit:1'] UNCHANGED (that invariant is the migration safety net — get it green before touching routes). Extend gpu/command-submission-metrics.ts to count multi-pass single-buffer frames. T2 proof: NEW test/webgpu/frame-graph-execute.test.ts (fake-device event recorder: a 3-render+1-compute graph ⇒ ONE createCommandEncoder/finish/submit, passes interleaved in compiled order; compute begin/dispatch/end inside the same encoder) + the unchanged frame-boundary.test.ts.

D1–D4 are SIGNED OFF — public API shape (declarative node + encode callback, string handles, edges drive order) is binding; implement exactly, do not redesign. ENV: vitest is headless (T1/T2 are vitest slices); WebGPU E2E (needed from T3) runs via scripts/webgpu-e2e.sh (SwiftShader+xvfb). KNOWN ENV: test/e2e/dof.spec.ts times out under SwiftShader on a clean tree (pre-existing). LESSON from T1: the repo tsconfig sets noUncheckedIndexedAccess — bounded array access needs ! (used elsewhere in src) or tsc -b fails in waves.`
