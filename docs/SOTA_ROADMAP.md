...
`M4 (Production-quality shadows) IN PROGRESS — 7/9 done (T1-T7). Gate green; shadow e2e all pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb): csm-directional, auto-shadow, point, spot, multi-light, shadow-bias. DONE: T1 frustum-fit; T2 practical splits; T3 authored knobs; T4 strength floor removed; T5 authored bias (units + shadow-bias acne route); T6 cascade blend (per-cascade helper + mix; shadow-bias continuity); T7 PCSS (filterRadius slot 27 + shadowType slot 28 packed; cascaded sampleDirectionalCascadeVisibility branches hard/PCF/PCSS; sampleDirectionalShadowPcss blocker-searches raw depth via textureLoad — NO extra sampler/binding — + variable-radius PCF; cascaded PCF now honors filterRadius; proven by WGSL unit + shadow-bias pillar contact-hardening route). NOTE: csm scene authors shadowType=2 so it renders via PCSS; non-cascaded/multi/point variants still use fixed PCF + tuned bias consts (cascaded directional is the milestone proof route). REMAINING M4 (not started): T8 alpha-tested shadow casters (independent) — in shadow-caster-pipeline-descriptor.ts/-resource.ts add an alpha-test pipeline variant: vertex.buffers + TEXCOORD_0, a SECOND bind group binding the material baseColor texture+sampler+alphaCutoff, and a SHADOW_CASTER_ALPHA_TEST_WGSL whose fs_main samples baseColor.a and discards below cutoff; select per-draw in shadow-caster-draw-list-plan.ts by material alphaMode=MASK; thread alphaCutoff/baseColor via the MeshDrawPacket. Proof: a checkerboard/alpha-cutout quad caster casts a PERFORATED shadow (lit+shadowed samples coexist in the footprint) vs a solid rectangle for opaque. T9 shadow atlas packer (deps T3) — new shadow-atlas-packer.ts (deterministic shelf/quadtree assigning non-overlapping ShadowAtlasRegions, drop 5th over-budget light), updateMode (realtime/once/interval) on LightShadowSettings + per-frame scheduler skipping static 'once' caster passes (depth cache persists), wire atlasRegion into the pass viewport/scissor. Proof: vitest packer unit (4x512 into 1024 non-overlapping + 5th dropped) + multi-light shared-atlas route + 'once' skip. ENV: WebGPU e2e turnkey via .claude/hooks/session-start.sh; run scripts/webgpu-e2e.sh <spec> (timeout 150s). PROOF TIP: SwiftShader needs robust region-average/variance assertions; the examples/shadow-bias.* dedicated-route + URL-param pattern is the template.`
...
#### `M4-T9` · Shadow atlas packer + per-light update policy (static/realtime) scaffold-to-behavior — 🟡 in-progress
