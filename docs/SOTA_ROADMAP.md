...
**Last updated:** `2026-05-30 18:15 PDT — claude/opus`
...
**Next recommended task:** `M5-T6 — SSAO applies to indirect/ambient/IBL only, not direct light. dependsOn none (the last remaining M5 task). Approach (B) per its spec: stop the post-ssao.ts:561 final-color multiply; instead bind the SSAO visibility texture into the standard shader and fold it into the ambient+IBL occlusion factor (combine with the existing occlusionTexture term ~standard-shader-source.ts:695 + standard-shader-ibl-sampling.ts), leaving direct light untouched. Proof: examples/ssao-indirect (or extend ssao) — crease probe darkens with SSAO while a direct-lit probe barely changes. M5 at 5/6.`
...
**Gate status:** `pnpm run check = pass (391 files / 2208 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect,hdr-exposure,tonemap-showcase,transmission,transmission-ior}.spec.ts = pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). M5-T1..T5 ✅ done. T1: split-sum DFG metal sphere. T2: cosine irradiance convolution. T3: equirect→cube single-asset auto-IBL. T4: rgba16float HDR scene buffer + appended hdr-tonemap post stage (exposure+operator+sRGB). T5: IOR-driven refraction (refract + viewer-facing normal) + thickness + Beer-Lambert (transmission-ior sweep shifts the background monotonically with ior; amber volume absorbs blue by thickness); existing transmission regression green. M4 remains COMPLETE (9/9).`
...
`clean — no in-flight work. M5-T1..T5 ✅ done; M5 in progress (5/6); next M5-T6 (the last M5 task). Gate green (pnpm run check = 391 files / 2208 tests); test/e2e/{ibl-brdf,ibl-irradiance,ibl-equirect,hdr-exposure,tonemap-showcase,transmission,transmission-ior}.spec.ts pass under SwiftShader+xvfb. T1: split-sum DFG — brdf-lut-compute-pipeline.ts + brdf-lut-resource.ts; iblSpecularBrdf variant (analytic Karis DFGApprox). T2: cosine irradiance convolution — irradiance-convolution-compute-pipeline.ts (convolves by default, opt-out convolveIrradiance:false). T3: equirect→cube single-asset auto-IBL — equirect-to-cube-{compute-pipeline,resource}.ts. T4 (f85b7fe): opt-in exposure? → rgba16float HDR scene buffer + appended post/post-tonemap.ts as last stage; in-material tonemap skipped when HDR. T5 (bfa7e39): ior/thickness/attenuationColor/attenuationDistance on StandardMaterialAsset (attenuationDistance=0 = JSON-safe no-absorption sentinel); KHR_materials_ior/volume parsed (mapStandardVolumeFields); transmissionVolume+attenuationColor vec4s packed into StandardMaterialUniform (68→76 floats); applyStandardTransmissionSampling rewritten to refract(-viewDir,N,1/ior)→walk thickness→project exit to grab buffer + Beer-Lambert pow(attenuationColor, thickness/attenuationDistance). KEY FIX: transmissive fragment normal faced AWAY from camera → refract() ior-insensitive; forced a viewer-facing normal (select(-N,N,dot≥0)) before refract. examples/transmission-ior.* + test/e2e/transmission-ior.spec.ts. NEXT — M5-T6 (SSAO→indirect/ambient/IBL only, NOT direct light; dependsOn none): approach (B) — stop the post-ssao.ts final-color multiply (~line 561 returns source.rgb*visibility over the whole composite), instead BIND the SSAO visibility texture into the standard shader and fold it into the ambient+IBL occlusion factor (combine with the existing occlusionTexture term ~standard-shader-source.ts:695 + multiply diffuseIbl/specularIbl by AO in standard-shader-ibl-sampling.ts), possibly a new binding in standard-shader-variant-bindings.ts. Keep the SSAO pass PRODUCING visibility; change its CONSUMPTION. Proof: examples/ssao-indirect (or extend examples/ssao) — a creased surface lit by ONE strong direct light + ambient/IBL; SSAO-on darkens the crease probe (>large threshold) while a direct-lit probe barely changes (<small threshold). ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present. LESSONS: (1) post-effect/shader WGSL must be proven by an actual GPU draw+readback — a shader compile error still counts a draw but writes nothing; (2) for view-dependent shading (refraction/reflection) on transmissive/double-sided surfaces, force the normal to face the viewer before refract/reflect, and verify GPU inputs (normal/viewDir/worldPos) by reading them back as color when a term looks input-insensitive; (3) Infinity is not JSON-safe — use a finite sentinel for "unbounded" material params.`
