...
**Last updated:** `2026-05-30 12:33 PDT — claude/opus`
...
**Active task:** `M5-T3 (in-progress) — projection compute pipeline + UV-mapping vitest landed (3f54524, Done-when #3 ✅); resource builder + equirectSource auto-wiring + example/live proof remain.`
...
**Next recommended task:** `Continue M5-T3 — projection compute pipeline + UV vitest are done (Done-when #3 ✅); next: equirect-to-cube-resource.ts (one-time dispatch), equirectSource auto-wiring in app-environment-resources.ts, examples/ibl-equirect.* + test/e2e/ibl-equirect.spec.ts (needs a 2:1 equirect .hdr — author/synthesize one). M5-T4 (HDR buffer+exposure) and M5-T6 (SSAO→indirect) are also ready (no deps). M5 at 2/6 done + T3 in-progress.`
...
**Gate status:** `pnpm run check = pass; test/e2e/{ibl-brdf,ibl-irradiance}.spec.ts = pass via scripts/webgpu-e2e.sh (SwiftShader + xvfb). T1: split-sum DFG metal sphere (iblSpecularBrdf not proof in key, environment.brdfLut{ready,256,rg16float}, grazing>facing, specularDiagnosticCodes:[]). T2: cosine irradiance convolution (environment.diffuse{convolved:true,faceSize:32}; convolved softens raw directional contrast + bleeds into the dark hemisphere). M5-T1, M5-T2 ✅ done. M4 remains COMPLETE (9/9).`
...
| [M5](#m5)   | 1    | in-progress | 2/6        | DFG / irradiance / transmission correctness routes ⬜                            | T1 ✅ split-sum DFG (examples/ibl-brdf). T2 ✅ cosine irradiance-convolution (examples/ibl-irradiance). T3 🟡 in-progress: equirect→cube projection pipeline + UV vitest landed (Done-when #3); resource builder + auto-wiring + live proof remain.                                          |
...
`M5-T1 + M5-T2 ✅ done; M5 in progress (2/6). Gate green (pnpm run check); test/e2e/{ibl-brdf,ibl-irradiance}.spec.ts pass under SwiftShader+xvfb. T1: split-sum environment-BRDF (DFG) — brdf-lut-compute-pipeline.ts (rg16float GGX DFG integration + integrateEnvironmentBrdf CPU mirror) + brdf-lut-resource.ts; iblSpecularBrdf shader variant (analytic Karis DFGApprox) supersedes iblSpecularProof, reusing the specular cube (group3 binding7); examples/ibl-brdf. T2: cosine irradiance convolution — irradiance-convolution-compute-pipeline.ts (cosine-hemisphere convolution + convolveIrradianceDirection CPU mirror); createDiffuseIblTextureResourceReport convolves by default (32² cube) when compute is available (hasIrradianceConvolutionDeviceSupport), opt-out convolveIrradiance:false; examples/ibl-irradiance (bright hemisphere, ?mode=convolved|raw). DESIGN NOTES: T1 uses the SOTA-sanctioned analytic DFGApprox (GPU LUT built+validated+reported but not yet sampled at a binding-8 — optional follow-up; DECISIONS §0017). T2's ibl-preparation-pass-plan.ts is NOT on the live path (no live callers) so convolution-executed is surfaced via the diffuse report's convolved flag. M5-T3 IN PROGRESS (3f54524): the equirect→cube projection COMPUTE PIPELINE CORE is done — `equirect-to-cube-compute-pipeline.ts` (`createEquirectToCubeComputePipeline`, cube-face dispatch + `cubeDirection()`→+Z-centered equirect UV `u=atan2(x,z)/2π+0.5`,`v=asin(y)/π+0.5`) + CPU mirrors + `test/webgpu/equirect-to-cube-compute-pipeline.test.ts`(Done-when #3 ✅). RESUME T3 with: (a)`equirect-to-cube-resource.ts`(one-time dispatch — model on`brdf-lut-resource.ts`/`createSpecularIblPmremTextureResource`: createTexture 2D equirect source + storage cube, beginComputePass, dispatch, submit, return 6 faces / cube `TextureGpuResource`); (b) add `equirectSource`to`app-environment-resources.ts` `WebGpuAppEnvironmentAssetInput`(line ~121) + when present (no pre-faceted faces) project → feed the diffuse (T2 convolution) + specular (PMREM) builders; (c)`examples/ibl-equirect.{html,main,worker}.js`+`test/e2e/ibl-equirect.spec.ts`(Done-when #1/#2/#4). ASSET BLOCKER: no 2:1 equirect`.hdr`in-repo (only the 6:1 cube atlas`examples/assets/pisa-studio-rgbe-cube.hdr`); author a small RGBE equirect via the existing `parseHdrRgbe`/`loadHdrFromUri` round-trip, OR synthesize equirect float/byte data and feed it directly (note the +Z-centered UV convention — author the equirect to match, or switch the pipeline to three.js's atan2(z,x) and update the vitest's +Z expectation to u=0.75). M5-T4 (HDR rgba16float scene buffer + exposure) and M5-T6 (SSAO→indirect only) are ALSO ready (no deps, no asset blocker) if T3's asset proves blocking. ENV: WebGPU e2e turnkey via scripts/webgpu-e2e.sh (SwiftShader+xvfb); references/ present.`
...
#### `M5-T3` · Equirectangular HDR -> cubemap projection pass; auto-wire single-asset IBL — 🟡 in-progress (2026-05-30 · 3f54524)
