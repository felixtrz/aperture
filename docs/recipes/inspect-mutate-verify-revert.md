...
Start a managed dev session from the app root (the `showcase/platformer` app
works as-is):
...
## Verbatim showcase transcript

This sequence runs as-is from `showcase/platformer/` (entity values from
`showcase/platformer/src/systems/setup.system.ts` `#spawnObjects`: the `flag`
entity is spawned at `FLAG.position` and no system rewrites it after `init`,
so the edit and revert are observable). `FLAG.position` is `[0, 3.481, -6]`
(`showcase/platformer/src/lib/platformer-data.ts`, `FLAG`). The `--json`
argument form is the one the e2e suite drives (`runCli(["tool", "input_key",
"--json", ...])` in `test/e2e/cli-ai-tools.spec.ts`).

```sh
pnpm exec aperture dev up --headless
pnpm exec aperture tool ecs_find_entities --json '{"key":"flag","limit":1}'
pnpm exec aperture tool ecs_snapshot --json '{"key":"flag","label":"before-edit"}'
pnpm exec aperture tool ecs_set_component_field --json '{"component":"aperture.transform.local","field":"translation","value":[1,2,3]}'
pnpm exec aperture tool ecs_diff --json '{"key":"flag","label":"after-edit"}'
pnpm exec aperture tool ecs_set_component_field --json '{"component":"aperture.transform.local","field":"translation","value":[0,3.481,-6]}'
pnpm exec aperture tool ecs_diff --json '{"key":"flag","label":"after-revert"}'
...
success case) and `showcase/platformer/src/lib/platformer-data.ts` (`FLAG`, the
spawn translation `[0, 3.481, -6]` being restored).

Expected reports along the way:

- `ecs_find_entities` → `ok: true`, `result.total: 1`,
  `result.summaries[0].key: "flag"` with
  `summaries[0].entity: { index, generation }`.
- `ecs_snapshot` → `ok: true`, `result.label: "before-edit"`,
  `result.summaries[0].localTransform.translation: [0, 3.481, -6]`.
