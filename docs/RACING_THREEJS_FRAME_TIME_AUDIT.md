...
- Latest heap allocation sample after numeric internal snapshot change-set
  ...
  retained-heap fix.
- New render-loop trace harness:
  `scripts/racing-render-loop-trace.mjs` / `pnpm run trace:racing -- ...`.
  ...
  extension, records Chrome trace/CPU-profile artifacts, summarizes
...
- Latest render-state parser cache run:
  `/tmp/racing-paired-render-state-cache/summary.json` (`2026-06-18`, command:
  `node scripts/racing-render-loop-trace.mjs ... --headed --no-trace
--no-cpu-profile`). Aperture idle phase total improved to
  `2.78 ms` p50 / `3.08 ms` p95, with `prepare` at `1.60 ms` p50 /
  `1.78 ms` p95. Aperture drive phase total was `4.15 ms` p50 /
...
--three-gl-diagnostics --headed`). This is a diagnostic run, not the fair
  benchmark path, because it wraps additional WebGL methods. It measured about
  `452` total wrapped GL calls per idle frame (`359` non-draw/state calls) and
  `480.5` total wrapped GL calls per drive frame (`386` non-draw/state calls).
  The dominant method counts were `uniformMatrix4fv` (`111-116`/frame),
  `bindVertexArray` (`71-73`/frame), `drawElements` (`68-70`/frame),
  `bindFramebuffer` (`27`/frame), `bindTexture` (`23`/frame), and
  `useProgram` (`19`/frame).
- Rejected WebGPU whole-frame timestamp attempt: adding command-encoder-level
  timestamp writes around `executeFrameGraph()` produced the browser timestamp
  floor (`0.001 us`) in Racing headed traces. That path was removed from the
  exposed diagnostics and must not be used as evidence that Aperture full-frame
...
- Latest trace artifact run:
  ...
  drive. Use this for phase/stack inspection, not headline tail-latency, because
  trace collection perturbed Aperture p99/max RAF cadence.
- Latest clean no-trace/no-profile run:
...
calls in idle and `93-98` while driving in the latest headed paired run. Aperture
reported `25` idle draw calls and `32-38` while driving. A separate opt-in
...
about `2.8 ms` p50 idle and `4.1 ms` p50 drive after render-state token caching.
`prepare` was the largest normal bucket (`1.6 ms` p50 idle, `2.4 ms` p50
drive), followed by drive `collect`/`sort`; normal `submit` was only about
`0.30-0.41 ms` p50. Three.js's injected JS `render` section stayed around
`0.5-0.6 ms` p50 / `<0.7 ms` p95. A
...
- Heavy generated worker summaries are now published on an elapsed-time cadence
  ...
  tick rate.
- Material pipeline render-state token parsing is now cached by stable pipeline
  key. This removes repeated string splitting/token scanning across material
  queue routing, shadow caster filtering, pipeline descriptor/report creation,
  and diagnostics for unchanged pipeline keys.
...
elapsed time. In the latest post-cache headed run, Aperture is at least
display-cadence tied with three.js in the short sample, but still needs repeated
confirmation and still has much higher heap. The latest write probe removes the
idle `WorldTransforms/storage` hot spot and keeps drive write
...
problem is CPU render phase cost, frame cadence/tail latency, heap retention,
residual change-set/bounds broadness, and particle-heavy drive work, not one
dominant dynamic-buffer byte source or one stale-snapshot message flood.
...
- Instrumentation:
  ...
  - Chrome CPU profiles, summarized into self-time buckets;
  - three.js user-timing phase marks around controls, physics, particles,
    audio, lap timing, and `renderer.render`;
  - wrapped WebGL draw methods for actual GL draw counts;
  - optional deep WebGL method counting via `--three-gl-diagnostics`;
  - non-blocking three.js WebGL timer queries when Chrome exposes
    `EXT_disjoint_timer_query`;
  - Aperture generated render diagnostics, phase timings, draw counts, and
    opt-in per-boundary GPU timestamps via `--aperture-gpu-timings`.
...
`/tmp/racing-paired-render-state-cache/summary.json`.
...
| -------- | ---------------------------------: | ---------------------------------: | -------------: | ----------------: | -----------------------------: | -------------------------: | ------------------------: | ---------------------------: | ---------------------: |
| Idle     | `16.66 / 17.67 / 17.68 / 17.68 ms` | `16.70 / 17.60 / 17.70 / 17.70 ms` |           `25` |              `93` |               `2.78 / 3.08 ms` |           `1.60 / 1.78 ms` |          `0.30 / 0.40 ms` |             `0.60 / 0.70 ms` |       `5.05 / 8.24 ms` |
| Drive    | `16.66 / 17.66 / 17.67 / 17.67 ms` | `16.70 / 17.70 / 17.75 / 17.80 ms` |        `32-38` |           `93-98` |               `4.15 / 4.97 ms` |           `2.44 / 3.13 ms` |          `0.41 / 0.52 ms` |             `0.50 / 0.60 ms` |       `4.85 / 8.25 ms` |

Interpretation: headline RAF cadence is at least tied in this short run, but
prior repeats have enough variance that this is not yet a completed benchmark
goal. The actionable delta remains the app-side CPU phase profile:
