...
| Subemitters                                      | `subEmitters`, composite `type: "composite"` effects            | Subemitter module is diagnostic-only; composite VFX assets (one `particle-effect` with `type: "composite"`) represent coordinated child emitters and expand into leaf packets at extraction |
