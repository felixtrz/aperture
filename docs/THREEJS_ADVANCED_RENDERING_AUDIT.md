...
geometry with an oldest-first live-decal cap surfaced in the frame report), CPU
...
re-realization — shipped as `examples/cloth-flag`), and custom-shader skinning
(scenario #5) shipped as parity plan F3 (`material.customWgsl({ skinned: true })`
prepends the versioned `apertureSkin(...)` header and binds the joint palette at
`@group(1) @binding(1)` — the `skinned-custom-material` dissolve character; morph
deltas for custom materials remain a deliberate deferral), leaving only one 🟡
(planar mirror #7). §10 ranks the gap closures by how much game-dev surface each
unlocks.
...
| Textures + samplers      | Texture uniforms; `texture()` nodes                                   | ✅ App — binding-layout variants (`sampleType` float/unfilterable/depth/sint/uint, `viewDimension` 2d/cube/3d/2d-array, `multisampled`, sampler `filtering`/`non-filtering`/`comparison`) close the old float/2D/filtering hard-coding (parity plan B4; 3d/2d-array added in E5) | Parity             |
...
| 4   | Vertex-animated foliage/flags (wind)                  | ✅       | ✅       | Displacement works and `entryPoints.shadowVertex` mirrors it into the shadow map (parity plan A4); custom-shader skinning shipped as #5                                                                                                                                                                                                                                                                                                                                                            |
| 5   | Custom shader on skinned characters                   | ✅       | ✅       | `material.customWgsl({ skinned: true })` binds the joint palette at `@group(1) @binding(1)` (the StandardMaterial location) + the JOINTS_0/WEIGHTS_0 vertex attributes, and prepends the versioned `apertureSkin(...)` header — a custom vertex shader skins with zero GPU wiring; composes with `lighting: "lit"` (parity plan F3, `examples/skinned-custom-material` = a skinned dissolve character). Morph deltas for custom materials are an honest deferral (skinning ships; morph is future) |
...
✅ 19 / 🟡 1 / ❌ 0 — **there is no remaining ❌**. Scenario #5 (custom shader on
skinned characters) flips to ✅ with parity plan F3: `material.customWgsl({
skinned: true })` prepends the versioned `apertureSkin(...)` contract header,
adds the JOINTS_0/WEIGHTS_0 vertex attributes, and binds the existing joint
palette at `@group(1) @binding(1)` (an extra binding in the transforms group, so
it fits `maxBindGroups` = 4 alongside the A1 lit `@group(3)` and composes with
`lighting: "lit"`), shipped with the `skinned-custom-material` dissolve-character
example — morph deltas for custom materials are the one honest deferral (skinning
ships; morph is a future opt-in). Scenario #17 (decals) flips
...
10. **Skinning/morph inputs for custom pipelines** (unblocks #5). _Shipped
    (skinning)_ — parity plan F3: `material.customWgsl({ skinned: true })`
    prepends the versioned `apertureSkin(...)` header, adds the JOINTS_0/WEIGHTS_0
    vertex attributes, and binds the existing joint palette at `@group(1)
@binding(1)` (the StandardMaterial location, so it composes with the A1 lit
    `@group(3)`); `examples/skinned-custom-material`. Morph deltas for custom
    materials are the one honest deferral (skinning ships; morph is a future
    opt-in — the standard morph path binds three extra storage buffers +
    per-instance descriptors, disproportionate to the skinning core).
