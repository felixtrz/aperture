...
```jsonc
{
  ...
  "snapshot": {
    ...
    "quads": {
      "version": 1,
      "instanceFloatStride": 24,
      "instanceWordStride": 8,
      "instanceFloats": {
        "$typedArray": "Float32Array",
        "base64": "…",
        "length": 24,
  ...
  "sourceAssets": {
    /* SerializedSourceAssetRegistry: handles + base64/Uint8Array mesh bytes, FULL (no patch state) */
  },
}
```
...
1. **Asset-in-Node (Track 1).** No image decoder, no `globalThis.location`; external/texture-asset configs crash or can't produce pixels. **Default:** ship for **procedural-asset apps first**; always inject the Node asset loader (P1.3) so external-asset apps load _structurally_ with placeholder textures + warnings; real pixels come from Track 2. Document in `--help` + AI_TOOLING.md.
