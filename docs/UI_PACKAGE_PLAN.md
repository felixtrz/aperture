...
| **M4 Conditional/responsive** | ✅ core | Layered **`resolveStyle`** (important > focus > active > hover > dark > responsive > base) + `activeBreakpoints` (sm…2xl), fully tested. _Follow-up:_ per-frame live restyle wiring needs interaction hover/active/focus state surfaced; `Viewport` scale mode needs view dimensions threaded into extraction.                                                  |
...
4. **Run Yoga inline on the existing simulation worker**, where layout already
   runs. **No dedicated layout worker.** The retained Yoga tree is a _derived
   extraction cache_ (reconciled with the "ECS is source of truth" invariant in
...
A framework-agnostic fluent tree that _compiles to ECS components_:

```ts
const root = ui.screen({ scale: "viewport" }, [
  ui.column(
    { padding: 16, gap: 8, backgroundColor: "#1b1d23", borderRadius: 12 },
    [
...
- **`ui` owns everything above the wire**: the authoring components
  ...
  _logic_ that fills the packets, the kits.
...
ownership of all UI _logic and authoring_, and is the **main thing to validate in
...
> _Lighter fallback if the seam proves awkward:_ leave the UI authoring +
...
loadYoga()` **once during worker system startup**, before the layout system
  ...
  `/load` explicitly.) _This corrects an earlier "sync-init" assumption._
...
2. **`UiFreezeLayout` (explicit).** A flag on a subtree root that suppresses dirty
   ...
   flag forces one pass. **Verified-cheap:** with Yoga, "freeze" is literally _not
   calling `calculateLayout`_ (rects persist; `getComputedLayout()` is a field
...
**worker-by-default**. Today's extraction is a _pure rebuild every frame_ with no
...
- ECS stays the only source of truth; the snapshot stays an immutable, serializable
  ...
  _internal optimization of the extraction step_, exactly like Bevy's `UiSurface`.
...
The only residual caveat (sub-pixel hit-test edges across _different_ native
...
input system across _three_ threads.
...
- The SAB transport's **packed encoder does not currently encode UI packets**
  ...
  fallback**). UI layout _runs_ on the worker regardless; but to put computed UI
...
- Nothing here _requires_ SAB. On non-isolated pages the transferable path carries
  the same packets.
...
- **WGSL port notes:** per-instance add `cornerRadii: vec4<f32>` (skip uikit's
  ...
  `fwidth(x)=abs(dpdx(x))+abs(dpdy(x))`. For _uniform_ borders the simpler iq
...
the worker only ever sees a stream of input events. _(A future non-DOM/native host
would need an OS IME bridge instead.)_
...
`UiScreenScaleMode.Viewport`, which today is _defined but never read_ (audit §3.9).
...
edge since its UI _is_ scene geometry).
...
- **Move** UI authoring components, hit-testing (`render/ui-hit-test.ts`), and
  extraction _logic_ into `ui`; **keep** the packet types + `RenderSnapshot`
...
- **`.changeset/config.json`** — add `@aperture-engine/ui` to the **`fixed`** group
  (lockstep versioning). _Required._
...
- **`webgpu`** gains a `workspace:^` dep on `ui` only if any UI _types_ it needs
...
_Reference implementation available locally at `references/uikit/` (gitignored):
panel SDF/border shader `panel/material/shader.ts`; clipping `clipping.ts`; text
input `text/input/hidden-input.ts` + `text/selection/_`+`text/layout/query.ts`;
scrollbars `scroll.ts`; style resolver `properties/_`+`pub-sub/src/index.ts`._

_Recommended next step:_ approve D1–D5 (or amend), then I scaffold **M0** — the
