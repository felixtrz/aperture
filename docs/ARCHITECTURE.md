...
## The ECS → Render Bridge

Aperture's ECS-to-render bridge follows a single principle: rendering is a
derived view of authoritative ECS state.
...
future render world may grow richer internal structure, but it must not become
...
The preferred mesh authoring direction stores mesh and material handles in
separate components, with unsuffixed names because Aperture is a 3D-only
runtime. The early combined `MeshRenderer` component has been removed from the
active API.
