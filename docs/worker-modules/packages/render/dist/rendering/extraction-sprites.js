import { assetHandleKey, Enabled, transformPoint, WorldTransform, } from "/aperture/worker-modules/packages/simulation/dist/index.js";
import { RenderLayer, RenderOrder, SpriteBillboardMode, SpriteBlendMode, SpriteCoordinateMode, SpriteSizeMode, Sprite, Visibility, validateSpriteInput, } from "./index.js";
import { createRenderSortKey, createStableRenderId, encodeQuadInstanceFlags, QUAD_INSTANCE_FLOAT_STRIDE, QUAD_INSTANCE_WORD_STRIDE, } from "./snapshot.js";
...
export function extractSpriteDraws(world, assets, transforms, bounds, diagnostics, cameraLayerMask, viewCullContexts, quadInstanceFloats, quadInstanceWords, quadBatches) {
    ...
    for (const entity of sortedEntities(query.entities)) {
        ...
        if (!validateTextureAssetState(input.texture, assets, entity, diagnostics)) {
            continue;
        }
        ...
        const boundsPacket = createSpriteBoundsPacket(bounds.length, entity, worldMatrix, width, height);
        ...
        const firstInstance = quadInstanceFloats.length / QUAD_INSTANCE_FLOAT_STRIDE;
...
function createSpriteBoundsPacket(boundsId, entity, worldMatrix, width, height) {
...
//# sourceMappingURL=extraction-sprites.js.map