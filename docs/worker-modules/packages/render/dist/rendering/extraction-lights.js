import { identityMat4, WorldTransform, } from "/aperture/worker-modules/packages/simulation/dist/index.js";
...
import { appendShadowRequest, diagnoseUnsupportedShadowRequest, readLightCookie, readShadowSettings, requiresLightTransform, } from "./extraction-light-settings.js";
...
export function extractLights(world, assets, transforms, diagnostics, environments, shadowRequests) {
    ...
    for (const entity of sortedEntities(query.entities)) {
        ...
        if (kind === "environment") {
            diagnoseUnsupportedShadowRequest(entity, kind, shadowSettings, diagnostics);
            const handle = readEnvironmentMapHandle(entity, diagnostics);
            if (handle === undefined ||
                (handle !== null &&
                    !validateEnvironmentMapAssetState(handle, assets, entity, diagnostics))) {
                continue;
            }
            environments.push({
                environmentId: createStableRenderId(entityRef(entity)),
                handle,
                color: Array.from(entity.getVectorView(Light, "color")),
        ...
        const worldTransformOffset = pushMatrix(transforms, hasWorldTransform ? readWorldMatrix(entity) : identityMat4());
        lights.push({
        ...
        appendShadowRequest(entity, kind, shadowSettings, shadowRequests, diagnostics);
    }
    return lights;
}
//# sourceMappingURL=extraction-lights.js.map