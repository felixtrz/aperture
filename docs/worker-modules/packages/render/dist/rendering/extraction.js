import { registerMetadataComponents, registerTransformComponents, } from "/aperture/worker-modules/packages/simulation/dist/index.js";
import { registerRenderAuthoringComponents } from "./index.js";
import { compareRenderSortKeys, createQuadSnapshotBuffers, QUAD_INSTANCE_FLOAT_STRIDE, } from "./snapshot.js";
import { extractAudioEmitters, extractAudioListener, } from "./extraction-audio.js";
import { createViewCullSignature, } from "./extraction-culling.js";
import { extractFogs } from "./extraction-fogs.js";
import { extractLights } from "./extraction-lights.js";
import { extractMeshDraws, } from "./extraction-meshes.js";
...
export { createRenderExtractionCache, } from "./extraction-meshes.js";
export function extractRenderSnapshot(world, assets, options = {}) {
    ...
    const bounds = [];
    const viewCullContexts = [];
    const views = extractViews(world, viewMatrices, diagnostics, viewCullContexts);
    const cameraLayerMask = views.reduce((mask, view) => mask | view.layerMask, 0);
    ...
    const lights = extractLights(world, assets, transforms, diagnostics, environments, shadowRequests);
    const meshDraws = extractMeshDraws(world, assets, transforms, bones, morphTargetWeights, morphTargetDeltas, morphInstanceDescriptors, instanceTints, instanceAttributes, instanceAttributePackets, bounds, diagnostics, cameraLayerMask, fogs, viewCullContexts, viewCullSignature, options.cache).sort((a, b) => compareRenderSortKeys(a.sortKey, b.sortKey));
    const shadowCasterLayerMask = shadowRequests.reduce((mask, request) => mask | request.casterLayerMask, 0);
    const shadowDiagnostics = [];
    const shadowCasterDraws = shadowRequests.length === 0
        ? []
        : extractMeshDraws(world, assets, transforms, bones, morphTargetWeights, morphTargetDeltas, morphInstanceDescriptors, instanceTints, instanceAttributes, instanceAttributePackets, bounds, shadowDiagnostics, shadowCasterLayerMask, fogs, viewCullContexts, viewCullSignature, options.cache, {
            frustumCull: false,
    ...
    const spriteDraws = extractSpriteDraws(world, assets, transforms, bounds, diagnostics, cameraLayerMask, viewCullContexts, quadInstanceFloats, quadInstanceWords, quadBatches).sort((a, b) => compareRenderSortKeys(a.sortKey, b.sortKey));
    const particleEmitters = extractParticleEmitters(world, assets, options.frame ?? 0, options.time ?? 0, transforms, bounds, diagnostics, cameraLayerMask, viewCullContexts).sort((a, b) => compareRenderSortKeys(a.sortKey, b.sortKey));
    ...
    const audioEmitters = extractAudioEmitters(world, assets, transforms, diagnostics);
    const audioListener = extractAudioListener(world, transforms, diagnostics, audioEmitters.length > 0);
    ...
    const proceduralSkies = extractProceduralSkies(world, diagnostics, cameraLayerMask);
    ...
    return {
        ...
        report: {
            ...
            cullStats: viewCullContexts.map((context) => ({ ...context.stats })),
        },
    };
}
...
function appendUniqueDiagnostics(target, source) {
    if (source.length === 0) {
        return;
    }
