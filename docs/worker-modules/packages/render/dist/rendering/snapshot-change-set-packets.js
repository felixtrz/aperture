...
export function viewPackets(snapshot) {
    return {
        ...
        signature: (view) => stableStringify({
            view: viewSignaturePacket(view),
            viewMatrix: matrixAt(snapshot?.viewMatrices, view.viewMatrixOffset),
            projectionMatrix: matrixAt(snapshot?.viewMatrices, view.projectionMatrixOffset),
            viewProjectionMatrix: matrixAt(snapshot?.viewMatrices, view.viewProjectionMatrixOffset),
        }),
    };
}
export function meshDrawPackets(snapshot) {
    return meshDrawPacketFamily(snapshot, snapshot?.meshDraws ?? [], "mesh-draw");
}
export function shadowCasterDrawPackets(snapshot) {
    return meshDrawPacketFamily(snapshot, snapshot?.shadowCasterDraws ?? [], "shadow-caster-draw");
}
function meshDrawPacketFamily(snapshot, packets, keyPrefix) {
    return {
        packets,
        state: snapshot ?? null,
        ...
        signature: (draw) => stableStringify({
            draw: meshDrawSignaturePacket(draw),
            worldTransform: matrixAt(snapshot?.transforms, draw.worldTransformOffset),
            instanceTint: draw.instanceTintOffset === undefined
                ? null
                : vec4At(snapshot?.instanceTints, draw.instanceTintOffset),
        }),
    };
}
export function lightPackets(snapshot) {
    return {
        ...
        signature: (light) => stableStringify({
            light: lightSignaturePacket(light),
            worldTransform: matrixAt(snapshot?.transforms, light.worldTransformOffset),
        }),
    };
}
export function environmentPackets(snapshot) {
    return {
        ...
        signature: (environment) => stableStringify({
            ...environment,
            handle: environment.handle === null
                ? null
                : assetHandleKey(environment.handle),
        }),
    };
}
export function proceduralSkyPackets(snapshot) {
    return {
...
export function runtimeUniformPackets(snapshot) {
    return {
...
export function shadowRequestPackets(snapshot) {
    return {
...
export function boundsPackets(snapshot) {
    return {
...
function boundsPacketKey(bounds) {
    ...
    const packed = bounds.entity.index * BOUNDS_ENTITY_KEY_GENERATION_STRIDE +
        bounds.entity.generation;
    if (bounds.entity.generation >= 0 &&
...
function formatBoundsPacketKey(key) {
...
function viewPacketsEqual(previous, next, previousState, nextState) {
    ...
    return (previous.viewId === next.viewId &&
        ...
        numericRangeEqual(previousSnapshot?.viewMatrices, previous.viewMatrixOffset, nextSnapshot?.viewMatrices, next.viewMatrixOffset, 16) &&
        numericRangeEqual(previousSnapshot?.viewMatrices, previous.projectionMatrixOffset, nextSnapshot?.viewMatrices, next.projectionMatrixOffset, 16) &&
        numericRangeEqual(previousSnapshot?.viewMatrices, previous.viewProjectionMatrixOffset, nextSnapshot?.viewMatrices, next.viewProjectionMatrixOffset, 16));
}
function meshDrawPacketsEqual(previous, next, previousState, nextState) {
    const previousSnapshot = snapshotState(previousState);
    const nextSnapshot = snapshotState(nextState);
    return (previous.renderId === next.renderId &&
        ...
        numericRangeEqual(previousSnapshot?.transforms, previous.worldTransformOffset, nextSnapshot?.transforms, next.worldTransformOffset, 16) &&
        optionalNumericRangeEqual(previousSnapshot?.instanceTints, previous.instanceTintOffset, nextSnapshot?.instanceTints, next.instanceTintOffset, 4));
}
function lightPacketsEqual(previous, next, previousState, nextState) {
    const previousSnapshot = snapshotState(previousState);
    const nextSnapshot = snapshotState(nextState);
    return (previous.lightId === next.lightId &&
        entityRefsEqual(previous.entity, next.entity) &&
...
function environmentPacketsEqual(previous, next) {
    return (previous.environmentId === next.environmentId &&
        nullableHandlesEqual(previous.handle, next.handle) &&
        vecEqual(previous.color, next.color, 4) &&
        previous.intensity === next.intensity &&
        previous.layerMask === next.layerMask);
}
function proceduralSkyPacketsEqual(previous, next) {
    return (previous.skyId === next.skyId &&
        entityRefsEqual(previous.entity, next.entity) &&
...
function runtimeUniformPacketsEqual(previous, next) {
    return (previous.uniformId === next.uniformId &&
        entityRefsEqual(previous.entity, next.entity) &&
        previous.key === next.key &&
        previous.version === next.version &&
        stableStringify(previous.values) === stableStringify(next.values));
}
function shadowRequestPacketsEqual(previous, next) {
    return (previous.shadowId === next.shadowId &&
...
function boundsPacketsEqual(previous, next) {
    return (entityRefsEqual(previous.entity, next.entity) &&
...
function optionalNumericRangeEqual(a, aOffset, b, bOffset, length) {
    if (aOffset === undefined || bOffset === undefined) {
        return aOffset === bOffset;
    }
    return numericRangeEqual(a, aOffset, b, bOffset, length);
}
function numericRangeEqual(a, aOffset, b, bOffset, length) {
    const aValid = a !== undefined && aOffset >= 0 && aOffset + length <= a.length;
    const bValid = b !== undefined && bOffset >= 0 && bOffset + length <= b.length;
    if (!aValid || !bValid) {
...
function optionalVecEqual(a, b, length) {
    if (a === undefined || b === undefined) {
        return a === b;
    }
    return vecEqual(a, b, length);
}
function vecEqual(a, b, length) {
...
function viewSignaturePacket(view) {
    const { viewMatrixOffset: _viewMatrixOffset, projectionMatrixOffset: _projectionMatrixOffset, viewProjectionMatrixOffset: _viewProjectionMatrixOffset, ...packet } = view;
    return packet;
}
function meshDrawSignaturePacket(draw) {
    const { worldTransformOffset: _worldTransformOffset, instanceTintOffset, boundsIndex: _boundsIndex, ...packet } = draw;
    return {
        ...packet,
        hasInstanceTint: instanceTintOffset !== undefined,
    };
}
function lightSignaturePacket(light) {
    const { worldTransformOffset: _worldTransformOffset, ...packet } = light;
    return packet;
}
function matrixAt(values, offset) {
    return numericSlice(values, offset, 16);
}
function vec4At(values, offset) {
    return numericSlice(values, offset, 4);
}
function numericSlice(values, offset, length) {
...
//# sourceMappingURL=snapshot-change-set-packets.js.map