...
import { attributesMatch, dataConstructorsMatch, materialSlotsMatch, required, requiredElementCount, } from "./mesh-merge-utils.js";
export function collectSourceLayouts(sources, diagnostics) {
    const layouts = [];
    let vertexBase = 0;
    let indexBase = 0;
...
export function validateCompatibility(input) {
    for (const source of input.sources) {
        ...
        for (let index = 0; index < input.firstMesh.vertexStreams.length; index += 1) {
        ...
        if (!materialSlotsMatch(input.firstMesh.materialSlots, mesh.materialSlots)) {
            input.diagnostics.push({
...
function validateStreamCompatibility(meshKey, expected, actual, diagnostics) {
    const compatible = expected.id === actual.id &&
    ...
    if (actual.arrayStride % elementSize !== 0 ||
...
//# sourceMappingURL=mesh-merge-validation.js.map