...
function identityMatrixInto(target, offset) {
...
export function importGltfSkins(input) {
    ...
    root.skins.forEach((rawSkin, skinIndex) => {
        ...
        if (accessorIndex === null) {
            ...
            for (let i = 0; i < jointCount; i += 1) {
                if (i < decoded.count) {
                    inverseBindMatrices.set(decoded.values.subarray(i * MATRIX_FLOATS, i * MATRIX_FLOATS + MATRIX_FLOATS), i * MATRIX_FLOATS);
                }
                else {
                    identityMatrixInto(inverseBindMatrices, i * MATRIX_FLOATS);
                }
...
export function findImportedSkin(report, skinIndex) {
    return report.skins.find((skin) => skin.skinIndex === skinIndex) ?? null;
}
//# sourceMappingURL=gltf-skin-import.js.map