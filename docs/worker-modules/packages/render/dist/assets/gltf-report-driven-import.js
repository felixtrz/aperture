...
import { createGlbBufferSourceDiagnostics, resolveGlbBufferBytes, } from "./gltf-report-driven-import-buffers.js";
import { createGltfReportDrivenMeshReports } from "./gltf-report-driven-import-meshes.js";
export { gltfReportDrivenGlbImportReportToJsonValue, gltfReportDrivenGlbImportReportToSourceStatusJsonValue, gltfReportDrivenImportReportToJson, gltfReportDrivenImportReportToJsonValue, } from "./gltf-report-driven-import-json.js";
export function createGltfReportDrivenImportReport(options) {
    const diagnostics = [];
    const root = validateGltfRootForAssetMapping(options.root);
    const assetMapping = options.createAssetMapping === true
    ...
    const meshReports = options.createMeshAssets === true
    ...
    if (options.provided?.root !== undefined) {
        diagnostics.push({
            ...
            severity: "error",
            message: "Report-driven import creates its own root validation report; provided.root is not accepted.",
        });
    }
    if (options.provided?.sceneTraversal !== undefined) {
        diagnostics.push({
            ...
            severity: "error",
            message: "Report-driven import creates its own scene traversal report; provided.sceneTraversal is not accepted.",
        });
    }
    if (options.createAssetMapping === true &&
        options.provided?.assetMapping !== undefined) {
        diagnostics.push({
            code: "gltfImport.assetMappingConflict",
            severity: "error",
            message: "Report-driven import cannot accept provided.assetMapping when createAssetMapping is true.",
        });
    }
    if (options.createMeshAssets === true &&
        options.provided?.meshConstruction !== undefined) {
        diagnostics.push({
            code: "gltfImport.meshConstructionConflict",
            severity: "error",
            message: "Report-driven import cannot accept provided.meshConstruction when createMeshAssets is true.",
        });
    }
...
export function createGltfReportDrivenImportReportFromGlb(options) {
    const { source, resolveBufferBytes: resolveExternalBufferBytes, ...importOptions } = options;
    ...
    const resolveBufferBytes = (bufferIndex) => resolveGlbBufferBytes(bufferIndex, container.container?.json ?? {}, container.container?.binaryChunk ?? null, resolveExternalBufferBytes, resolvedBuffers);
    const diagnostics = createGlbBufferSourceDiagnostics(container.container.json, container.container.binaryChunk, resolveBufferBytes);
...
//# sourceMappingURL=gltf-report-driven-import.js.map