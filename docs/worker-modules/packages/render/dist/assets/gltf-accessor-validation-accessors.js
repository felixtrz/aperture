...
import { expectationForSemantic, hasUnsupportedQuantizedComponentType, } from "./gltf-accessor-validation-expectations.js";
import { ACCESSOR_COMPONENTS, COMPONENT_BYTE_SIZE, arrayField, integerField, isRecord, toDiagnosticValue, } from "./gltf-accessor-validation-utils.js";
export function validateGltfAccessorReference(context, primitive, input) {
    ...
    if (expectation === null) {
        const unsupportedQuantizedComponentType = hasUnsupportedQuantizedComponentType(input.semantic, accessor);
        pushAccessorValidationDiagnostic(context, primitive, input, {
            ...
            field: `accessors[${input.accessorIndex}]`,
            value: toDiagnosticValue(unsupportedQuantizedComponentType
    ...
    const bufferView = validateBufferView(context, primitive, input, bufferViewIndex);
    if (bufferView === null) {
        return null;
    }
    const componentType = integerField(accessor.componentType);
    const accessorType = typeof accessor.type === "string" ? accessor.type : null;
    const componentByteSize = componentType === null ? undefined : COMPONENT_BYTE_SIZE.get(componentType);
    const componentCount = accessorType === null ? undefined : ACCESSOR_COMPONENTS.get(accessorType);
    if (componentType === null ||
        accessorType === null ||
        componentByteSize === undefined ||
        componentCount === undefined) {
        pushAccessorValidationDiagnostic(context, primitive, input, {
            code: componentByteSize === undefined
                ? "gltfAccessor.unsupportedComponentType"
                : "gltfAccessor.unsupportedAccessorType",
            field: `accessors[${input.accessorIndex}]`,
            value: toDiagnosticValue(componentByteSize === undefined ? accessor.componentType : accessorType),
    ...
    const byteStride = bufferView.byteStride === null ? elementByteSize : bufferView.byteStride;
    ...
    const sourceByteLength = count === 0 ? 0 : (count - 1) * byteStride + elementByteSize;
...
//# sourceMappingURL=gltf-accessor-validation-accessors.js.map