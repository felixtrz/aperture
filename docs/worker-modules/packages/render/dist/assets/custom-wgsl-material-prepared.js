import { createInstanceAttributeLayout, } from "../materials/index.js";
export function createPreparedCustomWgslMaterial(input) {
    const shaderHash = stableStringHash(input.shaderCode);
    const instanceAttributes = createInstanceAttributeLayout(input.source.instanceAttributes);
    const moduleKey = `custom-wgsl-module:${input.shaderSourceKey}:${shaderHash}`;
    const pipelineKey = customWgslMaterialPipelineKey(input.source, shaderHash, instanceAttributes);
    ...
    const bindings = [...(input.source.bindings ?? [])].sort((a, b) => a.binding - b.binding);
...
function customWgslMaterialPipelineKey(source, shaderHash, instanceAttributes) {
    return [
        source.familyKey,
        `shader:${shaderHash}`,
        ...
        `specialization:${stableStringHash(JSON.stringify(source.pipelineKey.specialization))}`,
...
function customWgslBindingLayoutSignature(binding) {
...
//# sourceMappingURL=custom-wgsl-material-prepared.js.map