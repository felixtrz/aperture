...
export function gltfReportDrivenImportReportToJsonValue(report) {
    return {
        ...
        orchestration: gltfLoaderOrchestrationReportToJsonValue(report.orchestration),
        diagnostics: report.diagnostics.map((diagnostic) => ({ ...diagnostic })),
    };
}
export function gltfReportDrivenGlbImportReportToJsonValue(report) {
    return {
        valid: report.valid,
        container: {
            ok: report.container.ok,
            byteLength: report.container.container?.byteLength ?? null,
            chunks: report.container.container?.chunks.map((chunk) => ({ ...chunk })) ?? [],
            diagnostics: report.container.diagnostics.map((diagnostic) => ({
                ...diagnostic,
            })),
        },
        importReport: report.importReport === null
            ? null
            : gltfReportDrivenImportReportToJsonValue(report.importReport),
        diagnostics: report.diagnostics.map((diagnostic) => ({ ...diagnostic })),
    };
}
export function gltfReportDrivenGlbImportReportToSourceStatusJsonValue(report) {
    return {
        valid: report.valid,
        byteLength: report.container.container?.byteLength ?? null,
        chunks: report.container.container?.chunks.map((chunk) => ({
            type: chunk.type,
...
export function gltfReportDrivenImportReportToJson(report) {
    return JSON.stringify(gltfReportDrivenImportReportToJsonValue(report));
}
//# sourceMappingURL=gltf-report-driven-import-json.js.map