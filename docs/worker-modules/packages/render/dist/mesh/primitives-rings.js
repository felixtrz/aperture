import { clampInteger, createPrimitiveMeshAsset, interleavePrimitiveVertexList, positiveFinite, } from "./primitives-builders.js";
export function createCapsuleMeshAsset(options = {}) {
    const radius = positiveFinite(options.radius, 0.5);
    const height = Math.max(positiveFinite(options.height, radius * 4), radius * 2);
    ...
    for (let y = 1; y <= capSegments; y += 1) {
        ...
        rings.push(primitiveRing({
            ...
            normalY,
            radialSegments,
            v: (capSegments + (bodyHalfHeight > 0 ? 1 : 0) + y) /
...
export function createTorusMeshAsset(options = {}) {
...
function createRingMeshAsset(input) {
...
//# sourceMappingURL=primitives-rings.js.map