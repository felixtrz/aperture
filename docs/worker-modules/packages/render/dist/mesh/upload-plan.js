export function createMeshGpuUploadPlan(mesh) {
    const diagnostics = [];
    const vertexStreams = mesh.vertexStreams.flatMap((stream) => createVertexUploadDescriptor(mesh, stream, diagnostics));
    const indexBuffer = mesh.indexBuffer === undefined
        ? undefined
...
function createVertexUploadDescriptor(mesh, stream, diagnostics) {
    ...
    if (!Number.isFinite(stream.arrayStride) ||
    ...
    const updateRanges = normalizeUpdateRanges(stream.updateRanges, source.byteLength, diagnostics, "vertexStreams.updateRanges", stream.id);
    const descriptor = {
...
function requiredVertexStreamByteLength(stream) {
    if (stream.vertexCount === 0) {
        return 0;
    }
    return stream.attributes.reduce((length, attribute) => Math.max(length, (stream.vertexCount - 1) * stream.arrayStride +
        attribute.offset +
        meshVertexFormatByteSize(attribute.format)), 0);
}
...
function createIndexUploadDescriptor(mesh, indexBuffer, diagnostics) {
    ...
    const updateRanges = normalizeUpdateRanges(indexBuffer.updateRanges, source.byteLength, diagnostics, "indexBuffer.updateRanges");
    const descriptor = {
...
function normalizeUpdateRanges(ranges, byteLength, diagnostics, field, streamId) {
    if (ranges === undefined) {
        return undefined;
    }
    const normalized = [];
    for (const range of ranges) {
        if (!Number.isInteger(range.byteOffset) ||
            ...
            diagnostics.push({
                ...
                field,
                message: "Mesh update ranges must be 4-byte aligned byte windows inside the source buffer.",
            });
            continue;
        }
