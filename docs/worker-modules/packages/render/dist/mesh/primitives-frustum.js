import { clampInteger, createPrimitiveMeshAsset, interleavePrimitiveVertexList, nonNegativeFinite, normalize, positiveFinite, } from "./primitives-builders.js";
export function createCylinderMeshAsset(options = {}) {
...
export function createConeMeshAsset(options = {}) {
    return createFrustumMeshAsset({
...
function createFrustumMeshAsset(input) {
    ...
    for (let y = 0; y <= input.heightSegments; y += 1) {
        ...
        const radius = input.radiusBottom + (input.radiusTop - input.radiusBottom) * v;
    ...
    for (let y = 0; y < input.heightSegments; y += 1) {
        ...
        const lowerRadius = input.radiusBottom + (input.radiusTop - input.radiusBottom) * lowerV;
        const upperRadius = input.radiusBottom + (input.radiusTop - input.radiusBottom) * upperV;
        for (let x = 0; x < input.radialSegments; x += 1) {
...
//# sourceMappingURL=primitives-frustum.js.map