import { quat as wgpuQuat, vec3 as wgpuVec3, } from "/aperture/worker-modules/node_modules/wgpu-matrix/dist/3.x/wgpu-matrix.module.js";
import { EPSILON } from "./constants.js";
import { quat } from "./constructors.js";
import { v3 } from "./scalars.js";
export function quatFromAxisAngle(axis, radians, out = quat()) {
...
export function quatNormalize(value, out = quat()) {
    return wgpuQuat.normalize(value, out);
}
export function quatFromEuler(x, y, z, order = "YXZ", out = quat()) {
    return wgpuQuat.fromEuler(x, y, z, toWgpuRotationOrder(order), out);
}
export function quatFromEulerYXZ(x, y, z, out = quat()) {
...
export function rotateVec3ByQuat(value, rotation, out = wgpuVec3.create()) {
    return wgpuVec3.transformQuat(value, rotation, out);
}
export function quatLookAt(eye, target, up = [0, 1, 0], out = quat()) {
    ...
    if (xl <= EPSILON) {
        const fallback = Math.abs(zx) > 0.9 ? [0, 0, 1] : [1, 0, 0];
...
function quatIdentityTo(out) {
    return wgpuQuat.identity(out);
}
function toWgpuRotationOrder(order) {
    return order.toLowerCase();
}
function q(values, index) {
    const value = values[index];
    if (value === undefined) {
        throw new RangeError(`QuatLike is missing numeric value at index ${index}.`);
    }
    return value;
}
function quatFromBasis(m00, m10, m20, m01, m11, m21, m02, m12, m22, out) {
