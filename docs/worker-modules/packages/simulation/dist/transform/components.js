import { EcsType, defineComponent, } from "../ecs/index.js";
import { composeTrsMatrix, identityMat4, } from "/aperture/worker-modules/packages/math/dist/index.js";
export function registerTransformComponents(world) {
...
export function registerMetadataComponents(world) {
    world.registerComponent(Enabled);
    world.registerComponent(Name);
    world.registerComponent(DebugMetadata);
    return world;
}
export const LocalTransform = defineComponent("aperture.transform.local", {
...
export const Parent = defineComponent("aperture.transform.parent", {
    entity: { type: EcsType.Entity, default: null },
}, "Authoritative ECS transform parent. Null means this entity is a transform root.");
export const WorldTransform = defineComponent("aperture.transform.world", {
...
export const Children = defineComponent("aperture.transform.children", {
...
export function createLocalTransform(input = {}) {
    return {
...
export function createParent(parent = null) {
    return { entity: parent };
}
export function createWorldTransform(matrix = identityMat4()) {
    return {
...
export function createRootTransform(input = {}) {
    const local = createLocalTransform(input);
    const matrix = composeTrsMatrix(local.translation, local.rotation, local.scale);
    return {
        local,
...
function matColumn(matrix, column) {
    const offset = column * 4;
    return tuple4(read(matrix, offset), read(matrix, offset + 1), read(matrix, offset + 2), read(matrix, offset + 3));
}
