import { APERTURE_ENTITY_DIFF_COMMAND_CHANNEL, APERTURE_ENTITY_FIND_COMMAND_CHANNEL, APERTURE_ENTITY_GET_COMMAND_CHANNEL, APERTURE_ENTITY_HIERARCHY_COMMAND_CHANNEL, APERTURE_ENTITY_SET_COMPONENT_COMMAND_CHANNEL, APERTURE_ENTITY_SNAPSHOT_COMMAND_CHANNEL, } from "../../commands.js";
import { createApertureEntityHierarchy, createApertureEntityLookupSnapshot, diffApertureEntityLookupSnapshots, findApertureEntities, getApertureEntitySummary, setApertureEntityComponentField, } from "../../entity-lookup.js";
import { isRecord, jsonSafeValue, numberFromValue, stringArrayFromValue, stringFromValue, } from "../payload.js";
export function createGeneratedEntityToolBridge(world) {
    let finds = 0;
    let gets = 0;
    let mutations = 0;
    let snapshots = 0;
    ...
    return {
        handle(command) {
            if (command.channel === APERTURE_ENTITY_FIND_COMMAND_CHANNEL) {
                const report = findApertureEntities(world, findQueryFromPayload(command.payload, 50));
                finds += 1;
            ...
            if (command.channel === APERTURE_ENTITY_SET_COMPONENT_COMMAND_CHANNEL) {
                const request = setComponentRequestFromPayload(command.payload, lastFind, lastGet);
                const report = "diagnostic" in request
                    ? {
                        ok: false,
            ...
            if (command.channel === APERTURE_ENTITY_SNAPSHOT_COMMAND_CHANNEL) {
                const snapshot = createApertureEntityLookupSnapshot(world, snapshotOptionsFromPayload(command.payload, `generated-snapshot-${snapshots + 1}`));
                snapshots += 1;
                lastRequest = entityToolRequest(command);
                    ...
                    diagnostics = [
                        {
                            ...
                            severity: "error",
                            message: "Entity diff requires a previous generated entity snapshot.",
                            data: { channel: command.channel },
                            suggestedFix: "Request aperture.devtools.entity.snapshot before requesting aperture.devtools.entity.diff.",
                        },
                    ];
                    lastDiff = null;
                    return true;
                }
                const nextSnapshot = createApertureEntityLookupSnapshot(world, snapshotOptionsFromPayload(command.payload, `generated-diff-${diffs + 1}`));
                const diff = diffApertureEntityLookupSnapshots(lastSnapshot, nextSnapshot);
                snapshots += 1;
                diffs += 1;
        ...
        call(tool, payload) {
            if (tool === "ecs_find_entities" || tool === "ecs_query") {
                const report = findApertureEntities(world, findQueryFromPayload(payload, 50));
                finds += 1;
            ...
            if (tool === "ecs_set_component_field") {
                const request = setComponentRequestFromPayload(payload, lastFind, lastGet);
                const report = "diagnostic" in request
                    ? {
                        ok: false,
                        diagnostic: request.diagnostic,
            ...
            if (tool === "ecs_snapshot") {
                const snapshot = createApertureEntityLookupSnapshot(world, snapshotOptionsFromPayload(payload, `generated-snapshot-${snapshots + 1}`));
                snapshots += 1;
                lastRequest = { channel: tool, payload: jsonSafeValue(payload) };
                lastSnapshot = snapshot;
                lastDiff = null;
            ...
            if (tool === "ecs_diff") {
                ...
                const nextSnapshot = createApertureEntityLookupSnapshot(world, snapshotOptionsFromPayload(payload, `generated-diff-${diffs + 1}`));
                const diff = diffApertureEntityLookupSnapshots(lastSnapshot, nextSnapshot);
                snapshots += 1;
                diffs += 1;
                lastSnapshot = nextSnapshot;
            ...
            if (tool === "ecs_get_component_schema") {
                ...
                diagnostics = report.diagnostics;
                return {
                    ok: report.diagnostics.every((diagnostic) => diagnostic.severity !== "error"),
                    result: report,
                    diagnostics: report.diagnostics,
                };
            }
                ...
                diagnostics: [
                    {
                        ...
                        data: { tool },
                        suggestedFix: "Use one of the supported ECS tools or add a focused generated worker handler.",
                    },
                ],
            };
        },
...
export function entityRefFromValue(value) {
    if (!isRecord(value)) {
        return null;
    }
...
function createComponentSchemaReport(world, payload) {
    const requested = stringFromValue(isRecord(payload) ? (payload["component"] ?? payload["id"]) : undefined);
        ...
        for (const component of entity.getComponents()) {
            ...
            components.set(component.id, {
                ...
                ...(typeof component.description === "string" &&
                    ...
                    : {}),
                fields: jsonSafeValue(component.schema),
            });
        }
    }
    const schemas = [...components.values()].sort((a, b) => a.id.localeCompare(b.id));
    const diagnostics = requested !== undefined && schemas.length === 0
        ? [
            {
                ...
                suggestedFix: "Check the component id or inspect an entity that currently has the component.",
            },
        ]
        : [];
    return { schemas, diagnostics };
}
function findQueryFromPayload(payload, fallbackLimit) {
...
function entityRefFromPayload(payload, lastFind, lastGet) {
    const record = isRecord(payload) ? payload : {};
...
function firstEntityFromFindReportPayload(payload) {
    const record = isRecord(payload) ? payload : {};
...
function setComponentRequestFromPayload(payload, lastFind, lastGet) {
    const record = isRecord(payload) ? payload : {};
    ...
    if (entity === null) {
        return {
            diagnostic: missingEntityRefDiagnostic(APERTURE_ENTITY_SET_COMPONENT_COMMAND_CHANNEL),
        };
    }
    if (component === undefined || field === undefined) {
        return {
            diagnostic: {
                ...
                suggestedFix: "Dispatch aperture.devtools.entity.setComponent with { entity, component, field, value }.",
            },
        };
    }
    return {
...
function missingEntityRefDiagnostic(channel) {
    return {
        code: "aperture.entityTools.missingEntityRef",
        severity: "error",
        message: "Entity tool command requires an entity { index, generation } reference.",
        data: { channel },
        suggestedFix: "Run aperture.devtools.entity.find first and pass the returned entity reference, or select an entity in the developer panel.",
    };
}
function snapshotOptionsFromPayload(payload, fallbackLabel) {
    const record = isRecord(payload) ? payload : {};
    const query = isRecord(record["query"]) ? record["query"] : record;
    const source = sourceFilterFromValue(query["source"]);
...
function entityToolRequest(command) {
    return {
        channel: command.channel,
        payload: jsonSafeValue(command.payload),
    };
}
function sourceFilterFromValue(value) {
    if (!isRecord(value)) {
        return undefined;
    }
...
function entityRefsFromValue(value) {
    ...
    const refs = value
        ...
        .filter((ref) => ref.index !== undefined && ref.generation !== undefined);
