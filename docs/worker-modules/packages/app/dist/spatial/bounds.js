import { raycast as raycastBounds } from "/aperture/worker-modules/packages/simulation/dist/index.js";
import { spatialEntryMatches } from "./filters.js";
import { entityRef, tuple3 } from "./math.js";
export function raycastBoundsHit(bounds, ray, options) {
    return raycastBoundsHits(bounds, ray, options)[0] ?? null;
}
export function raycastBoundsHits(bounds, ray, options) {
    const queryEntities = options.query?.entities;
    const candidates = queryEntities === undefined
        ? bounds
        : bounds.filter((candidate) => queryEntities.has(candidate.entity));
    const filteredCandidates = candidates.filter((candidate) => spatialEntryMatches(candidate, candidate.entity, options, "bounds"));
    const pickableByEntity = new Map();
 ...
 */
function applyPickablePolicy(hits, pickableByEntity) {
    ...
    const visible = cutoff === Number.POSITIVE_INFINITY
        ? hits
