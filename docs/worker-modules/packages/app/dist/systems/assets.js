...
import { createGlbUriLoadCache, createGltfEcsAuthoringCommandPlan, createGltfPrimitiveMaterialResolutionReport, createGltfUriLoadCache, createBasisUniversalKtx2Transcoder, createDracoMeshDecoder, createMeshoptDecoder as createMeshoptBufferDecoder, createWgslShaderAsset, createAudioClipAsset, createParticleEffectAsset, createTextureAsset, createStandardMaterialAsset, decodeImageUrlToTextureSource, loadGlbFromUri, loadGltfFromUri, registerGltfSourceAssetsFromReports, validateAudioClipAsset, validateParticleEffectAsset, validateTextureAsset, } from "/aperture/worker-modules/packages/render/dist/index.js";
import { assetHandleKey, createAnimationClipHandle, createAudioClipHandle, createEnvironmentMapHandle, createMaterialHandle, createParticleEffectHandle, createSamplerHandle, createSceneHandle, createShaderHandle, createTextureHandle, } from "/aperture/worker-modules/packages/simulation/dist/index.js";
...
export function createDefaultSystemGltfAssetDecoderProvider(options = {}) {
    const assetUrl = createDecoderAssetUrlResolver(options.baseUrl ?? DEFAULT_GLTF_DECODER_BASE_URL);
...
export function createSystemAssetAccess(options) {
    ...
    for (const [id, descriptor] of Object.entries(options.config?.assets ?? {})) {
        ...
        if (!options.registry.has(handle.renderHandle)) {
            const dependencies = systemAssetDependencies(handle);
            options.registry.register(handle.renderHandle, {
                label: descriptor.label ?? assetDescriptorLabel(id, descriptor),
                ...(dependencies.length === 0 ? {} : { dependencies }),
            });
        }
    }
    async function request(idOrHandle) {
        const handle = typeof idOrHandle === "string" ? lookup(idOrHandle) : idOrHandle;
        const registryHandle = handle.renderHandle;
        ...
        try {
            if (options.loader !== undefined) {
                ...
                handle.scene.value = scene;
            }
            else if (handle.kind === "shader") {
                const shaderAsset = await loadSystemShaderAsset(handle);
                options.registry.markReady(registryHandle, shaderAsset);
            }
            else if (handle.kind === "audio") {
                const audioAsset = await loadSystemAudioAsset(handle);
                options.registry.markReady(registryHandle, audioAsset);
            }
            else if (handle.kind === "texture") {
                const textureAsset = await loadSystemTextureAsset(handle);
                options.registry.markReady(registryHandle, textureAsset);
            }
            else if (handle.kind === "particle-effect") {
                const particleEffectAsset = loadSystemParticleEffectAsset(handle);
                options.registry.markReady(registryHandle, particleEffectAsset);
            }
    ...
    function lookup(id) {
        ...
        if (handle === undefined) {
            throw new ApertureSystemError("aperture.asset.unknown", `Asset '${id}' is not declared in aperture.config.ts.`, "Add the asset to the config assets object before using this.assets.");
        }
        return handle;
    }
    return {
        gltf(id) {
            ...
            if (handle.kind !== "gltf") {
                throw new ApertureSystemError("aperture.asset.kindMismatch", `Asset '${id}' is '${handle.kind}', not 'gltf'.`, "Use this.assets.gltf() only with asset.gltf() declarations.");
            }
            return handle;
        },
        texture(id) {
            ...
            if (handle.kind !== "texture") {
                throw new ApertureSystemError("aperture.asset.kindMismatch", `Asset '${id}' is '${handle.kind}', not 'texture'.`, "Use this.assets.texture() only with asset.texture() declarations.");
            }
            return handle;
        },
        hdr(id) {
            const handle = lookup(id);
            if (handle.kind !== "hdr") {
                throw new ApertureSystemError("aperture.asset.kindMismatch", `Asset '${id}' is '${handle.kind}', not 'hdr'.`, "Use this.assets.hdr() only with asset.hdr() declarations.");
            }
            return handle;
        },
        shader(id) {
            ...
            if (handle.kind !== "shader") {
                throw new ApertureSystemError("aperture.asset.kindMismatch", `Asset '${id}' is '${handle.kind}', not 'shader'.`, "Use this.assets.shader() only with asset.shader() declarations.");
            }
            return handle;
        },
        audio(id) {
            ...
            if (handle.kind !== "audio") {
                throw new ApertureSystemError("aperture.asset.kindMismatch", `Asset '${id}' is '${handle.kind}', not 'audio'.`, "Use this.assets.audio() only with asset.audio() declarations.");
            }
            return handle;
        },
        particleEffect(id) {
            ...
            if (handle.kind !== "particle-effect") {
                throw new ApertureSystemError("aperture.asset.kindMismatch", `Asset '${id}' is '${handle.kind}', not 'particle-effect'.`, "Use this.assets.particleEffect() only with asset.particleEffect() declarations.");
            }
            return handle;
        },
        request,
...
function createDecoderAssetUrlResolver(baseUrl) {
    const normalizedBase = baseUrl.endsWith("/") ? baseUrl : `${baseUrl}/`;
    return (path) => `${normalizedBase}${path}`;
}
function assetDescriptorLabel(id, descriptor) {
    return "url" in descriptor ? descriptor.url : id;
}
function systemAssetDependencies(handle) {
    if (handle.kind !== "particle-effect") {
        return [];
    }
    const particleEffect = handle;
    const dependencies = [];
...
function createSystemAssetHandle(id, descriptor) {
    if (descriptor.kind === "gltf") {
        return {
            ...
            scene: createSignal(null),
        };
    }
    if (descriptor.kind === "audio") {
        const audio = descriptor;
        return {
            id,
            kind: descriptor.kind,
            url: descriptor.url,
    ...
    if (descriptor.kind === "particle-effect") {
        const particleEffect = descriptor;
        return {
            id,
            kind: descriptor.kind,
            ...(descriptor.label === undefined ? {} : { label: descriptor.label }),
            ...
            ...(particleEffect.texture === undefined
                ? {}
                : {
                    texture: particleEffect.texture === null
                        ? null
            ...
            ...(particleEffect.sampler === undefined
                ? {}
                : {
                    sampler: particleEffect.sampler === null
                        ? null
    ...
    return {
        ...
        ready: createSignal(false),
        error: createSignal(null),
        renderHandle: descriptor.kind === "hdr"
            ? createEnvironmentMapHandle(id)
            : createShaderHandle(id),
    };
}
function loadSystemParticleEffectAsset(handle) {
    ...
    throw new ApertureSystemError("aperture.asset.particleEffectInvalid", `Particle effect asset '${handle.id}' is invalid. ${formatReportDiagnostics(report.diagnostics.map((diagnostic) => ({
        code: diagnostic.code,
        severity: "error",
        message: diagnostic.message,
...
async function loadSystemTextureAsset(handle) {
    const resolvedUrl = resolveAssetUrl(handle.url);
    if (resolvedUrl === null) {
        throw new ApertureSystemError("aperture.asset.invalidUrl", `Texture asset '${handle.id}' URL '${handle.url}' could not be resolved.`, "Use an absolute URL, a root-relative Vite public asset URL, or a data URL in aperture.config.ts.", {
            asset: handle.id,
            url: handle.url,
            kind: handle.kind,
            preload: handle.preload,
    ...
    if (typeof fetch !== "function") {
        throw new ApertureSystemError("aperture.asset.textureFetchUnavailable", `Texture asset '${handle.id}' cannot be fetched in this environment.`, "Provide an ApertureAssetLoader in tests/headless mode or run in a browser worker with fetch support.", {
            asset: handle.id,
            url: handle.url,
            kind: handle.kind,
            preload: handle.preload,
    ...
    return validatedTexture(handle, createTextureAsset({
        label: handle.label ?? handle.id,
        dimension: "2d",
        width: decoded.width,
...
function validatedTexture(handle, asset) {
    const report = validateTextureAsset(asset);
    if (report.valid) {
        return asset;
    }
    throw new ApertureSystemError("aperture.asset.textureInvalid", `Texture asset '${handle.id}' is invalid. ${formatReportDiagnostics(report.diagnostics.map((diagnostic) => ({
        code: diagnostic.code,
        severity: "error",
        message: diagnostic.message,
...
async function loadSystemAudioAsset(handle) {
    const resolvedUrl = resolveAssetUrl(handle.url);
    if (resolvedUrl === null) {
        throw new ApertureSystemError("aperture.asset.invalidUrl", `Audio asset '${handle.id}' URL '${handle.url}' could not be resolved.`, "Use an absolute URL, a root-relative Vite public asset URL, or a data URL in aperture.config.ts.", {
            asset: handle.id,
            url: handle.url,
            kind: handle.kind,
            preload: handle.preload,
    ...
    if (typeof fetch !== "function") {
        throw new ApertureSystemError("aperture.asset.audioFetchUnavailable", `Audio asset '${handle.id}' cannot be fetched in this environment.`, "Provide an ApertureAssetLoader in tests/headless mode or run in a browser worker with fetch support.", {
            asset: handle.id,
            url: handle.url,
            kind: handle.kind,
            preload: handle.preload,
    ...
    if (!response.ok) {
        throw new ApertureSystemError("aperture.asset.audioLoadFailed", `Audio asset '${handle.id}' failed to load with HTTP ${response.status}.`, "Check the audio URL in aperture.config.ts.", {
            asset: handle.id,
            url: handle.url,
    ...
    return validatedAudioClip(handle, createAudioClipAsset({
        ...base,
        bytes: await response.arrayBuffer(),
    }));
}
function validatedAudioClip(handle, asset) {
    const report = validateAudioClipAsset(asset);
    if (report.valid) {
        return asset;
    }
    throw new ApertureSystemError("aperture.asset.audioInvalid", `Audio asset '${handle.id}' is invalid. ${formatReportDiagnostics(report.diagnostics.map((diagnostic) => ({
        code: diagnostic.code,
        severity: "error",
        message: diagnostic.message,
...
async function loadSystemShaderAsset(handle) {
    const resolvedUrl = resolveAssetUrl(handle.url);
    if (resolvedUrl === null) {
        throw new ApertureSystemError("aperture.asset.invalidUrl", `Shader asset '${handle.id}' URL '${handle.url}' could not be resolved.`, "Use an absolute URL, a root-relative Vite public asset URL, or a data URL in aperture.config.ts.", {
            asset: handle.id,
            url: handle.url,
            kind: handle.kind,
            preload: handle.preload,
    ...
    if (typeof fetch !== "function") {
        throw new ApertureSystemError("aperture.asset.shaderFetchUnavailable", `Shader asset '${handle.id}' cannot be fetched in this environment.`, "Provide an ApertureAssetLoader in tests/headless mode or run in a browser worker with fetch support.", {
            asset: handle.id,
            url: handle.url,
            kind: handle.kind,
            preload: handle.preload,
    ...
    const response = await fetch(resolvedUrl);
    if (!response.ok) {
        throw new ApertureSystemError("aperture.asset.shaderLoadFailed", `Shader asset '${handle.id}' failed to load with HTTP ${response.status}.`, "Check the shader URL in aperture.config.ts.", {
            asset: handle.id,
            url: handle.url,
            status: response.status,
            kind: handle.kind,
...
async function loadSystemGltfAsset(input) {
    ...
    if (resolvedUrl === null) {
        throw new ApertureSystemError("aperture.asset.invalidUrl", `GLTF asset '${input.handle.id}' URL '${input.handle.url}' could not be resolved.`, "Use an absolute URL, a root-relative Vite public asset URL, or a data URL in aperture.config.ts.", {
    ...
    const loaded = sourceKind === "gltf"
        ? await loadGltfFromUri(resolvedUrl, {
            ...
            ...(input.gltfAssetDecoders?.createBasisKtx2Transcoder === undefined
                ? {}
                : {
                    createBasisKtx2Transcoder: input.gltfAssetDecoders.createBasisKtx2Transcoder,
                }),
            ...(input.gltfAssetDecoders?.ktx2TextureCompression === undefined
                ? {}
                : {
                    ktx2TextureCompression: input.gltfAssetDecoders.ktx2TextureCompression,
                }),
        })
        : await loadGlbFromUri(resolvedUrl, {
            ...
            ...(input.gltfAssetDecoders?.createMeshoptDecoder === undefined
                ? {}
                : {
                    createMeshoptDecoder: input.gltfAssetDecoders.createMeshoptDecoder,
                }),
            ...(input.gltfAssetDecoders?.createBasisKtx2Transcoder === undefined
                ? {}
                : {
    ...
    const importReport = loaded.loader === null
        ? null
    ...
    if (!loaded.ok || importReport === null) {
        throw new ApertureSystemError("aperture.asset.gltfLoadFailed", `GLTF asset '${input.handle.id}' failed to load. ${formatReportDiagnostics(loaded.diagnostics)}`, "Check the asset URL in aperture.config.ts and use a supported glTF/GLB file.");
    }
    if (importReport.assetMapping === null ||
        importReport.meshConstruction === null ||
        importReport.meshPrimitive === null) {
        throw new ApertureSystemError("aperture.asset.gltfNotRenderable", `GLTF asset '${input.handle.id}' did not produce renderable mesh/material reports.`, "Use a glTF/GLB with triangle mesh primitives and supported material inputs.");
    }
    const defaultMaterialHandle = createMaterialHandle(`${input.handle.id}.default-material`);
    ...
    if (!input.registry.has(defaultMaterialHandle)) {
        ...
        input.registry.markReady(defaultMaterialHandle, createStandardMaterialAsset({
            label: `${input.handle.id} default GLTF material`,
        }));
    }
    ...
    if (!registration.valid ||
        ...
        throw new ApertureSystemError("aperture.asset.gltfRegistrationFailed", `GLTF asset '${input.handle.id}' source assets could not be registered. ${formatReportDiagnostics(registration.diagnostics)}`, "Check for duplicate generated asset keys or unsupported glTF source assets.");
    }
    ...
    if (!primitiveMaterials.valid) {
        throw new ApertureSystemError("aperture.asset.gltfMaterialResolutionFailed", `GLTF asset '${input.handle.id}' primitive materials could not be resolved. ${formatReportDiagnostics(primitiveMaterials.diagnostics)}`, "Use supported glTF materials or provide material data for all primitives.");
    }
    ...
    if (!commandPlan.valid) {
        throw new ApertureSystemError("aperture.asset.gltfCommandPlanFailed", `GLTF asset '${input.handle.id}' could not be converted to ECS spawn commands. ${formatReportDiagnostics(commandPlan.diagnostics)}`, "Check the glTF scene hierarchy and mesh primitive data.");
    }
    ...
    const clips = importReport.animation.clips.map((imported) => {
        const handle = createAnimationClipHandle(`${input.handle.id}:animation:${imported.animationIndex}`);
...
export function systemAssetReadyMetadata(handle) {
    if (handle.kind !== "gltf") {
        return {};
    }
    ...
    return {
        ...
        materialAssetCount: scene.sourceRegistration.written.filter((entry) => entry.kind === "material").length,
        textures: scene.importReport.assetMapping?.textures.map((texture) => ({
            ...
            sourceData: texture.texture?.sourceData === undefined
                ? null
                : {
                    ...
                    mipLevelCount: texture.texture.sourceData.mipLevels?.length ?? 1,
                    mipLevels: texture.texture.sourceData.mipLevels?.map((level) => ({
...
function gltfSourceKindFromUrl(url) {
