import { Collider, ExternalForce, ExternalImpulse, KinematicTarget, PhysicsGravity, PhysicsColliderAxis, PhysicsColliderShapeKind, PhysicsBodyState, PhysicsCharacterController, PhysicsCharacterMassMode, PhysicsDebug, PhysicsJoint, PhysicsJointKind, PhysicsJointMotorMode, PhysicsJointMotorModel, PhysicsMaterial, PhysicsMaterialCombineRule, PhysicsVelocity, RigidBody, PhysicsRigidBodyType, } from "/aperture/worker-modules/packages/physics/dist/index.js";
import { RenderLayer, Sprite, SpriteBillboardMode, SpriteBlendMode, SpriteCoordinateMode, SpriteDepthMode, SpriteSizeMode, } from "/aperture/worker-modules/packages/render/dist/index.js";
import { AppEntityKey, AppEntitySource, AppEntityTags, Enabled, LocalTransform, Name, Parent, WorldTransform, } from "../../systems.js";
...
export function entitySummary(entity) {
    ...
    const physicsExternalForce = entityHasComponentId(entity, ExternalForce.id)
        ...
        : null;
    const physicsExternalImpulse = entityHasComponentId(entity, ExternalImpulse.id)
        ? physicsExternalImpulseSummary(entity)
        : null;
    const physicsKinematicTarget = entityHasComponentId(entity, KinematicTarget.id)
    ...
    const physicsGravity = entityHasComponentId(entity, PhysicsGravity.id)
        ...
        : null;
    const physicsCharacterController = entityHasComponentId(entity, PhysicsCharacterController.id)
...
export function collectActiveEntities(world) {
    const entityManager = world.entityManager;
    if (Array.isArray(entityManager.indexLookup)) {
        return entityManager.indexLookup.filter((entity) => entity !== null && entity?.active === true);
    }
    return [
        ...world.queryManager.registerQuery({ required: [] }).entities,
    ].filter((entity) => entity.active);
}
export function compareEntitySummaries(a, b) {
    return (a.entity.index - b.entity.index || a.entity.generation - b.entity.generation);
}
export function entityRefKey(ref) {
    return `${ref.index}:${ref.generation}`;
}
export function validEntityRef(ref) {
    return (Number.isInteger(ref.index) &&
        Number.isInteger(ref.generation) &&
        ref.index >= 0 &&
        ref.generation >= 0);
}
function localTransformSummary(entity) {
    return {
...
function worldTransformSummary(entity) {
    return {
        matrix: [
...
function renderLayerSummary(entity) {
    return {
        mask: entity.getValue(RenderLayer, "mask") ?? 1,
    };
}
function renderSpriteSummary(entity) {
...
function physicsRigidBodySummary(entity) {
...
function physicsColliderSummary(entity) {
    ...
    return {
        ...
        offsetTranslation: tuple3FromView(entity.getVectorView(Collider, "offsetTranslation")),
        offsetRotation: tuple4FromView(entity.getVectorView(Collider, "offsetRotation")),
...
function entityHasComponentId(entity, componentId) {
    return entity
        .getComponents()
        .some((component) => component.id === componentId);
}
function physicsVelocitySummary(entity) {
    return {
...
function physicsExternalForceSummary(entity) {
    return {
...
function physicsExternalImpulseSummary(entity) {
    return {
        impulse: tuple3FromView(entity.getVectorView(ExternalImpulse, "impulse")),
        angularImpulse: tuple3FromView(entity.getVectorView(ExternalImpulse, "angularImpulse")),
    };
}
function physicsKinematicTargetSummary(entity) {
    return {
        enabled: entity.getValue(KinematicTarget, "enabled") === true,
        translation: tuple3FromView(entity.getVectorView(KinematicTarget, "translation")),
        rotation: tuple4FromView(entity.getVectorView(KinematicTarget, "rotation")),
    };
}
function physicsGravitySummary(entity) {
    return {
        gravity: tuple3FromView(entity.getVectorView(PhysicsGravity, "gravity")),
    };
}
function physicsCharacterControllerSummary(entity) {
    ...
    return {
        ...
        maxSlopeClimbAngleEnabled: entity.getValue(PhysicsCharacterController, "maxSlopeClimbAngleEnabled") === true,
        maxSlopeClimbAngle: entity.getValue(PhysicsCharacterController, "maxSlopeClimbAngle") ??
            Math.PI / 4,
        minSlopeSlideAngleEnabled: entity.getValue(PhysicsCharacterController, "minSlopeSlideAngleEnabled") === true,
        minSlopeSlideAngle: entity.getValue(PhysicsCharacterController, "minSlopeSlideAngle") ??
            Math.PI / 3,
        snapToGroundDistance: entity.getValue(PhysicsCharacterController, "snapToGroundDistance") ?? 0,
        autostepEnabled: entity.getValue(PhysicsCharacterController, "autostepEnabled") === true,
        autostepMaxHeight: entity.getValue(PhysicsCharacterController, "autostepMaxHeight") ?? 0.1,
        autostepMinWidth: entity.getValue(PhysicsCharacterController, "autostepMinWidth") ?? 0.1,
        autostepIncludeDynamicBodies: entity.getValue(PhysicsCharacterController, "autostepIncludeDynamicBodies") === true,
        ...
        characterMass: entity.getValue(PhysicsCharacterController, "characterMass") ?? 0,
    };
}
function physicsMaterialSummary(entity) {
    const frictionCombine = entity.getValue(PhysicsMaterial, "frictionCombine");
    const restitutionCombine = entity.getValue(PhysicsMaterial, "restitutionCombine");
    return {
...
function physicsDebugSummary(entity) {
    return {
        colliderWireframes: entity.getValue(PhysicsDebug, "colliderWireframes") === true,
        contactNormals: entity.getValue(PhysicsDebug, "contactNormals") === true,
        bodyStateMarkers: entity.getValue(PhysicsDebug, "bodyStateMarkers") === true,
...
function physicsJointSummary(entity) {
...
function isPhysicsJointKind(value) {
    return typeof value === "string" && stringValueIn(value, PhysicsJointKind);
}
function isPhysicsCharacterMassMode(value) {
    return (typeof value === "string" && stringValueIn(value, PhysicsCharacterMassMode));
}
function isPhysicsMaterialCombineRule(value) {
    return (typeof value === "string" &&
        stringValueIn(value, PhysicsMaterialCombineRule));
}
function isPhysicsRigidBodyType(value) {
    return (typeof value === "string" && stringValueIn(value, PhysicsRigidBodyType));
}
function isPhysicsColliderShapeKind(value) {
    return (typeof value === "string" && stringValueIn(value, PhysicsColliderShapeKind));
}
function isPhysicsColliderAxis(value) {
    return typeof value === "string" && stringValueIn(value, PhysicsColliderAxis);
}
function isPhysicsJointMotorMode(value) {
    return (typeof value === "string" && stringValueIn(value, PhysicsJointMotorMode));
}
function isPhysicsJointMotorModel(value) {
    return (typeof value === "string" && stringValueIn(value, PhysicsJointMotorModel));
}
function isSpriteCoordinateMode(value) {
    return (typeof value === "string" && stringValueIn(value, SpriteCoordinateMode));
}
function isSpriteBillboardMode(value) {
    return typeof value === "string" && stringValueIn(value, SpriteBillboardMode);
}
function isSpriteSizeMode(value) {
    return typeof value === "string" && stringValueIn(value, SpriteSizeMode);
}
function isSpriteBlendMode(value) {
    return typeof value === "string" && stringValueIn(value, SpriteBlendMode);
}
function isSpriteDepthMode(value) {
...
function physicsBodyStateSummary(entity) {
    ...
    return {
        ...
        currentTranslation: tuple3FromView(entity.getVectorView(PhysicsBodyState, "currentTranslation")),
        currentRotation: tuple4FromView(entity.getVectorView(PhysicsBodyState, "currentRotation")),
        previousTranslation: tuple3FromView(entity.getVectorView(PhysicsBodyState, "previousTranslation")),
        previousRotation: tuple4FromView(entity.getVectorView(PhysicsBodyState, "previousRotation")),
        ...(typeof backendBodyId === "string" && backendBodyId.length > 0
            ? { backendBodyId }
            : {}),
    };
}
function entityRefFromEntity(entity) {
    return entity === null
        ? null
        : { index: entity.index, generation: entity.generation };
}
function tuple2FromView(view) {
    return [view[0] ?? 0, view[1] ?? 0];
}
function tuple3FromView(view) {
    return [view[0] ?? 0, view[1] ?? 0, view[2] ?? 0];
}
function tuple4FromView(view) {
    return [view[0] ?? 0, view[1] ?? 0, view[2] ?? 0, view[3] ?? 0];
}
function sourceSummary(entity) {
