import { collectActiveEntities, compareEntitySummaries, entitySummary, validEntityRef, } from "./summary.js";
export function findApertureEntities(world, query = {}) {
    const diagnostics = [];
    ...
    if (query.key !== undefined && matches.length > 1) {
        diagnostics.push({
            ...
            suggestedFix: "Keep app-authored keys globally unique, or use the returned { index, generation } pair for follow-up calls.",
        });
    }
    return {
        summaries,
...
export function getApertureEntitySummary(world, ref) {
...
function matchesEntityQuery(summary, query, nameRegex) {
    ...
    if (query.withComponents !== undefined &&
        !query.withComponents.every((componentId) => summary.componentIds.includes(componentId))) {
        return false;
    }
...
function matchesSource(summary, source) {
    ...
    if (source.gltfNodeIndex !== undefined &&
        candidate.gltfNodeIndex !== source.gltfNodeIndex) {
        return false;
    }
    if (source.gltfNodePath !== undefined &&
        candidate.gltfNodePath !== source.gltfNodePath) {
        return false;
    }
    return true;
}
function compileNamePattern(pattern, diagnostics) {
    ...
    try {
        ...
        diagnostics.push({
            ...
            suggestedFix: "Pass a valid JavaScript regular expression string, or search by exact key.",
        });
        return null;
    }
}
