 ...
 */
export function syncGeneratedDiagnostics(getDiagnostics, status, options = {}) {
    let lastErrorSig = "";
    let emptyWarned = false;
    const seenFrameDiags = new Set();
    const intervalMilliseconds = normalizeDiagnosticsSyncInterval(options.intervalMilliseconds);
    let disposed = false;
    let pendingTimer = null;
    const record = (value) => typeof value === "object" && value !== null
        ? value
        : null;
    const numberAt = (value) => typeof value === "number" && Number.isFinite(value) ? value : null;
    const surface = (diagnostics) => {
        ...
        if (error !== null) {
            ...
            if (sig !== lastErrorSig && (code !== "" || message !== "")) {
                lastErrorSig = sig;
                console.error(`[aperture] render error${code ? ` (${code})` : ""}: ${message}` +
                    (reason ? ` — ${reason}` : ""));
            }
        }
        else {
        ...
        if (counts !== null) {
            ...
            const anyDraws = meshDraws + spriteDraws + particleEmitters + quadInstances + uiNodes;
            if (views > 0 && anyDraws === 0) {
                if (!emptyWarned) {
                    emptyWarned = true;
                    console.warn("[aperture] render produced 0 draws for an active camera view — " +
        ...
        if (Array.isArray(frameDiags)) {
            for (const entry of frameDiags) {
                ...
                if (diag === null)
                    continue;
                const severity = typeof diag["severity"] === "string" ? diag["severity"] : "";
                if (severity !== "warning" && severity !== "error")
                    continue;
                const code = typeof diag["code"] === "string" ? diag["code"] : "";
                const message = typeof diag["message"] === "string" ? diag["message"] : code;
                const sig = `${severity}|${code}|${message}`;
                if (seenFrameDiags.has(sig))
                    continue;
...
function normalizeDiagnosticsSyncInterval(value) {
    if (typeof value !== "number" || !Number.isFinite(value)) {
        return DEFAULT_GENERATED_DIAGNOSTICS_SYNC_INTERVAL_MS;
    }
    return Math.max(MIN_GENERATED_DIAGNOSTICS_SYNC_INTERVAL_MS, Math.floor(value));
}
