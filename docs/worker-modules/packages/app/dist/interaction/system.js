import { Pickable, PickablePrecision, extractUiLayout, hitTestUiLayout, } from "/aperture/worker-modules/packages/render/dist/index.js";
...
export function runInteractionFrame(context, time) {
    const pointer = context.input.pointer.primary;
    const position = pointer.position.value;
    const pressed = pointer.pressed.value;
    const runtime = context.interaction;
    let hitEntity = null;
    let worldPoint = null;
    const uiDiagnostics = [];
    const uiLayout = extractUiLayout(context.world, uiDiagnostics, 0);
    ...
    if ((uiHit === null || !uiHit.blocksInput) &&
        ...
        if (hit !== null) {
            ...
            if (target.hasComponent(Pickable) &&
