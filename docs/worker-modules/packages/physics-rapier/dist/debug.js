import { createPhysicsAabbDebugLines, } from "/aperture/worker-modules/packages/physics/dist/index.js";
import { colliderEntries } from "./colliders.js";
import { jointAxis } from "./joints.js";
import { addScaledVec3, addVec3, bodyLocalPointToWorld, bodyLocalVectorToWorld, cloneVec3, normalizeQuat, normalizeVec3, quatFromRapierRotation, rotateVec3ByQuat, subtractVec3, vec3, } from "./math.js";
...
export function broadphaseAabbDebugLines(bodies, options) {
    return createPhysicsAabbDebugLines([...bodies.values()]
        .sort((a, b) => a.entity.localeCompare(b.entity))
        .flatMap((entry) => entry.colliders.map((collider) => ({ entry, collider })))
        .map(({ entry, collider }) => colliderAabb(entry, collider))
        .filter((aabb) => aabb !== null), options.broadphaseAabbColor).filter(isFiniteLine);
}
function colliderAabb(_entry, colliderEntry) {
    const localHalfExtents = colliderLocalHalfExtents(colliderEntry.descriptor.shape);
...
function colliderLocalHalfExtents(shape) {
    switch (shape.kind) {
        case "box":
...
function rotatedAabbHalfExtents(localHalfExtents, rotation) {
...
export function contactNormalDebugLines(world, bodies, options) {
    const lines = [];
    ...
    for (let left = 0; left < entries.length; left += 1) {
        for (let right = left + 1; right < entries.length; right += 1) {
            ...
            world.contactPair(leftEntry.collider.collider, rightEntry.collider.collider, (manifold, flipped) => {
...
export function bodyStateDebugLines(bodies, options) {
    ...
    return [...bodies.values()]
        ...
        const from = [translation.x, translation.y, translation.z];
        return {
            from,
...
export function jointFrameDebugLines(bodies, joints, options) {
    const lines = [];
    ...
    for (const entry of [...joints.values()].sort((a, b) => a.entity.localeCompare(b.entity))) {
        ...
        const axis = normalizeVec3(bodyLocalVectorToWorld(bodyA.body, jointAxis(entry.descriptor)));
        lines.push({
            from: anchorA,
            to: anchorB,
            color: frameColor,
        ...
        if (entry.descriptor.kind === "fixed") {
            lines.push(...fixedJointFrameBasisDebugLines(bodyA.body, anchorA, quatFromRapierRotation(entry.joint.frameX1()), axisLength, basisColors), ...fixedJointFrameBasisDebugLines(bodyB.body, anchorB, quatFromRapierRotation(entry.joint.frameX2()), axisLength, basisColors));
        }
    }
    return lines.filter(isFiniteLine);
}
function fixedJointFrameBasisDebugLines(body, anchor, frame, length, colors) {
    const basis = [
        [1, 0, 0],
        [0, 1, 0],
        [0, 0, 1],
...
function isFiniteLine(line) {
    return (line.from.every(Number.isFinite) &&
        line.to.every(Number.isFinite) &&
        line.color.every(Number.isFinite));
}
function colorForDebugVertex(colors, vertexIndex) {
