import { writePackedSnapshotPreviousTransforms, } from "/aperture/worker-modules/packages/render/dist/index.js";
import { writeBufferData } from "./app-frame-resource-utils.js";
import { createWebGpuAppFrameBoundaryTargets } from "./frame-target.js";
import { createWorldTransformGpuBuffer, writeWorldTransformBufferDescriptor, } from "../resources/transforms/world-transform-buffer.js";
export function createWebGpuAppSceneMotionVectorPlan(options) {
    const needsMotionVectors = options.app.postEffects.some((effect) => effect.enabled !== false && effect.requiresMotionVectors === true);
    ...
    if ((options.snapshot.spriteDraws?.length ?? 0) > 0 ||
...
export function prepareWebGpuAppPreviousObjectTransformResource(options) {
    ...
    const packed = writePackedSnapshotPreviousTransforms(options.currentTransforms, options.cache.previousWorldTransformsByRenderId, options.cache.previousWorldTransformsScratch);
    const descriptor = writeWorldTransformBufferDescriptor(packed, options.cache.previousWorldTransformDescriptorScratch, {
        label: previousWorldTransformBufferLabel(packed.floatCount ?? packed.data.length),
    });
    const diagnostics = [
        ...packed.diagnostics,
...
export function createWebGpuAppMotionVectorReport(options) {
    const missingPreviousBuffer = options.plan.required &&
    ...
    const status = !options.plan.required
        ? "disabled"
...
export function encodePostPassMotionVectorClearColor(options) {
    const current = readSnapshotViewProjectionMatrix(options.snapshot, options.view);
    if (current === null) {
        return [0.5, 0.5, 0.5, 1];
    }
    const previous = options.cache.previousViewProjectionByViewId.get(options.view.viewId);
    const motion = previous === undefined
        ? [0, 0]
...
export function rememberCurrentViewProjectionMatrices(snapshot, previousViewProjectionByViewId) {
    for (const view of snapshot.views) {
        const current = readSnapshotViewProjectionMatrix(snapshot, view);
        if (current === null) {
            continue;
        }
        const stored = previousViewProjectionByViewId.get(view.viewId) ?? new Float32Array(16);
        stored.set(current);
        previousViewProjectionByViewId.set(view.viewId, stored);
    }
}
function emptyPreviousObjectTransformHistoryReport() {
    return { total: 0, used: 0, fallback: 0, missing: [] };
}
function previousWorldTransformBufferLabel(floatCount) {
    return `PreviousWorldTransforms/storage/${floatCount}`;
}
function readSnapshotViewProjectionMatrix(snapshot, view) {
...
function screenMotionForViewProjectionMatrices(current, previous) {
...
function projectWorldOriginToNdc(matrix) {
