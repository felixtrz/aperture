...
import { createRenderShadowFrame, resolvePrimaryShadowCamera, } from "../shadows/render-shadow-frame.js";
...
export function standardAutoShadowPipelineKindFromSnapshot(snapshot) {
    if (!snapshotHasStandardShadowReceiver(snapshot)) {
        return null;
    }
    const directionalRequests = snapshot.shadowRequests.filter(isDirectionalShadowRequest);
...
export function createWebGpuAppAutoShadowFrame(options) {
    ...
    const meshViews = createAutoShadowCasterMeshViews({
        ...options,
        resourceLifetimeFrame: options.resourceLifetimeFrame ?? options.snapshot.frame,
    });
    ...
    const sceneMatrix = resolvePrimaryShadowCamera(options.snapshot) === null &&
...
export function createWebGpuAppAutoShadowFrameInputKey(snapshot) {
    ...
    const needsCameraFrustumFitBounds = primaryShadowCamera !== null && needsAutoFit;
    ...
    const supportedRequests = snapshot.shadowRequests.filter(isSupportedAutoShadowRequest);
    return [
...
export function autoShadowInputKeyUsesCamera(snapshot) {
    return (resolvePrimaryShadowCamera(snapshot) !== null &&
        autoShadowNeedsCameraFrustumFitBounds(snapshot));
}
export function autoShadowInputKeyUsesBounds(snapshot) {
 ...
 */
function computeShadowSceneMatrix(snapshot) {
    ...
    const casterFit = computeCasterShadowFit(snapshot, lightDirection, floorY, ceilingY);
    if (casterFit === null) {
        return null;
    }
    const radius = Math.max(casterFit.inPlaneRadius * SHADOW_RADIUS_MARGIN, SHADOW_MIN_RADIUS);
    const depthRadius = Math.max(casterFit.depthRadius * SHADOW_RADIUS_MARGIN, SHADOW_MIN_RADIUS);
 ...
 */
function computeCasterShadowFit(snapshot, lightDirection, floorY, ceilingY) {
    const points = [];
    let minX = Infinity;
    let minY = Infinity;
    let minZ = Infinity;
    ...
    const canProject = lightDirection !== null && lightDirection[1] < -1e-3 && floorY !== null;
    const include = (x, y, z) => {
        points.push([x, y, z]);
            ...
            for (let cy = 0; cy < 2; cy += 1) {
                for (let cz = 0; cz < 2; cz += 1) {
                    ...
                    if (canProject && py > floorY) {
                        ...
                        include(px + t * lightDirection[0], floorY, pz + t * lightDirection[2]);
                    }
                }
            }
        }
    ...
    const lightSpaceFit = lightDirection === null
        ? null
...
function fitShadowPointsInLightSpace(points, lightDirection) {
...
function lightSpaceBasis(lightDirection) {
 ...
 */
function directionalLightDirection(snapshot) {
 ...
 */
function receiverFloorY(snapshot) {
    let floor = Infinity;
 ...
 */
function receiverCeilingY(snapshot) {
...
function autoShadowNeedsCameraFrustumFitBounds(snapshot) {
    return snapshot.shadowRequests.some((request) => isDirectionalShadowRequest(request) &&
        (request.orthographicSize ?? 0) <= 0);
}
function collectCameraFitShadowBoundsIndices(snapshot) {
    ...
    for (const draw of snapshot.meshDraws) {
        ...
        const isReceiver = directionalRequests.some((request) => (draw.layerMask & request.receiverLayerMask) !== 0);
        if (isReceiver) {
            indices.add(draw.boundsIndex);
        }
    }
...
function collectFallbackShadowBoundsIndices(snapshot) {
    const indices = new Set();
    for (const draw of shadowCasterDrawsForSnapshot(snapshot)) {
        if (isShadowCasterDraw(draw, snapshot.shadowRequests)) {
            indices.add(draw.boundsIndex);
...
function shadowBoundsInputKey(snapshot, indices) {
    if (indices.size === 0) {
    ...
    return [...indices]
        ...
        const { min, max } = bounds.worldAabb;
        return `${index}:${numberKey(min[0])},${numberKey(min[1])},${numberKey(min[2])}:${numberKey(max[0])},${numberKey(max[1])},${numberKey(max[2])}`;
    })
        .join("|");
}
...
function shadowCameraInputKey(camera) {
    if (camera === null) {
        return "camera:none";
    }
    return [
...
function shadowRequestsInputKey(requests) {
    return `requests:${requests.map(shadowRequestInputKey).join("|")}`;
}
function shadowRequestInputKey(request) {
    return [
        request.shadowId,
        request.lightId,
...
function shadowLightsInputKey(snapshot, requests) {
...
function shadowCastersInputKey(snapshot, requests) {
    const casters = shadowCasterDrawsForSnapshot(snapshot).filter((draw) => isSupportedShadowCasterDraw(draw, requests));
...
function matrixAtKey(values, offset) {
...
function nullableAssetHandleKey(handle) {
...
function createAutoShadowCasterMeshViews(options) {
    ...
    const executableMeshes = [];
    const preparedByMeshKey = new Set();
...
function snapshotHasStandardShadowReceiver(snapshot) {
    return snapshot.meshDraws.some((draw) => draw.receivesShadow !== false &&
        draw.batchKey.pipelineKey.startsWith("standard|"));
}
function shadowCasterDrawsForSnapshot(snapshot) {
    return snapshot.shadowCasterDraws ?? snapshot.meshDraws;
}
function isShadowCasterDraw(draw, shadowRequests) {
    return (draw.castsShadow !== false &&
        isDepthOnlyShadowCasterDrawSupported(draw) &&
        shadowRequests.some((request) => isDirectionalShadowRequest(request) &&
 ...
 */
function isSupportedShadowCasterDraw(draw, shadowRequests) {
    return (draw.castsShadow !== false &&
        isDepthOnlyShadowCasterDrawSupported(draw) &&
        shadowRequests.some((request) => isSupportedAutoShadowRequest(request) &&
            (draw.layerMask & request.casterLayerMask) !== 0));
}
...
function isSupportedAutoShadowRequest(request) {
    return (isDirectionalShadowRequest(request) ||
        isPointShadowRequest(request) ||
        isSpotShadowRequest(request));
}
//# sourceMappingURL=auto-shadow-frame.js.map