...
import { createMaterialDependencyReadinessReport, isCustomWgslMaterialAsset, writePackedSnapshotInstanceTintsForVertexBuffer, writePackedSnapshotTransforms, writePackedSnapshotViewUniforms, } from "/aperture/worker-modules/packages/render/dist/index.js";
import { createWebGpuAppDrawResourceSetPlan } from "./draw-resource-set.js";
import { emptyPreparedAppTextureSamplerResources, sourceAssetCacheKey, } from "./app-texture-sampler-resources.js";
...
import { collectQueuedBuiltInAppResourceSet, createSingleQueuedBuiltInAppResourceItem, } from "../render/queues/queued-built-in-app-resource-set.js";
import { prepareLocalLightClusterCookieResources } from "../lighting/local-light-cookie-resources.js";
import { canReuseClusteredLocalLightShadowMatricesForCookies, hasReadyStandardDiffuseIblResources, hasReadyStandardSpecularIblBrdfResources, hasReadyStandardSpecularIblProofResources, standardShadowPipelineKind, withStandardClusteredLocalLightPipelineKeys, withStandardIblPipelineKeys, withStandardShadowPipelineKeys, } from "../materials/standard/standard-app-pipeline-keys.js";
...
import { createWebGpuAppMaterialDependencyDiagnostic, diagnoseSnapshotMaterialDependencies, } from "./material-dependencies.js";
...
import { createEmptyRenderSnapshot, createWebGpuAppSnapshotUpdateMetadata, renderSnapshotTimeSeconds, } from "./snapshot.js";
...
import { createQueuedBuiltInRouteFailureDiagnosticsSummary, resolveStandardAreaLightLtcResources, } from "./queued-built-in-support.js";
...
import { collectMultiUnlitAppResourceSet, createMultiUnlitAppFrameResources, } from "./multi-unlit.js";
import { QUEUED_BUILT_IN_APP_RESOURCE_ADAPTER_VALIDATION, QUEUED_BUILT_IN_MATERIAL_ADAPTERS, } from "./queued-built-in-adapters.js";
...
export async function renderWebGpuAppFrame(context, resourceCache, options) {
    const { app, sourceAssets } = context;
    const reuse = createWebGpuAppResourceReuseReport();
    const phaseTimer = createWebGpuAppRenderPhaseTimer(options.phaseTimingSamples);
    const extractedSnapshot = options.snapshot;
    const autoStandardMaterialShadowReceiverResources = options.autoStandardMaterialShadowReceiverResources !== false;
    ...
    if (extractedSnapshot === undefined) {
        ...
        return renderReport({
            ...
            resourceReuse: reuse,
            phaseTimings: phaseTimer.report(resourceCache.phaseTimingHistory, emptySnapshot.frame),
            diagnostics: [
                {
                    code: "webGpuApp.missingSnapshot",
                    message: "Renderer-only WebGPU app rendering requires a RenderSnapshot from the simulation worker.",
                },
            ],
        });
    }
    const shadowPipelineKind = options.standardMaterialShadowReceiverResources === undefined
        ? autoStandardMaterialShadowReceiverResources
            ? standardAutoShadowPipelineKindFromSnapshot(extractedSnapshot)
            : null
        : standardShadowPipelineKind(options.standardMaterialShadowReceiverResources);
    const shadowSnapshot = shadowPipelineKind === null
        ? extractedSnapshot
        : withStandardShadowPipelineKeys(extractedSnapshot, shadowPipelineKind);
    const iblSnapshot = hasReadyStandardDiffuseIblResources(options.standardMaterialIblResources)
        ? withStandardIblPipelineKeys(shadowSnapshot, hasReadyStandardSpecularIblProofResources(options.standardMaterialIblResources), hasReadyStandardSpecularIblBrdfResources(options.standardMaterialIblResources))
        : shadowSnapshot;
    ...
    if (!localLightCookieResources.valid) {
        return renderReport({
            ...
            snapshot: iblSnapshot,
            resourceReuse: reuse,
            phaseTimings: phaseTimer.report(resourceCache.phaseTimingHistory, iblSnapshot.frame),
            diagnostics: [
    ...
    const snapshot = withStandardClusteredLocalLightPipelineKeys(iblSnapshot, {
        supportedCookieResources: localLightCookieResources.resources?.supportedResources ?? [],
        cookieTextureViewDimension: localLightCookieResources.resources?.textureViewDimension ?? null,
        reuseShadowMatricesForCookies: canReuseClusteredLocalLightShadowMatricesForCookies(options.standardMaterialShadowReceiverResources, localLightCookieResources.resources),
    });
    const resourceLifetimeFrame = options.resourceLifetimeFrame ?? snapshot.frame;
    const updateMetadata = createWebGpuAppSnapshotUpdateMetadata(snapshot, options);
    ...
    const hasGlyphBatches = (snapshot.quadBatches ?? []).some((batch) => batch.kind === "glyph");
    ...
    if (firstDraw === undefined || firstView === undefined) {
        const materialDependencyDiagnostics = diagnoseSnapshotMaterialDependencies(sourceAssets, snapshot);
        return renderReport({
            ok: false,
            snapshot,
            resourceReuse: reuse,
            phaseTimings: phaseTimer.report(resourceCache.phaseTimingHistory, snapshot.frame),
            diagnostics: [
                ...snapshot.diagnostics,
                ...materialDependencyDiagnostics,
                {
                    code: "webGpuApp.emptySnapshot",
                    message: "WebGPU app render requires at least one view and one mesh draw.",
                },
            ],
        });
    }
    const snapshotMaterialDependencyDiagnostics = diagnoseSnapshotMaterialDependencies(sourceAssets, snapshot);
    ...
    const meshEntry = sourceAssets.get(firstDraw.mesh);
    const materialEntry = sourceAssets.get(firstDraw.material);
    ...
    if (!materialDependencyReadiness.ready) {
        return renderReport({
            ...
            diagnostics: [
                ...snapshot.diagnostics,
                createWebGpuAppMaterialDependencyDiagnostic(materialDependencyReadiness),
            ],
        });
    }
    ...
    const materialFamilyLabel = materialIsCustom
        ? material.familyKey
        : material.kind;
    const firstMaterialKindSupported = builtInMaterial !== null &&
        isBuiltInMaterialQueueFamily(builtInMaterial.kind);
    const mixedCustomBuiltInSnapshot = createBuiltInOnlySnapshotForMixedCustomWgslRoute(sourceAssets, snapshot);
    if (mixedCustomBuiltInSnapshot !== null) {
        ...
        if (!queuedBuiltIn.valid || queuedBuiltIn.resourceSet === null) {
            const diagnosticsSummary = createQueuedBuiltInRouteFailureDiagnosticsSummary(queuedBuiltIn.diagnostics, QUEUED_BUILT_IN_APP_RESOURCE_ADAPTER_VALIDATION);
            return renderReport({
                ok: false,
                snapshot,
                resourceReuse: reuse,
    ...
    const shouldUseQueuedBuiltInRoute = multiUnlit === null &&
    ...
    const materialKind = multiUnlit === null &&
    ...
    const getBindGroupLayout = pipelineHandle.getBindGroupLayout.bind(pipelineHandle);
    ...
    const materialKey = sourceAssetCacheKey(firstDraw.material, materialEntry?.version ?? -1);
    const singleBuiltInItem = multiUnlit === null && builtInMaterial !== null
    ...
    const preparedTextures = singleBuiltInItem === null
    ...
    if (multiUnlit !== null) {
        ...
        const textureSamplerDependencies = createPreparedMaterialTextureSamplerDependencies(preparedTextures);
    ...
    const meshResourceKeys = new Map();
    const materialResourceKeys = new Map();
    meshResourceKeys.set(assetHandleKey(firstDraw.mesh), frameResources.mesh.resourceKey);
    if ("materials" in frameResources && multiUnlit !== null) {
        for (let index = 0; index < multiUnlit.materialKeys.length; index += 1) {
            ...
            if (materialResource !== undefined && materialHandleKey !== undefined) {
                materialResourceKeys.set(materialHandleKey, materialResource.resourceKey);
            }
        }
    }
    else if ("material" in frameResources) {
        materialResourceKeys.set(assetHandleKey(firstDraw.material), frameResources.material.resourceKey);
    }
    ...
    const framePlan = writeRenderFramePlanFromSnapshot({
        ...
        resolveMeshResourceKey: (draw) => meshResourceKeys.get(assetHandleKey(draw.mesh)) ?? null,
        resolveMaterialResourceKey: (draw) => materialResourceKeys.get(assetHandleKey(draw.material)) ?? null,
    ...
    const boundaries = await assembleWebGpuAppFrameBoundaries({
        ...
        label: options.label ?? "aperture-webgpu-app",
        reuse,
        enableRenderBundles: shouldUseRenderBundlesForSnapshotSchedule(updateMetadata.snapshotUpdateSchedule),
        ...(options.clearColor === undefined
            ? {}
            : { clearColor: options.clearColor }),
        ...(options.gpuTimings === undefined
...
function createBuiltInOnlySnapshotForMixedCustomWgslRoute(assets, snapshot) {
    const builtInDraws = [];
    const builtInShadowCasterDraws = [];
