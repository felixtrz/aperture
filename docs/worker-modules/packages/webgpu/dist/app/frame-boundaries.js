import { countWebGpuAppFrameBoundaryTargetSubmissions, createWebGpuAppFrameBoundaryTargets, findLastSwapchainTargetIndex, resolveWebGpuAppTargetViewRectangles, webGpuAppFrameBoundaryTargetSubmissionKey, } from "./frame-target.js";
import { assembleFrameBoundary, buildFrameBoundaryTargetPlan, } from "../render/frame/frame-boundary.js";
...
import { executeFrameGraph, } from "../render/graph/frame-graph-execute.js";
import { planGpuOcclusionFeedbackCulling, } from "../gpu/occlusion-query.js";
...
import { createWebGpuAppDepthAttachmentForTarget, createWebGpuAppMsaaColorTargetForTarget, createWebGpuAppMsaaReport, } from "./attachments.js";
import { createWebGpuAppGpuTimingForTarget, } from "./gpu-readback.js";
import { assembleWebGpuAppTransmissionGrabPass, buildWebGpuAppTransmissionGrabBoundaryOptions, } from "./transmission-grab.js";
import { buildShadowCasterDepthAttachmentPlan, } from "./shadow-caster-graph-pass.js";
import { createWebGpuAppOcclusionQueryResources, createWebGpuAppRenderBundleCommandKey, createWebGpuAppRenderBundleReport, } from "./frame-boundary-support.js";
import { appendWebGpuAppOcclusionCullingPlan, collectOcclusionQueryRenderIds, commandsWithoutOcclusionQueryCommands, commandsWithoutSkippedOcclusionDraws, createWebGpuAppOcclusionCullingReport, normalizeOcclusionQueryCommands, recordWebGpuAppOcclusionCullingFallback, } from "./occlusion-culling.js";
...
import { buildUserPassNode, createUserPassSkippedOnLegacyRouteDiagnostic, } from "./user-pass.js";
export async function assembleWebGpuAppFrameBoundaries(options) {
    ...
    const postEffects = [];
    const diagnostics = [];
    const activePostEffects = options.app.postEffects.filter((effect) => effect.enabled !== false);
    let firstBoundary = null;
    let firstDepthAttachment;
    let readbackBoundary = null;
    const gpuTimingReadbacks = [];
    const gpuTimingDiagnostics = [];
    const occlusionQueryReadbacks = [];
    const occlusionQueryDiagnostics = [];
    ...
    const renderBundleViewCommands = [];
    const submittedTargetCounts = new Map();
    const submittedTargetLayerMasks = new Map();
    const targetSubmissionTotals = countWebGpuAppFrameBoundaryTargetSubmissions(targetPlan.targets);
    const lastSwapchainTargetIndex = findLastSwapchainTargetIndex(targetPlan.targets);
    ...
    const forwardGraphEligible = options.app.useFrameGraph === true && activePostEffects.length === 0;
    ...
    if (forwardGraph === null &&
        ...
        diagnostics.push(createUserPassSkippedOnLegacyRouteDiagnostic(enabledUserPassNames));
    }
    const forwardGraphPayloads = new Map();
    const forwardGraphEntries = [];
    ...
    const forwardGraphShadowEntries = [];
    const forwardGraphShadowReads = [];
    if (forwardGraph !== null) {
        const shadowDevice = options.app.initialization.device;
    ...
    for (let targetIndex = 0; targetIndex < targetPlan.targets.length; targetIndex += 1) {
        ...
        const background = proceduralSky.commands.length > 0
            ? proceduralSky
        ...
        const commandsForView = writeCommandsForView(options.commands, options.snapshot, target.view, options.cache.frameScratch.viewCommands, background.commands);
        const renderBundleCommandsForView = options.renderBundleCommands === undefined
            ? commandsForView
            : writeCommandsForView(options.renderBundleCommands, options.snapshot, target.view, renderBundleViewCommands, background.commands);
        const occlusionCandidateRenderIds = collectOcclusionQueryRenderIds(commandsForView);
        ...
        const commands = commandsWithoutSkippedOcclusionDraws(commandsForView, occlusionCullingPlan.skippedRenderIds, options.cache.frameScratch.occlusionCulledCommands);
        const occlusionRenderIds = normalizeOcclusionQueryCommands(commands);
        occlusionCulling.queriedDraws += occlusionRenderIds.length;
        const occlusionQueries = occlusionRenderIds.length === 0
            ? null
        ...
        const occlusionFallback = occlusionRenderIds.length > 0 && occlusionQueries?.resources === null;
        const commandsForBoundary = occlusionFallback
            ? commandsWithoutOcclusionQueryCommands(commands, options.cache.frameScratch.occlusionFallbackCommands)
            : commands;
        ...
        const depthAttachment = createWebGpuAppDepthAttachmentForTarget(options.app, options.cache, target);
        const msaaColorTarget = createWebGpuAppMsaaColorTargetForTarget(options.app, options.cache, target);
        ...
        const targetSubmissionKey = webGpuAppFrameBoundaryTargetSubmissionKey(target);
        const previousTargetSubmissions = submittedTargetCounts.get(targetSubmissionKey) ?? 0;
        const targetSubmissionTotal = targetSubmissionTotals.get(targetSubmissionKey) ?? 0;
        const loadExistingTarget = previousTargetSubmissions > 0;
        const previousTargetLayerMask = submittedTargetLayerMasks.get(targetSubmissionKey);
        const canLoadExistingTargetDepth = previousTargetLayerMask === undefined ||
            renderLayerMasksOverlap(previousTargetLayerMask, target.view.layerMask);
        const storeMsaaColorForLaterLoad = msaaColorTarget.resource !== null &&
            previousTargetSubmissions + 1 < targetSubmissionTotal;
        const targetClearColor = Array.from(options.clearColor ?? target.view.clearColor);
        const colorLoadOp = loadExistingTarget
            ? "load"
            : "clear";
        const depthLoadOp = loadExistingTarget &&
            canLoadExistingTargetDepth &&
            !isTransparentOverlayClearColor(targetClearColor)
            ? "load"
            : "clear";
        const colorStoreOp = storeMsaaColorForLaterLoad
            ? "store"
            : "discard";
        const finalTargetSubmission = previousTargetSubmissions + 1 >= targetSubmissionTotal;
        const includeReadback = options.readbackSamples !== undefined &&
            readbackBoundary === null &&
            target.source === "swapchain" &&
            targetIndex === lastSwapchainTargetIndex;
        const targetPassName = target.renderTargetKey === null
            ? "main"
            : `main:${target.renderTargetKey}`;
        const gpuTiming = options.gpuTimings === true
            ? await createWebGpuAppGpuTimingForTarget(options.app, options.cache, options.label, target)
            : null;
        ...
        const overlayCommands = target.source === "swapchain" ? (options.overlayCommands ?? []) : [];
        const commandsForBoundaryWithOverlay = overlayCommands.length === 0
            ? commandsForBoundary
        ...
        if (target.source === "swapchain" && activePostEffects.length > 0) {
            ...
            const sceneOcclusionQueries = postTarget.boundaries[0]?.occlusionQueries ?? null;
            for (const boundary of postTarget.boundaries) {
                occlusionQueryDiagnostics.push(...(boundary.occlusionQueries?.diagnostics ?? []));
            }
            if (occlusionQueries?.resources !== undefined &&
                occlusionQueries.resources !== null &&
            ...
            firstDepthAttachment ??= createWebGpuAppDepthAttachmentReport(options.snapshot, depthAttachment);
            ...
            if (postTarget.valid) {
                submittedTargetCounts.set(targetSubmissionKey, previousTargetSubmissions + 1);
                submittedTargetLayerMasks.set(targetSubmissionKey, target.view.layerMask);
            }
            diagnostics.push(...postTarget.diagnostics);
            continue;
        }
        if (options.transmissionSceneColorResources !== undefined &&
            ...
            if (forwardGraph !== null) {
                ...
                const grab = buildWebGpuAppTransmissionGrabBoundaryOptions(grabPassOptions);
                ...
                const grabCommands = snapshotForwardGraphCommands(options.cache.frameScratch.forwardGraphCommandLists, forwardGraphPayloads.size, grab.boundaryOptions.commands);
                ...
                const transmissionGrabPass = assembleWebGpuAppTransmissionGrabPass(grabPassOptions);
        ...
        const renderBundleCommands = renderBundleCommandCount === commandsForBoundaryWithOverlay.length
        ...
        if (forwardGraph !== null) {
            registerForwardGraphTarget({
                ...
                boundaryOptions: {
                    ...boundaryOptions,
                    commands: snapshotForwardGraphCommands(options.cache.frameScratch.forwardGraphCommandLists, forwardGraphPayloads.size, commandsForBoundaryWithOverlay),
                },
                target,
                targetSubmissionKey,
        ...
        plannedCommands += commandsForBoundaryWithOverlay.length;
        drawCalls += countDrawCommands(commandsForBoundaryWithOverlay);
        diagnostics.push(...boundary.texture.diagnostics, ...(boundary.attachments?.diagnostics ?? []), ...(boundary.encoder?.diagnostics ?? []), ...(boundary.begin?.diagnostics ?? []), ...(boundary.execution?.diagnostics ?? []), ...(boundary.renderBundle?.diagnostics ?? []), ...(boundary.end?.diagnostics ?? []), ...(boundary.occlusionQueries?.diagnostics ?? []), ...(boundary.rectangle?.diagnostics ?? []), ...(boundary.finish?.diagnostics ?? []), ...(boundary.submit?.diagnostics ?? []));
    }
    ...
    const forwardUserPasses = forwardGraph === null
        ? null
    ...
    if (forwardGraph !== null &&
        ...
        const queue = options.app.initialization.device.queue;
        ...
        const exec = executeFrameGraph({
            ...
            resources: {
                ...
                resolveRenderBoundary: (node) => forwardGraphPayloads.get(node.name) ?? null,
            },
            label: options.label,
        });
        ...
        const userPassGraphReport = forwardUserPasses === null
            ? null
            : {
                order: compiled.orderedNodes.map((node) => node.name),
                userPasses: forwardUserPasses.nodes.map((userNode) => {
                    const node = exec.nodes.find((entry) => entry.name === userNode.nodeName);
                    const executedCommands = node === undefined
                        ? 0
                        : node.kind === "compute"
        ...
        for (const shadowEntry of forwardGraphShadowEntries) {
            ...
            diagnostics.push(...(encode?.begin?.diagnostics ?? []), ...(encode?.execution?.diagnostics ?? []), ...(encode?.end?.diagnostics ?? []));
        }
        ...
        for (const grabEntry of forwardGraphGrabEntries) {
            ...
            diagnostics.push(...grabBoundary.texture.diagnostics, ...(grabBoundary.attachments?.diagnostics ?? []), ...(grabBoundary.encoder?.diagnostics ?? []), ...(grabBoundary.begin?.diagnostics ?? []), ...(grabBoundary.rectangle?.diagnostics ?? []), ...(grabBoundary.execution?.diagnostics ?? []), ...(grabBoundary.end?.diagnostics ?? []), ...(grabBoundary.finish?.diagnostics ?? []), ...(grabBoundary.submit?.diagnostics ?? []));
        }
        for (const entry of forwardGraphEntries) {
            ...
            allTargetsValid &&= boundary.valid;
            occlusionQueryDiagnostics.push(...(boundary.occlusionQueries?.diagnostics ?? []));
            if (entry.occlusionQueries?.resources !== undefined &&
        ...
        for (const userNode of forwardUserPasses?.nodes ?? []) {
            if (userNode.kind === "compute") {
                const computeNode = exec.nodes.find((node) => node.name === userNode.nodeName);
            ...
            boundaries.push(boundary);
            allTargetsValid &&= boundary.valid;
            diagnostics.push(...boundary.texture.diagnostics, ...(boundary.attachments?.diagnostics ?? []), ...(boundary.begin?.diagnostics ?? []), ...(boundary.execution?.diagnostics ?? []), ...(boundary.end?.diagnostics ?? []), ...(boundary.rectangle?.diagnostics ?? []));
        }
    }
    const renderBundleReport = createWebGpuAppRenderBundleReport(boundaries);
    return {
        valid: targetPlan.targets.length > 0 &&
            allTargetsValid &&
...
function registerForwardGraphTarget(args) {
    ...
    args.payloads.set(nodeName, {
        ...
        commands: opts.commands,
        label: opts.label,
        colorTargetSource: opts.colorTarget?.source === "offscreen-target"
            ? "offscreen-target"
            : "current-texture",
 ...
 */
function snapshotForwardGraphCommands(lists, index, commands) {
...
function registerForwardGraphUserPasses(args) {
    ...
    const userPasses = (args.app.userPassRegistry?.list() ?? []).filter((descriptor) => descriptor.enabled !== false);
    ...
    if (host === undefined) {
        args.diagnostics.push({
            ...
            severity: "warning",
            message: "Registered user passes were skipped: the forward FrameGraph route rendered no swapchain target this frame to host them.",
            data: { passes: userPasses.map((descriptor) => descriptor.name) },
        });
        return null;
    }
    const device = args.app.initialization.device;
    const context = args.app.initialization.context;
    ...
    const sceneColorView = host.texture.texture?.createView?.();
    const resolvers = {
        view: (handle) => handle === "scene-color"
            ? sceneColorView
            : handle === "depth"
                ? host.depthView
                : undefined,
        buffer: () => undefined,
        createBindGroup: (entries) => device.createBindGroup?.(entries),
    };
    const nodes = [];
    let plannedCommands = 0;
    let drawCalls = 0;
    for (const descriptor of userPasses) {
        ...
        args.payloads.set(built.name, {
            ...
            commands: built.commands,
            label: built.name,
            colorTargetSource: host.colorTarget?.source === "offscreen-target"
                ? "offscreen-target"
                : "current-texture",
            readbackTexture: plan.texture.texture,
        });
...
function isTransparentOverlayClearColor(clearColor) {
    const alpha = clearColor?.[3];
    return typeof alpha === "number" && Number.isFinite(alpha) && alpha < 1;
}
function renderLayerMasksOverlap(a, b) {
    return (a & b) !== 0;
}
//# sourceMappingURL=frame-boundaries.js.map