import { createPreparedMaterialStore, createPreparedMeshStore, preparedMaterialStoreSummaryToJsonValue, preparedMeshStoreSummaryToJsonValue, RENDER_SNAPSHOT_CHANGE_SET_FAMILIES, } from "/aperture/worker-modules/packages/render/dist/index.js";
import { collectWebGpuAppMaterialDependencyReadiness, } from "./app-diagnostics-summary.js";
import { createAppTextureSamplerResourceCacheSummary, writeAppTextureSamplerResourceCacheSummary, } from "./app-texture-sampler-resources.js";
import { createPreparedBuiltInMaterialCacheEvictionReport, createPreparedMeshGpuResourceCacheEvictionReport, } from "./prepared-resource-cache-eviction.js";
import { createPreparedAppMaterialCacheSummary, } from "../materials/core/prepared-app-material-resource.js";
import { writePreparedBuiltInMaterialStoreSummary, } from "../materials/core/prepared-built-in-material-store.js";
import { createPreparedMeshGpuResourceCacheSummary, writePreparedMeshGpuResourceCacheSummary, } from "../resources/meshes/prepared-mesh-cache.js";
import { createLocalLightClusterDescriptor, localLightClusterDeferredSamplingDiagnostics, localLightClusterReportFromDescriptor, snapshotShouldUseClusteredLocalLights, } from "../lighting/local-light-clusters.js";
...
export function webGpuAppRenderReportToJsonValue(report, options = {}) {
    const materialDependencyReadiness = collectWebGpuAppMaterialDependencyReadiness(report.diagnostics);
    const resourceReuse = options.detail === "status"
        ? compactWebGpuAppResourceReuseReport(report.resourceReuse)
        : { ...report.resourceReuse };
    const renderChangeSet = report.snapshotChangeSet === undefined
        ? undefined
    ...
    return {
        ...
        ...(report.snapshotUpdateSchedule === undefined
            ? {}
            : {
                renderUpdateSchedule: toWebGpuAppJsonValue(report.snapshotUpdateSchedule),
            }),
...
function compactRenderSnapshotChangeSet(changeSet) {
    return {
        ...
        ...(changeSet.keys === undefined
            ? {}
            : {
                keys: Object.fromEntries(RENDER_SNAPSHOT_CHANGE_SET_FAMILIES.map((family) => [
                    family,
...
function compactRenderSnapshotFamilyChangeKeys(keys) {
    if (keys === undefined) {
        return null;
    }
    return {
...
function compactWebGpuAppResourceReuseReport(report) {
    return {
        ...report,
        preparedMeshCache: {
...
function compactWebGpuAppDiagnosticsSummary(summary) {
        ...
        ...(summary.directLighting === undefined
            ...
            : {
                directLighting: {
                    ...
                    resources: {
                        lightGpuBuffer: summary.directLighting.resources.lightGpuBufferResourceKey !==
                            null,
                        lightBindGroupLayout: summary.directLighting.resources.lightBindGroupLayoutKey !==
                            null,
                        lightBindGroup: summary.directLighting.resources.lightBindGroupResourceKey !==
                            null,
                    },
                    shaderMetadata: {
                        valid: summary.directLighting.shaderMetadata.valid,
                        diagnosticCount: summary.directLighting.shaderMetadata.diagnostics.length,
                    },
...
function compactRenderShadowFrameReport(shadow) {
    return toWebGpuAppJsonValue({
        ready: shadow.ready,
        ...
        commandBufferSubmission: {
            ...
            submittedCommandBuffers: shadow.commandBufferSubmission.submittedCommandBuffers,
            commandBufferKeyCount: shadow.commandBufferSubmission.commandBufferKeys.length,
            sections: { ...shadow.commandBufferSubmission.sections },
        },
        diagnosticCount: shadow.diagnostics.length,
    });
}
function renderShadowFrameReportToJsonValue(shadow) {
    return {
        ready: shadow.ready,
        status: shadow.status,
        shadowKind: shadow.shadowKind,
        ...
        casterDrawList: shadowCasterDrawListReportToJsonValue(shadow.casterDrawList),
        ...
        diagnostics: shadow.diagnostics.map((diagnostic) => toWebGpuAppJsonValue(diagnostic)),
    };
}
function shadowCasterDrawListReportToJsonValue(report) {
    return {
        ready: report.ready,
...
export function webGpuAppPickReportToJsonValue(report) {
    return {
        ok: report.ok,
        x: report.x,
        y: report.y,
...
export function webGpuAppRenderReportToJson(report) {
    return JSON.stringify(webGpuAppRenderReportToJsonValue(report));
}
export function toWebGpuAppJsonValue(value, seen = new WeakSet()) {
    if (value === null ||
        typeof value === "string" ||
...
export function renderReport(input) {
    const resourceReuse = input.resourceReuse ?? createWebGpuAppResourceReuseReport();
    const localLightClusters = createWebGpuAppLocalLightClusterReport(input.snapshot, input.resources ?? null, resourceReuse);
    const localLightCookies = createWebGpuAppLocalLightCookieReport(input.localLightCookieResources ?? null);
    const diagnostics = [
        ...input.diagnostics,
    ...
    return {
        ...
        counts: {
            ...
            quadInstances: input.snapshot.quads === undefined
                ? 0
...
function createWebGpuAppLocalLightCookieReport(resources) {
    if (resources === null) {
        return undefined;
    }
    return {
...
function createWebGpuAppLocalLightClusterReport(snapshot, resources, reuse) {
    ...
    if (resource !== null) {
        ...
        return {
            ...report,
            routes: clusterResources.map((routeResource) => localLightClusterReportFromDescriptor(routeResource.descriptor, {
    ...
    return localLightClusterReportFromDescriptor(createLocalLightClusterDescriptor(snapshot), {
        buffersCreated: reuse.localLightClusterBuffersCreated,
        buffersReused: reuse.localLightClusterBuffersReused,
        bufferWrites: reuse.localLightClusterBufferWrites,
        bufferWritesSkipped: reuse.localLightClusterBufferWritesSkipped,
...
function collectWebGpuAppLocalLightClusterResources(result) {
    ...
    if ("standard" in resources) {
        ...
        for (const standardResources of resources.standard) {
            ...
            if (localLightClusters !== undefined &&
...
export function createWebGpuAppDepthAttachmentReport(snapshot, resource) {
    return {
...
function countOpaqueDepthWritePipelineKeys(snapshot) {
...
export function createWebGpuAppResourceReuseReport() {
    return {
        pipelineHits: 0,
        pipelineMisses: 0,
        ...
        preparedMeshCacheEviction: createPreparedMeshGpuResourceCacheEvictionReport(),
        preparedMeshFacade: preparedMeshStoreSummaryToJsonValue(createPreparedMeshStore()),
        ...
        preparedMaterialCacheEviction: createPreparedBuiltInMaterialCacheEvictionReport(),
        preparedMaterialFacade: preparedMaterialStoreSummaryToJsonValue(createPreparedMaterialStore()),
...
export function writeWebGpuAppPreparedMaterialCacheSummary(summary, cache) {
    return writePreparedBuiltInMaterialStoreSummary(summary, cache.preparedMaterials);
}
export function writeWebGpuAppPreparedMeshCacheSummary(summary, cache) {
    return writePreparedMeshGpuResourceCacheSummary(summary, cache.preparedMeshes);
}
export function writeWebGpuAppTextureSamplerCacheSummary(summary, cache) {
    return writeAppTextureSamplerResourceCacheSummary(summary, cache);
}
export async function waitForSubmittedWork(device) {
...
export function frameBoundariesNeedGpuDrain(boundaries) {
    return (boundaries.readbackBoundary !== null ||
