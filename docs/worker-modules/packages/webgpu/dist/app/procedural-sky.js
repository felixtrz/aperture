...
import { createProceduralSkyRenderPipelineResource, PROCEDURAL_SKY_UNIFORM_FLOAT_COUNT, proceduralSkyPipelineCacheKey, } from "../render/skybox/procedural-sky-pipeline.js";
...
export async function writeProceduralSkyCommandsForView(options) {
    ...
    const sky = selectProceduralSkyForView(options.snapshot.proceduralSkies ?? [], options.view);
    if (sky === null) {
        return { valid: true, commands: options.target, diagnostics: [] };
    }
    const diagnostics = [];
    const pipeline = await getOrCreateWebGpuAppProceduralSkyPipeline(options.app, options.cache, options.reuse);
...
function selectProceduralSkyForView(skies, view) {
...
async function getOrCreateWebGpuAppProceduralSkyPipeline(app, cache, reuse) {
    const colorFormat = webGpuAppScenePassColorFormat(app);
    const key = proceduralSkyPipelineCacheKey(colorFormat, WEBGPU_APP_DEPTH_FORMAT, app.msaa.sampleCount);
...
function getOrCreateProceduralSkyUniformResource(input) {
        ...
        if (cached.valueKey !== valueKey) {
            if (!writeWebGpuBufferData(input.device, cached.buffer, input.data)) {
                input.diagnostics.push({
                    ...
                    message: "WebGPU device cannot write updated procedural sky uniform data.",
                });
                return null;
            }
    ...
    if (!uniformBuffer.ok) {
        input.diagnostics.push(bufferDiagnostic("proceduralSkyFrame.uniformBufferFailed", uniformBuffer.message));
        return null;
    }
...
function validSkyPacket(sky) {
    return (tupleFiniteNonNegative(sky.topColor) &&
...
function tupleFinite(values) {
    return (Number.isFinite(values[0]) &&
        Number.isFinite(values[1]) &&
        Number.isFinite(values[2]));
}
function tupleFiniteNonNegative(values) {
    return (finiteNonNegative(values[0]) &&
...
function proceduralSkyUniformValueKey(data) {
    const parts = [];
...
//# sourceMappingURL=procedural-sky.js.map