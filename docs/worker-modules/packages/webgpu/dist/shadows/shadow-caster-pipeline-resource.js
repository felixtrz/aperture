import { renderPipelineResourceKey, shaderModuleResourceKey, } from "../resources/core/resource-keys.js";
import { createShadowCasterMatrixBindGroupLayoutDescriptor, SHADOW_CASTER_MATRIX_BIND_GROUP_LAYOUT_KEY, } from "./shadow-caster-matrix-bind-group-resource.js";
import { UNLIT_PRIMITIVE_VERTEX_BUFFER_LAYOUT, } from "../materials/unlit/unlit-pipeline.js";
...
`;
export function createShadowCasterPipelineResourceReport(options) {
    ...
    const metadataEntries = options.descriptor.descriptors.length > 0
            ...
            diagnostics: [
                {
                    ...
                    severity: "warning",
                    message: "Shadow caster pipeline resource creation requires depth-only pipeline descriptor metadata.",
                },
            ],
        });
    }
            ...
            diagnostics: [
                {
                    ...
                    severity: "warning",
                    message: "WebGPU device cannot create the shadow caster shader module.",
                },
            ],
        });
    }
            ...
            diagnostics: [
                {
                    ...
                    severity: "warning",
                    message: "WebGPU device cannot create the shadow caster render pipeline.",
                },
            ],
        });
    }
    const createRenderPipeline = options.device.createRenderPipeline.bind(options.device);
    ...
    const explicitLayout = createExplicitShadowCasterPipelineLayout(options.device);
    try {
        const createdResources = pendingMetadata.map((metadata) => {
            const descriptor = createBrowserShadowCasterPipelineDescriptor(metadata, shaderModule, explicitLayout.pipelineLayout ?? "auto");
            const resource = {
...
export function createBrowserShadowCasterPipelineDescriptor(metadata, shaderModule, layout = "auto") {
    return {
        label: metadata.label,
        layout,
        vertex: {
            ...
            buffers: resolveShadowCasterVertexBufferLayouts(metadata.vertex.meshLayoutKey),
        },
        primitive: {
...
function createExplicitShadowCasterPipelineLayout(device) {
    ...
    const matrixBindGroupLayout = device.createBindGroupLayout(createShadowCasterMatrixBindGroupLayoutDescriptor());
...
export function shadowCasterPipelineResourceReportToJsonValue(value) {
    return {
        ready: value.ready,
        status: value.status,
...
export function shadowCasterPipelineResourceReportToJson(value) {
...
function report(input) {
    ...
    return {
        ...
        sections: {
            pipelineDescriptor: input.descriptorCount > 0 || input.status === "not-required",
...
function shadowCasterMeshLayoutTokenFormat(token) {
...
function isShadowCasterColorFormat(format) {
    return (format === "float32x3" ||
        format === "float32x4" ||
        format === "unorm8x4" ||
        format === "unorm16x4");
}
function isShadowCasterWeightFormat(format) {
    ...
    return [
        {
            ...
            severity: "warning",
            message: "Shadow caster pipeline resource is available, but shadow pass submission is deferred.",
        },
        {
            code: "shadowCasterPipelineResource.shaderSamplingDeferred",
            severity: "warning",
            message: "Shadow caster pipeline resource is available, but StandardMaterial shadow sampling remains deferred.",
        },
    ];
}
//# sourceMappingURL=shadow-caster-pipeline-resource.js.map