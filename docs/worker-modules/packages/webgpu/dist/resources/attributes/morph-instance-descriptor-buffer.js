...
const DEFAULT_MORPH_INSTANCE_DESCRIPTOR_BUFFER_USAGE = WEBGPU_BUFFER_USAGE_FLAGS.STORAGE | WEBGPU_BUFFER_USAGE_FLAGS.COPY_DST;
function morphInstanceDescriptorBufferResourceKeyForRenderId(renderId) {
    return morphInstanceDescriptorBufferResourceKey(`render:${renderId}`);
}
export function createMorphInstanceDescriptorBufferDescriptor(snapshot, draw, options = {}) {
    const diagnostics = [];
    ...
    if (!Number.isInteger(usage) || usage <= 0) {
        diagnostics.push({
            ...
            renderId: draw.renderId,
            field: "usage",
            message: "Morph instance descriptor storage-buffer usage flags must be a positive integer.",
        });
    }
    ...
    return {
        ...
        plan: {
            descriptor: {
                label: options.label ?? `MorphInstanceDescriptors/render:${draw.renderId}`,
...
export function createMorphInstanceDescriptorGpuBuffer(options) {
            ...
            diagnostics: [
                {
                    ...
                    message: "Cannot create a morph instance descriptor GPU buffer from a null descriptor plan.",
                },
            ],
        };
    }
    const resourceKey = morphInstanceDescriptorBufferResourceKeyForRenderId(options.plan.renderId);
...
//# sourceMappingURL=morph-instance-descriptor-buffer.js.map