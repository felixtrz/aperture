...
const BLOOM_KERNEL_SIZES = [6, 10, 14, 18, 22, 26];
const BLOOM_BLUR_PARAMETER_BUFFER_BYTES = 16;
export function createWebGpuBloomPostEffect(options = {}) {
    ...
    const radius = clampFinite(options.radius ?? legacyRadiusPixelsToBloomRadius(options.radiusPixels), 0, 1);
    ...
    const horizontalSlots = Array.from({ length: levelCount }, () => createWebGpuPostPassTextureCacheSlot());
    const verticalSlots = Array.from({ length: levelCount }, () => createWebGpuPostPassTextureCacheSlot());
    const blurParameterSlots = Array.from({ length: levelCount }, () => ({ current: null }));
    const brightpassCommandSlot = {
        current: null,
    };
    const horizontalCommandSlots = Array.from({ length: levelCount }, () => ({ current: null }));
    const verticalCommandSlots = Array.from({ length: levelCount }, () => ({ current: null }));
    const compositeCommandSlot = {
        current: null,
    };
    let sampler = null;
    return {
...
function prepareBloomGraph(options) {
    const passes = [];
    passes.push(prepareSingleInputBloomGraphPass({
        device: options.device,
    ...
    passes.push(prepareBloomCompositeGraphPass({
        device: options.device,
        pipelines: options.pipelines,
        sampler: options.sampler,
        effectId: options.effectId,
...
function prepareSingleInputBloomGraphPass(options) {
    const diagnostics = [];
    const blurParameters = options.kind === "brightpass"
        ? null
    ...
    const commands = pipelineResult === null ||
...
function createOrReuseSingleInputBloomCommands(options) {
    ...
    const resourceKey = options.blurParameters === undefined
    ...
    const layout = options.pipeline.getBindGroupLayout?.(0);
    if (layout === undefined) {
        options.diagnostics.push({
...
function sameObjectSequence(left, right) {
...
function preparedBloomPass(effectId, label, commands, diagnostics, graph) {
    return {
        effectId,
        label,
        commands,
...
function legacyRadiusPixelsToBloomRadius(value) {
    if (value === undefined)
        return 0;
    return clampFinite(value / 16, 0, 1);
}
function halfDimension(value) {
    return Math.max(1, Math.round(value / 2));
}
function bloomKernelSize(index) {
    return BLOOM_KERNEL_SIZES[index] ?? 26;
}
function gaussianCoefficients(kernelSize) {
...
function bloomPostEffectWgsl(options) {
    const fragment = options.kind === "composite"
        ...
        : options.kind === "brightpass"
            ...
            : bloomGaussianBlurFragmentWgsl({
                direction: options.kind === "blur-horizontal" ? [1, 0] : [0, 1],
...
function bloomCompositeFragmentWgsl(options) {
    ...
    const textureBindings = Array.from({ length: levelCount }, (_, index) => `@group(0) @binding(${index + 2}) var bloomTexture${index}: texture_2d<f32>;`).join("\n");
...
//# sourceMappingURL=post-bloom.js.map