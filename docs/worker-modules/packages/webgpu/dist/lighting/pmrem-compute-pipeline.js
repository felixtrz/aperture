...
export function createPmremComputePipeline(options) {
    ...
    if (options.device.createShaderModule === undefined) {
        return failure("pmremComputePipeline.createShaderModuleUnavailable", "WebGPU device cannot create PMREM shader modules.");
    }
    if (options.device.createBindGroupLayout === undefined) {
        return failure("pmremComputePipeline.createBindGroupLayoutUnavailable", "WebGPU device cannot create PMREM bind group layouts.");
    }
    if (options.device.createPipelineLayout === undefined) {
        return failure("pmremComputePipeline.createPipelineLayoutUnavailable", "WebGPU device cannot create PMREM pipeline layouts.");
    }
    if (options.device.createComputePipeline === undefined) {
        return failure("pmremComputePipeline.createComputePipelineUnavailable", "WebGPU device cannot create PMREM compute pipelines.");
    }
    let shaderModule;
    try {
        ...
        return failure("pmremComputePipeline.shaderModuleCreationFailed", messageFromError(error));
    }
    let bindGroupLayout;
    try {
        ...
        return failure("pmremComputePipeline.bindGroupLayoutCreationFailed", messageFromError(error));
    }
    let pipelineLayout;
    try {
        ...
        return failure("pmremComputePipeline.pipelineLayoutCreationFailed", messageFromError(error));
    }
    let pipeline;
    try {
        ...
        return failure("pmremComputePipeline.pipelineCreationFailed", messageFromError(error));
    }
    return {
        valid: true,
        resource: {
...
function pmremComputeShader(format) {
    return `
struct PmremParams {
  width: u32,
  height: u32,
...
//# sourceMappingURL=pmrem-compute-pipeline.js.map