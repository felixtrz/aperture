...
// Unreal Engine 4"; see references/three.js EnvironmentBRDF.js / DFGLUT.js).
...
export function createBrdfLutComputePipeline(options) {
    ...
    if (options.device.createShaderModule === undefined) {
        return failure("brdfLutComputePipeline.createShaderModuleUnavailable", "WebGPU device cannot create BRDF-LUT shader modules.");
    }
    if (options.device.createBindGroupLayout === undefined) {
        return failure("brdfLutComputePipeline.createBindGroupLayoutUnavailable", "WebGPU device cannot create BRDF-LUT bind group layouts.");
    }
    if (options.device.createPipelineLayout === undefined) {
        return failure("brdfLutComputePipeline.createPipelineLayoutUnavailable", "WebGPU device cannot create BRDF-LUT pipeline layouts.");
    }
    if (options.device.createComputePipeline === undefined) {
        return failure("brdfLutComputePipeline.createComputePipelineUnavailable", "WebGPU device cannot create BRDF-LUT compute pipelines.");
    }
    let shaderModule;
    try {
        ...
        return failure("brdfLutComputePipeline.shaderModuleCreationFailed", messageFromError(error));
    }
    let bindGroupLayout;
    try {
        ...
        return failure("brdfLutComputePipeline.bindGroupLayoutCreationFailed", messageFromError(error));
    }
    let pipelineLayout;
    try {
        ...
        return failure("brdfLutComputePipeline.pipelineLayoutCreationFailed", messageFromError(error));
    }
    let pipeline;
    try {
        ...
        return failure("brdfLutComputePipeline.pipelineCreationFailed", messageFromError(error));
    }
    return {
        valid: true,
        resource: {
...
function importanceSampleGgx(xiX, xiY, roughness) {
 ...
 */
export function integrateEnvironmentBrdf(nDotV, roughness, sampleCount = BRDF_LUT_DEFAULT_SAMPLE_COUNT) {
    ...
    for (let i = 0; i < sampleCount; i += 1) {
        ...
        if (nDotL > 0.0) {
            const g = geometrySchlickGgx(clampedNdotV, roughness) *
...
function brdfLutComputeShader(format) {
    return `
...
//# sourceMappingURL=brdf-lut-compute-pipeline.js.map