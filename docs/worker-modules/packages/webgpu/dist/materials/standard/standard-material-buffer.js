...
import { validateStandardMaterialProofPoint, } from "/aperture/worker-modules/packages/render/dist/index.js";
...
export const STANDARD_MATERIAL_UNIFORM_BYTE_LENGTH = STANDARD_MATERIAL_UNIFORM_FLOATS * Float32Array.BYTES_PER_ELEMENT;
...
export const DEFAULT_STANDARD_MATERIAL_BUFFER_USAGE = WEBGPU_BUFFER_USAGE_FLAGS.UNIFORM | WEBGPU_BUFFER_USAGE_FLAGS.COPY_DST;
export function packStandardMaterial(material) {
    ...
    const diagnostics = proofPoint.diagnostics.map((diagnostic) => ({
        code: diagnostic.code,
        field: diagnostic.field,
        severity: diagnostic.severity,
        message: diagnostic.message,
    }));
    const dependencies = collectStandardMaterialDependencies(material, diagnostics);
    ...
    uniformFloat32.set(readTextureTransform(material.metallicRoughnessTexture), 24);
...
function packAttenuationDistance(distance) {
...
function readTextureTransform(binding) {
...
export function createStandardMaterialBufferDescriptor(packed, options = {}) {
    const diagnostics = [];
    ...
    if (!isPositiveInteger(usage)) {
        diagnostics.push({
            ...
            field: "usage",
            message: "Standard material uniform buffer usage flags must be a positive integer.",
        });
    }
    if (packed === null) {
        diagnostics.push({
            code: "standardMaterialBuffer.nullPackedMaterial",
            message: "Cannot create a standard material buffer descriptor from null packed material data.",
        });
        return { valid: false, plan: null, diagnostics };
    }
    if (packed.uniform.byteLength !== STANDARD_MATERIAL_UNIFORM_BYTE_LENGTH ||
...
export function createStandardMaterialPreparationPlan(material, options = {}) {
    const packing = packStandardMaterial(material);
    const buffer = createStandardMaterialBufferDescriptor(packing.packed, options);
    ...
    const materialBufferResourceKey = materialUniformBufferResourceKey(materialBuffer.descriptor.label ?? "standard");
...
function collectStandardMaterialDependencies(material, diagnostics) {
    return {
        baseColor: textureDependency("baseColorTexture", material.baseColorTexture, diagnostics),
        metallicRoughness: textureDependency("metallicRoughnessTexture", material.metallicRoughnessTexture, diagnostics),
        normal: textureDependency("normalTexture", material.normalTexture, diagnostics),
        occlusion: textureDependency("occlusionTexture", material.occlusionTexture, diagnostics),
        emissive: textureDependency("emissiveTexture", material.emissiveTexture, diagnostics),
        clearcoat: textureDependency("clearcoatTexture", material.clearcoatTexture, diagnostics),
        clearcoatRoughness: textureDependency("clearcoatRoughnessTexture", material.clearcoatRoughnessTexture, diagnostics),
        transmission: textureDependency("transmissionTexture", material.transmissionTexture, diagnostics),
        sheenColor: textureDependency("sheenColorTexture", material.sheenColorTexture, diagnostics),
        sheenRoughness: textureDependency("sheenRoughnessTexture", material.sheenRoughnessTexture, diagnostics),
        iridescence: textureDependency("iridescenceTexture", material.iridescenceTexture, diagnostics),
        iridescenceThickness: textureDependency("iridescenceThicknessTexture", material.iridescenceThicknessTexture, diagnostics),
    };
}
function textureDependency(field, binding, diagnostics) {
    if (binding === null) {
...
function standardMaterialFeatureFlags(material) {
    let flags = 0;
    ...
    if (material.sheenRoughnessTexture !== null) {
        flags |= STANDARD_MATERIAL_FEATURE_FLAGS.SHEEN_ROUGHNESS_TEXTURE;
    }
...
function readColor(values, index, field) {
...
//# sourceMappingURL=standard-material-buffer.js.map