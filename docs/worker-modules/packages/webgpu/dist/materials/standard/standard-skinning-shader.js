...
export const STANDARD_SKINNING_BIND_GROUP_LAYOUT_KEY = "standard/group-1:world-transforms@0,skin-joint-matrices@1";
export function standardSkinningEnabledFromBatchKey(batchKey) {
    return (batchKey?.skinned === true ||
        (typeof batchKey?.pipelineKey === "string" &&
            batchKey.pipelineKey.split("|").includes(STANDARD_SKINNING_FEATURE)));
}
export function appendStandardSkinningFeatureName(names, features) {
    if (features.skinned === true) {
        names.push(STANDARD_SKINNING_FEATURE);
    }
}
export function hasStandardSkinningFeature(features) {
    return features.skinned === true;
}
export function addStandardSkinningVertexSlots(composer, features) {
    if (!hasStandardSkinningFeature(features)) {
        return;
    }
    composer.addBinding("skinJointMatrices", "@group(1) @binding(1) var<storage, read> skinJointMatrices: array<mat4x4f>;");
    composer.addInputField("joints0", `  @location(${STANDARD_SKINNING_JOINTS_LOCATION}) joints0: vec4u,`);
    composer.addInputField("weights0", `  @location(${STANDARD_SKINNING_WEIGHTS_LOCATION}) weights0: vec4f,`);
...
//# sourceMappingURL=standard-skinning-shader.js.map