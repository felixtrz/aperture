export function createStandardMaterialIblShadowBindingReadinessReport(input) {
    ...
    if (input.iblPassPlan.status === "missing") {
        diagnostics.push({
            ...
            severity: "warning",
            message: "StandardMaterial IBL binding readiness requires IBL preparation pass planning.",
        });
    }
    else {
    ...
    if (input.shadowViewProjection.status === "missing" ||
        input.shadowCasterDrawList.status === "missing") {
        diagnostics.push({
            code: "standardMaterialIblShadowBinding.missingShadowPlan",
            severity: "warning",
            message: "StandardMaterial shadow binding readiness requires shadow matrix and caster draw-list planning.",
        });
    }
    else {
    ...
    if (slots.length > 0) {
        diagnostics.push({
            ...
            severity: "warning",
            message: "StandardMaterial IBL/shadow binding slots are planned, but bind group layout changes are deferred.",
        });
        diagnostics.push({
            code: "standardMaterialIblShadowBinding.shaderSamplingDeferred",
            severity: "warning",
            message: "StandardMaterial IBL/shadow binding slots are planned, but shader sampling is deferred.",
        });
    }
    const status = diagnostics.some((diagnostic) => diagnostic.code === "standardMaterialIblShadowBinding.missingIblPlan" ||
    ...
    return {
        ...
        sections: {
            ...
            shadowPlanning: input.shadowViewProjection.status !== "missing" &&
                input.shadowCasterDrawList.status !== "missing",
            bindGroupLayout: false,
            shaderSampling: false,
        },
...
export function standardMaterialIblShadowBindingReadinessReportToJsonValue(report) {
    return {
        ready: report.ready,
...
export function standardMaterialIblShadowBindingReadinessReportToJson(report) {
    return JSON.stringify(standardMaterialIblShadowBindingReadinessReportToJsonValue(report));
}
//# sourceMappingURL=standard-material-ibl-shadow-binding-readiness.js.map