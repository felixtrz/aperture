import { STANDARD_AREA_LIGHT_LTC_FRESNEL_BINDING, STANDARD_AREA_LIGHT_LTC_MATRIX_BINDING, STANDARD_AREA_LIGHT_LTC_SAMPLER_BINDING, } from "./standard-area-light-ltc-resource.js";
import { LOCAL_LIGHT_CLUSTER_COOKIE_MATRIX_BINDING, LOCAL_LIGHT_CLUSTER_COOKIE_SAMPLER_BINDING, LOCAL_LIGHT_CLUSTER_COOKIE_TEXTURE_BINDING, } from "../../lighting/local-light-clusters.js";
export function appendAreaLightLtcEntries(entries, resources) {
    if (resources === null) {
        return;
    }
    entries.push({
...
export function appendLocalLightClusterEntries(entries, resources) {
    if (resources === null) {
        return;
    }
    entries.push({
...
export function appendLocalLightCookieEntries(entries, resources, matrixEntryEnabled = true) {
    if (resources === null) {
        return;
    }
    entries.push({
...
export function appendShadowEntries(shadowResources, entries, diagnostics, bindings = { matrix: 2, depth: 3, sampler: 4 }) {
    ...
    const depthResource = shadowResources.depthTextureResources.resources.find((resource) => resource.allocation.resource !== null);
...
//# sourceMappingURL=standard-light-shadow-bind-group-entries.js.map