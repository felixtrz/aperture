...
import { applyOutputStageToFragmentWgsl, createTonemapPipelineKey, } from "../../output/output-stage-tonemap.js";
import { createOutputColorSpacePipelineKey, } from "../../output/output-stage-color-space.js";
import { SpriteBlendMode, } from "/aperture/worker-modules/packages/render/dist/index.js";
...
export const PARTICLE_BURST_RENDER_PIPELINE_KEY = "aperture/gpu-particles-burst-render";
...
`.trim();
export function particleComputePipelineCacheKey() {
    return PARTICLE_COMPUTE_PIPELINE_KEY;
}
export function particleRenderPipelineCacheKey(colorFormat, depthFormat = null, sampleCount = 1, blendMode = SpriteBlendMode.Additive, tonemap = "none", outputColorSpace = "linear") {
    const outputStage = tonemap === "none" && outputColorSpace === "linear"
        ? ""
...
export function particleBurstRenderPipelineCacheKey(colorFormat, depthFormat = null, sampleCount = 1, blendMode = SpriteBlendMode.Additive, tonemap = "none", outputColorSpace = "linear") {
    const outputStage = tonemap === "none" && outputColorSpace === "linear"
        ? ""
        : `:${createTonemapPipelineKey(tonemap)}:${createOutputColorSpacePipelineKey(outputColorSpace)}`;
...
export async function createParticleRenderPipelineResource(options) {
    ...
    const shaderModule = await createWebGpuShaderModule({
        ...
        descriptor: {
            ...
            code: applyOutputStageToFragmentWgsl(burst ? PARTICLE_BURST_RENDER_WGSL : PARTICLE_RENDER_WGSL, tonemap, outputColorSpace, burst
                ? PARTICLE_BURST_RENDER_PIPELINE_KEY
                : PARTICLE_RENDER_PIPELINE_KEY),
        },
    });
    ...
    try {
        return {
            ...
            resource: {
                cacheKey: burst
                    ? particleBurstRenderPipelineCacheKey(options.colorFormat, options.depthFormat ?? null, options.sampleCount ?? 1, blendMode, tonemap, outputColorSpace)
                    : particleRenderPipelineCacheKey(options.colorFormat, options.depthFormat ?? null, options.sampleCount ?? 1, blendMode, tonemap, outputColorSpace),
                shaderModule: shaderModule.module,
...
function particleBlendState(blendMode) {
    switch (blendMode) {
...
//# sourceMappingURL=particle-pipeline.js.map