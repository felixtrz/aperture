...
function uiOutputStageKeySuffix(tonemap, outputColorSpace) {
...
`.trim();
export function uiPanelPipelineCacheKey(colorFormat, depthFormat = null, sampleCount = 1, tonemap = "none", outputColorSpace = "linear") {
...
export function uiImagePipelineCacheKey(colorFormat, depthFormat = null, sampleCount = 1, tonemap = "none", outputColorSpace = "linear") {
...
export async function createUiPanelRenderPipelineResource(options) {
    ...
    return createUiQuadRenderPipelineResource({
        ...
        tonemap,
        outputColorSpace,
        cacheKey: uiPanelPipelineCacheKey(options.colorFormat, options.depthFormat ?? null, options.sampleCount ?? 1, tonemap, outputColorSpace),
    });
}
export async function createUiImageRenderPipelineResource(options) {
    ...
    return createUiQuadRenderPipelineResource({
        ...
        tonemap,
        outputColorSpace,
        cacheKey: uiImagePipelineCacheKey(options.colorFormat, options.depthFormat ?? null, options.sampleCount ?? 1, tonemap, outputColorSpace),
    });
}
async function createUiQuadRenderPipelineResource(options) {
    const shaderModule = await createWebGpuShaderModule({
        ...
        descriptor: {
            label: options.label,
            code: applyOutputStageToFragmentWgsl(options.code, options.tonemap ?? "none", options.outputColorSpace ?? "linear", options.label),
        },
    });
...
//# sourceMappingURL=ui-quad-pipeline.js.map