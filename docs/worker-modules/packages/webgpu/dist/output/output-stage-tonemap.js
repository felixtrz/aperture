import { createOutputColorSpacePipelineKey, createOutputColorSpaceWgsl, } from "./output-stage-color-space.js";
...
export const DEFAULT_TONEMAP_OPERATOR = "none";
const STANDARD_FRAGMENT_RETURN = "return vec4f(color, alpha);";
const STANDARD_FRAGMENT_ANCHOR = "@fragment\nfn fs_main";
export function isTonemapOperator(value) {
    return (typeof value === "string" &&
        TONEMAP_OPERATORS.includes(value));
}
export function parseTonemapOperator(value) {
...
export function resolveTonemapOperator(value) {
    return parseTonemapOperator(value) ?? DEFAULT_TONEMAP_OPERATOR;
}
export function createTonemapPipelineKey(operator) {
    return `tonemap:${operator}`;
}
export function createOutputTonemapWgsl(operator, options) {
    ...
    return base.replace("fn apertureOutputTonemap(color: vec3f) -> vec3f {", "fn apertureOutputTonemap(inputColor: vec3f, exposure: f32) -> vec3f {\n  let color = inputColor * exposure;");
}
function createOutputTonemapOperatorWgsl(operator) {
    switch (operator) {
        case "none":
            return `
...
export function applyOutputTonemapToStandardShader(shader, operator, outputColorSpace = "linear") {
    if (operator === "none" && outputColorSpace === "linear") {
        return shader;
    }
    if (!shader.code.includes(STANDARD_FRAGMENT_RETURN)) {
        throw new Error(`Cannot apply ${createTonemapPipelineKey(operator)} and ${createOutputColorSpacePipelineKey(outputColorSpace)} to '${shader.label}' because the standard fragment output marker was not found.`);
    }
    const outputWgsl = [
    ...
    const codeWithFunction = shader.code.includes(STANDARD_FRAGMENT_ANCHOR)
        ? shader.code.replace(STANDARD_FRAGMENT_ANCHOR, `${outputWgsl}\n\n${STANDARD_FRAGMENT_ANCHOR}`)
        : `${outputWgsl}\n\n${shader.code}`;
    return {
        ...shader,
        label: [
        ...
        code: codeWithFunction.replace(STANDARD_FRAGMENT_RETURN, "return vec4f(apertureOutputColorSpace(apertureOutputTonemap(color)), alpha);"),
    };
}
const BUILT_IN_FRAGMENT_ENTRY = "@fragment\nfn fs_main(input: VertexOutput) -> @location(0) vec4f";
...
const BUILT_IN_INNER_ENTRY = "fn apertureOutputStageInner(input: VertexOutput) -> vec4f";
/**
 ...
 */
export function applyOutputStageToFragmentWgsl(code, operator, outputColorSpace = "linear", label = "built-in shader") {
    if (operator === "none" && outputColorSpace === "linear") {
        return code;
    }
    if (!code.includes(BUILT_IN_FRAGMENT_ENTRY)) {
        throw new Error(`Cannot apply ${createTonemapPipelineKey(operator)} and ${createOutputColorSpacePipelineKey(outputColorSpace)} to '${label}' because the built-in fragment entry '${BUILT_IN_FRAGMENT_ENTRY}' was not found.`);
    }
    const outputWgsl = [
        createOutputTonemapWgsl(operator),
        createOutputColorSpaceWgsl(outputColorSpace),
...
export function applyOutputStageToBuiltInShader(shader, operator, outputColorSpace = "linear") {
    const code = applyOutputStageToFragmentWgsl(shader.code, operator, outputColorSpace, shader.label);
    if (code === shader.code) {
        return shader;
    }
    return {
...
//# sourceMappingURL=output-stage-tonemap.js.map