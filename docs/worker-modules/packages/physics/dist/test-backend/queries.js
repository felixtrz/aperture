...
export function raycastSphere(ray, body, collider, maxDistance) {
...
export function castSphereBounds(queryRadius, cast, body, collider) {
...
export function projectPointToBody(point, body, collider) {
    const center = colliderCenter(body, collider);
    const centerToPoint = subtract(point, center);
    const distanceToCenter = Math.hypot(centerToPoint[0], centerToPoint[1], centerToPoint[2]);
...
export function nearestCharacterHit(bodies, character, cast, options) {
    const hits = [];
...
export function characterMovementAfterHit(desiredTranslation, hit, slide) {
    ...
    const slideRemaining = intoNormal < 0
        ? subtract(remaining, scale(hit.normal, intoNormal))
        : remaining;
    return add(applied, slideRemaining);
}
export function isCharacterGrounded(bodies, character, up, options, snapDistance) {
...
export function queryAllowsCollider(body, collider, options) {
    if (body.entity === options.excludeEntity ||
