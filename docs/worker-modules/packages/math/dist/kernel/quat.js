...
export function fromAxisAngle(axisIn, radians, dst) {
    const axis = axisIn;
    const d = allocQuat(dst);
...
export function fromEuler(x, y, z, order, dst) {
    const d = allocQuat(dst);
 ...
 */
export function fromMat(mIn, dst) {
    const m = mIn;
    const d = allocQuat(dst);
...
export function normalize(qIn, dst) {
    const q = qIn;
    const d = allocQuat(dst);
    const x = q[0];
    const y = q[1];
...
export function conjugate(qIn, dst) {
    const q = qIn;
    const d = allocQuat(dst);
    d[0] = -q[0];
    d[1] = -q[1];
...
export function slerp(aIn, bIn, t, dst) {
    const a = aIn;
    const b = bIn;
    const d = allocQuat(dst);
    const ax = a[0];
