...
import {
  clearColor,
  createStandardTextureControlStatus,
...
try {
  ...
  if (canvas === null) {
    ...
    if (!created.ok) {
      ...
      const scene = registerStandardTextureControlScene(
        aperture,
        sourceAssets,
        scenario,
      );
...
function startWorkerSnapshotLoop(aperture, app, scene) {
  const worker = new Worker(
    "/aperture/worker-modules/examples/standard-texture-control.worker.js",
    {
      name: "aperture-standard-texture-control-simulation",
      type: "module",
    },
  );
  const loop = {
