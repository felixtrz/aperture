...
self.addEventListener("error", (event) => {
  self.postMessage({
    ...
    message: event.message || "The SSAO worker raised an error.",
    location: {
...
async function handleMessage(data) {
  try {
    ...
    if (data?.type === "frame") {
      if (scene === null) {
        throw new Error("SSAO worker scene is not initialized.");
      }
...
function createWorkerScene(aperture, canvasSize) {
  ...
  const registered = registerSsaoScene(aperture, app.assets);
