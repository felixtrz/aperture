...
import { createStandardTextureShaderVariantKey, createStandardTextureVariantShader, validateStandardShaderMetadata, } from "./standard-shader.js";
import { STANDARD_SKINNING_BIND_GROUP_LAYOUT_KEY, standardSkinningEnabledFromBatchKey, } from "./standard-skinning-shader.js";
import { STANDARD_MORPH_TARGET_BIND_GROUP_LAYOUT_KEY, STANDARD_MORPH_TARGET_DELTAS_BINDING, STANDARD_MORPH_TARGET_DESCRIPTORS_BINDING, STANDARD_SKINNED_MORPH_TARGET_BIND_GROUP_LAYOUT_KEY, standardMorphTargetsEnabledFromBatchKey, } from "./standard-morph-target-shader.js";
import { STANDARD_LIGHT_CASCADED_SHADOW_BIND_GROUP_LAYOUT_KEY, STANDARD_LIGHT_CASCADED_SHADOW_IBL_BIND_GROUP_LAYOUT_KEY, } from "./standard-light-shadow-bind-group.js";
import { CLUSTERED_LOCAL_LIGHT_ARRAY_COOKIE_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_ARRAY_SHADOW_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_CUBE_COOKIE_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_COOKIE_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_POINT_ARRAY_SHADOW_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_SHADOW_COOKIE_PIPELINE_FEATURE, } from "../../lighting/local-light-clusters.js";
export const STANDARD_DEFERRED_PIPELINE_FEATURES = [];
export function createStandardPipelineDescriptorPlan(input) {
    const diagnostics = [];
    const batchKey = input.batchKey;
    const shaderFeaturePlan = createStandardPipelineShaderFeaturePlan(batchKey, input.shader);
    ...
    const colorFormats = secondColorFormat === null
        ? [input.colorFormat]
        : [input.colorFormat, secondColorFormat];
    const colorTargets = secondColorFormat === null
        ? [colorTarget]
...
export function createStandardPipelineShaderFeaturePlan(batchKey, shader) {
    ...
    return {
        ...
        morphTargets: {
            ...
            deltaBufferBinding: features.morphed === true ? STANDARD_MORPH_TARGET_DELTAS_BINDING : null,
            descriptorBufferBinding: features.morphed === true
                ? STANDARD_MORPH_TARGET_DESCRIPTORS_BINDING
                : null,
        },
    };
}
function standardBindGroupLayoutKeys(batchKey) {
    const features = standardTextureFeatures(batchKey);
    const lightGroupKey = features.shadowMap === true && features.pointShadowMap === true
        ? standardMultiShadowLightGroupLayoutKey(features)
        : features.pointShadowMap === true
            ? `standard/lights-point-shadow/group-3:light-floats@0,light-metadata@1,matrix@2,${features.clusteredLocalLightPointArrayShadows === true
                ? "depth-array@3"
                : "depth-cube@3"},sampler@4`
    ...
    return [
        ...
        standardClusteredLocalLightGroupLayoutKey(standardTransmissionLightGroupLayoutKey(lightGroupKey, features), features),
    ];
}
function standardMultiShadowLightGroupLayoutKey(features) {
    const directionalDepthKey = features.clusteredLocalLightArrayShadows === true
        ? "directional-depth-array@3"
        : "directional-depth@3";
    const spotBindings = features.clusteredLocalLights === true
        ? ""
        : ",spot-matrix@5,spot-depth@6,spot-sampler@7";
    const pointDepthKey = features.clusteredLocalLightPointArrayShadows === true
        ? "point-depth-array@9"
        : "point-depth-cube@9";
...
function standardTransmissionLightGroupLayoutKey(lightGroupKey, features) {
    return features.transmission === true
        ? `${lightGroupKey},transmission-scene-color@14,transmission-scene-sampler@15`
        : lightGroupKey;
}
function standardClusteredLocalLightGroupLayoutKey(lightGroupKey, features) {
    ...
    const cookieTextureKey = features.clusteredLocalLightCubeCookies === true
    ...
    const cookieMatrixKey = features.clusteredLocalLightShadowCookies === true
...
export function resolveStandardShaderForBatchKey(batchKey, shader) {
    if (shader !== undefined) {
        return shader;
    }
    const features = standardTextureFeatures(batchKey);
...
function standardTextureFeatures(batchKey) {
    const tokens = typeof batchKey?.pipelineKey === "string" &&
        batchKey.pipelineKey.trim().length > 0
        ? batchKey.pipelineKey.split("|")
        : [];
    const clusteredLocalLightCubeCookies = tokens.includes(CLUSTERED_LOCAL_LIGHT_CUBE_COOKIE_PIPELINE_FEATURE);
    const clusteredLocalLightArrayCookies = tokens.includes(CLUSTERED_LOCAL_LIGHT_ARRAY_COOKIE_PIPELINE_FEATURE);
    const clusteredLocalLightArrayShadows = tokens.includes(CLUSTERED_LOCAL_LIGHT_ARRAY_SHADOW_PIPELINE_FEATURE);
    const clusteredLocalLightPointArrayShadows = tokens.includes(CLUSTERED_LOCAL_LIGHT_POINT_ARRAY_SHADOW_PIPELINE_FEATURE);
    const clusteredLocalLightShadowCookies = tokens.includes(CLUSTERED_LOCAL_LIGHT_SHADOW_COOKIE_PIPELINE_FEATURE);
    return {
        ...
        clusteredLocalLights: tokens.includes(CLUSTERED_LOCAL_LIGHT_PIPELINE_FEATURE),
        clusteredLocalLightCookies: tokens.includes(CLUSTERED_LOCAL_LIGHT_COOKIE_PIPELINE_FEATURE) ||
        ...
        vertexColor: typeof batchKey?.meshLayoutKey === "string" &&
            batchKey.meshLayoutKey.split(/[|,]/).some(isColor0LayoutToken),
    };
}
function standardTransformBindGroupLayoutKey(features) {
...
function standardVertexBufferSemantics(features) {
...
function standardShaderVariantKey(batchKey) {
    const features = standardTextureFeatures(batchKey);
...
function validateStandardPipelineTokens(tokens, diagnostics) {
...
function isDeferredPipelineFeature(feature) {
    return STANDARD_DEFERRED_PIPELINE_FEATURES.includes(feature);
}
function validateBatchKey(batchKey, diagnostics) {
    ...
    if (batchKey.topology === undefined) {
        diagnostics.push({
            ...
            message: "Standard pipeline descriptor planning requires batchKey.topology.",
        });
    }
}
...
//# sourceMappingURL=standard-pipeline-descriptor.js.map