export function applyStandardIridescenceSampling(code, options = {
    textureSample: null,
    thicknessTextureSample: null,
}) {
    void options;
    return code
        .replace(`fn evaluateDirectLight(
  normal: vec3f,`, `${STANDARD_IRIDESCENCE_WGSL}

fn evaluateDirectLight(
  normal: vec3f,`)
        .replaceAll(`  roughness: f32,
`, `  roughness: f32,
  iridescence: f32,
  iridescenceThickness: f32,
`)
        .replace(/\n(\s*)roughness,\n/gu, "\n$1roughness,\n$1iridescence,\n$1iridescenceThickness,\n")
        .replace(`  let f0 = mix(vec3f(0.04), baseColor, vec3f(metallic));
  let fresnel = fresnelSchlick(max(dot(halfVector, viewDir), 0.0), f0);`, `  let f0 = mix(vec3f(0.04), baseColor, vec3f(metallic));
  ...
  fresnel = mix(fresnel, iridescenceFresnel, iridescence);`)
        .replace(`  let f0 = mix(vec3f(0.04), baseColor, vec3f(metallic));
  let fresnel = fresnelSchlick(max(dot(viewDir, lightDir), 0.0), f0);`, `  let f0 = mix(vec3f(0.04), baseColor, vec3f(metallic));
  let iridescenceIor = clamp(material.iridescenceFactorIorThickness.y, 1.0, 2.333);
...
export function standardIridescenceMaterialStatements(options = {
    textureSample: null,
    thicknessTextureSample: null,
}) {
    const iridescenceFactorExpression = options.textureSample === null
        ? "material.iridescenceFactorIorThickness.x"
        : `material.iridescenceFactorIorThickness.x * ${options.textureSample}`;
    const iridescenceThicknessExpression = options.thicknessTextureSample === undefined ||
...
export function applyStandardClearcoatSampling(code, options = { textureSample: null, roughnessTextureSample: null }) {
    void options;
  ...
  clearcoatRoughness: f32,
        ...
        .replace(`      direct = direct + evaluateDirectLight(
        normal,
        viewDir,
        ...
        .replaceAll(`        roughness,
      ) * shadowFactor;`, `        roughness,
        clearcoatFactor,
        clearcoatRoughness,
      ) * shadowFactor;`)
        .replaceAll(`          roughness,
    ...
    return withClearcoatFactor.replace(`  var brdf = diffuse + specular;
  return brdf * radiance * nDotL;`, `  let clearcoatFresnel = fresnelSchlick(
...
export function standardClearcoatMaterialStatements(options = { textureSample: null, roughnessTextureSample: null }) {
    const clearcoatTextureFactor = options.textureSample ?? "1.0";
    const clearcoatRoughnessFactor = options.roughnessTextureSample === undefined ||
...
export function applyStandardSheenSampling(code, options) {
    void options;
    return code
        .replace(`fn evaluateDirectLight(
  normal: vec3f,
  ...
  baseColor: vec3f,
  metallic: f32,
  roughness: f32,`, `fn evaluateDirectLight(
  normal: vec3f,
  viewDir: vec3f,
  lightDir: vec3f,
  radiance: vec3f,
  ...
  metallic: f32,
  roughness: f32,
  clearcoatFactor: f32,`, `fn evaluateDirectLight(
  normal: vec3f,
  viewDir: vec3f,
  lightDir: vec3f,
  radiance: vec3f,
  ...
  clearcoatFactor: f32,`)
        ...
        .replace(`      direct = direct + evaluateAreaLight(
        lightIndex,
        input.worldPosition,
        normal,
        viewDir,
        ...
        .replace(`  return brdf * radiance * nDotL;`, `  let sheenInvRoughness = 1.0 / max(sheenRoughness * sheenRoughness, 0.0001);
...
export function standardSheenMaterialStatements(options) {
    const sheenColorExpression = options.colorTextureSample === null
        ? "material.sheenColorRoughnessFactor.rgb"
        : `material.sheenColorRoughnessFactor.rgb * ${options.colorTextureSample}`;
    const sheenRoughnessExpression = options.roughnessTextureSample === null
...
export function applyStandardFogSampling(code, features) {
    const fogFactor = features.fogLinear === true
    ...
    return code.replace(`fn evaluateDirectLight(
  normal: vec3f,`, `fn applyDistanceFog(color: vec3f, distanceToCamera: f32) -> vec3f {
${fogFactor}
...
//# sourceMappingURL=standard-shader-extension-sampling.js.map