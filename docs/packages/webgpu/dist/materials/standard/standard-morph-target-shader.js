...
export const STANDARD_MORPH_TARGET_BIND_GROUP_LAYOUT_KEY = "standard/group-1:world-transforms@0,morph-target-weights@2,morph-target-deltas@4,morph-instance-descriptors@5";
export const STANDARD_SKINNED_MORPH_TARGET_BIND_GROUP_LAYOUT_KEY = "standard/group-1:world-transforms@0,skin-joint-matrices@1,morph-target-weights@2,morph-target-deltas@4,morph-instance-descriptors@5";
export function standardMorphTargetsEnabledFromBatchKey(batchKey) {
    return (batchKey?.morphed === true ||
        (typeof batchKey?.pipelineKey === "string" &&
            batchKey.pipelineKey.split("|").includes(STANDARD_MORPH_TARGET_FEATURE)));
}
export function appendStandardMorphTargetFeatureName(names, features) {
    if (features.morphed === true) {
        names.push(STANDARD_MORPH_TARGET_FEATURE);
    }
}
export function hasStandardMorphTargetFeature(features) {
    return features.morphed === true;
}
export function addStandardMorphTargetVertexSlots(composer, features) {
    if (!hasStandardMorphTargetFeature(features)) {
        return;
    }
    composer.addBinding("morphTargetWeights", MORPH_BINDINGS_WGSL);
    composer.addInputField("morphVertexIndex", "  @builtin(vertex_index) morphVertexIndex: u32,");
...
//# sourceMappingURL=standard-morph-target-shader.js.map