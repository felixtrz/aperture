import { CLUSTERED_LOCAL_LIGHT_ARRAY_COOKIE_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_ARRAY_SHADOW_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_CUBE_COOKIE_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_COOKIE_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_POINT_ARRAY_SHADOW_PIPELINE_FEATURE, CLUSTERED_LOCAL_LIGHT_SHADOW_COOKIE_PIPELINE_FEATURE, createLocalLightClusterDescriptor, } from "../../lighting/local-light-clusters.js";
export function hasReadyStandardDiffuseIblResources(resources) {
    return (resources !== undefined &&
        resources.bindGroupResource.status === "available" &&
        resources.bindGroupResource.resource !== null &&
        resources.diffuseTextureResource?.resources.some((resource) => resource.valid && resource.resource !== null) === true &&
        resources.samplerResource?.resources.some((resource) => resource.valid && resource.resource !== null) === true);
}
export function hasReadyStandardSpecularIblProofResources(resources) {
    return (hasReadyStandardDiffuseIblResources(resources) &&
        resources.specularTextureResource?.resources.some((resource) => resource.valid && resource.resource !== null) === true);
}
export function hasReadyStandardSpecularIblBrdfResources(resources) {
    return (hasReadyStandardSpecularIblProofResources(resources) &&
        resources?.brdfLutTextureResource?.ready === true &&
        resources.brdfLutTextureResource.resource !== null);
}
export function withStandardShadowPipelineKeys(snapshot, shadowKind = "directional") {
    let changed = false;
    const shadowFeatures = shadowKind === "multi-spot-array-point-array"
        ? [
            "shadowMap",
            "pointShadowMap",
    ...
    const meshDraws = snapshot.meshDraws.map((draw) => {
        ...
        for (const shadowFeature of shadowFeatures) {
            if (!pipelineKey.includes(`|${shadowFeature}|`)) {
                pipelineKey = pipelineKey.replace(/^standard\|/, `standard|${shadowFeature}|`);
            }
        }
        return {
            ...draw,
...
export function standardShadowPipelineKind(resources) {
    if (resources.shadowKind !== undefined) {
        return resources.shadowKind;
    }
    return resources.depthTextureResources.resources.some((resource) => resource.viewDimension === "2d-array" &&
        (resource.layerCount ?? resource.faceCount) > 1)
        ? "directional-cascaded"
        : "directional";
}
export function canReuseClusteredLocalLightShadowMatricesForCookies(shadowResources, cookieResources) {
    ...
    if (cookieResources.textureLayout === "atlas" &&
    ...
    const supportedSpotLightIds = new Set(spotResources.depthTextureResources.resources
        .filter((resource) => (resource.viewDimension === "2d" ||
        resource.viewDimension === "2d-array") &&
        resource.allocation.resource !== null)
        .map((resource) => resource.lightId));
    return cookieResources.supportedResources.every((resource) => resource.textureViewDimension === "2d" &&
...
export function withStandardIblPipelineKeys(snapshot, includeSpecularProof, includeSpecularBrdf = false) {
    let changed = false;
...
export function withStandardClusteredLocalLightPipelineKeys(snapshot, options = {}) {
    ...
    const cookieSamplingReady = descriptor.shadowCookieMetadata.cookie.samplingSupported;
    let changed = false;
    const meshDraws = snapshot.meshDraws.map((draw) => {
        const pipelineKey = draw.batchKey.pipelineKey;
        if (!pipelineKey.startsWith("standard|")) {
        ...
        const clusteredPipelineKey = standardClusteredLocalLightPipelineKey(pipelineKey, cookieSamplingReady, options.cookieTextureViewDimension ?? null, options.reuseShadowMatricesForCookies === true);
...
function standardClusteredLocalLightPipelineKey(pipelineKey, cookieSamplingReady, cookieTextureViewDimension = null, reuseShadowMatricesForCookies = false) {
    ...
    const rest = tokens
        .slice(1)
        .filter((token) => token !== CLUSTERED_LOCAL_LIGHT_PIPELINE_FEATURE &&
    ...
    const cookieSamplingSupportedForDraw = cookieSamplingReady && !rest.includes("cascadedShadowMap");
    const reuseShadowMatricesForCookiesForDraw = reuseShadowMatricesForCookies &&
...
//# sourceMappingURL=standard-app-pipeline-keys.js.map