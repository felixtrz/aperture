...
export function createIndirectColorChannelShaderVariant(shader) {
    const fragmentEntry = shader.entryPoints.fragment;
    const escapedFragmentEntry = fragmentEntry.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
    const signaturePattern = new RegExp(`@fragment\\nfn ${escapedFragmentEntry}\\(([\\s\\S]*?)\\) -> @location\\(0\\) vec4f \\{`, "u");
    ...
    const code = shader.code
        .replace(signaturePattern, (_match, parameters) => `${INDIRECT_FRAGMENT_OUTPUT_STRUCT}\n\n@fragment\nfn ${fragmentEntry}(${parameters}) -> StandardIndirectFragmentOutput {`)
        .replace(returnPattern, (_match, expression) => {
...
//# sourceMappingURL=standard-indirect-channel-shader.js.map