...
import { hasAnyStandardTextureFeature, hasStandardFogFeature, } from "./standard-shader-features.js";
import { standardTextureUvExpression, standardTextureVariantBindings, standardTextureVariantComment, standardTextureVariantDeclaration, standardTextureVariantShaderLabel, usesCompactClusteredLocalMultiShadow, } from "./standard-shader-variant.js";
import { applyStandardClearcoatSampling, applyStandardClusteredLocalLightSampling, applyStandardFogSampling, applyStandardIridescenceSampling, applyStandardMultiShadowMapSampling, applyStandardPointShadowMapSampling, applyStandardShadowMapSampling, applyStandardSheenSampling, applyStandardSpecularIblBrdfSampling, applyStandardSpecularIblProofSampling, standardClearcoatMaterialStatements, standardIridescenceMaterialStatements, standardSheenMaterialStatements, standardTransmissionColorMutationStatements, } from "./standard-shader-sampling.js";
import { createStandardFragmentComposer, replaceStandardFragmentSlots, STANDARD_AMBIENT_DIFFUSE_BRDF_EXPRESSION, } from "./standard-shader-composer.js";
import { createStandardVertexComposer, replaceStandardVertexSlots, } from "./standard-vertex-composer.js";
import { STANDARD_DIFFUSE_IBL_SAMPLE_WGSL, STANDARD_SPECULAR_IBL_BRDF_SAMPLE_WGSL, STANDARD_SPECULAR_IBL_PROOF_SAMPLE_WGSL, } from "./standard-shader-ibl-sampling.js";
export { STANDARD_BASE_COLOR_METALLIC_ROUGHNESS_TEXTURE_SHADER_VARIANT, STANDARD_BASE_COLOR_TEXTURE_SHADER_VARIANT, STANDARD_CASCADED_SHADOW_MAP_SHADER_VARIANT, STANDARD_CLEARCOAT_SHADER_VARIANT, STANDARD_DIFFUSE_IBL_SHADER_VARIANT, STANDARD_DIRECT_LIGHT_SHADER_VARIANT, STANDARD_IRIDESCENCE_SHADER_VARIANT, STANDARD_MATERIAL_MVP_LIGHTING_MODEL, STANDARD_METALLIC_ROUGHNESS_TEXTURE_SHADER_VARIANT, STANDARD_MULTI_SHADOW_MAP_SHADER_VARIANT, STANDARD_POINT_SHADOW_MAP_SHADER_VARIANT, STANDARD_SHADOW_MAP_SHADER_VARIANT, STANDARD_SHEEN_SHADER_VARIANT, STANDARD_SPECULAR_IBL_PROOF_SHADER_VARIANT, STANDARD_TRANSMISSION_SHADER_VARIANT, createStandardTextureShaderVariantKey, } from "./standard-shader-features.js";
export { STANDARD_MESH_WGSL } from "./standard-shader-source.js";
export const STANDARD_BASE_COLOR_TEXTURED_MESH_WGSL = createStandardTextureVariantWgsl({
...
export const STANDARD_METALLIC_ROUGHNESS_TEXTURED_MESH_WGSL = createStandardTextureVariantWgsl({
...
export const STANDARD_BASE_COLOR_METALLIC_ROUGHNESS_TEXTURED_MESH_WGSL = createStandardTextureVariantWgsl({
    baseColorTexture: true,
    metallicRoughnessTexture: true,
    normalTexture: false,
    occlusionTexture: false,
...
export const STANDARD_SHADOW_RECEIVER_MESH_WGSL = createStandardTextureVariantWgsl({
    baseColorTexture: false,
    metallicRoughnessTexture: false,
    normalTexture: false,
    occlusionTexture: false,
...
export const STANDARD_CASCADED_SHADOW_RECEIVER_MESH_WGSL = createStandardTextureVariantWgsl({
    baseColorTexture: false,
    metallicRoughnessTexture: false,
    normalTexture: false,
    occlusionTexture: false,
...
export const STANDARD_MESH_SHADER = {
...
export const STANDARD_BASE_COLOR_TEXTURED_MESH_SHADER = createStandardTextureVariantShader({
    baseColorTexture: true,
    metallicRoughnessTexture: false,
    normalTexture: false,
    occlusionTexture: false,
...
export const STANDARD_METALLIC_ROUGHNESS_TEXTURED_MESH_SHADER = createStandardTextureVariantShader({
    baseColorTexture: false,
    metallicRoughnessTexture: true,
    normalTexture: false,
    occlusionTexture: false,
...
export const STANDARD_BASE_COLOR_METALLIC_ROUGHNESS_TEXTURED_MESH_SHADER = createStandardTextureVariantShader({
    baseColorTexture: true,
    metallicRoughnessTexture: true,
    normalTexture: false,
    occlusionTexture: false,
...
export const STANDARD_SHADOW_RECEIVER_MESH_SHADER = createStandardTextureVariantShader({
    baseColorTexture: false,
    metallicRoughnessTexture: false,
    normalTexture: false,
    occlusionTexture: false,
...
export function createStandardMeshShaderModuleDescriptor(shader = STANDARD_MESH_SHADER) {
    return {
        label: shader.label,
        code: shader.code,
...
export function validateStandardShaderMetadata(shader) {
    const diagnostics = [];
...
function hasBinding(bindings, id) {
    return bindings.some((binding) => binding.id === id);
}
export function createStandardTextureVariantShader(features) {
...
function createStandardTextureVariantWgsl(features) {
    const baseColorUv = standardTextureUvExpression(features, "baseColor");
    const metallicRoughnessUv = standardTextureUvExpression(features, "metallicRoughness");
    const clearcoatUv = standardTextureUvExpression(features, "clearcoat");
    const clearcoatRoughnessUv = standardTextureUvExpression(features, "clearcoatRoughness");
    ...
    const sheenRoughnessUv = standardTextureUvExpression(features, "sheenRoughness");
    const iridescenceUv = standardTextureUvExpression(features, "iridescence");
    const iridescenceThicknessUv = standardTextureUvExpression(features, "iridescenceThickness");
    ...
    let code = STANDARD_MESH_WGSL.replace(`// Direct lights use a small metallic/roughness GGX BRDF. Texture sampling,
// image-based lighting, and shadows are deferred.`, standardTextureVariantComment(features)).replace(`@group(2) @binding(0) var<uniform> material: StandardMaterialUniform;
@group(3) @binding(0) var<storage, read> lightFloats: array<f32>;`, standardTextureVariantDeclaration(features));
  ...
  return normalize(mat3x3f(tangent, bitangent, normal) * tangentNormal);
    ...
    if (features.texCoord1 === true) {
        ...
        code = code.replace(`fn saturate(value: f32) -> f32 {`, `fn standardTextureUv(texCoord: u32, uv0: vec2f, uv1: vec2f) -> vec2f {
  if (texCoord == 1u) {
    return uv1;
  }
...
fn saturate(value: f32) -> f32 {`);
    ...
    if (features.baseColorTexture ||
        ...
        code = code.replace(`fn saturate(value: f32) -> f32 {`, `fn standardTextureTransformUv(uv: vec2f, offset: vec2f, scale: vec2f, rotation: f32) -> vec2f {
  let scaled = uv * scale;
  let c = cos(rotation);
  let s = sin(rotation);
  let rotated = vec2f(
...
fn saturate(value: f32) -> f32 {`);
  ...
  let baseColorSample = textureSample(baseColorTexture, baseColorSampler, baseColorUv);`);
        fragment.setBaseColorExpression("baseColorSample.rgb * material.baseColorFactor.rgb");
        fragment.setAlphaExpression("baseColorSample.a * material.baseColorFactor.a");
    }
    ...
    if (features.instanceTint === true) {
        ...
        vertex.addPostUvOutputAssignment("  output.instanceTint = input.instanceTint;");
        fragment.setBaseColorMutable();
        fragment.setAlphaMutable();
        fragment.addBaseColorAlphaMutation("  baseColor = baseColor * input.instanceTint.rgb;");
        fragment.addBaseColorAlphaMutation("  alpha = alpha * input.instanceTint.a;");
    }
  ...
  let metallicRoughnessSample = textureSample(
    ...
    metallicRoughnessUv,
        ...
        fragment.setMetallicExpression("clamp(material.metallicFactor * metallicRoughnessSample.b, 0.0, 1.0)");
        fragment.setRoughnessExpression("clamp(material.roughnessFactor * metallicRoughnessSample.g, 0.045, 1.0)");
    }
  ...
  let emissive = material.emissiveFactor * emissiveSample.rgb;`
        ...
        fragment.setIndirectDiffuseTerm("ambientDiffuse", "ambientDiffuse", `  let ambientDiffuse = ${STANDARD_AMBIENT_DIFFUSE_BRDF_EXPRESSION} * occlusion;`);
    ...
    if (features.shadowMap === true && features.pointShadowMap === true) {
        code = applyStandardMultiShadowMapSampling(code, {
            compactClusteredLocalShadows: usesCompactClusteredLocalMultiShadow(features),
    ...
    const removeGlobalPointShadowReceiverFactor = features.clusteredLocalLights === true &&
        features.pointShadowMap === true &&
        features.shadowMap !== true;
    const removeGlobalSpotShadowReceiverFactor = features.clusteredLocalLights === true &&
        features.shadowMap === true &&
    ...
    samplePointShadowReceiverFactor(input.worldPosition),
        ...
        fragment.addMaterialStatement("  let receiverPointShadowFactor = samplePointShadowReceiverFactor(input.worldPosition);");
        fragment.setDirectTerm("receiverPointShadowDirect", "direct * receiverPointShadowFactor");
    }
    if (features.iblDiffuse === true) {
        fragment.addIndirectDiffuseTerm("diffuseIbl", "diffuseIbl", STANDARD_DIFFUSE_IBL_SAMPLE_WGSL);
    }
    if (features.iblSpecularBrdf === true) {
        code = applyStandardSpecularIblBrdfSampling(code);
        fragment.addIndirectSpecularTerm("specularIblBrdf", "specularIblBrdf", STANDARD_SPECULAR_IBL_BRDF_SAMPLE_WGSL);
    }
    else if (features.iblSpecularProof === true) {
        code = applyStandardSpecularIblProofSampling(code);
        fragment.addIndirectSpecularTerm("specularIblProof", "specularIblProof", STANDARD_SPECULAR_IBL_PROOF_SAMPLE_WGSL);
    }
    if (features.clearcoat === true) {
        fragment.addMetallicRoughnessStatement(standardClearcoatMaterialStatements({
            textureSample: features.clearcoatTexture === true
                ? `textureSample(clearcoatTexture, clearcoatSampler, ${clearcoatUv}).r`
                : null,
            roughnessTextureSample: features.clearcoatRoughnessTexture === true
    ...
    if (features.sheen === true) {
        fragment.addMetallicRoughnessStatement(standardSheenMaterialStatements({
            colorTextureSample: features.sheenColorTexture === true
                ? `textureSample(sheenColorTexture, sheenColorSampler, ${sheenColorUv}).rgb`
                : null,
            roughnessTextureSample: features.sheenRoughnessTexture === true
    ...
    if (features.iridescence === true) {
        fragment.addMetallicRoughnessStatement(standardIridescenceMaterialStatements({
            textureSample: features.iridescenceTexture === true
                ? `textureSample(iridescenceTexture, iridescenceSampler, ${iridescenceUv}).r`
                : null,
            thicknessTextureSample: features.iridescenceThicknessTexture === true
    ...
    if (features.transmission === true) {
        fragment.setAlphaMutable();
        fragment.addColorMutationStatement(standardTransmissionColorMutationStatements({
            textureSample: features.transmissionTexture === true
    ...
    if (hasStandardFogFeature(features)) {
        ...
        fragment.addColorMutationStatement("  let foggedColor = applyDistanceFog(color, length(view.cameraPosition.xyz - input.worldPosition));");
        fragment.addColorMutationStatement("  let standardIndirectFoggedColor = applyDistanceFog(standardIndirectColor, length(view.cameraPosition.xyz - input.worldPosition));");
    ...
    if (features.clusteredLocalLights === true) {
        code = applyStandardClusteredLocalLightSampling(code, {
            ...
            pointArrayShadowMap: features.clusteredLocalLightPointArrayShadows === true,
            spotShadowMap: features.shadowMap === true &&
                (features.cascadedShadowMap !== true ||
                    features.clusteredLocalLightArrayShadows === true),
            localLightCookies: features.clusteredLocalLightCookies === true,
            localLightShadowCookies: features.clusteredLocalLightShadowCookies === true,
            ...
            removeGlobalPointShadowReceiverFactor: removeGlobalPointShadowReceiverFactor,
            removeGlobalSpotShadowReceiverFactor: removeGlobalSpotShadowReceiverFactor,
        });
    }
    addStandardSkinningVertexSlots(vertex, features);
    addStandardMorphTargetVertexSlots(vertex, features);
    if (features.morphed === true) {
        vertex.addLocalStatement("  let morphed = apertureMorph(input.position, input.normal, input.instanceIndex, input.morphVertexIndex);");
    }
    if (features.skinned === true) {
        const skinPositionExpression = features.morphed === true ? "morphed.position" : "input.position";
        const skinNormalExpression = features.morphed === true ? "morphed.normal" : "input.normal";
        vertex.addLocalStatement(`  let skinnedPosition = apertureSkinPosition(${skinPositionExpression}, input.joints0, input.weights0);`);
        vertex.addLocalStatement(`  let skinnedNormal = apertureSkinDirection(${skinNormalExpression}, input.joints0, input.weights0);`);
        ...
        if (features.normalTexture === true) {
            vertex.addLocalStatement("  let skinnedTangent = apertureSkinDirection(input.tangent.xyz, input.joints0, input.weights0);");
    ...
    return replaceStandardVertexSlots(replaceStandardFragmentSlots(code, fragment), vertex);
}
//# sourceMappingURL=standard-shader.js.map