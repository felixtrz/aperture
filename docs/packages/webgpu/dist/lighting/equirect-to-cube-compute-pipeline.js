...
//
// Direction<->UV mapping (see references/three.js/src/nodes/utils/EquirectUV.js,
// which uses atan(dir.z, dir.x)). We center the equirect on +Z instead — i.e.
...
export function createEquirectToCubeComputePipeline(options) {
    ...
    if (options.device.createShaderModule === undefined) {
        return failure("equirectToCubePipeline.createShaderModuleUnavailable", "WebGPU device cannot create equirect-to-cube shader modules.");
    }
    if (options.device.createBindGroupLayout === undefined) {
        return failure("equirectToCubePipeline.createBindGroupLayoutUnavailable", "WebGPU device cannot create equirect-to-cube bind group layouts.");
    }
    if (options.device.createPipelineLayout === undefined) {
        return failure("equirectToCubePipeline.createPipelineLayoutUnavailable", "WebGPU device cannot create equirect-to-cube pipeline layouts.");
    }
    if (options.device.createComputePipeline === undefined) {
        return failure("equirectToCubePipeline.createComputePipelineUnavailable", "WebGPU device cannot create equirect-to-cube compute pipelines.");
    }
    let shaderModule;
    try {
        ...
        return failure("equirectToCubePipeline.shaderModuleCreationFailed", messageFromError(error));
    }
    let bindGroupLayout;
    try {
        ...
        return failure("equirectToCubePipeline.bindGroupLayoutCreationFailed", messageFromError(error));
    }
    let pipelineLayout;
    try {
        ...
        return failure("equirectToCubePipeline.pipelineLayoutCreationFailed", messageFromError(error));
    }
    let pipeline;
    try {
        ...
        return failure("equirectToCubePipeline.pipelineCreationFailed", messageFromError(error));
    }
    return {
        valid: true,
        resource: {
...
export function equirectToCubeUv(direction) {
...
export function cubeFaceCenterDirection(face) {
    switch (face) {
        case 0:
            return [1, 0, 0];
        case 1:
...
function equirectToCubeShader(format) {
    return `
...
//# sourceMappingURL=equirect-to-cube-compute-pipeline.js.map