import { createWebGpuIdBufferIdForEntity, WEBGPU_ID_BUFFER_EMPTY_ID, WEBGPU_ID_BUFFER_FORMAT, } from "./id-buffer.js";
...
import { createWebGpuDepthStencilDescriptor, resolveWebGpuPipelineRenderState, } from "../materials/core/material-render-state.js";
...
export async function createWebGpuIdBufferPickPipelineResource(options) {
    ...
    const renderState = resolveWebGpuPipelineRenderState(options.batchKey.pipelineKey, options.depthFormat);
    const descriptor = {
    ...
    const depthStencil = createWebGpuDepthStencilDescriptor(options.depthFormat, renderState);
    const finalDescriptor = depthStencil === null ? descriptor : { ...descriptor, depthStencil };
    try {
...
function pushPickPipelineErrorScope(device) {
    try {
...
async function popPickPipelineErrorScope(device) {
...
function createPickPipelineLayout(device) {
            ...
            diagnostics: [
                {
                    ...
                    message: "ID-buffer picking requires createBindGroupLayout and createPipelineLayout to share frame bind groups.",
                },
            ],
        };
    }
...
export function webGpuIdBufferPickPipelineCacheKey(batchKey) {
    return [
        "id-buffer-pick",
...
export function createWebGpuIdBufferPickIdStorageValues(snapshot) {
    const transformCount = Math.max(1, Math.floor(snapshot.transforms.length / 16));
    ...
    for (const draw of snapshot.meshDraws) {
        ...
        if (Number.isInteger(transformIndex) &&
            transformIndex >= 0 &&
...
export function createWebGpuIdBufferPickBindGroup(options) {
    ...
    const layout = options.pipeline.layouts.ids ?? pipeline.getBindGroupLayout?.(2);
    if (layout === undefined) {
        return {
            valid: false,
            resource: null,
            diagnostics: [
                {
                    code: "idBufferPick.pipelineLayoutUnavailable",
                    message: "WebGPU ID-buffer picking pipeline did not expose group 2 layout.",
                },
            ],
        };
    }
...
export async function readWebGpuIdBufferPickPixel(options) {
    ...
    try {
        ...
        }, { buffer, bytesPerRow: readbackBytesPerRow, rowsPerImage: 1 }, { width: 1, height: 1, depthOrArrayLayers: 1 });
    ...
    if (buffer.getMappedRange === undefined) {
        return {
            ...
            message: "WebGPU ID-buffer picking readback buffer cannot expose mapped bytes.",
            origin,
        };
    }
    try {
...
function validatePickBatchKey(batchKey) {
    ...
    if (batchKey.skinned || batchKey.morphed) {
        return {
            code: "idBufferPick.unsupportedBatchKey",
            pipelineKey: batchKey.pipelineKey,
            message: "ID-buffer picking currently supports rigid, unmorphed mesh draws.",
        };
    }
...
function hasSupportedPickMeshLayout(meshLayoutKey) {
    const tokens = meshLayoutKey.split(/[|,]/);
    const semantics = new Set(tokens.map(meshLayoutTokenSemantic).filter(isNonEmptyString));
    return (semantics.has("POSITION") &&
...
//# sourceMappingURL=id-buffer-pick.js.map