...
import { createDiffuseIblTextureResourceReport, createSpecularIblTextureResourceReport, diffuseIblTextureResourceReportToJsonValue, specularIblTextureResourceReportToJsonValue, } from "../lighting/ibl-texture-resource.js";
import { createEquirectToCubeResource, } from "../lighting/equirect-to-cube-resource.js";
import { createIblSamplerResourceReport, iblSamplerResourceReportToJsonValue, } from "../lighting/ibl-sampler-resource.js";
import { createIblSamplerDescriptorReadinessReport, iblSamplerDescriptorReadinessReportToJsonValue, } from "../lighting/ibl-sampler-descriptor-readiness.js";
import { createIblTexturePreparationReport, iblTexturePreparationReportToJsonValue, } from "../lighting/ibl-texture-preparation.js";
import { createIblResourceDescriptorReport, iblResourceDescriptorReportToJsonValue, } from "../lighting/ibl-resource-descriptor.js";
import { createStandardMaterialIblBindGroupDescriptorReadinessReport, createStandardMaterialIblBindGroupResourceReport, standardMaterialIblBindGroupDescriptorReadinessReportToJsonValue, standardMaterialIblBindGroupResourceReportToJsonValue, } from "../materials/standard/standard-material-ibl-bind-group.js";
...
export function createWebGpuEnvironmentResourceCache() {
    return {
        ...
        shadowCasterWorldTransformScratch: createShadowCasterWorldTransformScratch(),
        shadowCasterCommandTopology: new Map(),
    };
}
export function registerWebGpuAppEnvironmentResourceCache(app, cache) {
    APP_ENVIRONMENT_RESOURCE_CACHES.set(app, cache);
}
export function getOrCreateWebGpuAppEnvironmentResourceCache(app) {
...
export function createWebGpuEnvironmentResourceCacheSummary() {
    return {
...
export function writeWebGpuEnvironmentResourceCacheSummary(summary, cache) {
...
export function createWebGpuEnvironmentResourceReuseReport() {
    return {
...
export function prepareWebGpuAppIblResourceReports(options) {
    ...
    return {
        ...
        samplerResources,
        reuse,
        cacheSummary: writeWebGpuEnvironmentResourceCacheSummary(createWebGpuEnvironmentResourceCacheSummary(), cache),
    };
}
export function prepareWebGpuAppEnvironmentAssets(options) {
    const device = options.device ?? deviceFromApp(options.app);
    const cache = getOrCreateWebGpuAppEnvironmentResourceCache(options.app);
    const preparedAssets = options.assets.map((asset, index) => prepareWebGpuAppEnvironmentAsset({
        app: options.app,
        device,
        cache,
        asset,
    ...
    const requestedActiveKey = options.activeEnvironmentMapResourceKey ??
        (options.activeHandle === undefined || options.activeHandle === null
            ? null
            : assetHandleKey(options.activeHandle));
    const activeEnvironmentMapResourceKey = requestedActiveKey ?? preparedAssets[0]?.environmentMapResourceKey ?? null;
    const active = activeEnvironmentMapResourceKey === null
        ? null
        : (preparedAssets.find((asset) => asset.environmentMapResourceKey === activeEnvironmentMapResourceKey) ?? null);
    return {
...
export function webGpuPreparedEnvironmentAssetSetToJsonValue(set) {
    return {
        ...
        assets: set.assets.map((asset) => webGpuPreparedEnvironmentAssetToJsonValue(asset)),
    };
}
function prepareWebGpuAppEnvironmentAsset(input) {
    const environmentMapResourceKey = assetHandleKey(input.asset.handle);
    const version = input.asset.version === undefined ? null : String(input.asset.version);
    const diffuseResourceKey = versionedEnvironmentResourceKey(input.asset.diffuseResourceKey, version);
    const specularResourceKey = versionedEnvironmentResourceKey(input.asset.specularResourceKey, version);
    ...
    const bindGroupDescriptor = createStandardMaterialIblBindGroupDescriptorReadinessReport({
    ...
    const samplerKeys = resources.samplerResources.resources.flatMap((resource) => resource.resource === null ? [] : [resource.resource.resourceKey]);
    const base = {
        handle: input.asset.handle,
        environmentMapResourceKey,
        version,
        ready: (equirectProjection?.ready ?? true) &&
        ...
        cacheSummary: writeWebGpuEnvironmentResourceCacheSummary(createWebGpuEnvironmentResourceCacheSummary(), input.cache),
    };
    return input.asset.label === undefined
        ? base
        : { ...base, label: input.asset.label };
}
function webGpuPreparedEnvironmentAssetToJsonValue(asset) {
    return {
        ...
        resourceKeys: {
            ...
            bindGroupResourceKey: asset.bindGroupResource.resource?.resourceKey ?? null,
        },
        reuse: { ...asset.reuse },
        cacheSummary: { ...asset.cacheSummary },
        reports: {
            descriptors: iblResourceDescriptorReportToJsonValue(asset.descriptorReport),
            texturePreparation: iblTexturePreparationReportToJsonValue(asset.texturePreparation),
            samplerDescriptors: iblSamplerDescriptorReadinessReportToJsonValue(asset.samplerDescriptors),
            ...(asset.equirectProjection === undefined
                ? {}
                : {
                    equirectProjection: equirectToCubeResourceReportToJsonValue(asset.equirectProjection),
                }),
            diffuseTexture: diffuseIblTextureResourceReportToJsonValue(asset.diffuseTextureResource),
            specularTexture: specularIblTextureResourceReportToJsonValue(asset.specularTextureResource),
            samplerResources: iblSamplerResourceReportToJsonValue(asset.samplerResources),
            bindGroupDescriptor: standardMaterialIblBindGroupDescriptorReadinessReportToJsonValue(asset.bindGroupDescriptor),
            bindGroupResource: standardMaterialIblBindGroupResourceReportToJsonValue(asset.bindGroupResource),
        },
    };
}
function equirectProjectionForAsset(input) {
    ...
    return createEquirectToCubeResource({
        ...
        resourceKey: versionedEnvironmentResourceKey(source.resourceKey ?? `${input.environmentMapResourceKey}:equirect-cube`, input.version),
        label: source.label ?? input.asset.label ?? input.environmentMapResourceKey,
    });
}
function equirectToCubeResourceReportToJsonValue(report) {
    return {
        ready: report.ready,
...
function summarizePreparedEnvironmentAssets(assets) {
    return assets.reduce((totals, asset) => ({
        assetCount: totals.assetCount + 1,
        readyAssetCount: totals.readyAssetCount + (asset.ready ? 1 : 0),
        diffuseTextureResourcesCreated: totals.diffuseTextureResourcesCreated +
            asset.reuse.diffuseTextureResourcesCreated,
        diffuseTextureResourcesReused: totals.diffuseTextureResourcesReused +
            asset.reuse.diffuseTextureResourcesReused,
        specularTextureResourcesCreated: totals.specularTextureResourcesCreated +
            asset.reuse.specularTextureResourcesCreated,
        specularTextureResourcesReused: totals.specularTextureResourcesReused +
            asset.reuse.specularTextureResourcesReused,
        samplerResourcesCreated: totals.samplerResourcesCreated + asset.reuse.samplerResourcesCreated,
        samplerResourcesReused: totals.samplerResourcesReused + asset.reuse.samplerResourcesReused,
        standardIblBindGroupsCreated: totals.standardIblBindGroupsCreated +
            asset.reuse.standardIblBindGroupsCreated,
        standardIblBindGroupsReused: totals.standardIblBindGroupsReused +
...
function environmentPacket(environmentId, handle) {
    return {
        environmentId,
        handle,
        color: [1, 1, 1, 1],
...
function diffuseSourcesForAsset(input) {
    ...
    if (input.equirectProjection?.ready !== true ||
    ...
    return [
        {
            ...
            sourceResourceKey: input.diffuseResourceKey,
            environmentMapResourceKey: input.environmentMapResourceKey,
            label: input.asset.equirectSource?.label ??
                input.asset.label ??
...
function versionedEnvironmentResourceKey(resourceKey, version) {
    return version === null ? resourceKey : `${resourceKey}@${version}`;
}
function deviceFromApp(app) {
    const maybeApp = app;
    return maybeApp.initialization?.device ?? {};
}
//# sourceMappingURL=app-environment-resources.js.map