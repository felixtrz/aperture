 ...
 */
export function writeVersionedBufferData(device, buffer, source, content, stamp) {
 ...
 */
export function writeBufferDataDirtyRange(device, buffer, source, range) {
    if (range === null) {
        return "skipped";
    }
...
export function writeBufferData(device, buffer, data) {
 ...
 */
export function writeBufferSubData(device, buffer, data, range) {
    const queue = device.queue;
    if (queue?.writeBuffer === undefined) {
        return false;
    }
    const dataByteOffset = range.dataByteOffset ?? 0;
    if (!Number.isInteger(range.bufferByteOffset) ||
    ...
    queue.writeBuffer(buffer, range.bufferByteOffset, data.buffer, data.byteOffset + dataByteOffset, range.byteLength);
    return true;
}
//# sourceMappingURL=app-frame-resource-utils.js.map