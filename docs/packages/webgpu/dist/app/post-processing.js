...
import { resolveWebGpuAppPostPassColorHistory, } from "../post/post-color-history.js";
...
export function assembleWebGpuAppPostProcessedSwapchainTarget(options) {
    if (options.useFrameGraph === true) {
        const viaGraph = assembleWebGpuAppPostProcessedSwapchainTargetViaGraph(options);
        if (viaGraph !== null) {
            return viaGraph;
        }
            ...
            return invalidPostProcessedSwapchainTarget(options, [
                {
                    ...
                    severity: "error",
                    message: "Post-processing graph execution was required to fold shadow caster passes into the frame, but this post route is not graph-compatible.",
                },
            ]);
        }
    }
    ...
    if (skippedUserPasses.length > 0) {
        diagnostics.push(createUserPassSkippedOnLegacyRouteDiagnostic(skippedUserPasses));
    }
    const device = options.app.initialization.device;
    const queue = options.app.initialization.device.queue;
    const context = options.app.initialization.context;
    ...
    const requiresMotionVectors = options.effects.some((effect) => effect.requiresMotionVectors === true);
    let motionVectorTexture;
    ...
    let indirectColorTexture;
    if (options.indirectColorFormat !== undefined &&
    ...
    const requiresDepthTexture = options.effects.some((effect) => effect.requiresDepthTexture === true);
    const depthTexture = requiresDepthTexture
        ? {
        ...
        : undefined;
    const motionVectorAttachmentView = options.motionVectorColorFormat === undefined ||
        options.motionVectorColorFormat === null
        ? undefined
        : motionVectorTexture?.texture.createView?.();
    const useSceneMotionVectorAttachment = motionVectorTexture !== undefined &&
        motionVectorAttachmentView !== undefined;
    const indirectColorAttachmentView = options.indirectColorFormat === undefined ||
        options.indirectColorFormat === null
        ? undefined
        : indirectColorTexture?.texture.createView?.();
    const useSceneIndirectAttachment = indirectColorTexture !== undefined &&
    ...
    let input = sceneTexture.resource;
    ...
    for (let effectIndex = 0; effectIndex < options.effects.length; effectIndex += 1) {
        ...
        if (!isLast) {
            const intermediate = createOrReuseWebGpuPostPassTexture({
                device: options.app.initialization.device,
                slot: (effectIndex + options.snapshot.frame) % 2 === 0
        ...
        const postOk = postBoundary.valid &&
            prepared.diagnostics.length === 0 &&
    ...
    if (overlayCommands.length > 0) {
        ...
        if (overlayBoundary.readback !== null &&
...
function createPostSceneRenderBundleOptions(args) {
    ...
    const bundledCommandCount = Math.min(args.commands.length, args.options.renderBundleCommandCount ?? args.commands.length);
    const bundleCommands = bundledCommandCount === args.commands.length
        ? args.commands
        : args.commands.slice(0, bundledCommandCount);
    if (args.options.enableRenderBundles !== true ||
...
function invalidPostProcessedSwapchainTarget(options, diagnostics) {
    return {
        valid: false,
        boundaries: [],
        renderTarget: {
 ...
 */
export function assembleWebGpuAppPostProcessedSwapchainTargetViaGraph(options) {
    const requiresMotionVectors = options.effects.some((effect) => effect.requiresMotionVectors === true);
    const motionVectorColorFormat = options.motionVectorColorFormat === undefined ||
        options.motionVectorColorFormat === null
        ? null
    ...
    const depthTexture = requiresDepthTexture
        ...
        : undefined;
    const diagnostics = [...sceneTexture.diagnostics];
    ...
    let motionVectorTexture = null;
    let motionVectorHandle = null;
    ...
    const registerNode = (args) => {
        ...
        const writes = [
            {
                ...
                attachment: args.writeAttachment ?? "clear",
            },
        ];
        for (const extra of args.additionalWriteHandles ?? []) {
            ...
            writes.push({ handle: extra, attachment: "clear" });
        }
    ...
    const shadowNodeNames = [];
    const shadowReadHandles = [];
    ...
    const userPasses = (options.app.userPassRegistry?.list() ?? []).filter((descriptor) => descriptor.enabled !== false);
    ...
    if (options.present !== false) {
        for (let effectIndex = 0; effectIndex < options.effects.length; effectIndex += 1) {
            ...
            let outputTexture = null;
            let historyForEffect;
            let effectHistoryHandle = null;
            if (usesColorHistory) {
            ...
            if (prepared.graph !== undefined) {
                ...
                let lastOutputResource = null;
                let lastOutput = "offscreen";
            ...
            const effectReads = motionVectorHandle !== null && effect.requiresMotionVectors === true
    ...
    const overlayNodeName = options.present === false || overlayCommands.length === 0
        ? null
    ...
    for (const shadowNodeName of shadowNodeNames) {
        ...
        diagnostics.push(...(shadowEncode?.begin?.diagnostics ?? []), ...(shadowEncode?.execution?.diagnostics ?? []), ...(shadowEncode?.end?.diagnostics ?? []));
    }
    if (overlayNodeName !== null) {
        const overlayEncode = encodeByName.get(overlayNodeName);
        valid &&= (overlayEncode?.valid ?? false) && frameOk;
        diagnostics.push(...(overlayEncode?.begin?.diagnostics ?? []), ...(overlayEncode?.execution?.diagnostics ?? []), ...(overlayEncode?.end?.diagnostics ?? []));
    }
    const postEffects = effectMetas.map((meta) => {
        ...
        const effectDrawCalls = meta.plannedDrawCalls !== null
...
function assembleWebGpuAppPreparedPostEffectGraph(options) {
    ...
    for (let graphPassIndex = 0; graphPassIndex < options.graph.passes.length; graphPassIndex += 1) {
        ...
        const postBoundary = graphPass.output === "offscreen"
...
function appendFrameBoundaryDiagnostics(diagnostics, boundary) {
    diagnostics.push(...boundary.texture.diagnostics, ...(boundary.attachments?.diagnostics ?? []), ...(boundary.encoder?.diagnostics ?? []), ...(boundary.begin?.diagnostics ?? []), ...(boundary.execution?.diagnostics ?? []), ...(boundary.end?.diagnostics ?? []), ...(boundary.occlusionQueries?.diagnostics ?? []), ...(boundary.finish?.diagnostics ?? []), ...(boundary.submit?.diagnostics ?? []));
}
