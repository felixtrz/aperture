import { makePerspective, multiplyMat4, toVec3Tuple, vec3Dot, } from "@aperture-engine/simulation";
const EPSILON = 1e-6;
export function createSpotShadowMatrixComputationReport(input) {
    ...
    if (input.viewProjection.status === "missing") {
        diagnostics.push({
            ...
            severity: "warning",
            message: "Spot shadow matrix computation requires view/projection planning.",
        });
    }
    if (input.viewProjection.status === "unsupported") {
        diagnostics.push({
            ...
            severity: "warning",
            message: "Spot shadow matrix computation only supports spot shadow plans.",
        });
    }
    const matrices = [];
    ...
    return {
        ...
        sections: {
            ...
            transformData: !diagnostics.some((diagnostic) => diagnostic.code === "spotShadowMatrix.missingLightTransform"),
...
export function spotShadowMatrixComputationReportToJsonValue(report) {
    return {
        ready: report.ready,
...
export function spotShadowMatrixComputationReportToJson(report) {
    return JSON.stringify(spotShadowMatrixComputationReportToJsonValue(report));
}
function computeSpotShadowMatrix(input, plan) {
...
//# sourceMappingURL=spot-shadow-matrix-computation.js.map