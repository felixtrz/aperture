export function createShadowMapDescriptorReport(input) {
    const sourcesByKey = new Map((input.descriptors ?? []).map((descriptor) => [
    ...
    const descriptors = input.shadowRequests.map((request) => {
        ...
        const source = sourcesByKey.get(descriptorKey(request.shadowId, request.lightId));
        if (source === undefined) {
            diagnostics.push({
        ...
        const cascadeCount = lightKind === "directional"
        ...
        const viewDimension = source?.viewDimension ??
            (lightKind === "point"
                ...
                : lightKind === "directional" &&
                    ...
                    ? "2d-array"
                    : "2d");
        const layerCount = Math.max(1, source?.layerCount ??
            (lightKind === "directional" ? cascadeCount : faceCount));
        const textureWidth = normalizeTextureExtent(source?.textureWidth, source?.mapSize);
        const textureHeight = normalizeTextureExtent(source?.textureHeight, source?.mapSize);
        const atlasRegion = normalizeAtlasRegion(source?.atlasRegion, textureWidth, textureHeight);
        return {
            shadowId: request.shadowId,
            lightId: request.lightId,
            lightKind,
            ...
            filterRadiusTexels: normalizeFilterRadiusTexels(source?.filterRadiusTexels, lightKind),
            cascadeCount,
            faceCount,
            viewDimension,
            layerCount,
...
export function shadowMapDescriptorReportToJsonValue(report) {
    return {
        ready: report.ready,
...
export function shadowMapDescriptorReportToJson(report) {
    return JSON.stringify(shadowMapDescriptorReportToJsonValue(report));
}
function normalizeFilterRadiusTexels(value, lightKind) {
...
function descriptorKey(shadowId, lightId) {
    return `${shadowId}:${lightId}`;
}
function clampCascadeCount(value) {
    if (!Number.isInteger(value)) {
        return 1;
    }
    return Math.min(4, Math.max(1, value));
}
function normalizeTextureExtent(value, mapSize) {
    const fallback = Number.isInteger(mapSize) && mapSize !== undefined && mapSize > 0
        ? mapSize
        : 0;
...
function normalizeAtlasRegion(value, textureWidth, textureHeight) {
    if (value === undefined) {
        return undefined;
    }
...
function normalizeAtlasInteger(value) {
    if (!Number.isInteger(value) || value < 0) {
        return null;
    }
    return value;
}
//# sourceMappingURL=shadow-map-descriptor.js.map