import { planRenderPassCommands, } from "../render/passes/render-pass-commands.js";
export function createShadowCasterCommandRecordPlanReport(options) {
    ...
    const pipelines = new Map((options.pipelines ?? []).map((pipeline) => [
        pipeline.pipelineKey,
        pipeline,
    ]));
    const matrixBindGroups = new Map((options.matrixBindGroups ?? []).map((bindGroup) => [
        bindGroup.matrixResourceKey,
        bindGroup,
    ]));
    const matrixBindGroupsByPass = new Map((options.matrixBindGroups ?? []).flatMap((bindGroup) => bindGroup.passKey === undefined ? [] : [[bindGroup.passKey, bindGroup]]));
    const meshes = new Map((options.meshes ?? []).map((mesh) => [mesh.meshResourceKey, mesh]));
    const commandPlans = new Map(options.commandPlan.commands.map((command) => [command.passKey, command]));
    const diagnostics = [];
    const resolvedDrawsByPass = new Map();
    if (options.frameResources.counts.readyDraws === 0) {
        diagnostics.push({
            code: "shadowCasterCommandRecord.frameResourcesNotReady",
            severity: "warning",
            message: "Shadow caster command records require at least one ready caster frame-resource record.",
        });
    }
    for (const record of options.frameResources.records) {
        ...
        const pipeline = record.pipelineKey === null
            ? undefined
            : pipelines.get(record.pipelineKey);
        const matrixBindGroup = matrixBindGroupsByPass.get(record.passKey) ??
            (record.matrixResourceKey === null
                ? undefined
                : matrixBindGroups.get(record.matrixResourceKey));
        const mesh = record.meshResourceKey === null
            ? undefined
        ...
        const worldTransformIndex = options.worldTransformIndexByPassDraw?.get(passDrawKey);
        const useWorldTransform = worldTransformIndex !== undefined &&
            matrixBindGroup?.worldTransformResourceKey !== undefined;
        const bakedIndex = options.bakedMatrixIndexByPassDraw?.get(passDrawKey);
        const useBaked = !useWorldTransform &&
        ...
        if (vertexBuffers.length !== record.vertexBufferResourceKeys.length ||
            indexBuffer === null ||
            transformPackedOffset === null) {
            continue;
        }
        const activeBindGroup = useBaked && options.bakedMatrixBindGroup !== undefined
    ...
    for (const [passKey, draws] of resolvedDrawsByPass) {
        ...
        const commandKey = commandPlans.get(passKey)?.commandKey ??
    ...
    if (records.length > 0) {
        diagnostics.push({
            ...
            severity: "warning",
            message: "Shadow caster command records are executable, but command-buffer finish and queue submission are deferred.",
        });
        diagnostics.push({
            code: "shadowCasterCommandRecord.shaderSamplingDeferred",
            severity: "warning",
            message: "Shadow caster command records are executable, but StandardMaterial shadow sampling remains deferred.",
        });
    }
    const hasBlockingDiagnostics = diagnostics.some((diagnostic) => diagnostic.code !== "shadowCasterCommandRecord.passSubmissionDeferred" &&
        diagnostic.code !== "shadowCasterCommandRecord.shaderSamplingDeferred");
    return report({
        status: hasBlockingDiagnostics || commandRecords.length === 0
            ? "missing"
            : "ready",
        frameResources: options.frameResources,
        pipelines: options.pipelines ?? [],
...
export function shadowCasterCommandRecordPlanReportToJsonValue(value) {
    return {
        ready: value.ready,
        status: value.status,
        counts: { ...value.counts },
...
export function shadowCasterCommandRecordPlanReportToJson(value) {
    return JSON.stringify(shadowCasterCommandRecordPlanReportToJsonValue(value));
}
function resolveVertexBuffers(record, mesh, diagnostics) {
    const byKey = new Map(mesh.vertexBuffers.map((buffer) => [buffer.resourceKey, buffer]));
    const vertexBuffers = [];
...
function resolveIndexBuffer(record, mesh, diagnostics) {
...
function mapCommandDiagnostics(passKey, diagnostics) {
...
function createRecord(passKey, commandKey, draws, commands) {
    return {
        passKey,
        commandKey,
        renderIds: draws.flatMap((draw) => draw.renderIds),
        commandCount: commands.length,
        drawCalls: commands.filter((command) => command.kind === "draw" || command.kind === "drawIndexed").length,
        indexedDrawCalls: commands.filter((command) => command.kind === "drawIndexed").length,
        pipelineKeys: unique(draws.map((draw) => draw.pipelineKey)),
        pipelineResourceKeys: unique(commands.flatMap((command) => command.kind === "setPipeline" ? [command.pipelineKey] : [])),
        bindGroupResourceKeys: unique(commands.flatMap((command) => command.kind === "setBindGroup" ? [command.resourceKey] : [])),
        vertexBufferResourceKeys: unique(commands.flatMap((command) => command.kind === "setVertexBuffer" ? [command.resourceKey] : [])),
        indexBufferResourceKeys: unique(commands.flatMap((command) => command.kind === "setIndexBuffer" ? [command.resourceKey] : [])),
        drawCommandKeys: draws.flatMap((draw) => draw.renderIds.map((renderId) => `${passKey}:draw:${renderId}`)),
    };
}
function pushShadowCasterResolvedDraw(draws, draw) {
...
function canCoalesceShadowCasterDraw(previous, draw) {
    if (previous === undefined) {
        return false;
    }
    return (previous.pipelineKey === draw.pipelineKey &&
...
function shadowCasterResolvedDrawResourcesMatch(a, b) {
    if (a.bindGroups.length !== b.bindGroups.length ||
    ...
    return (a.indexBuffer.resourceKey === b.indexBuffer.resourceKey &&
...
function matrixOffsetBytesToPackedOffset(offsetBytes) {
    if (!Number.isInteger(offsetBytes) ||
        offsetBytes < 0 ||
        offsetBytes % 4 !== 0) {
        return null;
    }
...
function report(input) {
    ...
    const indexedDrawCalls = input.records.reduce((sum, record) => sum + record.indexedDrawCalls, 0);
    return {
        ...
        sections: {
            frameResources: input.frameResources.status === "not-required" ||
                input.frameResources.counts.readyDraws > 0,
            commandPlans: input.frameResources.status === "not-required" ||
...
//# sourceMappingURL=shadow-caster-command-record-plan.js.map