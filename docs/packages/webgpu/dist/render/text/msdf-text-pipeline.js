...
`.trim();
export async function createMsdfTextRenderPipelineResource(options) {
    ...
    const shaderModule = await createWebGpuShaderModule({
        ...
        descriptor: {
            ...
            code: applyOutputStageToFragmentWgsl(MSDF_TEXT_WGSL, tonemap, outputColorSpace, MSDF_TEXT_PIPELINE_KEY),
        },
    });
    ...
    try {
        return {
            valid: true,
            resource: {
                cacheKey: msdfTextPipelineCacheKey(options.colorFormat, options.depthFormat ?? null, options.sampleCount ?? 1, tonemap, outputColorSpace),
                shaderModule: shaderModule.module,
...
export function msdfTextPipelineCacheKey(colorFormat, depthFormat = null, sampleCount = 1, tonemap = "none", outputColorSpace = "linear") {
...
//# sourceMappingURL=msdf-text-pipeline.js.map