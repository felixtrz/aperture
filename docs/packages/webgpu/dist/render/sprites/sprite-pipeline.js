...
`.trim();
export async function createSpriteRenderPipelineResource(options) {
    ...
    const shaderModule = await createWebGpuShaderModule({
        ...
        descriptor: {
            ...
            code: applyOutputStageToFragmentWgsl(SPRITE_WGSL, tonemap, outputColorSpace, "aperture/sprite-billboard"),
        },
    });
    ...
    try {
        return {
            valid: true,
            resource: {
                cacheKey: spritePipelineCacheKey(options.colorFormat, options.depthFormat ?? null, options.sampleCount ?? 1, tonemap, outputColorSpace, depthMode),
                shaderModule: shaderModule.module,
...
export function spritePipelineCacheKey(colorFormat, depthFormat, sampleCount = 1, tonemap = "none", outputColorSpace = "linear", depthMode = "test") {
