...
export function createMotionVectorBuiltInShaderVariant(shader) {
    const codeWithUniform = addPreviousViewProjectionUniform(shader.code);
    const codeWithPreviousTransforms = addPreviousWorldTransformStorage(codeWithUniform);
    ...
    const code = wrapFragmentWithMotionVectorOutput(codeWithAssignments, shader.entryPoints.fragment);
    return {
        ...shader,
...
function addPreviousViewProjectionUniform(code) {
    if (code.includes("previousViewProjection: mat4x4f")) {
        return code;
    }
    return code.replace("  cameraPosition: vec4f,\n", "  cameraPosition: vec4f,\n  previousViewProjection: mat4x4f,\n");
}
function addPreviousWorldTransformStorage(code) {
    if (code.includes("previousWorldTransforms: array<mat4x4f>")) {
        return code;
    }
    return code.replace("@group(1) @binding(0) var<storage, read> worldTransforms: array<mat4x4f>;", `@group(1) @binding(0) var<storage, read> worldTransforms: array<mat4x4f>;
@group(1) @binding(${PREVIOUS_WORLD_TRANSFORM_BINDING}) var<storage, read> previousWorldTransforms: array<mat4x4f>;`);
}
function addMotionVectorVaryings(code) {
    if (code.includes("motionClipPosition: vec4f")) {
        return code;
    }
    return code.replace(/(struct VertexOutput \{[\s\S]*?)(\n};)/, "$1\n  @location(14) motionClipPosition: vec4f,\n  @location(15) previousMotionClipPosition: vec4f,$2");
}
function addMotionVectorAssignments(code) {
    if (code.includes("output.previousMotionClipPosition")) {
        return code;
    }
    const codeWithPreviousWorld = addPreviousWorldLocal(code);
    const previousLocalPosition = previousLocalPositionExpression(codeWithPreviousWorld);
    if (codeWithPreviousWorld.includes("output.position = view.viewProjection * worldPosition;")) {
        return codeWithPreviousWorld.replace("  output.position = view.viewProjection * worldPosition;\n", `  output.position = view.viewProjection * worldPosition;
...
`);
    ...
    return codeWithPreviousWorld.replace("  output.position = view.viewProjection * world * vec4f(input.position, 1.0);\n", `  let worldPosition = world * vec4f(input.position, 1.0);
  output.position = view.viewProjection * worldPosition;
  output.motionClipPosition = output.position;
  output.previousMotionClipPosition = view.previousViewProjection * previousWorld * vec4f(input.position, 1.0);
`);
}
function addPreviousWorldLocal(code) {
    if (code.includes("let previousWorld = previousWorldTransforms")) {
        return code;
    }
    return code.replace("  let world = worldTransforms[input.instanceIndex];", "  let world = worldTransforms[input.instanceIndex];\n  let previousWorld = previousWorldTransforms[input.instanceIndex];");
}
function previousLocalPositionExpression(code) {
...
function addPreviousWorldTransformBinding(bindings) {
...
function wrapFragmentWithMotionVectorOutput(code, fragmentEntryPoint) {
    const colorEntryPoint = `${fragmentEntryPoint}_color`;
    const fragmentPattern = new RegExp(`@fragment\\s+fn ${escapeRegExp(fragmentEntryPoint)}\\(([\\s\\S]*?)\\)\\s*->\\s*@location\\(0\\)\\s*vec4f\\s*\\{`, "u");
    ...
    const fragmentArgumentList = fragmentParameterInfo
        ...
        .join(", ");
    const vertexInputName = fragmentParameterInfo.find((parameter) => parameter.type === "VertexOutput")
        ?.name ??
        fragmentParameterInfo[0]?.name ??
        "input";
    const colorCode = code.replace(fragmentPattern, `fn ${colorEntryPoint}($1) -> vec4f {`);
    return `${colorCode}
...
function parseWgslParameterList(parameters) {
    return splitTopLevelWgslParameters(parameters).map(parseWgslParameter);
}
function splitTopLevelWgslParameters(parameters) {
    const result = [];
    let start = 0;
    let parenDepth = 0;
    let angleDepth = 0;
...
function parseWgslParameter(parameter) {
    const parameterWithoutAttributes = parameter.replace(/^(?:\s*@\w+(?:\([^)]*\))?\s*)+/u, "");
    const match = /^\s*([A-Za-z_]\w*)\s*:\s*([A-Za-z_]\w*(?:<[^>]*>)?)/u.exec(parameterWithoutAttributes);
    return {
        name: match?.[1] ?? "input",
        type: match?.[2] ?? "",
    };
...
//# sourceMappingURL=motion-vector-shader.js.map