import { canGenerateTextureMipmaps, generateTextureMipmaps, } from "./generate-mipmaps.js";
...
export function createTextureGpuResource(options) {
    const colorSpaceDiagnostic = validateTextureDescriptorColorSpace(options.resourceKey, options.descriptor);
    ...
    if (options.device.createTexture === undefined) {
        return failure("textureResource.createTextureUnavailable", options.resourceKey, "WebGPU device cannot create textures.");
    }
    let texture;
    const mipGenerationRequested = shouldGenerateMipmaps(options.descriptor, options.upload);
    try {
        texture = options.device.createTexture(textureDescriptorForWebGpu(options.descriptor, {
            generateMipmaps: mipGenerationRequested,
            upload: options.upload !== undefined,
        }));
    }
    catch (error) {
        return failure("textureResource.textureCreationFailed", options.resourceKey, error instanceof Error ? error.message : "Texture creation failed.");
    }
    if (texture.createView === undefined) {
        return failure("textureResource.createViewUnavailable", options.resourceKey, "Created texture cannot create texture views.");
    }
    if (options.upload !== undefined) {
        if (options.device.queue?.writeTexture === undefined) {
            return failure("textureResource.writeTextureUnavailable", options.resourceKey, "WebGPU queue cannot upload texture data.");
        }
        const uploadLayoutDiagnostic = validateTextureUploadInput(options.resourceKey, options.descriptor, options.upload);
        ...
        try {
            for (const level of textureUploadLevels(options.descriptor, options.upload)) {
                const copySize = textureUploadCopySize(options.descriptor, level.width, level.height);
                options.device.queue.writeTexture({ texture, mipLevel: level.mipLevel }, level.data, {
            ...
            return failure("textureResource.textureUploadFailed", options.resourceKey, error instanceof Error ? error.message : "Texture upload failed.");
        }
    }
    let mipGeneration;
    if (mipGenerationRequested) {
        try {
            ...
            return failure("textureResource.mipmapGenerationFailed", options.resourceKey, error instanceof Error
                ? error.message
    ...
    try {
        ...
        return failure("textureResource.textureViewCreationFailed", options.resourceKey, error instanceof Error ? error.message : "Texture view creation failed.");
    }
    return {
        valid: true,
        resource: {
...
function validateTextureDescriptorColorSpace(resourceKey, descriptor) {
    if (descriptor.colorSpace === undefined) {
        return null;
    }
    if (isSrgbTextureFormat(descriptor.format) !==
...
function textureDescriptorForWebGpu(descriptor, options = {}) {
    const { colorSpace: _colorSpace, semantic: _semantic, ...webGpuDescriptor } = descriptor;
...
function validateTextureUploadInput(resourceKey, descriptor, upload) {
    ...
    for (const level of levels) {
        const diagnostic = validateTextureUploadLevelLayout(resourceKey, descriptor, level);
        if (diagnostic !== null) {
            return diagnostic;
        }
    }
    if (shouldGenerateMipmaps(descriptor, upload) &&
...
function textureUploadLevels(descriptor, upload) {
    if (upload.mipLevels !== undefined) {
...
function textureUploadCopySize(descriptor, width, height) {
...
function validateTextureUploadLevelLayout(resourceKey, descriptor, upload) {
    const width = upload.width;
    const height = upload.height;
    const layout = textureFormatUploadLayout(descriptor.format);
    const blockRows = layout === null ? height : Math.ceil(height / layout.blockHeight);
    const minimumBytesPerRow = layout === null
        ? null
        : Math.ceil(width / layout.blockWidth) * layout.blockByteLength;
    if (!Number.isInteger(upload.bytesPerRow) ||
        upload.bytesPerRow <= 0 ||
        (minimumBytesPerRow !== null && upload.bytesPerRow < minimumBytesPerRow)) {
        return {
            code: "textureResource.invalidBytesPerRow",
            resourceKey,
            message: minimumBytesPerRow === null
    ...
    if (upload.rowsPerImage !== undefined &&
    ...
    if (minimumBytesPerRow !== null) {
        ...
        const minimumByteLength = ((depthOrArrayLayers - 1) * rowsPerImage + (blockRows - 1)) *
            upload.bytesPerRow +
            minimumBytesPerRow;
...
function shouldGenerateMipmaps(descriptor, upload) {
    return (upload !== undefined &&
        upload.mipLevels === undefined &&
        descriptor.size[2] === 1 &&
        (descriptor.mipLevelCount ?? 1) > 1);
}
function textureFormatUploadLayout(format) {
    switch (format) {
        case "r8unorm":
...
function texelLayout(byteLength) {
    return { blockWidth: 1, blockHeight: 1, blockByteLength: byteLength };
}
function blockLayout(blockByteLength) {
    return { blockWidth: 4, blockHeight: 4, blockByteLength };
}
function isSrgbTextureFormat(format) {
    return format.endsWith("-srgb");
}
export function createSamplerGpuResource(options) {
    if (options.device.createSampler === undefined) {
        return failure("samplerResource.createSamplerUnavailable", options.resourceKey, "WebGPU device cannot create samplers.");
    }
    ...
    try {
        ...
        return failure("samplerResource.samplerCreationFailed", options.resourceKey, error instanceof Error ? error.message : "Sampler creation failed.");
    }
}
function samplerDescriptor(sampler) {
    return {
        label: sampler.label,
...
function failure(code, resourceKey, message) {
    return {
        valid: false,
        resource: null,
...
//# sourceMappingURL=texture-resources.js.map