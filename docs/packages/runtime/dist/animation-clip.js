...
export function animationChannelKeyframeCount(channel) {
 ...
 */
export function sampleAnimationChannel(channel, time, out) {
...
function sampleLinear(channel, i0, i1, p, result) {
...
function sampleCubicSpline(channel, i0, i1, td, p, result) {
    const values = channel.values;
    const componentCount = channel.componentCount;
...
function readKeyframeValue(channel, index, result) {
...
function keyframeValueOffset(channel, index) {
...
function quaternionHemisphereSign(values, offsetA, offsetB) {
    const dot = values[offsetA] * values[offsetB] +
        values[offsetA + 1] * values[offsetB + 1] +
        values[offsetA + 2] * values[offsetB + 2] +
        values[offsetA + 3] * values[offsetB + 3];
    return dot < 0 ? -1 : 1;
}
function normalizeQuaternionTuple(result) {
...
function fillDefaultValue(path, result) {
    if (path === "rotation") {
        result[0] = 0;
        result[1] = 0;
        result[2] = 0;
...
function ensureTuple(out, length) {
