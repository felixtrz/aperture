...
export function bindingPressed(binding, resource) {
    ...
    if (isLegacyGamepadBinding(binding)) {
        return gamepadButtonPressed(resource, binding.gamepad);
    }
    return false;
}
export function bindingAxis1d(binding, resource) {
    if (isLegacyKeyboardBinding(binding)) {
...
export function bindingAxis2d(binding, resource) {
...
function keyboard1dValue(keyboard, binding) {
    return (anyPressed(keyboard, binding.positive ?? []) -
        anyPressed(keyboard, binding.negative ?? []));
}
function keyboard2dValue(keyboard, binding) {
    return {
        x: anyPressed(keyboard, binding.positiveX ?? []) -
            anyPressed(keyboard, binding.negativeX ?? []),
        y: anyPressed(keyboard, binding.positiveY ?? []) -
            anyPressed(keyboard, binding.negativeY ?? []),
    };
}
function anyPressed(keyboard, codes) {
    return codes.some((code) => keyboard.pressed(code)) ? 1 : 0;
}
function gamepadButtonPressed(resource, button, index) {
    const device = index === undefined
        ? resource.gamepads.primary
        : resource.gamepads.byIndex(index);
    return device?.pressed(button) === true || device?.down(button) === true;
}
function pointerPressed(pointer, name) {
    return pointer[name].pressed.value || pointer[name].pressedThisFrame.value;
}
function gamepadButtonValue(resource, binding) {
    const device = binding.gamepadIndex === undefined
        ? resource.gamepads.primary
        : resource.gamepads.byIndex(binding.gamepadIndex);
    return device?.button(binding.button).value ?? 0;
}
function gamepadStickValue(resource, binding) {
    const device = binding.gamepadIndex === undefined
        ? resource.gamepads.primary
        : resource.gamepads.byIndex(binding.gamepadIndex);
    const stick = binding.stick === "left" ? device?.leftStick : device?.rightStick;
    if (stick === undefined) {
        return { x: 0, y: 0 };
    }
    const value = { x: 0, y: 0 };
    stick.read(value);
    return {
        x: applyGamepadDeadzone(value.x, binding.deadzone ?? DEFAULT_GAMEPAD_DEADZONE),
        y: applyGamepadDeadzone(value.y, binding.deadzone ?? DEFAULT_GAMEPAD_DEADZONE),
    };
}
function gamepadAxisValue(resource, binding) {
...
function isLegacyKeyboardBinding(binding) {
    return "keyboard" in binding;
}
function isLegacyPointerBinding(binding) {
    return "pointer" in binding && !("kind" in binding);
}
function isLegacyGamepadBinding(binding) {
    return "gamepad" in binding;
}
function bindingHasKind(binding, kind) {
