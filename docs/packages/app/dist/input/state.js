...
import { beginActionFrame, createInputActions, setAxis1dActionValue, setAxis2dActionValue, setButtonActionPressed, } from "./actions.js";
...
export function createInputResource(config) {
    return new InputResourceImpl(config);
}
export function advanceInputResource(input, events = []) {
    input.advanceFrame(events);
}
class InputResourceImpl {
    actions;
    pointer = {
    ...
    keyboard = createStatefulKeyboardState();
    gamepads = createStatefulGamepadsState();
    gamepad = {};
    xr = {
        active: createSignal(false),
    };
    #descriptors;
    #virtualActions = new Map();
    #diagnostics = [];
    constructor(config) {
        this.#descriptors = normalizeActionDescriptors(config?.input?.actions ?? {});
    ...
    #resolveActions() {
        for (const [name, descriptor] of Object.entries(this.#descriptors)) {
            ...
            if (descriptor.kind === "button" && action.kind === "button") {
                const pressed = descriptor.bindings.some((binding) => bindingPressed(binding, this)) ||
            ...
            if (descriptor.kind === "axis1d" && action.kind === "axis1d") {
                const value = clamp(descriptor.bindings.reduce((sum, binding) => sum + bindingAxis1d(binding, this), virtual?.value ?? 0), -1, 1);
            ...
            if (descriptor.kind === "axis2d" && action.kind === "axis2d") {
                ...
                setAxis2dActionValue(action, clamp(value.x, -1, 1), clamp(value.y, -1, 1));
            }
        }
        this.#clearVirtualActionEdges();
    }
    #clearVirtualActionEdges() {
...
function createPointerButtonState() {
    return {
...
function pointerButtons(pointer) {
    return [pointer.primary, pointer.secondary, pointer.middle];
}
function normalizeActionDescriptors(actions) {
    const output = {};
...
function normalizeActionDescriptor(descriptor) {
