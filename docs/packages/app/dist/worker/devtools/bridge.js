import { PhysicsCharacterController, PhysicsCharacterMassMode, PhysicsDebug, PhysicsJoint, collectUnsupportedPhysicsJointFeatures, createUnsupportedJointImpulseReadbackFeature, validatePhysicsCharacterMove, } from "@aperture-engine/physics";
import { LocalTransform, serializeEntityRef, } from "@aperture-engine/simulation";
import { createApertureDevtoolsResponse, } from "../../commands.js";
...
import { booleanFromValue, isRecord, numberFromValue, stringFromValue, tuple3FromValue, tuple4FromValue, } from "../payload.js";
import { callCameraTool } from "./camera.js";
...
export function createGeneratedDevtoolsBridge(options) {
    const savedCameraStates = new Map();
    return {
        handle(request) {
            try {
                const result = callGeneratedDevtoolsTool(options, request, savedCameraStates);
                options.port.postMessage(createApertureDevtoolsResponse({
...
function callGeneratedDevtoolsTool(bridge, request, savedCameraStates) {
    ...
    if (request.tool.startsWith("input_")) {
        const result = callInputDevtoolsTool(bridge.app, request.tool, request.payload, { enqueueInputEvent: bridge.enqueueInputEvent });
        if (result !== null) {
            return result;
        }
    }
    ...
    if (request.tool === "physics_apply_force" ||
    ...
    if (request.tool === "physics_sleep_body" ||
        request.tool === "physics_wake_body") {
        return callPhysicsSleepWakeBodyTool(bridge.app, request.tool, request.payload);
    }
    if (request.tool === "physics_raycast_first" ||
    ...
    if (request.tool === "physics_debug_geometry" ||
...
function callStepAndDiffTool(bridge, payload) {
            ...
            diagnostics: [
                {
                    ...
                    suggestedFix: "Call ecs_snapshot with the entities or query you want to compare, then call ecs_step_and_diff.",
                },
            ],
            result: null,
        };
                ...
                diagnostics: [
                    {
                        ...
                        suggestedFix: "Pass { id: 'your.resource.id' }, or omit id to list all generated app resources.",
                    },
                ],
            };
        }
            ...
            diagnostics: [
                {
                    ...
                    suggestedFix: "Call resource_get without an id to list resources currently initialized by generated worker systems.",
                },
            ],
        };
    }
...
function callResourceSetTool(app, payload) {
    const record = isRecord(payload) ? payload : {};
            ...
            diagnostics: [
                {
                    ...
                    suggestedFix: "Pass { id: 'your.resource.id', values: { fieldName: value } }.",
                },
            ],
        };
    }
            ...
            diagnostics: [
                {
                    ...
                    suggestedFix: "Pass { values: { fieldName: value } } using fields reported by resource_get.",
                },
            ],
        };
    }
...
function resourceNotFoundDiagnostic(id) {
    return {
        code: "aperture.resource.notFound",
        severity: "warning",
        message: `Generated app resource '${id}' was not found.`,
...
function devtoolsErrorDiagnostic(error) {
    if (error instanceof ApertureSystemError) {
        return {
            code: error.code,
            severity: "error",
...
function physicsEntityFromPayload(app, payload, diagnostic) {
    const record = isRecord(payload) ? payload : {};
...
function callPhysicsCommandTool(app, tool, payload) {
    const record = isRecord(payload) ? payload : {};
    const resolved = physicsEntityFromPayload(app, payload, {
        ...
        suggestedFix: "Run ecs_find_entities or ecs_snapshot, then pass the returned entity reference as { entity }.",
    });
    if (!resolved.ok) {
    ...
    if (tool === "physics_apply_force") {
        ...
        if (force === null) {
            return invalidPhysicsCommandDiagnostic("aperture.physics.applyForce.invalidPayload", "physics_apply_force requires a finite force [x, y, z] tuple.", "Pass { entity: { index, generation }, force: [x, y, z] } and optional point/torque tuples.");
        }
        ...
        if (point === null || torque === null) {
            return invalidPhysicsCommandDiagnostic("aperture.physics.applyForce.invalidOptions", "physics_apply_force received a malformed point or torque tuple.", "Use finite [x, y, z] tuples for optional point and torque fields.");
        }
    ...
    if (tool === "physics_apply_impulse") {
        ...
        if (impulse === null) {
            return invalidPhysicsCommandDiagnostic("aperture.physics.applyImpulse.invalidPayload", "physics_apply_impulse requires a finite impulse [x, y, z] tuple.", "Pass { entity: { index, generation }, impulse: [x, y, z] } and optional point/angularImpulse tuples.");
        }
        const point = optionalTuple3FromRecord(record, "point");
        const angularImpulse = optionalTuple3FromRecord(record, "angularImpulse");
        if (point === null || angularImpulse === null) {
            return invalidPhysicsCommandDiagnostic("aperture.physics.applyImpulse.invalidOptions", "physics_apply_impulse received a malformed point or angularImpulse tuple.", "Use finite [x, y, z] tuples for optional point and angularImpulse fields.");
        }
    ...
    if (tool === "physics_set_linear_velocity" ||
        ...
        const velocity = tuple3FromValue(record["velocity"] ??
            record["value"] ??
            (tool === "physics_set_linear_velocity"
                ? record["linear"]
                : record["angular"]));
        if (velocity === null) {
            return invalidPhysicsCommandDiagnostic(tool === "physics_set_linear_velocity"
                ? "aperture.physics.setLinearVelocity.invalidPayload"
                : "aperture.physics.setAngularVelocity.invalidPayload", `${tool} requires a finite velocity [x, y, z] tuple.`, "Pass { entity: { index, generation }, velocity: [x, y, z] }.");
        }
        if (tool === "physics_set_linear_velocity") {
    ...
    const transform = physicsTransformFromPayload(isRecord(record["transform"]) ? record["transform"] : record);
    if (transform === null) {
        return invalidPhysicsCommandDiagnostic("aperture.physics.setKinematicTarget.invalidPayload", "physics_set_kinematic_target requires a finite target transform.", "Pass { entity: { index, generation }, translation: [x, y, z], rotation?: [x, y, z, w] } or { transform }.");
    }
...
function physicsCommandResult(app, entity, command) {
    return {
        entity: {
            index: entity.index,
...
function invalidPhysicsCommandDiagnostic(code, message, suggestedFix) {
    return {
        ok: false,
        diagnostics: [
            {
...
function callPhysicsSleepWakeBodyTool(app, tool, payload) {
    const resolved = physicsEntityFromPayload(app, payload, {
        code: tool === "physics_sleep_body"
            ? "aperture.physics.sleepBody.missingEntity"
            : "aperture.physics.wakeBody.missingEntity",
        message: `${tool} requires an entity reference with { index, generation }.`,
        suggestedFix: "Run ecs_find_entities or ecs_snapshot, then pass the returned body entity reference as { entity }.",
    });
    if (!resolved.ok) {
        return { ok: false, diagnostics: [resolved.diagnostic] };
    }
    const changed = tool === "physics_sleep_body"
            ...
            diagnostics: [
                {
                    ...
                    suggestedFix: "Step once to sync the ECS body into the active same-worker backend, confirm the backend supports body sleep/wake controls, then retry with an active RigidBody entity.",
                },
            ],
        };
    }
...
function callPhysicsEventsTool(app, payload) {
    const record = isRecord(payload) ? payload : {};
    const kind = stringFromValue(record["kind"]);
    const family = stringFromValue(record["family"]);
    const entity = eventEntityRefFromValue(record["entity"] ?? record["entityRef"] ?? record["ref"]);
    ...
    if (kind !== undefined && !isPhysicsEventKind(kind)) {
        return invalidPhysicsCommandDiagnostic("aperture.physics.events.invalidKind", `physics_events does not recognize event kind '${kind}'.`, `Pass one of: ${physicsEventKinds.join(", ")}.`);
    }
    if (family !== undefined && !isPhysicsEventFamily(family)) {
        return invalidPhysicsCommandDiagnostic("aperture.physics.events.invalidFamily", `physics_events does not recognize event family '${family}'.`, `Pass one of: ${physicsEventFamilies.join(", ")}.`);
    }
    if (hasPayloadKey(record, "entity") && entity === undefined) {
        return invalidPhysicsCommandDiagnostic("aperture.physics.events.invalidEntity", "physics_events received a malformed entity filter.", "Pass an entity ref as { index, generation } or an existing backend entity token like '12:0'.");
    }
    if (hasPayloadKey(record, "joint") && joint === undefined) {
        return invalidPhysicsCommandDiagnostic("aperture.physics.events.invalidJoint", "physics_events received a malformed joint filter.", "Pass a joint entity ref as { index, generation } or an existing backend entity token like '12:0'.");
    }
    const physics = app.context.physics.summary();
    const matchedEvents = physics.events.filter((event) => physicsEventMatches(event, { kind, family, entity, joint }));
    const events = limit === undefined ? matchedEvents : matchedEvents.slice(0, limit);
    return {
        ok: true,
        result: {
            filters: {
...
function hasPayloadKey(record, key) {
...
function eventEntityRefFromValue(value) {
...
function isPhysicsEventKind(kind) {
    return physicsEventKinds.includes(kind);
}
function isPhysicsEventFamily(family) {
    return physicsEventFamilies.includes(family);
}
function physicsEventMatches(event, filters) {
    return ((filters.kind === undefined || event.kind === filters.kind) &&
...
function physicsEventIsInFamily(event, family) {
    switch (family) {
        case "contacts":
            return (event.kind === "collisionStart" ||
        ...
        case "collisions":
            return (event.kind === "collisionStart" ||
                ...
                event.kind === "collisionEnd");
        case "triggers":
            return (event.kind === "triggerEnter" ||
...
function callPhysicsJointStatusTool(app, payload) {
    const resolved = physicsEntityFromPayload(app, payload, {
        code: "aperture.physics.jointStatus.missingEntity",
        message: "physics_joint_status requires an entity reference with { index, generation }.",
        suggestedFix: "Run ecs_find_entities or ecs_snapshot, then pass the returned joint entity reference as { entity }.",
    });
    if (!resolved.ok) {
        return { ok: false, diagnostics: [resolved.diagnostic] };
    }
            ...
            diagnostics: [
                {
                    ...
                    severity: "warning",
                    message: "physics_joint_status received an entity without a PhysicsJoint component.",
                    suggestedFix: "Pass a joint entity from ecs_find_entities or ecs_snapshot with PhysicsJoint in withComponents.",
                },
            ],
        };
    }
    ...
    const authoredUnsupportedFeatures = backend === null || joint === undefined
        ? []
        : collectUnsupportedPhysicsJointFeatures(backend.kind, ref, physicsJointDescriptorFromSummary(joint));
    const syncUnsupportedFeatures = physics.unsupportedFeatures.filter((feature) => feature.entity === ref);
    const capabilities = backend?.capabilities ?? null;
    const isUnitJoint = joint?.kind === "revolute" || joint?.kind === "prismatic";
    const jointImpulseReadbackSupported = capabilities?.jointImpulseReadback === true;
    const jointImpulseReadbackUnsupportedFeature = backend === null || jointImpulseReadbackSupported
        ? null
    ...
    return {
        ...
        result: {
            ...
            capabilities: {
                ...
                combinedPositionVelocityMotors: (capabilities?.combinedPositionVelocityMotors ?? false) &&
                    isUnitJoint,
                pairedNonFixedFrameB: joint?.kind === "fixed" ||
...
function physicsJointDescriptorFromSummary(joint) {
    return {
        kind: joint.kind,
...
function callPhysicsBreakJointTool(app, payload) {
    const record = isRecord(payload) ? payload : {};
    const resolved = physicsEntityFromPayload(app, payload, {
        code: "aperture.physics.breakJoint.missingEntity",
        message: "physics_break_joint requires an entity reference with { index, generation }.",
        suggestedFix: "Run ecs_find_entities or ecs_snapshot, then pass the returned entity reference as { entity }.",
    });
    if (!resolved.ok) {
        return { ok: false, diagnostics: [resolved.diagnostic] };
            ...
            diagnostics: [
                {
                    ...
                    severity: "warning",
                    message: "physics_break_joint did not break a joint because the entity has no enabled PhysicsJoint component.",
                    suggestedFix: "Pass an active entity with an enabled PhysicsJoint component, or inspect it with ecs_get_entity first.",
                },
            ],
        };
    }
...
function callPhysicsRaycastTool(app, tool, payload) {
...
function callPhysicsOverlapShapeTool(app, payload) {
    ...
    return {
        ...
        result: {
            physics: app.context.physics.summary(),
            options: parsed.options ?? null,
            hits: app.context.physics.overlapShape(parsed.shape, parsed.transform, parsed.options),
        },
    };
}
function callPhysicsCastShapeFirstTool(app, payload) {
    ...
    return {
        ...
        result: {
            physics: app.context.physics.summary(),
            options: parsed.options ?? null,
            hit: app.context.physics.castShapeFirst(parsed.shape, parsed.cast, parsed.options),
        },
    };
}
function callPhysicsProjectPointTool(app, payload) {
    const record = isRecord(payload) ? payload : {};
            ...
            diagnostics: [
                {
                    ...
                    severity: "error",
                    message: "physics_project_point requires a finite point [x, y, z] tuple.",
                    suggestedFix: "Pass { point: [x, y, z] } and optional query filters.",
                },
            ],
        };
    }
    const options = physicsQueryOptionsFromPayload(app, isRecord(record["options"]) ? record["options"] : record);
    return {
        ok: true,
        result: {
            physics: app.context.physics.summary(),
...
function callPhysicsMoveCharacterTool(app, payload) {
            ...
            diagnostics: [
                {
                    ...
                    severity: "warning",
                    message: "physics_move_character could not move the requested entity because no backend character body/collider was found.",
                    suggestedFix: "Run ecs_step once to sync authored physics, then pass an active entity with a kinematic body and collider.",
                },
            ],
        };
    }
...
function callPhysicsDebugTool(app, tool, payload) {
    const parsed = physicsDebugOptionsPayload(payload);
    if (!parsed.ok) {
        return { ok: false, diagnostics: [parsed.diagnostic] };
    }
    const options = Object.keys(parsed.options).length === 0
...
function physicsDebugOptionsFromWorld(world) {
...
function physicsEntityRotation(entity) {
...
function physicsDebugOptionsPayload(payload) {
    const record = isRecord(payload) ? payload : {};
    const optionsRecord = isRecord(record["options"])
        ? record["options"]
        : record;
    const colliderWireframes = optionalBooleanFromRecord(optionsRecord, "colliderWireframes");
    const contactNormals = optionalBooleanFromRecord(optionsRecord, "contactNormals");
    const bodyStateMarkers = optionalBooleanFromRecord(optionsRecord, "bodyStateMarkers");
    const broadphaseAabbs = optionalBooleanFromRecord(optionsRecord, "broadphaseAabbs");
    const jointFrames = optionalBooleanFromRecord(optionsRecord, "jointFrames");
    const contactNormalLength = optionalNumberFromRecord(optionsRecord, "contactNormalLength");
    const bodyStateMarkerLength = optionalNumberFromRecord(optionsRecord, "bodyStateMarkerLength");
    const jointFrameLength = optionalNumberFromRecord(optionsRecord, "jointFrameLength");
    const contactNormalColor = optionalTuple4FromRecord(optionsRecord, "contactNormalColor");
    const activeBodyColor = optionalTuple4FromRecord(optionsRecord, "activeBodyColor");
    const sleepingBodyColor = optionalTuple4FromRecord(optionsRecord, "sleepingBodyColor");
    const broadphaseAabbColor = optionalTuple4FromRecord(optionsRecord, "broadphaseAabbColor");
    const jointFrameColor = optionalTuple4FromRecord(optionsRecord, "jointFrameColor");
    const jointAxisColor = optionalTuple4FromRecord(optionsRecord, "jointAxisColor");
    if (colliderWireframes === null ||
        contactNormals === null ||
        ...
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "physics_debug_geometry received malformed debug geometry options.",
                suggestedFix: "Use boolean debug flags, finite positive lengths, and RGBA color tuples such as { broadphaseAabbs: true, broadphaseAabbColor: [1, 0.65, 0.15, 1] }.",
            },
        };
    }
    const options = {
...
function physicsRaycastPayload(app, payload) {
    const record = isRecord(payload) ? payload : {};
    ...
    if (origin === null || direction === null) {
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "Physics raycast tools require finite origin and nonzero direction [x, y, z] tuples.",
                suggestedFix: "Pass { origin: [x, y, z], direction: [x, y, z] } or { ray: { origin, direction } }.",
            },
        };
    }
...
function physicsCharacterMovePayload(app, payload) {
    const record = isRecord(payload) ? payload : {};
    const entity = physicsCharacterEntityFromPayload(app, record);
    if (!entity.ok) {
        return entity;
    }
    const desiredTranslation = tuple3FromValue(record["desiredTranslation"] ?? record["translation"] ?? record["movement"]);
    if (desiredTranslation === null) {
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "physics_move_character requires finite desiredTranslation [x, y, z] values.",
                suggestedFix: "Pass { entity: { index, generation }, desiredTranslation: [x, y, z] } and optional settings/options.",
            },
        };
    }
    ...
    const ecsSettings = entity.entityObject === undefined
        ? undefined
        : physicsCharacterSettingsFromEntity(entity.entityObject);
    const moveSettings = mergePhysicsCharacterSettings(ecsSettings, settings.settings);
    const options = physicsQueryOptionsFromPayload(app, isRecord(record["options"]) ? record["options"] : record);
    const move = {
    ...
    if (diagnostics.length > 0) {
        return {
            ...
            diagnostic: {
                code: "aperture.physics.moveCharacter.invalidPayload",
                severity: "error",
                message: "physics_move_character received invalid character movement settings.",
                data: { diagnostics },
                suggestedFix: "Use finite movement vectors, positive controller dimensions, nonnegative snap distances, and a nonempty entity ref.",
            },
        };
    }
    return {
...
function physicsCharacterEntityFromPayload(app, record) {
    ...
    if (ref === null) {
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "physics_move_character requires an entity reference string or { index, generation } object.",
                suggestedFix: "Run ecs_find_entities or ecs_snapshot, then pass the returned entity reference as { entity }.",
            },
        };
    }
...
function physicsCharacterSettingsFromPayload(value) {
    if (value === undefined) {
        return { ok: true };
    }
    if (!isRecord(value)) {
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "physics_move_character settings must be an object when provided.",
                suggestedFix: "Pass settings such as { snapToGroundDistance, slide, autostep } or omit settings.",
            },
        };
    }
    const settings = {};
    const offset = optionalNumberFromRecord(value, "offset");
    const maxSlopeClimbAngle = optionalNumberFromRecord(value, "maxSlopeClimbAngle");
    const minSlopeSlideAngle = optionalNumberFromRecord(value, "minSlopeSlideAngle");
    const snapToGroundDistance = optionalNumberFromRecord(value, "snapToGroundDistance");
    let characterMass;
    ...
    if (value["characterMass"] === null) {
        ...
        const parsedCharacterMass = optionalNumberFromRecord(value, "characterMass");
    ...
    const applyImpulsesToDynamicBodies = optionalBooleanFromRecord(value, "applyImpulsesToDynamicBodies");
    if (offset === null ||
    ...
    if (value["autostep"] === false) {
        ...
        const maxHeight = autostep === null
            ? null
            : optionalNumberFromRecord(autostep, "maxHeight");
        const minWidth = autostep === null ? null : optionalNumberFromRecord(autostep, "minWidth");
        const includeDynamicBodies = autostep === null
            ? null
...
function physicsCharacterSettingsFromEntity(entity) {
    if (!entity.hasComponent(PhysicsCharacterController) ||
        entity.getValue(PhysicsCharacterController, "enabled") !== true) {
        return undefined;
    }
    const characterMassMode = entity.getValue(PhysicsCharacterController, "characterMassMode");
    const settings = {
        ...
        snapToGroundDistance: entity.getValue(PhysicsCharacterController, "snapToGroundDistance") ?? 0,
        autostep: entity.getValue(PhysicsCharacterController, "autostepEnabled") === true
            ? {
                maxHeight: entity.getValue(PhysicsCharacterController, "autostepMaxHeight") ?? 0.1,
                minWidth: entity.getValue(PhysicsCharacterController, "autostepMinWidth") ??
                    0.1,
                includeDynamicBodies: entity.getValue(PhysicsCharacterController, "autostepIncludeDynamicBodies") === true,
            }
            : false,
        applyImpulsesToDynamicBodies: entity.getValue(PhysicsCharacterController, "applyImpulsesToDynamicBodies") === true,
    };
    if (entity.getValue(PhysicsCharacterController, "maxSlopeClimbAngleEnabled") ===
        true) {
        settings.maxSlopeClimbAngle =
            entity.getValue(PhysicsCharacterController, "maxSlopeClimbAngle") ??
                Math.PI / 4;
    }
    if (entity.getValue(PhysicsCharacterController, "minSlopeSlideAngleEnabled") ===
        true) {
...
function mergePhysicsCharacterSettings(base, override) {
    if (base === undefined) {
        return override;
    }
...
function tuple3FromView(view) {
    return [view[0] ?? 0, view[1] ?? 0, view[2] ?? 0];
}
function optionalNumberFromRecord(record, field) {
    if (record[field] === undefined) {
        return undefined;
    }
    return numberFromValue(record[field]) ?? null;
}
function optionalBooleanFromRecord(record, field) {
    if (record[field] === undefined) {
        return undefined;
    }
    return booleanFromValue(record[field]) ?? null;
}
function optionalTuple3FromRecord(record, field) {
    if (record[field] === undefined) {
        return undefined;
    }
    return tuple3FromValue(record[field]) ?? null;
}
function optionalTuple4FromRecord(record, field) {
    if (record[field] === undefined) {
        return undefined;
    }
...
function invalidCharacterSettingsDiagnostic() {
    return {
        ...
        diagnostic: {
            code: "aperture.physics.moveCharacter.invalidSettings",
            severity: "error",
            message: "physics_move_character settings contain malformed controller values.",
            suggestedFix: "Use finite numeric settings, finite up vectors, boolean flags, and autostep { maxHeight, minWidth } or false.",
        },
    };
}
function physicsQueryOptionsFromPayload(app, record) {
    const collisionGroups = numberFromValue(record["collisionGroups"]);
    const includeSensors = booleanFromValue(record["includeSensors"]);
    const excludeEntity = physicsExcludeEntityFromPayload(app, record["excludeEntity"] ?? record["excludeEntityRef"]);
    const options = {
...
function physicsExcludeEntityFromPayload(app, value) {
...
function physicsShapeQueryPayload(app, payload) {
    const record = isRecord(payload) ? payload : {};
    ...
    if (shape === null || transform === null) {
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "physics_overlap_shape requires { shape, transform } with finite primitive shape and transform values.",
                suggestedFix: "Pass a shape such as { kind: 'sphere', radius } and transform { translation, rotation? }.",
            },
        };
    }
    const options = physicsQueryOptionsFromPayload(app, isRecord(record["options"]) ? record["options"] : record);
...
function physicsShapeCastPayload(app, payload) {
    const record = isRecord(payload) ? payload : {};
    ...
    if (shape === null || from === null || to === null) {
        return {
            ...
            diagnostic: {
                ...
                severity: "error",
                message: "physics_cast_shape_first requires { shape, from, to } with finite primitive shape and transform values.",
                suggestedFix: "Pass a shape plus from/to transforms, each with translation and optional rotation.",
            },
        };
    }
    const options = physicsQueryOptionsFromPayload(app, isRecord(record["options"]) ? record["options"] : record);
...
function physicsTransformFromPayload(value) {
    if (!isRecord(value)) {
        return null;
    }
...
function physicsShapeFromPayload(value) {
    if (!isRecord(value)) {
        return null;
    }
...
function physicsColliderAxisFromValue(value) {
