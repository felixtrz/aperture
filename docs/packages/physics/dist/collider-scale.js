import { PhysicsColliderAxis } from "./components.js";
const SCALE_EPSILON = 0.000001;
function approxEqual(a, b) {
    return (Math.abs(a - b) <= SCALE_EPSILON * Math.max(1, Math.abs(a), Math.abs(b)));
}
export function isNonUnitScale(scale) {
    return (!approxEqual(scale[0], 1) ||
 ...
 */
function axisScaleComponents(scale, axis) {
 ...
 */
export function scaleColliderShape(shape, scale) {
    const sx = Math.abs(scale[0]);
    const sy = Math.abs(scale[1]);
    const sz = Math.abs(scale[2]);
    switch (shape.kind) {
        ...
        case "cylinder":
        case "cone": {
            const { radial, axial } = axisScaleComponents(scale, shape.axis ?? PhysicsColliderAxis.Y);
            return {
                ...shape,
 ...
 */
export function isColliderScaleApproximated(shape, scale) {
    const sx = Math.abs(scale[0]);
    const sy = Math.abs(scale[1]);
    const sz = Math.abs(scale[2]);
    switch (shape.kind) {
        ...
        case "cylinder":
        case "cone": {
            const { radial } = axisScaleComponents(scale, shape.axis ?? PhysicsColliderAxis.Y);
...
//# sourceMappingURL=collider-scale.js.map