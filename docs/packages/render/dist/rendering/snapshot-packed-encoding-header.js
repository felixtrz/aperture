import { SNAPSHOT_PACKET_ENCODING_MAGIC, SNAPSHOT_PACKET_ENCODING_VERSION, SNAPSHOT_PACKET_HEADER_WORDS, SNAPSHOT_PACKET_HEADER_WORD_INDEX, } from "./snapshot-packed-encoding-constants.js";
export function writeSnapshotPacketHeader(words, counts) {
...
export function readSnapshotPacketHeaderCounts(words) {
    ...
    return {
        ...
        shadowCasterDraws: words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.ShadowCasterDraws] ?? 0,
        ...
        particleEmitters: words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.ParticleEmitters] ?? 0,
        audioEmitters: words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.AudioEmitters] ?? 0,
        audioListeners: words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.AudioListeners] ?? 0,
        shadowRequests: words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.ShadowRequests] ?? 0,
...
function assertSnapshotPacketHeader(words) {
    if (words.length < SNAPSHOT_PACKET_HEADER_WORDS) {
        throw new RangeError("Snapshot packet buffer is missing its header.");
    }
    if (words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.Magic] !==
        SNAPSHOT_PACKET_ENCODING_MAGIC) {
        throw new RangeError("Snapshot packet buffer has an unsupported magic.");
    }
    if (words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.Version] !==
        SNAPSHOT_PACKET_ENCODING_VERSION) {
        throw new RangeError(`Snapshot packet buffer version ${words[SNAPSHOT_PACKET_HEADER_WORD_INDEX.Version]} is unsupported.`);
    }
}
//# sourceMappingURL=snapshot-packed-encoding-header.js.map