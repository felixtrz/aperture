import { createVertexDataArray, required, requiredElementCount, setVertexData, } from "./mesh-merge-utils.js";
export function mergeVertexStreams(firstMesh, sources) {
    return firstMesh.vertexStreams.map((firstStream, streamIndex) => {
        const totalElements = sources.reduce((sum, source) => sum +
            requiredElementCount(required(source.mesh.vertexStreams[streamIndex])), 0);
        ...
        for (const source of sources) {
            ...
            setVertexData(data, sourceStream.data.subarray(0, sourceElementCount), elementOffset);
...
export function mergeIndexBuffer(sources, diagnostics) {
    const totalVertexCount = sources.reduce((sum, source) => sum + source.vertexCount, 0);
    const totalIndexCount = sources.reduce((sum, source) => sum + (source.mesh.indexBuffer?.data.length ?? 0), 0);
    const format = totalVertexCount > 0xffff ||
        sources.some((source) => source.mesh.indexBuffer?.format === "uint32")
        ? "uint32"
        : "uint16";
    const data = format === "uint32"
...
export function mergeSubmeshes(sources, indexed, ranges) {
    const submeshes = [];
    for (const source of sources) {
        for (let sourceSubmeshIndex = 0; sourceSubmeshIndex < source.mesh.submeshes.length; sourceSubmeshIndex += 1) {
...
//# sourceMappingURL=mesh-merge-assembly.js.map