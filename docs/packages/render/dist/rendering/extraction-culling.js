export function isVisibleInAnyMatchingView(worldAabb, layerMask, viewCullContexts) {
...
export function firstMatchingSortView(layerMask, viewCullContexts) {
    for (const context of viewCullContexts) {
        if ((context.layerMask & layerMask) !== 0) {
            return context;
        }
...
export function computeViewDepth(viewMatrix, worldCenter) {
    const x = worldCenter[0] ?? 0;
    const y = worldCenter[1] ?? 0;
    const z = worldCenter[2] ?? 0;
    return -(x * (viewMatrix[2] ?? 0) +
        y * (viewMatrix[6] ?? 0) +
        z * (viewMatrix[10] ?? 0) +
        (viewMatrix[14] ?? 0));
}
function aabbIntersectsFrustum(aabb, planes) {
...
export function createFrustumPlanes(matrix) {
...
function readMat4(matrix, index) {
...
export function createViewCullSignature(viewCullContexts) {
...
function hashCullNumber(hash, value) {
...
//# sourceMappingURL=extraction-culling.js.map