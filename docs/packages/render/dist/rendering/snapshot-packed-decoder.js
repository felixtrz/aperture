import { readAudioEmitterPacket, readAudioListenerPacket, readBoundsPacket, readEnvironmentPacket, readFogPacket, readLightPacket, readMeshDrawPacket, readParticleEmitterPacket, readQuadBatchPacket, readShadowRequestPacket, readViewPacket, } from "./snapshot-packed-codecs.js";
...
export function decodeSnapshotPackets(words, registry) {
    const counts = readSnapshotPacketHeaderCounts(words);
    const expectedWords = SNAPSHOT_PACKET_HEADER_WORDS +
    ...
    if (words.length < expectedWords) {
        throw new RangeError(`Snapshot packet buffer is truncated: requires ${expectedWords} words, received ${words.length}.`);
    }
    const views = [];
    const meshDraws = [];
...
//# sourceMappingURL=snapshot-packed-decoder.js.map