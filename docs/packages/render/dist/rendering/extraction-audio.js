import { Enabled, WorldTransform, } from "@aperture-engine/simulation";
import { AudioEmitter, AudioListener, AudioSimulationSpace, Camera, RenderLayer, } from "./authoring.js";
 ...
 */
export function extractAudioEmitters(world, assets, transforms, diagnostics) {
    ...
    for (const entity of sortedEntities(query.entities)) {
        ...
        const clip = parseAudioClipHandle(entity.getValue(AudioEmitter, "clipId") ?? "");
        if (clip === null) {
            diagnostics.push(diagnostic("render.audio.invalidClip", entity));
            continue;
        }
        const entry = assets.get(clip);
        if (entry === undefined) {
            diagnostics.push(diagnostic("render.audio.clipMissing", entity, clip));
            continue;
        }
        if (entry.status !== "ready" || entry.asset === null) {
            diagnostics.push(diagnostic(`render.audio.clip.${entry.status}`, entity, clip));
            continue;
        }
        const layerMask = entity.hasComponent(RenderLayer)
            ? (entity.getValue(RenderLayer, "mask") ?? 1)
            : 1;
        const worldTransformOffset = pushMatrix(transforms, readWorldMatrix(entity));
        const simulationSpace = entity.getValue(AudioEmitter, "simulationSpace") ===
        ...
        const refDistance = finitePositive(entity.getValue(AudioEmitter, "refDistance"), 1);
        const maxDistanceRaw = finitePositive(entity.getValue(AudioEmitter, "maxDistance"), 10000);
        const maxDistance = maxDistanceRaw > refDistance ? maxDistanceRaw : refDistance + 1;
        packets.push({
            ...
            timeScale: finiteNonNegative(entity.getValue(AudioEmitter, "timeScale"), 1),
            priority: finiteInteger(entity.getValue(AudioEmitter, "priority"), 0),
            panningModel: entity.getValue(AudioEmitter, "panningModel") === "HRTF"
                ? "HRTF"
                : "equalpower",
            simulationSpace,
            distanceModel: parseDistanceModel(entity.getValue(AudioEmitter, "distanceModel")),
            refDistance,
            maxDistance,
            rolloffFactor: finiteNonNegative(entity.getValue(AudioEmitter, "rolloffFactor"), 1),
            coneInnerAngle: finiteNumber(entity.getValue(AudioEmitter, "coneInnerAngle"), 360),
            coneOuterAngle: finiteNumber(entity.getValue(AudioEmitter, "coneOuterAngle"), 360),
            coneOuterGain: finiteNumber(entity.getValue(AudioEmitter, "coneOuterGain"), 0),
            offsetSec: finiteNonNegative(entity.getValue(AudioEmitter, "offsetSec"), 0),
            loopStart: finiteNonNegative(entity.getValue(AudioEmitter, "loopStart"), 0),
            loopEnd: finiteNonNegative(entity.getValue(AudioEmitter, "loopEnd"), 0),
            seed: finiteInteger(entity.getValue(AudioEmitter, "seed"), 1),
            boundsCenter: readVec3(entity.getVectorView(AudioEmitter, "boundsCenter")),
            audibilityRadius: finiteNonNegative(entity.getValue(AudioEmitter, "audibilityRadius"), 0),
            occlusion: clamp01(finiteNumber(entity.getValue(AudioEmitter, "occlusion"), 0)),
            lowpassFrequency: finitePositive(entity.getValue(AudioEmitter, "lowpassFrequency"), 22000),
 ...
 */
export function extractAudioListener(world, transforms, diagnostics, hasEmitters) {
    ...
    for (const entity of sortedEntities(query.entities)) {
        ...
        chosen = {
            ...
            masterGain: finiteNonNegative(entity.getValue(AudioListener, "masterGain"), 1),
        };
    }
...
function highestPriorityCamera(world) {
...
function parseDistanceModel(value) {
...
//# sourceMappingURL=extraction-audio.js.map