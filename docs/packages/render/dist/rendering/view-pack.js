...
export { PACKED_VIEW_UNIFORM_FLOAT_STRIDE, VIEW_CAMERA_POSITION_FLOAT_OFFSET, VIEW_FOG_COLOR_FLOAT_OFFSET, VIEW_FOG_PARAMS_FLOAT_OFFSET, VIEW_PREVIOUS_VIEW_PROJECTION_FLOAT_OFFSET, VIEW_PROJECTION_FLOAT_COUNT, } from "./view-pack-types.js";
export function packSnapshotViewUniforms(snapshot, options = {}) {
    const diagnostics = [];
    const views = [];
    const seen = new Set();
    const validViews = [];
    ...
    const data = new Float32Array(validViews.length * PACKED_VIEW_UNIFORM_FLOAT_STRIDE);
    validViews.forEach((view, index) => {
        const sourceOffset = view.viewProjectionMatrixOffset;
        const packedOffset = index * PACKED_VIEW_UNIFORM_FLOAT_STRIDE;
        data.set(snapshot.viewMatrices.subarray(sourceOffset, sourceOffset + VIEW_PROJECTION_FLOAT_COUNT), packedOffset);
        writeCameraPosition(data, packedOffset + VIEW_CAMERA_POSITION_FLOAT_OFFSET, snapshot.viewMatrices, view.viewMatrixOffset);
        writePreviousViewProjection(data, packedOffset + VIEW_PREVIOUS_VIEW_PROJECTION_FLOAT_OFFSET, snapshot.viewMatrices, view, options.previousViewProjectionByViewId);
        writeFogParameters(data, packedOffset + VIEW_FOG_COLOR_FLOAT_OFFSET, packedOffset + VIEW_FOG_PARAMS_FLOAT_OFFSET, snapshot.fogs ?? [], view);
        views.push({
            viewId: view.viewId,
            sourceOffset,
            packedOffset,
...
//# sourceMappingURL=view-pack.js.map