...
export async function decodeImageBytesWithBrowserCanvas(input) {
    ...
    if (typeof Blob === "undefined" || typeof createImageBitmap !== "function") {
        throw new Error("No browser image decoder is available; provide decodeImageData for this glTF image source.");
    }
    ...
    try {
        ...
        return {
            ...
            sourceData: {
                bytes: new Uint8Array(pixels.data.buffer, pixels.data.byteOffset, pixels.data.byteLength),
...
function blobPartFromBytes(bytes) {
...
function createImageDecodeCanvas(width, height) {
    ...
    throw new Error("No browser canvas implementation is available for image decode.");
}
//# sourceMappingURL=gltf-texture-browser-decoder.js.map