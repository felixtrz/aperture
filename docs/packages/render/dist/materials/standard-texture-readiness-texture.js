import { SUPPORTED_STANDARD_TEXCOORDS, isSupportedStandardTexCoord, } from "./standard-texture-readiness-expectations.js";
...
export function inspectReadyStandardTexture(input) {
    ...
    const colorSpaceReady = input.expectation.colorSpaces.includes(input.texture.colorSpace);
    ...
    if (!colorSpaceReady) {
        input.diagnostics.push({
            ...
            expectedColorSpaces: input.expectation.colorSpaces,
            actualColorSpace: input.texture.colorSpace,
            message: `StandardMaterial ${input.expectation.field} texture '${input.textureKey}' should use color space '${input.expectation.colorSpaces.join("' or '")}', not '${input.texture.colorSpace}'.`,
        });
    }
    if (!formatReady) {
...
//# sourceMappingURL=standard-texture-readiness-texture.js.map