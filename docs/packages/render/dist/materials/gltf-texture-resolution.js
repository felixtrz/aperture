import { isDecodedImageData, isImageDataResolverReport, isPromiseLike, validDecodedImage, } from "./gltf-texture-utils.js";
export function resolveDecodedImage(input) {
    ...
    if (isPromiseLike(resolved)) {
        input.diagnostics.push({
            ...
            slot: input.options.slot,
            imageIndex: input.imageIndex,
            message: "Image data resolver returned a Promise; use createTextureAssetFromGltfTextureAsync() for async glTF texture mapping.",
        });
        return null;
    }
...
function normalizeAndValidateDecodedImage(input) {
    ...
    if (!validDecodedImage(image)) {
        input.diagnostics.push({
            ...
            slot: input.options.slot,
            imageIndex: input.imageIndex,
            message: "Decoded image data must include dimensions, row stride, and bytes.",
        });
        return null;
    }
    return image;
...
//# sourceMappingURL=gltf-texture-resolution.js.map