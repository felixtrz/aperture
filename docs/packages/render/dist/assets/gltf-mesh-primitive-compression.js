import { integerField, isRecord, toDiagnosticValue, } from "./gltf-mesh-primitive-utils.js";
export function inspectUnsupportedCompression(input) {
    const extensions = input.primitive.extensions;
    if (!isRecord(extensions)) {
        return;
    }
    const supported = new Set(input.options.supportedCompressedPrimitiveExtensions ?? []);
...
export function mapCompressedPrimitive(input, attributes) {
    const supported = new Set(input.options.supportedCompressedPrimitiveExtensions ?? []);
    ...
    if (compressedAttributes.some((attribute) => attribute.semantic === "POSITION")) {
        return {
            extensionName: "KHR_draco_mesh_compression",
            bufferView,
...
function flattenAttributeReferences(attributes) {
    return [
        attributes.position,
        attributes.normal,
...
//# sourceMappingURL=gltf-mesh-primitive-compression.js.map