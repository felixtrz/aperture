import { bytesView, decodedMeshoptBufferView, isRecord, meshoptExtensionForBufferView, meshoptExtensionNameForBufferView, } from "./gltf-report-driven-import-meshopt-extension.js";
export function decodeGltfMeshoptBufferViews(input) {
    ...
    const compressedBufferViewIndexes = input.root.bufferViews
        .map((bufferView, bufferViewIndex) => meshoptExtensionNameForBufferView(bufferView) === null
        ? null
        : bufferViewIndex)
        .filter((bufferViewIndex) => bufferViewIndex !== null);
    ...
    const transformedBuffers = buffers.map((buffer) => isRecord(buffer) ? { ...buffer } : buffer);
    const transformedBufferViews = input.root.bufferViews.map((bufferView) => isRecord(bufferView) ? { ...bufferView } : bufferView);
    ...
    for (const bufferViewIndex of compressedBufferViewIndexes) {
        ...
        try {
            const decoded = input.decoder.decodeGltfBuffer(sourceBytes.subarray(extension.byteOffset, extension.byteOffset + extension.byteLength), {
            ...
            transformedBufferViews[bufferViewIndex] = decodedMeshoptBufferView(bufferView, decodedBufferIndex, decoded.byteLength);
        }
        catch (error) {
            diagnostics.push({
    ...
    return {
        ...
        resolveBufferBytes: (bufferIndex) => decodedBuffers.get(bufferIndex) ?? input.resolveBufferBytes(bufferIndex),
        diagnostics,
    };
}
//# sourceMappingURL=gltf-report-driven-import-meshopt.js.map