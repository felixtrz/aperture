import { KTX2_SUPERCOMPRESSION_BASIS_LZ, KTX2_SUPERCOMPRESSION_NONE, KTX2_VK_FORMAT_UNDEFINED, VK_FORMAT_R8G8B8A8_SRGB, VK_FORMAT_R8G8B8A8_UNORM, } from "./ktx2-constants.js";
import { parseKtx2Container } from "./ktx2-container.js";
import { bytesView, featureSetHas } from "./ktx2-utils.js";
export function decodeKtx2TextureData(source) {
    ...
    if (container.supercompressionScheme !== KTX2_SUPERCOMPRESSION_NONE) {
        throw new Error("Only uncompressed KTX2 textures are supported by the built-in decoder; BasisU supercompression requires a transcoder.");
    }
    ...
    const levels = container.levels.map((level, levelIndex) => {
        ...
        if (level.byteLength < expectedByteLength) {
            throw new Error(`KTX2 level ${levelIndex} payload is smaller than expected.`);
        }
        return {
            bytes: new Uint8Array(bytes.subarray(level.byteOffset, level.byteOffset + expectedByteLength)),
            bytesPerRow: width * 4,
            rowsPerImage: height,
            width,
            height,
...
export async function decodeKtx2TextureDataAsync(source, options = {}) {
    ...
    if (container.supercompressionScheme !== KTX2_SUPERCOMPRESSION_BASIS_LZ ||
        ...
        throw new Error(`Unsupported KTX2 supercompression scheme ${container.supercompressionScheme}.`);
    }
    if (options.basisTranscoder === undefined) {
        throw new Error("BasisU-compressed KTX2 textures require a configured Basis Universal transcoder.");
    }
    return options.basisTranscoder.decode(source, options);
}
export function createKtx2TextureCompressionSupportFromFeatures(features) {
    return {
...
function textureFormatFromVkFormat(vkFormat) {
    switch (vkFormat) {
...
//# sourceMappingURL=ktx2-decoder.js.map