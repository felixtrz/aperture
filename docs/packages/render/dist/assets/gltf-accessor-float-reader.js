...
function readComponentAsFloat(view, byteOffset, componentType, normalized) {
    switch (componentType) {
        case GLTF_COMPONENT_FLOAT:
...
export function decodeGltfFloatAccessor(input) {
    ...
    const componentCount = accessorType === null
        ? undefined
        : ACCESSOR_TYPE_COMPONENTS.get(accessorType);
    const componentType = typeof accessor.componentType === "number" ? accessor.componentType : null;
    ...
    const accessorByteOffset = typeof accessor.byteOffset === "number" ? accessor.byteOffset : 0;
    const normalized = accessor.normalized === true;
    const bufferViews = Array.isArray(root.bufferViews) ? root.bufferViews : [];
    const bufferViewIndex = typeof accessor.bufferView === "number" ? accessor.bufferView : null;
    ...
    if (bufferViewIndex !== null) {
        ...
        const bufferIndex = typeof bufferView.buffer === "number" ? bufferView.buffer : null;
        if (bufferIndex === null) {
            return null;
        }
        ...
        const bufferViewByteOffset = typeof bufferView.byteOffset === "number" ? bufferView.byteOffset : 0;
        const byteStride = typeof bufferView.byteStride === "number" && bufferView.byteStride > 0
        ...
        for (let element = 0; element < count; element += 1) {
            ...
            for (let component = 0; component < componentCount; component += 1) {
                ...
                values[element * componentCount + component] = readComponentAsFloat(view, byteOffset, componentType, normalized);
            }
        }
    }
...
function resolveBufferViewDataView(bufferViews, bufferViewIndex, resolveBufferBytes) {
...
function applyGltfSparseOverride(input) {
    ...
    const indexComponentType = typeof indices.componentType === "number" ? indices.componentType : null;
    if (indexComponentType !== GLTF_COMPONENT_UNSIGNED_BYTE &&
    ...
    const indexBytes = componentByteSize(indexComponentType);
    const indexData = resolveBufferViewDataView(input.bufferViews, indices.bufferView, input.resolveBufferBytes);
    const valueData = resolveBufferViewDataView(input.bufferViews, sparseValues.bufferView, input.resolveBufferBytes);
    if (indexData === null || valueData === null) {
        return false;
    }
    const indexByteOffset = typeof indices.byteOffset === "number" ? indices.byteOffset : 0;
    const valueByteOffset = typeof sparseValues.byteOffset === "number" ? sparseValues.byteOffset : 0;
    for (let entry = 0; entry < sparseCount; entry += 1) {
        ...
        const targetIndex = readComponentAsFloat(indexData.view, indexOffset, indexComponentType, false);
        ...
        for (let component = 0; component < input.componentCount; component += 1) {
            const valueOffset = valueData.base +
                valueByteOffset +
            ...
            input.values[targetIndex * input.componentCount + component] =
                readComponentAsFloat(valueData.view, valueOffset, input.componentType, input.normalized);
        }
    }
    return true;
}
//# sourceMappingURL=gltf-accessor-float-reader.js.map