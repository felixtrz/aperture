...
export { createMergedImageDataResolver, emptyDecodedImages, mergeDecodedImageData, normalizeConcurrency, } from "./glb-uri-image-merge.js";
export async function decodeExternalImages(input) {
    ...
    const results = await mapWithConcurrency(images.map((image, imageIndex) => ({ image, imageIndex })), input.imageDecodeConcurrency, ({ image, imageIndex }) => decodeGlbExternalImage({
        image,
        imageIndex,
        root: input.root,
        binary: input.binary,
...
//# sourceMappingURL=glb-uri-images.js.map