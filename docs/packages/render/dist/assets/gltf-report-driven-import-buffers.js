export function resolvedExternalBufferByteLengths(options) {
...
export function resolveGlbBufferBytes(bufferIndex, root, binaryChunk, resolveExternalBufferBytes, resolvedBuffers) {
    ...
    const resolved = (isExternalBuffer
...
export function createGlbBufferSourceDiagnostics(root, binaryChunk, resolveBufferBytes) {
    ...
    buffers.forEach((buffer, bufferIndex) => {
        ...
        if (bufferIndex === 0 && binaryChunk === null) {
            diagnostics.push({
                ...
                severity: "error",
                bufferIndex,
                message: "GLB buffer 0 requires bytes, but the container has no BIN chunk.",
            });
        }
    });
    return diagnostics;
...
//# sourceMappingURL=gltf-report-driven-import-buffers.js.map