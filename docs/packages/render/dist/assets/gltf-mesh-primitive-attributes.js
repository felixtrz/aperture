...
export function mapGltfMeshPrimitiveAttributes(input) {
    ...
    const position = mapGltfMeshPrimitiveAttributeReference(input, attributes, "POSITION");
    if (position === null) {
        return null;
    }
    const normal = mapGltfMeshPrimitiveAttributeReference(input, attributes, "NORMAL");
    const texcoord0 = mapGltfMeshPrimitiveAttributeReference(input, attributes, "TEXCOORD_0");
    const texcoord1 = mapGltfMeshPrimitiveAttributeReference(input, attributes, "TEXCOORD_1");
    const tangent = mapGltfMeshPrimitiveAttributeReference(input, attributes, "TANGENT");
    const color0 = mapGltfMeshPrimitiveAttributeReference(input, attributes, "COLOR_0");
    const joints0 = mapGltfMeshPrimitiveAttributeReference(input, attributes, "JOINTS_0");
    const weights0 = mapGltfMeshPrimitiveAttributeReference(input, attributes, "WEIGHTS_0");
    const morphTargets = mapGltfMeshPrimitiveMorphTargetAttributeReferences(input);
    const hasOptionalAttributeError = input.diagnostics.some((diagnostic) => diagnostic.severity === "error" &&
...
//# sourceMappingURL=gltf-mesh-primitive-attributes.js.map