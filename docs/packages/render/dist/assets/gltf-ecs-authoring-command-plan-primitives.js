...
import { appendGltfEcsEntityCommands, gltfIdentityLocalTransformCommandValue, } from "./gltf-ecs-authoring-command-plan-entities.js";
import { gltfEcsMeshReadinessStatus, } from "./gltf-ecs-authoring-command-plan-primitive-readiness.js";
import { gltfEcsPrimitiveEntityKey, skipGltfEcsMeshNotReady, skipGltfEcsMissingPrimitiveMaterialResolution, skipGltfEcsUnresolvedPrimitiveMaterial, } from "./gltf-ecs-authoring-command-plan-primitive-skips.js";
export { createGltfEcsMeshReadiness, } from "./gltf-ecs-authoring-command-plan-primitive-readiness.js";
export function appendGltfEcsPrimitiveCommands(input) {
    if (input.node.meshIndex === null ||
        input.options.primitiveMaterialReport === undefined ||
        input.options.meshRegistrationReport === undefined) {
        return;
    }
    const unresolved = input.options.primitiveMaterialReport.unresolved.filter((entry) => entry.meshIndex === input.node.meshIndex);
    const resolved = input.options.primitiveMaterialReport.resolved.filter((entry) => entry.meshIndex === input.node.meshIndex);
...
function gltfSkinCommandValue(input) {
    ...
    const skeletonEntityKey = skin.skeletonNodeIndex === null
        ? null
...
function assetIdFromHandleKey(handleKey, kind) {
    const prefix = `${kind}:`;
    return handleKey.startsWith(prefix)
        ? handleKey.slice(prefix.length)
        : handleKey;
}
//# sourceMappingURL=gltf-ecs-authoring-command-plan-primitives.js.map