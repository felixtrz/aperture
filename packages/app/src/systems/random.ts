...
export interface ApertureRandom {
  ...
  /** Export implementation state for deterministic restore. */
  snapshot?(): ApertureRandomState;
  /** Restore implementation state captured by `snapshot()`. */
  restore?(state: ApertureRandomState): void;
  /** Next float in [0, 1). */
  next(): number;
...
export interface ApertureRandomState {
  readonly seed: number;
  readonly state: number;
}

export function createApertureRandom(
  seedOrState: number | ApertureRandomState = 0,
): ApertureRandom {
  // mulberry32
  const seed = typeof seedOrState === "number" ? seedOrState : seedOrState.seed;
  let state =
    typeof seedOrState === "number" ? seed >>> 0 : seedOrState.state >>> 0;
  ...
  return {
    ...
    restore(value: ApertureRandomState): void {
      state = value.state >>> 0;
    },
    next,
    int(maxExclusive: number): number {
...
export function snapshotApertureRandom(
  random: ApertureRandom,
): ApertureRandomState | null {
  return random.snapshot?.() ?? null;
}

export function restoreApertureRandom(
  random: ApertureRandom,
  state: ApertureRandomState,
): boolean {
  if (random.restore === undefined) {
    return false;
  }
