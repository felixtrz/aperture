...
import {
  ...
  type MeshAsset,
  type MeshSpatialAdapterDiagnostic,
} from "@aperture-engine/render";
import {
  createPhysicsTriangleMeshGeometryFromSpatialMesh,
  validatePhysicsHeightfieldGeometry,
...
export interface AssetBackedPhysicsColliderGeometryProviderOptions {
...
export function createAssetBackedPhysicsColliderGeometryProvider(
  options: AssetBackedPhysicsColliderGeometryProviderOptions,
  ...
  return {
    triangleMesh(meshId) {
      ...
      const entry = options.assets.get<"mesh", MeshAsset>(handle);
      ...
      if (entry === undefined) {
        return {
          ...
          error: assetGeometryError({
            ...
            suggestedFix:
              "Register the mesh asset in the app AssetRegistry before stepping physics.",
          }),
        };
      }
      ...
      if (entry.status !== "ready") {
        return {
          ...
          error: assetGeometryError({
            ...
            suggestedFix:
              "Wait until the mesh source asset is ready before enabling the asset-backed collider.",
      ...
      if (!isMeshAsset(entry.asset)) {
        return {
          ...
          error: assetGeometryError({
            ...
            suggestedFix:
              "Store a render MeshAsset under the mesh handle used by Collider.meshId.",
      ...
      if (spatial.mesh === null) {
        const result: PhysicsColliderGeometryResult<PhysicsTriangleMeshGeometry> =
          {
            ok: false,
            error: assetGeometryError({
              code: "physics.collider.asset.invalid",
              ...
              suggestedFix:
                "Use triangle-list mesh data with float32x3 POSITION and uint16/uint32 indices.",
              details: {
      ...
      if (heightfield === null) {
        return {
          ...
          error: assetGeometryError({
            ...
            suggestedFix:
              "Add a finite PhysicsHeightfieldGeometry entry to the provider heightfields map before stepping physics.",
          }),
        };
      }
...
function createMeshHandleFromColliderMeshId(meshId: string) {
  return createMeshHandle(
    meshId.startsWith("mesh:") ? meshId.slice(5) : meshId,
  );
}

function readHeightfield(
  heightfields:
    | ReadonlyMap<string, PhysicsHeightfieldGeometry>
    | Readonly<Record<string, PhysicsHeightfieldGeometry>>
    | undefined,
  assetId: string,
): PhysicsHeightfieldGeometry | null {
...
function isHeightfieldMap(
  value:
    | ReadonlyMap<string, PhysicsHeightfieldGeometry>
    | Readonly<Record<string, PhysicsHeightfieldGeometry>>,
): value is ReadonlyMap<string, PhysicsHeightfieldGeometry> {
...
function isMeshAsset(value: unknown): value is MeshAsset {
  ...
  const asset = value as Partial<MeshAsset>;
...
function assetGeometryError(input: {
  readonly code: PhysicsColliderGeometryError["code"];
  readonly feature: string;
  readonly assetId: string;
  readonly message: string;
  readonly suggestedFix: string;
  readonly details?: Readonly<Record<string, unknown>>;
}): PhysicsColliderGeometryError {
  return {
    code: input.code,
...
function diagnosticsToDetails(
  diagnostics: readonly MeshSpatialAdapterDiagnostic[],
