...
} from "./gltf-animation-import-report.js";
...
const TRS_COMPONENT_COUNTS: Record<string, number> = {
  translation: 3,
  rotation: 4,
  scale: 3,
};

export interface GltfImportedAnimationClip {
  readonly animationIndex: number;
  readonly clip: AnimationClip;
}

export interface GltfAnimationImportResult {
  readonly clips: readonly GltfImportedAnimationClip[];
  readonly report: GltfAnimationImportReport;
}
...
function asChannelPath(value: unknown): AnimationChannelPath | null {
...
function asInterpolation(value: unknown): AnimationInterpolation | null {
...
export function importGltfAnimations(input: {
  ...
  const clips: GltfImportedAnimationClip[] = [];
  ...
  root.animations.forEach((rawAnimation, animationIndex) => {
    ...
    const channels = Array.isArray(rawAnimation.channels)
      ...
      : [];
    const parsedChannels: AnimationKeyframeChannel[] = [];
    ...
    const name =
      typeof rawAnimation.name === "string" && rawAnimation.name.length > 0
        ? rawAnimation.name
...
function parseChannel(input: {
  ...
  readonly rawChannel: Record<string, unknown>;
  readonly animationIndex: number;
  readonly channelIndex: number;
  readonly diagnostics: GltfAnimationImportDiagnostic[];
}): AnimationKeyframeChannel | null {
  const target = input.rawChannel.target as Record<string, unknown>;
  ...
  const sampler =
    samplerIndex === null ? undefined : input.samplers[samplerIndex];
