...
const TYPED_ARRAY_CONSTRUCTORS: Readonly<
  Record<string, TypedArrayConstructor>
> = {
  Float32Array,
  Float64Array,
  Int8Array,
...
export function decodeTypedArrayTree(value: unknown): unknown {
  if (hasTypedArrayTag(value)) {
    return decodeTypedArray(readTypedArrayJson(value));
  }
...
function decodeTypedArray(value: TypedArrayJson): TypedArray {
  ...
  const expectedByteLength = value.length * Constructor.BYTES_PER_ELEMENT;

  if (bytes.byteLength !== expectedByteLength) {
    throw new RangeError(
      `Typed array '${value[TYPED_ARRAY_TAG]}' length ${value.length} requires ${expectedByteLength} byte(s), but snapshot JSON contained ${bytes.byteLength}.`,
    );
  }
...
function hasTypedArrayTag(value: unknown): value is Record<string, unknown> {
  return isPlainRecord(value) && TYPED_ARRAY_TAG in value;
}

function readTypedArrayJson(value: Record<string, unknown>): TypedArrayJson {
  const typedArray = value[TYPED_ARRAY_TAG];
  const base64 = value["base64"];
  const length = value["length"];

  if (
    typeof typedArray !== "string" ||
    ...
    throw new TypeError(
      "Malformed typed-array JSON node: expected string $typedArray, string base64, and non-negative integer length.",
    );
  }

  return {
    [TYPED_ARRAY_TAG]: typedArray,
    base64,
    length,
  };
}
...
function bytesToBase64(bytes: Uint8Array): string {
  ...
  for (let index = 0; index < bytes.length; index += 3) {
    ...
    chars.push(
      ...
      hasByte1
        ? (BASE64_ALPHABET[((byte1 & 0b1111) << 2) | (byte2 >> 6)] ?? "A")
        : "=",
...
function base64ToBytes(base64: string): Uint8Array {
  if (!isCanonicalBase64(base64)) {
    throw new RangeError("Invalid base64 payload in snapshot JSON.");
  }
  ...
  for (const char of clean) {
    ...
    if (sextet === undefined) {
      throw new RangeError(
        `Invalid base64 character '${char}' in snapshot JSON.`,
      );
    }
