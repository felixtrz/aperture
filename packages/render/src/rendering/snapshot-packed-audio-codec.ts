import type { AudioEmitterPacket, AudioListenerPacket } from "./snapshot.js";
import {
  audioAudibilityId,
  audioAudibilityValue,
  audioDistanceModelId,
  audioDistanceModelValue,
  audioPanningModelId,
  audioPanningModelValue,
  audioSimulationSpaceId,
  audioSimulationSpaceValue,
  audioVoiceKeyKindId,
  audioVoiceKeyKindValue,
  readEntity,
  readFloat64,
  readRequiredHandle,
  readSigned32,
...
export function writeAudioEmitterPacket(
  words: Uint32Array,
  offset: number,
...
export function readAudioEmitterPacket(
  words: Uint32Array,
  offset: number,
  registry: SnapshotPacketEncodingRegistry,
): AudioEmitterPacket {
  ...
  const packet: AudioEmitterPacket = {
    key:
      keyKind === "entity"
...
export function writeAudioListenerPacket(
  words: Uint32Array,
  offset: number,
  packet: AudioListenerPacket,
): void {
...
export function readAudioListenerPacket(
  words: Uint32Array,
  offset: number,
): AudioListenerPacket {
  return {
...
function audioEmitterFlags(packet: AudioEmitterPacket): number {
  let flags = 0;

  if (packet.loop) flags |= AUDIO_FLAG_LOOP;
  if (packet.autoplay) flags |= AUDIO_FLAG_AUTOPLAY;
  if (packet.muted) flags |= AUDIO_FLAG_MUTED;
  if (packet.seed !== undefined) flags |= AUDIO_FLAG_HAS_SEED;
  if (packet.boundsCenter !== undefined) flags |= AUDIO_FLAG_HAS_BOUNDS_CENTER;
