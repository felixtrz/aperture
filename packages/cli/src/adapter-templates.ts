...
- Scene recipe (what the starter systems already do). The \`minimal\` starter
  keeps the default daylight environment and adds one shadow-casting
  directional sun (\`spawn.light({ kind: "directional", shadow: {...} })\`).
  The \`game\` and \`glb-viewer\` starters instead call
  \`spawn.lightRig({ preset: "studio-neutral", environmentMap })\`, which
  authors its own environment light plus key/rim lights — that deliberately
  suppresses the default daylight rig, so do not add one back. Either way:
