...
async function serveRequest(
  ...
  try {
    ...
    if (!isPathContainedBy(resolvedPath, mount.realDir)) {
      res.statusCode = 403;
      res.end("Forbidden");
      return;
    }
    ...
    if (
      realFilePath === null ||
      !isPathContainedBy(realFilePath, mount.realDir)
    ) {
      res.statusCode = 403;
      res.end("Forbidden");
      return;
...
function isPathContainedBy(filePath: string, root: string): boolean {
