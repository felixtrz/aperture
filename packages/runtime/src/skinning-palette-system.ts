...
import { Skin, type SkinSkeleton } from "@aperture-engine/render";
...
function readWorldMatrix(entity: Entity, out: Mat4): Mat4 {
...
function writeIdentityBlock(palette: Float32Array, offset: number): void {
 ...
 */
export function updateSkeletonPalettes(world: EcsWorld): number {
  ...
  for (const entity of query.entities) {
    const skeleton = entity.getValue(Skin, "skeleton") as
      | SkinSkeleton
      | null
      | undefined;
    const palette = entity.getValue(Skin, "jointMatrices") as
      | Float32Array
      | null
      | undefined;
    ...
    for (let index = 0; index < joints.length; index += 1) {
      ...
      const inverseBind = skeleton.inverseBindMatrices.subarray(
        offset,
        offset + MATRIX_FLOATS,
      );
