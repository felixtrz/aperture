...
export function set(x: number, y: number, dst?: Vec2): Vec2 {
  const d = dst ?? new Float32Array(2);
  d[0] = x;
  d[1] = y;
  return d;
}

export function copy(vIn: NumArray, dst?: Vec2): Vec2 {
...
export function add(aIn: NumArray, bIn: NumArray, dst?: Vec2): Vec2 {
...
export function subtract(aIn: NumArray, bIn: NumArray, dst?: Vec2): Vec2 {
  const a = aIn as unknown as T2;
  const b = bIn as unknown as T2;
  const d = dst ?? new Float32Array(2);
  d[0] = a[0] - b[0];
...
export function scale(vIn: NumArray, k: number, dst?: Vec2): Vec2 {
  const v = vIn as unknown as T2;
  const d = dst ?? new Float32Array(2);
  d[0] = v[0] * k;
  d[1] = v[1] * k;
...
export function normalize(vIn: NumArray, dst?: Vec2): Vec2 {
  const v = vIn as unknown as T2;
  const d = dst ?? new Float32Array(2);
  const x = v[0];
  const y = v[1];
