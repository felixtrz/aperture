...
export interface EntityBoundsBvhOptions {
  readonly maxLeafSize?: number;
  readonly maxDepth?: number;
}

export interface EntityBoundsBvhQueryStats {
  candidateEntityCount: number;
  testedEntityCount: number;
  visitedNodeCount: number;
}

export interface EntityBoundsBvhRaycastOptions extends RaycastOptions {
  readonly stats?: EntityBoundsBvhQueryStats;
}

interface EntityBoundsItem<TEntity> {
  bounds: RaycastableBounds<TEntity>;
  readonly sourceIndex: number;
}

interface EntityBoundsNode<TEntity> {
  bounds: Aabb;
  readonly depth: number;
  readonly left?: EntityBoundsNode<TEntity>;
  readonly right?: EntityBoundsNode<TEntity>;
  readonly items?: readonly EntityBoundsItem<TEntity>[];
}

interface IndexedEntityBoundsHit<TEntity> extends RaycastHit<TEntity> {
  readonly sourceIndex: number;
}
...
export function createEntityBoundsBvh<TEntity>(
  bounds: readonly RaycastableBounds<TEntity>[],
  options: EntityBoundsBvhOptions = {},
): EntityBoundsBvh<TEntity> {
  return new EntityBoundsBvh(bounds, options);
}

export function createEntityBoundsBvhQueryStats(): EntityBoundsBvhQueryStats {
  return { candidateEntityCount: 0, testedEntityCount: 0, visitedNodeCount: 0 };
}

export class EntityBoundsBvh<TEntity> {
  private readonly items: EntityBoundsItem<TEntity>[];
  private readonly indexByEntity = new Map<TEntity, number>();
  private root: EntityBoundsNode<TEntity> | null;
  private dirty = false;

  constructor(
    bounds: readonly RaycastableBounds<TEntity>[],
    private readonly options: EntityBoundsBvhOptions = {},
  ) {
    ...
    this.root = buildEntityNode(
      [...this.items],
      0,
      normalizePositiveInteger(options.maxDepth, DEFAULT_MAX_DEPTH),
      normalizePositiveInteger(options.maxLeafSize, DEFAULT_MAX_LEAF_SIZE),
    );
  }
  ...
  raycast(
    origin: Vec3Like,
    direction: Vec3Like,
    options: EntityBoundsBvhRaycastOptions = {},
  ): RaycastHit<TEntity>[] {
    ...
    const hits: IndexedEntityBoundsHit<TEntity>[] = [];
    const stack = [this.root];

    while (stack.length > 0) {
      const node = stack.pop() as EntityBoundsNode<TEntity>;
      ...
      if (node.items !== undefined) {
        for (const item of node.items) {
          const objectLayerMask =
            (item.bounds.layerMask ?? DEFAULT_OBJECT_LAYER_MASK) >>> 0;
          ...
          if (
            item.bounds.worldSphere !== undefined &&
  ...
  updateBounds(
    entity: TEntity,
    bounds: Omit<RaycastableBounds<TEntity>, "entity">,
  ): boolean {
    ...
    const item = this.items[index] as EntityBoundsItem<TEntity>;
...
function buildEntityNode<TEntity>(
  items: readonly EntityBoundsItem<TEntity>[],
  depth: number,
  maxDepth: number,
  maxLeafSize: number,
): EntityBoundsNode<TEntity> | null {
  if (items.length === 0) {
    return null;
  }
  ...
  const sorted = [...items].sort(
    (a, b) =>
      centroid(a.bounds.worldAabb, axis) - centroid(b.bounds.worldAabb, axis),
  );
  const split = Math.max(1, Math.floor(sorted.length / 2));
  const left = buildEntityNode(
    sorted.slice(0, split),
    depth + 1,
    maxDepth,
    maxLeafSize,
  ) as EntityBoundsNode<TEntity>;
  const right = buildEntityNode(
    sorted.slice(split),
    depth + 1,
    maxDepth,
    maxLeafSize,
...
function refitEntityNode<TEntity>(node: EntityBoundsNode<TEntity>): Aabb {
  if (node.items !== undefined) {
    node.bounds = boundsForItems(node.items);
    return node.bounds;
  }

  node.bounds = unionAabb(
    refitEntityNode(node.left as EntityBoundsNode<TEntity>),
    refitEntityNode(node.right as EntityBoundsNode<TEntity>),
  );
  return node.bounds;
}

function boundsForItems<TEntity>(
  items: readonly EntityBoundsItem<TEntity>[],
): Aabb {
...
function unionAabb(a: Aabb, b: Aabb): Aabb {
  return {
    min: vec3(
      Math.min(v3(a.min, 0), v3(b.min, 0)),
      Math.min(v3(a.min, 1), v3(b.min, 1)),
      Math.min(v3(a.min, 2), v3(b.min, 2)),
    ),
    max: vec3(
      Math.max(v3(a.max, 0), v3(b.max, 0)),
      Math.max(v3(a.max, 1), v3(b.max, 1)),
      Math.max(v3(a.max, 2), v3(b.max, 2)),
    ),
  };
}

function longestAxis(bounds: Aabb): number {
...
function centroid(bounds: Aabb, axis: number): number {
  return (v3(bounds.min, axis) + v3(bounds.max, axis)) / 2;
}

function normalizeRay(origin: Vec3Like, direction: Vec3Like): Ray | null {
...
function resetStats(
  stats: EntityBoundsBvhQueryStats | undefined,
  candidateEntityCount: number,
): void {
  if (stats !== undefined) {
