...
import {
  ...
  type CreateMsdfTextRenderPipelineResourceResult,
  type MsdfTextRenderPipelineResource,
} from "../render/text/msdf-text-pipeline.js";
...
interface WebGpuAppTextContext {
...
interface PreparedMsdfTextFrameResources {
  readonly pipeline: CreateMsdfTextRenderPipelineResourceResult | null;
  readonly resources: MsdfTextFrameResources;
}

interface MsdfTextRenderBatch {
  readonly renderId: number;
  readonly texture: NonNullable<QuadBatchPacket["texture"]>;
  readonly sampler?: QuadBatchPacket["sampler"];
...
export async function prepareMsdfTextFrameResourcesForSnapshot(options: {
  readonly app: WebGpuAppTextContext;
...
}): Promise<PreparedMsdfTextFrameResources> {
  ...
  const pipeline = await getOrCreateWebGpuAppMsdfTextPipeline(
    options.app,
    options.cache,
  );
...
export function createMsdfTextFrameResources(options: {
  ...
  if (!viewBuffer.ok) {
    diagnostics.push(
      bufferDiagnostic("msdfTextFrame.viewBufferFailed", viewBuffer.message),
    );
  }
  ...
  if (!transformBuffer.ok) {
    diagnostics.push(
      bufferDiagnostic(
        "msdfTextFrame.transformBufferFailed",
  ...
  if (!glyphBuffer.ok) {
    diagnostics.push(
      bufferDiagnostic("msdfTextFrame.glyphBufferFailed", glyphBuffer.message),
    );
  }
  ...
  const defaultSampler = getOrCreateMsdfTextDefaultSampler(
    options.app,
    options.cache,
    options.reuse,
    diagnostics,
...
export async function getOrCreateWebGpuAppMsdfTextPipeline(
...
function createMsdfTextRenderInput(
  snapshot: RenderSnapshot,
  diagnostics: unknown[],
): {
  readonly data: Float32Array;
  readonly batches: readonly MsdfTextRenderBatch[];
} {
  ...
  const batches: MsdfTextRenderBatch[] = [];
