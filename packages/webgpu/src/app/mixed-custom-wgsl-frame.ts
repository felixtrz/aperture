...
import type { CreateQueuedBuiltInFrameResourcesResult } from "../render/queues/queued-built-in-frame-resource-set.js";
...
export interface MixedCustomWgslAppFrameResources {
  ...
  >["mesh"];
  readonly viewUniform: NonNullable<
    CreateQueuedBuiltInFrameResourcesResult["resources"]
  >["viewUniform"];
  readonly worldTransforms: NonNullable<
    CreateQueuedBuiltInFrameResourcesResult["resources"]
  >["worldTransforms"];
  readonly meshResources: readonly MeshGpuBufferResource[];
  readonly bindGroups: readonly UnlitBindGroupResource[];
  readonly unlit: NonNullable<
    CreateQueuedBuiltInFrameResourcesResult["resources"]
  >["unlit"];
  readonly matcap: NonNullable<
    CreateQueuedBuiltInFrameResourcesResult["resources"]
  >["matcap"];
  readonly standard: NonNullable<
    CreateQueuedBuiltInFrameResourcesResult["resources"]
  >["standard"];
  readonly debugNormal: NonNullable<
    CreateQueuedBuiltInFrameResourcesResult["resources"]
  >["debugNormal"];
  readonly custom: readonly NonNullable<
    Awaited<ReturnType<typeof createCustomWgslAppFrameResources>>["resources"]
  >[];
}

export interface MixedCustomWgslAppFrameResourcesResult {
  readonly valid: boolean;
  readonly resources: MixedCustomWgslAppFrameResources | null;
  readonly builtIn: CreateQueuedBuiltInFrameResourcesResult | null;
  readonly custom: readonly Awaited<
    ReturnType<typeof createCustomWgslAppFrameResources>
  >[];
  readonly diagnostics: readonly unknown[];
}

interface PreparedCustomDrawResources {
  readonly renderIds: readonly number[];
  readonly meshKey: string;
  readonly materialKey: string;
  readonly pipelineResult: CreateCustomWgslMaterialRenderPipelineResourceResult;
  readonly pipelinePlanResult: WebGpuAppPipelinePlanResult;
...
export async function renderMixedCustomWgslWebGpuAppFrame(options: {
  ...
  const firstPipeline =
    preparedBuiltIn.firstPipeline ?? firstCustomPipeline ?? null;
  const resourcesResult = createMixedCustomWgslResourcesResult({
    builtIn: preparedBuiltIn.resourcesResult,
    custom: preparedCustom.resources.map(
      (resource) => resource.resourcesResult,
    ),
    diagnostics: [
  ...
  const customResourcesByRenderId = indexCustomResourcesByRenderId(
    preparedCustom.resources,
  );
  const customPipelineKeysByRenderId = new Map(
    preparedCustom.resources.flatMap((resource) =>
      resource.renderIds.map((renderId) => [
        renderId,
  ...
  const framePlan = writeRenderFramePlanFromSnapshot({
    ...
    resolveMeshResourceKey: (draw) =>
      customResourcesByRenderId.get(draw.renderId)?.meshResourceKey ??
      preparedBuiltIn.meshResourceKeys.get(assetHandleKey(draw.mesh)) ??
      null,
    resolveMaterialResourceKey: (draw) =>
      customResourcesByRenderId.get(draw.renderId)?.materialResourceKey ??
    ...
    pipelines: [
      ...
      ...preparedCustom.resources.map(
        (resource) => resource.pipelinePlanResult,
      ),
    ],
...
async function prepareCustomDrawResources(options: {
  ...
  readonly cache: WebGpuAppResourceCache;
  readonly snapshot: RenderSnapshot;
  readonly viewUniforms: Parameters<
    typeof createCustomWgslAppFrameResources
  >[0]["viewUniforms"];
  readonly worldTransforms: Parameters<
    typeof createCustomWgslAppFrameResources
  >[0]["worldTransforms"];
  readonly reuse: WebGpuAppResourceReuseReport;
}): Promise<{
  ...
  const resources: PreparedCustomDrawResources[] = [];
  const diagnostics: unknown[] = [];
  const resourceSetByKey = new Map<string, PreparedCustomDrawResources>();
  ...
  const preparedRenderIdCount = resources.reduce(
    (total, resource) => total + resource.renderIds.length,
    0,
  );

  return {
    valid:
      diagnostics.length === 0 && preparedRenderIdCount === customDraws.length,
    resources,
    diagnostics,
  };
}
...
async function prepareCustomDrawResourceSet(options: {
  ...
  const meshEntry = options.assets.get<"mesh", MeshAsset>(options.draw.mesh);
  const materialEntry = options.assets.get<"material", SourceMaterialAsset>(
    options.draw.material,
  );
      ...
      diagnostics: [
        {
          ...
          message:
            "Mixed custom WGSL app route requires ready mesh and custom WGSL material source assets.",
  ...
  const preparedEntry = options.cache.preparedMaterialFacade.get(
    options.draw.material,
  );
      ...
      diagnostics: [
        {
          ...
          message:
            "Custom WGSL material source was not prepared before mixed frame resource creation.",
          renderId: options.draw.renderId,
        },
      ],
    };
...
function createMixedCustomWgslResourcesResult(input: {
  readonly builtIn: CreateQueuedBuiltInFrameResourcesResult;
  readonly custom: readonly Awaited<
    ReturnType<typeof createCustomWgslAppFrameResources>
  >[];
  readonly diagnostics: readonly unknown[];
}): MixedCustomWgslAppFrameResourcesResult {
  const customResources = input.custom.flatMap((result) =>
    result.resources === null ? [] : [result.resources],
  );
  const resources =
    input.builtIn.resources === null ||
    customResources.length !== input.custom.length
      ...
      : {
          ...
          bindGroups: [
            ...
            ...customResources.flatMap((resource) =>
              resource.bindGroups.map(asUnlitBindGroupResource),
            ),
          ],
...
function indexCustomResourcesByRenderId(
  resources: readonly PreparedCustomDrawResources[],
): ReadonlyMap<number, PreparedCustomDrawResources> {
  const result = new Map<number, PreparedCustomDrawResources>();
