...
import {
  ...
  type CreateUiQuadRenderPipelineResourceResult,
  type UiQuadRenderPipelineResource,
} from "../render/ui/ui-quad-pipeline.js";
import { getOrCreateWebGpuAppMsdfTextPipeline } from "./text.js";
import type { MsdfTextRenderPipelineResource } from "../render/text/msdf-text-pipeline.js";
...
export interface UiFrameResources {
  readonly valid: boolean;
  readonly commands: readonly RenderPassCommand[];
  readonly diagnostics: readonly unknown[];
}

interface UiQuadInstance {
  readonly node: UiNodePacket;
  readonly instance: number;
}

interface UiTextBatch {
  readonly node: UiNodePacket;
...
export async function prepareUiFrameResourcesForSnapshot(options: {
  readonly app: WebGpuAppUiContext;
  readonly assets: AssetRegistry;
  readonly cache: WebGpuAppResourceCache;
  readonly snapshot: RenderSnapshot;
  readonly viewUniforms: PackedSnapshotViewUniforms;
  readonly reuse: WebGpuAppResourceReuseReport;
}): Promise<UiFrameResources> {
  ...
  const viewUniformData = createUiViewUniformData({
    ...
    source: options.viewUniforms.data.subarray(
      0,
  ...
  const textInput = createUiTextInput(
    options.snapshot,
    uiNodes,
    options.assets,
  );
  ...
  const panelResources =
    panelPipeline?.resource === undefined || panelPipeline.resource === null
      ? { commands: [] as RenderPassCommand[], diagnostics: [] as unknown[] }
  ...
  const imageResources =
    imagePipeline?.resource === undefined || imagePipeline.resource === null
      ? { commands: [] as RenderPassCommand[], diagnostics: [] as unknown[] }
  ...
  const textResources =
    textPipeline?.resource === undefined || textPipeline.resource === null
      ? { commands: [] as RenderPassCommand[], diagnostics: [] as unknown[] }
  ...
  diagnostics.push(
    ...panelResources.diagnostics,
    ...imageResources.diagnostics,
    ...textResources.diagnostics,
  );

  const commandGroups = new Map<number, readonly RenderPassCommand[]>();

  for (const instance of panelInput.instances) {
    commandGroups.set(
      instance.node.uiId,
      commandsForRenderId(panelResources.commands, instance.node.uiId),
    );
  }
  ...
  for (const instance of imageInput.instances) {
    commandGroups.set(
      instance.node.uiId,
      commandsForRenderId(imageResources.commands, instance.node.uiId),
    );
  }
  ...
  if (commands.length === 0) {
    diagnostics.push({
      ...
      message:
        "UI frame preparation found renderable UI nodes but emitted no draw commands.",
    });
  }
...
export async function getOrCreateWebGpuAppUiPanelPipeline(
...
export async function getOrCreateWebGpuAppUiImagePipeline(
  app: WebGpuAppUiContext,
  cache: WebGpuAppResourceCache,
): Promise<CreateUiQuadRenderPipelineResourceResult> {
  const { tonemap, outputColorSpace } = resolveUiOutputStage(app);
...
function createUiPanelCommands(options: {
  ...
  readonly cache: WebGpuAppResourceCache;
  readonly snapshot: RenderSnapshot;
  readonly pipeline: UiQuadRenderPipelineResource;
  readonly viewBuffer: unknown;
  readonly input: ReturnType<typeof createUiPanelInput>;
}): {
  readonly commands: readonly RenderPassCommand[];
  readonly diagnostics: readonly unknown[];
} {
...
function createUiImageCommands(options: {
  ...
  readonly pipeline: UiQuadRenderPipelineResource;
  readonly viewBuffer: unknown;
  readonly input: ReturnType<typeof createUiImageInput>;
}): {
  readonly commands: readonly RenderPassCommand[];
  readonly diagnostics: readonly unknown[];
} {
  ...
  const defaultSampler = getOrCreateUiImageDefaultSampler(
    options.app,
    options.cache,
    options.reuse,
    diagnostics,
  ...
  for (const instance of options.input.instances) {
    ...
    const sampler =
      instance.node.sampler === undefined || instance.node.sampler === null
        ? {
...
function createUiTextCommands(options: {
  ...
  readonly snapshot: RenderSnapshot;
  readonly pipeline: MsdfTextRenderPipelineResource;
  readonly viewBuffer: unknown;
  readonly input: ReturnType<typeof createUiTextInput>;
}): {
  readonly commands: readonly RenderPassCommand[];
  readonly diagnostics: readonly unknown[];
} {
  ...
  if (!transformBuffer.ok) {
    diagnostics.push(
      bufferDiagnostic(
        "uiFrame.textTransformBufferFailed",
  ...
  if (!glyphBuffer.ok) {
    diagnostics.push(
      bufferDiagnostic("uiFrame.textGlyphBufferFailed", glyphBuffer.message),
    );
  }
  ...
  const defaultSampler = getOrCreateUiTextDefaultSampler(
    options.app,
    options.cache,
    options.reuse,
    diagnostics,
...
function createUiPanelInput(nodes: readonly UiNodePacket[]): {
  readonly data: Float32Array;
  readonly instances: readonly UiQuadInstance[];
} {
  const panels = nodes.filter((node) => node.kind === "panel");
  const data = new Float32Array(
    Math.max(1, panels.length) * UI_QUAD_FLOAT_STRIDE,
  );
  const instances: UiQuadInstance[] = [];
...
function createUiImageInput(nodes: readonly UiNodePacket[]): {
  readonly data: Float32Array;
  readonly instances: readonly UiQuadInstance[];
} {
  const images = nodes.filter((node) => node.kind === "image");
  const data = new Float32Array(
    Math.max(1, images.length) * UI_QUAD_FLOAT_STRIDE,
  );
  const instances: UiQuadInstance[] = [];
...
function createUiTextInput(
  ...
  textNodes.forEach((node, transformIndex) => {
    ...
    const fontEntry = assets.get<"font-atlas", MsdfFontAtlasAsset>(fontHandle);
    ...
    const layout = layoutMsdfText(font, {
      ...
      maxWidth:
        node.maxWidth !== undefined && node.maxWidth > 0
          ? node.maxWidth
    ...
    const glyphsByPage = new Map<number, typeof layout.glyphs>();
...
function writeUiGlyphData(
  target: number[],
  input: {
    readonly node: UiNodePacket;
    readonly glyph: ReturnType<typeof layoutMsdfText>["glyphs"][number];
  ...
  target.push(
    ...
    glyph.uvRect[2],
    glyph.uvRect[3],
    input.font.distanceRange,
    input.font.scaleW,
    input.font.scaleH,
    1,
    input.transformIndex,
    glyph.glyphId,
    glyph.charIndex,
    glyph.lineIndex,
    input.node.clip.x,
    input.node.clip.y,
    input.node.clip.width,
    input.node.clip.height,
  );
}
...
function createUiViewUniformData(options: {
  ...
  const records =
    options.viewUniforms.views.length === 0
...
function parseFontAtlasHandle(value: string): FontAtlasHandle | null {
...
function getOrCreateUiImageDefaultSampler(
  app: WebGpuAppUiContext,
  cache: WebGpuAppResourceCache,
  reuse: WebGpuAppResourceReuseReport,
  diagnostics: unknown[],
): SamplerGpuResource | null {
  return getOrCreateUiDefaultSampler(
    app,
    cache,
    reuse,
    diagnostics,
    "ui-image:default-sampler",
    "UiImageDefaultSampler",
  );
}

function getOrCreateUiTextDefaultSampler(
  app: WebGpuAppUiContext,
  cache: WebGpuAppResourceCache,
  reuse: WebGpuAppResourceReuseReport,
  diagnostics: unknown[],
  ...
  return getOrCreateUiDefaultSampler(
    ...
    reuse,
    diagnostics,
    "ui-text:default-sampler",
    "UiTextDefaultSampler",
  );
}

function getOrCreateUiDefaultSampler(
  app: WebGpuAppUiContext,
  cache: WebGpuAppResourceCache,
  reuse: WebGpuAppResourceReuseReport,
  diagnostics: unknown[],
