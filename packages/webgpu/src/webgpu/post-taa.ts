...
interface CachedTaaPostPipeline {
  readonly key: string;
  readonly pipeline: unknown;
}

export function createWebGpuTaaPostEffect(
  options: CreateWebGpuTaaPostEffectOptions = {},
): WebGpuPostEffect {
  ...
  let cachedPipeline: CachedTaaPostPipeline | null = null;
  let sampler: unknown | null = null;
  let previousOutput: WebGpuPostPassTextureResource | null = null;
...
function historyTextureForFrame(
  previousOutput: WebGpuPostPassTextureResource | null,
  input: WebGpuPostPassTextureResource,
): WebGpuPostPassTextureResource {
  if (
    previousOutput !== null &&
...
function taaPostEffectWgsl(historyWeight: number): string {
  return /* wgsl */ `
struct VertexOutput {
