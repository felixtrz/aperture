...
import type {
  ...
  DrawIndirectCommand,
  RenderPassCommand,
} from "../passes/render-pass-commands.js";

export type IndirectDrawCommandStatus =
  | "disabled"
  | "skipped"
  | "created"
  | "updated"
...
export type IndirectDrawFallbackReason =
  | "no-candidates"
...
export interface IndirectDrawBufferResource {
...
export interface IndirectDrawCommandCache {
  resource: IndirectDrawBufferResource | null;
  readonly args: Uint32Array;
  readonly signedArgs: Int32Array;
  readonly commands: RenderPassCommand[];
}

export interface PrepareIndirectDrawCommandsOptions {
  readonly device: WebGpuBufferDeviceLike;
  readonly cache: IndirectDrawCommandCache;
...
export interface IndirectDrawCommandReport {
  readonly valid: boolean;
  readonly status: IndirectDrawCommandStatus;
  ...
  readonly fallbackReason: IndirectDrawFallbackReason | null;
  readonly diagnostics: readonly IndirectDrawCommandDiagnostic[];
}

export interface PrepareIndirectDrawCommandsResult {
  readonly commands: readonly RenderPassCommand[];
  readonly report: IndirectDrawCommandReport;
}

const indirectDrawArgumentStride = 5;
const indirectDrawArgumentStrideBytes = indirectDrawArgumentStride * 4;

export function createIndirectDrawCommandCache(
  capacity = 0,
): IndirectDrawCommandCache {
  const args = new Uint32Array(
    Math.max(0, capacity) * indirectDrawArgumentStride,
  );
...
export function prepareIndirectDrawCommands(
  options: PrepareIndirectDrawCommandsOptions,
): PrepareIndirectDrawCommandsResult {
  ...
  if (
    !options.supportsIndirectFirstInstance &&
    ...
    return fallback(options, {
      ...
      diagnostic: {
        ...
        reason: "first-instance-unsupported",
        message:
          "Indirect draws with non-zero firstInstance require the 'indirect-first-instance' WebGPU feature.",
      },
    });
  }
  ...
  if (options.device.createBuffer === undefined) {
    return fallback(options, {
      ...
      diagnostic: {
        ...
        reason: "create-buffer-unavailable",
        message:
          "WebGPU device cannot create an indirect draw argument buffer.",
      },
    });
  }
  ...
  if (options.device.queue?.writeBuffer === undefined) {
    return fallback(options, {
      ...
      diagnostic: {
        ...
        reason: "queue-write-buffer-unavailable",
        message:
          "WebGPU queue cannot upload indirect draw argument buffer contents.",
      },
    });
  }

  const resourceKey = `indirect-draw-buffer:${options.label}`;
  const allocated = ensureIndirectDrawBuffer(
    options.device,
    options.cache,
    resourceKey,
    candidates.length,
  ...
  options.device.queue.writeBuffer(
    ...
    0,
    options.cache.args.buffer,
    options.cache.args.byteOffset,
    candidates.length * indirectDrawArgumentStrideBytes,
  );

  const transformed = writeIndirectCommandList(
    options.commands,
    candidates,
    allocated.resource,
    options.cache,
  );
  const indexedIndirectDraws = candidates.filter(
...
function candidateDrawCommands(
  commands: readonly RenderPassCommand[],
  minInstanceCount: number,
): readonly (DrawCommand | DrawIndexedCommand)[] {
  return commands.filter(
    (command): command is DrawCommand | DrawIndexedCommand =>
      (command.kind === "draw" || command.kind === "drawIndexed") &&
      command.instanceCount >= minInstanceCount,
  );
}

function ensureIndirectDrawBuffer(
  device: WebGpuBufferDeviceLike,
  cache: IndirectDrawCommandCache,
  resourceKey: string,
  count: number,
):
  | {
      readonly ok: true;
      readonly resource: IndirectDrawBufferResource;
  ...
  const result = createWebGpuBuffer({
    ...
    descriptor: {
      ...
      size: byteLength,
      usage:
        WEBGPU_BUFFER_USAGE_FLAGS.INDIRECT | WEBGPU_BUFFER_USAGE_FLAGS.COPY_DST,
    },
  });
...
function writeIndirectArguments(
  cache: IndirectDrawCommandCache,
  candidates: readonly (DrawCommand | DrawIndexedCommand)[],
): void {
...
function ensureArgumentCapacity(
  cache: IndirectDrawCommandCache,
  count: number,
): void {
  ...
  (cache as { args: Uint32Array }).args = args;
  (cache as { signedArgs: Int32Array }).signedArgs = new Int32Array(
    args.buffer,
  );
}

function writeIndirectCommandList(
  commands: readonly RenderPassCommand[],
  candidates: readonly (DrawCommand | DrawIndexedCommand)[],
  resource: IndirectDrawBufferResource,
  cache: IndirectDrawCommandCache,
...
function indirectCommand(
  command: DrawCommand | DrawIndexedCommand,
  resource: IndirectDrawBufferResource,
...
function fallback(
  ...
  input: {
    readonly candidates: number;
    readonly reason: IndirectDrawFallbackReason;
    readonly diagnostic: IndirectDrawCommandDiagnostic;
  },
): PrepareIndirectDrawCommandsResult {
  return {
    commands: options.commands,
...
function firstInstanceForDrawCommand(
  command: DrawCommand | DrawIndexedCommand,
): number {
