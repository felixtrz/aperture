...
// deep-equal results.
//
// Two rules govern store/load annotation:
//   * Store-on-no-clear: when a later pass uses a target without clearing it, the
//     earlier producing pass must store. We generalize this so a later *read* of a
//     handle also forces the producer to store.
//   * History-texture lifetime: the history-texture handle kind consumed here is
//     kept alive across frames so its previous-frame contents survive.
//
