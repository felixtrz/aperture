...
import type { CreateTextureGpuResourceResult } from "../resources/textures/texture-resources.js";

export function createDefaultDiffuseIblUpload(size: number, format: string) {
...
export function createDefaultSpecularIblUpload(size: number, format: string) {
...
export function createPaddedCubeFaceUpload(
  face: Uint8Array,
  faceSize: number,
...
export function createCubeFacesUpload(
...
export function bytesPerPixelForPmremFormat(
  format: PmremComputeStorageFormat,
): number | null {
  switch (format) {
    case "rgba8unorm":
...
export function missingTextureResult(): CreateTextureGpuResourceResult {
  return {
    valid: false,
    resource: null,
    diagnostics: [],
...
export function mipLevelCountForSize(size: number): number {
