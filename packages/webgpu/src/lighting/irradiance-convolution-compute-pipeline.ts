...
export type IrradianceConvolutionStorageFormat = "rgba8unorm" | "rgba16float";

export interface IrradianceConvolutionDeviceLike {
...
export type IrradianceConvolutionPipelineDiagnosticCode =
  | "irradianceConvolutionPipeline.createShaderModuleUnavailable"
  | "irradianceConvolutionPipeline.createBindGroupLayoutUnavailable"
  | "irradianceConvolutionPipeline.createPipelineLayoutUnavailable"
  | "irradianceConvolutionPipeline.createComputePipelineUnavailable"
  | "irradianceConvolutionPipeline.shaderModuleCreationFailed"
  | "irradianceConvolutionPipeline.bindGroupLayoutCreationFailed"
  | "irradianceConvolutionPipeline.pipelineLayoutCreationFailed"
  | "irradianceConvolutionPipeline.pipelineCreationFailed";

export interface IrradianceConvolutionPipelineDiagnostic {
  readonly code: IrradianceConvolutionPipelineDiagnosticCode;
  readonly message: string;
}

export interface IrradianceConvolutionPipelineResource {
...
export interface CreateIrradianceConvolutionComputePipelineResult {
  readonly valid: boolean;
  readonly resource: IrradianceConvolutionPipelineResource | null;
  readonly diagnostics: readonly IrradianceConvolutionPipelineDiagnostic[];
}

export interface IrradianceConvolutionDispatchSize {
  readonly x: number;
  readonly y: number;
  readonly z: number;
}
...
export function createIrradianceConvolutionComputePipeline(
  options: CreateIrradianceConvolutionComputePipelineOptions,
  ...
  if (options.device.createShaderModule === undefined) {
    return failure(
      "irradianceConvolutionPipeline.createShaderModuleUnavailable",
      "WebGPU device cannot create irradiance-convolution shader modules.",
    );
  }
  ...
  if (options.device.createBindGroupLayout === undefined) {
    return failure(
      "irradianceConvolutionPipeline.createBindGroupLayoutUnavailable",
      "WebGPU device cannot create irradiance-convolution bind group layouts.",
    );
  }
  ...
  if (options.device.createPipelineLayout === undefined) {
    return failure(
      "irradianceConvolutionPipeline.createPipelineLayoutUnavailable",
      "WebGPU device cannot create irradiance-convolution pipeline layouts.",
    );
  }
  ...
  if (options.device.createComputePipeline === undefined) {
    return failure(
      "irradianceConvolutionPipeline.createComputePipelineUnavailable",
      "WebGPU device cannot create irradiance-convolution compute pipelines.",
    );
  }
...
}): IrradianceConvolutionDispatchSize {
  return {
 ...
 */
export function convolveIrradianceDirection(
  sample: (direction: [number, number, number]) => number,
  normal: readonly [number, number, number],
  sampleCount: number = IRRADIANCE_CONVOLUTION_DEFAULT_SAMPLE_COUNT,
): number {
...
function failure(
  code: IrradianceConvolutionPipelineDiagnosticCode,
  message: string,
): CreateIrradianceConvolutionComputePipelineResult {
  return {
    valid: false,
