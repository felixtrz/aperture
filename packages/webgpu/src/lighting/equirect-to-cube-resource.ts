...
import {
  ...
  type EquirectToCubeDeviceLike,
  type EquirectToCubeStorageFormat,
} from "./equirect-to-cube-compute-pipeline.js";
import {
...
export type EquirectToCubeResourceDiagnosticCode =
  | "equirectToCubeResource.deviceUnsupported"
  | "equirectToCubeResource.pipelineUnavailable"
  | "equirectToCubeResource.invalidSource"
  | "equirectToCubeResource.dispatchFailed";

export interface EquirectToCubeResourceDiagnostic {
  readonly code: EquirectToCubeResourceDiagnosticCode;
...
export interface EquirectSource {
  ...
  /** Tightly-packed rgba8unorm bytes (width * height * 4). */
  readonly data: Uint8Array;
}

export interface EquirectToCubeResourceReport {
  readonly ready: boolean;
  readonly faceSize: number;
  readonly faceCount: 6;
  readonly format: EquirectToCubeStorageFormat;
  readonly projection: "equirect-to-cube";
  readonly resource: TextureGpuResource | null;
  readonly diagnostics: readonly EquirectToCubeResourceDiagnostic[];
}

export interface CreateEquirectToCubeResourceOptions {
  readonly device: TextureGpuDeviceLike;
  readonly equirect: EquirectSource;
  readonly faceSize?: number;
  readonly format?: EquirectToCubeStorageFormat;
...
interface EquirectToCubeFullDeviceLike
  extends TextureGpuDeviceLike, EquirectToCubeDeviceLike {
  ...
  readonly createShaderModule: NonNullable<
    EquirectToCubeDeviceLike["createShaderModule"]
  >;
  readonly createBindGroupLayout: NonNullable<
    EquirectToCubeDeviceLike["createBindGroupLayout"]
  >;
  readonly createPipelineLayout: NonNullable<
    EquirectToCubeDeviceLike["createPipelineLayout"]
  >;
  readonly createComputePipeline: NonNullable<
    EquirectToCubeDeviceLike["createComputePipeline"]
  >;
...
const DEFAULT_FORMAT: EquirectToCubeStorageFormat = "rgba8unorm";
const SOURCE_FORMAT = "rgba8unorm";

export function hasEquirectToCubeDeviceSupport(
  device: TextureGpuDeviceLike,
): device is EquirectToCubeFullDeviceLike {
  const maybeDevice = device as EquirectToCubeFullDeviceLike;
...
export function createEquirectToCubeResource(
  options: CreateEquirectToCubeResourceOptions,
): EquirectToCubeResourceReport {
  ...
  const report = (
    ready: boolean,
    resource: TextureGpuResource | null,
    diagnostics: readonly EquirectToCubeResourceDiagnostic[],
  ): EquirectToCubeResourceReport => ({
    ready,
    faceSize,
    faceCount: 6,
    ...
    return report(false, null, [
      {
        ...
        severity: "warning",
        message:
          "Equirect source must be a positive width x height with at least width*height*4 rgba8unorm bytes.",
      },
    ]);
  }
    ...
    return report(false, null, [
      {
        ...
        severity: "warning",
        message:
          "Equirect-to-cube projection requires texture, compute pipeline, bind group, command encoder, uniform buffer, and queue support.",
      },
    ]);
  }
  ...
  if (!pipeline.valid || pipeline.resource === null) {
    return report(
      ...
      pipeline.diagnostics.map((diagnostic) => ({
        code: "equirectToCubeResource.pipelineUnavailable" as const,
  ...
  try {
    ...
    for (let y = 0; y < height; y += 1) {
      padded.set(
        data.subarray(y * sourceBytesPerRow, (y + 1) * sourceBytesPerRow),
        y * bytesPerRow,
      );
    }

    device.queue.writeTexture(
      { texture: sourceTexture, origin: [0, 0, 0] },
      padded,
    ...
    device.queue.writeBuffer(
      params,
      0,
      new Uint32Array([faceSize, faceSize, 6, 0]),
    );
    ...
    if (encoder.finish === undefined) {
      throw new Error(
        "Equirect command encoder cannot finish command buffers.",
      );
    }
    ...
    const resource: TextureGpuResource = {
      resourceKey,
      texture: cube,
      view,
      descriptor: {
