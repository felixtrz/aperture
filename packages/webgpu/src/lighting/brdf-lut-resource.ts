...
import {
  ...
  type BrdfLutComputeDeviceLike,
  type BrdfLutComputeStorageFormat,
} from "./brdf-lut-compute-pipeline.js";
import {
...
export type BrdfLutResourceDiagnosticCode =
  | "brdfLutResource.deviceUnsupported"
  | "brdfLutResource.pipelineUnavailable"
  | "brdfLutResource.dispatchFailed";

export interface BrdfLutResourceDiagnostic {
  readonly code: BrdfLutResourceDiagnosticCode;
  readonly severity: "warning" | "error";
  readonly message: string;
}

export interface BrdfLutResourceReport {
  readonly ready: boolean;
  readonly size: number;
  readonly format: BrdfLutComputeStorageFormat;
  readonly resource: TextureGpuResource | null;
  readonly diagnostics: readonly BrdfLutResourceDiagnostic[];
}

export interface CreateBrdfIntegrationLutResourceOptions {
...
interface BrdfLutDeviceLike
  extends TextureGpuDeviceLike, BrdfLutComputeDeviceLike {
  readonly createTexture: NonNullable<TextureGpuDeviceLike["createTexture"]>;
  readonly createShaderModule: NonNullable<
    BrdfLutComputeDeviceLike["createShaderModule"]
  >;
  readonly createBindGroupLayout: NonNullable<
    BrdfLutComputeDeviceLike["createBindGroupLayout"]
  >;
  readonly createPipelineLayout: NonNullable<
    BrdfLutComputeDeviceLike["createPipelineLayout"]
  >;
  readonly createComputePipeline: NonNullable<
    BrdfLutComputeDeviceLike["createComputePipeline"]
  >;
...
const DEFAULT_BRDF_LUT_FORMAT: BrdfLutComputeStorageFormat = "rg16float";

export function hasBrdfLutDeviceSupport(
  device: TextureGpuDeviceLike,
): device is BrdfLutDeviceLike {
  const maybeDevice = device as BrdfLutDeviceLike;
...
export function createBrdfIntegrationLutResource(
  options: CreateBrdfIntegrationLutResourceOptions,
): BrdfLutResourceReport {
      ...
      diagnostics: [
        {
          ...
          severity: "warning",
          message:
            "BRDF integration LUT requires texture, compute pipeline, bind group, command encoder, uniform buffer, and queue support.",
        },
      ],
    };
  }
  ...
  if (!pipeline.valid || pipeline.resource === null) {
    return {
      ...
      diagnostics: pipeline.diagnostics.map((diagnostic) => ({
        code: "brdfLutResource.pipelineUnavailable" as const,
  ...
  try {
    ...
    if (encoder.finish === undefined) {
      throw new Error(
        "BRDF LUT command encoder cannot finish command buffers.",
      );
    }
