...
/**
 * Per-cascade frustum-fit + texel-stabilized ortho derivation: fit each
 * cascade's camera-view slice into a light-space AABB, size the ortho to the
 * slice's largest diagonal (so the extent depends only on the frustum SHAPE, not
 * the camera position — shadows do not swim), and snap the cascade center to
 * integer texel multiples. This keeps Aperture's existing
 * makeLookAt/makeOrthographic (non-reverse-Z) convention so the shader's
 * shadowClip.z*0.5+0.5 remap stays valid.
 *
