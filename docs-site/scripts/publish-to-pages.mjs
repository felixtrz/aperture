...
async function main() {
  ...
  for (const showcaseId of showcaseIds) {
    const showcaseDist = path.join(repoRoot, "showcase", showcaseId, "dist");
    if (!(await exists(path.join(showcaseDist, "index.html")))) {
      throw new Error(
        `showcase/${showcaseId}/dist/index.html is missing. Run docs-site build:showcases before publishing.`,
      );
    }
    for (const file of await listFiles(showcaseDist)) {
      const outputFile = path.join("showcase", showcaseId, file);
      nextFiles.add(outputFile);
      copyOps.push({
        from: path.join(showcaseDist, file),
        to: path.join(pagesDir, outputFile),
        file: outputFile,
        sourceLabel: path.join("showcase", showcaseId, "dist", file),
      });
    }
  }
  ...
  console.log(
    `${dryRun ? "Dry run" : "Publishing"} ${copyOps.length} file(s) from docs-site/dist and showcase dist outputs to ${relative(
      pagesDir,
    )}.`,
  );
