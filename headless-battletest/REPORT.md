...
I packaged this as a reusable guarantee: `headless-battletest/determinism-regression.mjs` runs all four apps twice and asserts bit-identical bundle digests (4/4 ✓) — a drop-in CI check the project could adopt. This is something you fundamentally cannot get cheaply in a browser RAF loop, and it's the best argument for the whole headless approach. **Caveat (F11):** this hard gate only fires in the *one-shot* command — the warm `serve` session never aborts on `--determinism error` and only exposes violations via `get-status.diagnostics`.
