...
- Open observation (root cause UNDETERMINED): dynamic boxes free-fell straight THROUGH a fixed ground (y -> -42 over 3s = pure free-fall, no contact). This persisted even with EXPLICIT box collider shapes on both ground and boxes (identical digest), so it is NOT a missing-shape issue. So gravity integrates but dynamic-vs-fixed contact resolution does not happen in this low-level headless setup. Diagnostic: all 9 colliders REGISTER (sync.colliderCount=9) but contact/event count stays 0 — i.e. bodies integrate under gravity but narrow-phase contact DETECTION never fires in the bare stepAndExtract path. The physics facade registers a fixed-step task calling stepPhysicsWorld(); since gravity DOES apply, that task runs every fixed step — so the gap is specifically that narrow-phase contact detection within stepPhysicsWorld never fires in this bare path (free-fall to -42 over 3s = exactly 0 collision impulses). DECISIVE: even a SINGLE box dropped on a fixed ground (both with explicit box colliders, colliderCount=2) falls straight through to -40.7 — so it is NOT a stack/multi-body setup issue; the bare path resolves zero contacts for any configuration tried. Likely the bare createApertureApp + manual stepAndExtract path is missing collision-pipeline setup that the browser worker provides via startOptions, OR a real gap. Negative results: neither physicsInterpolation:true nor fixedStep.maxSubsteps fixes it (box still falls through in all variants) — so it's not a simple missing option. Needs the engine author to confirm; not reachable via the headless CLI anyway (F12). Caveat: physics is unreachable via the headless CLI anyway (F12); determinism (above) holds regardless. Flagging because if F12 is fixed, contact resolution must also work for headless physics validation to be meaningful beyond free-fall.
...
- MCP exposes 8 reference tools too (reference_search, reference_api_lookup, reference_find_examples, reference_explain_diagnostic, reference_list_components/systems, reference_find_dependents, reference_file_content) — verified search + explain_diagnostic(aperture.determinism.nondeterministicGlobal) return results. So the full agent docs/reference loop works headlessly via MCP, accounting for all 46 MCP tools (38 app/ecs/input/camera/resource/asset + 8 reference).
