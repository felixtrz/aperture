...
- **FIX (concrete):** enumerate entities in `apertureEntityComponentSchemas` via `collectActiveEntities(world)` (the same helper `findApertureEntities` uses) instead of the raw `registerQuery({ required: [] }).entities`. Then all live entities — including runtime-spawned ones — are scanned.
