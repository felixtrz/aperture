...
- Source confirmation: `renderBundleToPng` (`packages/cli/src/render/driver.ts`), shared by the CLI `render` command AND the MCP `frame_capture`/`render_bundle` tools, calls `chromium.launch()` + provisions Xvfb on every invocation and `close()`s both in `finally` — there is no persistent/warm browser, so each on-demand capture pays the full ~4 s boot. This is the concrete basis for the "add a warm render slot" recommendation.

### F15 contrast — node/TRS animation works at the SAME 0.01 scale that freezes skinning (F15 is skinning-specific)
- Built a minimal glTF (`make-node-anim-gltf.mjs` → `app/public/models/spincube.gltf` + `.bin`): a cube with a **node rotation** animation channel, authored at **node scale 0.01** (mesh-world determinant = 0.01³ = 1e-6 — the exact F15 freezing boundary). Vertices are ±50 so the world size (~1 unit) is framable while the determinant stays 1e-6 (determinant comes only from node scale).
- `nodeanim.headless.config.ts` + `nodeanim-src/setup.system.ts` play the clip. Result: `meshDraws:1`, the cube's extracted world transform **changes across frames** (4 elems, maxAbsDiff 0.0063, consistent with rotating a 0.01-scaled basis), and `aperture render` frame 5 vs 20 **differ** (`artifacts/nodeanim_spincube.png`, cube caught mid-spin). Node animation is applied and visible.
- Conclusion: node/TRS animation goes through the same `updateAnimationDrivers` → `resolveWorldTransforms` path but never calls `invertMat4(meshWorld)` (only `updateSkeletonPalettes` does), so it is UNAFFECTED at det=1e-6. This isolates F15 precisely to the skinning-palette inversion, not the animation system in general.
...
### FINDING F16 (LOW) — animation().playClip()/crossFade() throw a raw Error on an unknown clip id, aborting the run
- `anim.playClip("Bogus", { loop: "repeat" })` on the soldier (a rig that HAS clips) → the whole headless run aborts exit 1: `aperture.cli.failed: System 'XfadeSystem' threw during update(): AnimationMixer.play: unknown clip id "Bogus" Suggested fix: Fix the exception raised in XfadeSystem.update(); the original stack is preserved below.`
- Root cause: `AnimationMixer.play` (runtime/animation-mixer.ts) does `const clip = this.clips.get(clipId); if (clip === undefined) throw new Error(...)`. The public `createAnimationAccess.playClip` only guards the empty-rig case (`if (mixer.clipIds.length === 0) return`) and otherwise calls `mixer.play(clipId)` uncaught; `crossFade`/`crossFadeTo` do the same. So "rig has zero clips" is a graceful no-op, but "rig has clips, this id is a typo" is fatal — an odd asymmetry.
- Verified by temporarily changing the xfade demonstrator's `playClip("Idle")` to `playClip("Bogus")` (rig with 4 clips) → exit 1 with the raw Error (reverted after). The thrown value is a bare `Error`, not the engine's usual `ApertureSystemError` with a stable code + hint, and it carries the misleading "original stack is preserved below" tail (O12) with no stack.
- Impact/DX: a mistyped clip name, or a clip name valid for one model but not another spawned through shared code, kills the sim rather than surfacing a structured diagnostic. Fix: validate the id in `playClip`/`crossFade` and no-op-with-diagnostic (or throw a coded `ApertureSystemError`) on a miss, mirroring the empty-clips guard. Workaround: gate on `anim.clipIds.includes(id)`.
