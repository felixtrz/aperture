...
- Attributed footgun: dynamic boxes fell THROUGH the fixed ground with `collider: true`. Root cause: `physics.colliderGeometry.kind` is only "none"|"assets" — collider shapes are derived from ASSET (GLB) geometry, so a PROCEDURAL mesh.box() with `collider: true` gets no fitting collider and tunnels through. The call 'succeeds' (no error) but produces no effective collider — a quiet footgun. Procedural-mesh physics needs an explicit collider shape in the spawn. (Low-level API; physics is unreachable via the headless CLI anyway, F12.)
